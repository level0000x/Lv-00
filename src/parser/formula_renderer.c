...
static const GreekLetterMapping greek_letters[] = {
    {"alpha", "\\alpha"},
    {"beta", "\\beta"},
    {"gamma", "\\gamma"},
    {"delta", "\\delta"},
...
static bool needs_parentheses(const FormulaNode *node, NodeType parent_op, bool is_right) {
    ...
    if (node->type >= NODE_UNARY_OP_NEG && node->type <= NODE_UNARY_OP_LOG) {
        if (parent_op == NODE_BINARY_OP_MUL || parent_op == NODE_BINARY_OP_DIV ||
            parent_op == NODE_BINARY_OP_POW) {
            return true;
        }
        return false;
    }
    ...
    switch (node->type) {
        case NODE_BINARY_OP_ADD: node_prec = 1; break;
        case NODE_BINARY_OP_SUB: node_prec = 1; break;
        case NODE_BINARY_OP_MUL: node_prec = 2; break;
        case NODE_BINARY_OP_DIV: node_prec = 3; break;  /* 分数形式 */
        case NODE_BINARY_OP_POW: node_prec = 4; break;
        default: return false;
    }
    
    switch (parent_op) {
        case NODE_BINARY_OP_ADD: parent_prec = 1; break;
        case NODE_BINARY_OP_SUB: parent_prec = 1; break;
        case NODE_BINARY_OP_MUL: parent_prec = 2; break;
        case NODE_BINARY_OP_DIV: parent_prec = 3; break;
        case NODE_BINARY_OP_POW: parent_prec = 4; break;
        default: return false;
    }
    ...
    /* 同优先级时，右侧减法需要括号 */
    if (node_prec == parent_prec && is_right && 
        (parent_op == NODE_BINARY_OP_SUB || parent_op == NODE_BINARY_OP_DIV)) {
...
static int render_latex_internal(const FormulaNode *node, char *buffer, size_t size,
    ...
    switch (node->type) {
        case NODE_NUMBER:
            if (node->data.number.is_integer) {
                ...
                /* 分数渲染为 \frac{a}{b} */
                written = snprintf(buffer, size, "\\frac{%lld}{%llu}",
        ...
        case NODE_VARIABLE:
            if (is_greek_letter(node->data.variable.name)) {
                written = snprintf(buffer, size, "%s", 
                                  formula_latex_greek_name(node->data.variable.name));
            } else {
        ...
        case NODE_GEOM_POINT:
            {
                ...
                if (node->data.geom_point.name) {
                    written = snprintf(buffer, size, "%s = \\left(%s\\right)", 
                                      node->data.geom_point.name, coords_buf);
                } else {
        ...
        case NODE_GEOM_CIRCLE:
            {
                ...
                if (node->data.geom_circle.center) {
                    if (node->data.geom_circle.center->type == NODE_IDENTIFIER) {
                        snprintf(center_buf, sizeof(center_buf), "%s", 
                                node->data.geom_circle.center->data.identifier.name);
                    } else {
                ...
                written = snprintf(buffer, size, "\\text{circle } %s \\text{ with center } %s \\text{ and radius } %s",
                                  node->data.geom_circle.name ? node->data.geom_circle.name : "O",
                                  center_buf, radius_buf);
            }
            break;
        ...
        case NODE_GEOM_ARC:
            {
                ...
                written = snprintf(buffer, size,
                    "\\overset{\\frown}{%s} \\text{ with center } %s, \\text{ radius } %s, \\text{ from } %s \\text{ to } %s",
                    node->data.geom_arc.name ? node->data.geom_arc.name : "AB",
                    center_buf, radius_buf, start_buf, end_buf);
            }
            break;
...
static int render_python_internal(const FormulaNode *node, char *buffer, size_t size,
                                  const RenderOptions *options) {
    if (!node) {
        return -1;
    }
        ...
        case NODE_NUMBER:
            if (node->data.number.is_integer) {
                ...
                if (options && options->fraction_mode) {
                    written = snprintf(buffer, size, "Fraction(%lld, %llu)",
                                      (long long)node->data.number.numerator,
                                      (unsigned long long)node->data.number.denominator);
                } else {
                    double val = (double)node->data.number.numerator / 
        ...
        case NODE_GEOM_POINT:
            {
                ...
                if (node->data.geom_point.name) {
                    written = snprintf(buffer, size, "%s = Point(%s)", 
                                      node->data.geom_point.name, coords_buf);
                } else {
        ...
        case NODE_GEOM_ARC:
            {
                ...
                written = snprintf(buffer, size,
                    "Arc('%s', center=%s, radius=%s, start_angle=%s, end_angle=%s)",
                    node->data.geom_arc.name ? node->data.geom_arc.name : "AB",
                    center_buf, radius_buf, start_buf, end_buf);
            }
            break;
...
static int render_dsl_internal(const FormulaNode *node, char *buffer, size_t size,
                               const RenderOptions *options) {
    if (!node) {
        return -1;
    }
    ...
    switch (node->type) {
        case NODE_NUMBER:
            if (node->data.number.is_integer) {
                ...
                written = snprintf(buffer, size, "%lld/%llu",
                                  (long long)node->data.number.numerator,
                                  (unsigned long long)node->data.number.denominator);
            }
            break;
        ...
        case NODE_GEOM_POINT:
            {
                ...
                written = snprintf(buffer, size, "point %s(%s)", 
                                  node->data.geom_point.name ? node->data.geom_point.name : "P",
                                  coords_buf);
            }
            break;
        ...
        case NODE_GEOM_SEGMENT:
            {
                ...
                written = snprintf(buffer, size, "segment %s(%s, %s)", 
                                  node->data.geom_segment.name ? node->data.geom_segment.name : "AB",
                                  ep1_buf, ep2_buf);
            }
            break;
        ...
        case NODE_GEOM_TRIANGLE:
            {
                ...
                written = snprintf(buffer, size, "triangle %s(%s, %s, %s)", 
                                  node->data.geom_triangle.name ? node->data.geom_triangle.name : "ABC",
...
char *formula_render_circle_latex(const char *name, const char *center, const FormulaNode *radius) {
    if (!name || !center || !radius) return NULL;
...
char *formula_render_fraction_latex(int64_t numerator, uint64_t denominator) {
    ...
    if (result) {
        snprintf(result, FRACTION_LATEX_BUF_SIZE, "\\frac{%lld}{%llu}", 
                (long long)numerator, (unsigned long long)denominator);
    }
    return result;
}
