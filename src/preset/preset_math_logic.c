...
/** 数理逻辑模块预设函数块总数 */
#define ADVANCED_MATH_LOGIC_PRESET_COUNT 20
...
bool preset_math_logic_register(void)
{
    ...
    /* -------------------- 析取 -------------------- */
    {
        ...
        if (register_logic_preset(
                ...
                "命题析取：P ∨ Q，当且仅当 P 或 Q 至少一个为真时结果为真",
                inputs, 2, PRESET_TYPE_BOOLEAN,
                "P \\lor Q",
                "O(1)", true, false)) {
            success_count++;
    ...
    /* -------------------- 全称实例化 -------------------- */
    {
        ...
        if (register_logic_preset(
                ...
                "全称实例化：从 ∀x P(x) 推出 P(c)，c 为论域中任意元素",
                inputs, 2, PRESET_TYPE_FORMULA,
    ...
    /* -------------------- 存在泛化 -------------------- */
    {
        ...
        if (register_logic_preset(
                ...
                "存在泛化：从 P(c) 推出 ∃x P(x)，c 为论域中某个元素",
                inputs, 2, PRESET_TYPE_FORMULA,
    ...
    /* -------------------- 全称泛化 -------------------- */
    {
        ...
        if (register_logic_preset(
                ...
                "全称泛化：从 P(c) 推出 ∀x P(x)，要求 c 为任意常量且未被特殊假定",
                inputs, 2, PRESET_TYPE_FORMULA,
                "P(c) \\vdash \\forall x \\, P(x), \\quad c \\text{ 为任意常量}",
                "O(1)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 存在实例化 -------------------- */
    {
        ...
        if (register_logic_preset(
                ...
                "存在实例化：从 ∃x P(x) 引入 P(c)，c 为此前未出现的新常量",
                inputs, 2, PRESET_TYPE_FORMULA,
                "\\exists x \\, P(x) \\vdash P(c), \\quad c \\text{ 为新常量}",
                "O(1)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 自然演绎 -------------------- */
    {
        ...
        if (register_logic_preset(
                ...
                "自然演绎系统：基于引入规则（∧I, ∨I, →I, ∀I, ∃I）和消去规则（∧E, ∨E, →E, ∀E, ∃E）的证明",
                inputs, 2, PRESET_TYPE_BOOLEAN,
                "\\Gamma \\vdash_{\\text{ND}} \\varphi",
                "O(2^{|\\Gamma|})", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 归结原理 -------------------- */
    {
        ...
        if (register_logic_preset(
                ...
                "归结原理：将公式集转化为子句集，通过归结消解推导空子句以证明不可满足性",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "S \\vdash_{\\text{Res}} \\Box \\Leftrightarrow S \\text{ 不可满足}",
    ...
    /* -------------------- 表方法 -------------------- */
    {
        ...
        if (register_logic_preset(
                ...
                "语义表方法：通过系统性地分解公式构建反证树，判定公式的可满足性",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "\\lnot \\varphi \\text{ 的表封闭} \\Leftrightarrow \\varphi \\text{ 有效}",
    ...
    /* -------------------- 模型论可满足性 -------------------- */
    {
        ...
        if (register_logic_preset(
                ...
                "模型论可满足性：判定一阶公式集 Γ 是否存在一致的模型（Γ 有模型）",
                inputs, 1, PRESET_TYPE_BOOLEAN,
    ...
    /* -------------------- 图灵机判定 -------------------- */
    {
        ...
        if (register_logic_preset(
                ...
                "判定给定描述是否定义了一台合法的图灵机（状态集、字母表、转移函数完备性检验）",
                inputs, 1, PRESET_TYPE_BOOLEAN,
    ...
    /* -------------------- 递归函数判定 -------------------- */
    {
        ...
        if (register_logic_preset(
                ...
                "判定给定函数是否为递归函数（可通过图灵机计算）",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "f \\text{ 是递归函数} \\Leftrightarrow \\exists T_M: T_M(x) = f(x)",
                "不可判定", false, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 停机问题 -------------------- */
    {
        ...
        if (register_logic_preset(
                ...
                "停机问题：判定图灵机 M 在输入 w 上是否最终停机（经典不可判定问题）",
                inputs, 2, PRESET_TYPE_BOOLEAN,
    ...
    /* 返回是否所有预设都注册成功 */
    /* lv00_log_info("数理逻辑预设注册完成，共 %d 个预设", success_count) */
