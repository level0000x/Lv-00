...
bool preset_algebraic_geometry_register(void)
{
    ...
    /* -------------------- ag_affine_variety_construct：仿射代数集构造 -------------------- */
    {
        ...
        if (register_ag_preset(
                ...
                "仿射代数集构造：由多项式理想 I 构造仿射代数集 V(I) = {x : f(x)=0, f in I}",
    ...
    /* -------------------- ag_ideal_variety_correspondence：理想-代数集对应 -------------------- */
    {
        ...
        if (register_ag_preset(
                ...
                "理想-代数集对应：Hilbert零点定理 V(I(V(J))) = sqrt(J)，建立理想与代数集的对应",
    ...
    /* -------------------- ag_variety_dimension：代数集维数 -------------------- */
    {
        ...
        if (register_ag_preset(
                ...
                "代数集维数：计算仿射代数集的Krull维数 dim V = tr.deg_k k(V)",
    ...
    /* -------------------- ag_projective_closure：射影闭包 -------------------- */
    {
        ...
        if (register_ag_preset(
                ...
                "射影闭包：计算仿射代数集的射影闭包，添加无穷远点",
                inputs, 1, PRESET_TYPE_REGION,
    ...
    /* -------------------- ag_bezout_theorem：Bezout定理 -------------------- */
    {
        ...
        if (register_ag_preset(
                ...
                "Bezout定理：两个平面曲线 C_d 和 C_e 的交点数（计重数）= d * e",
    ...
    /* -------------------- ag_rational_curve：有理曲线判定 -------------------- */
    {
        ...
        if (register_ag_preset(
                ...
                "有理曲线判定：判定曲线是否为有理曲线（g=0 时存在有理参数化）",
    ...
    /* -------------------- ag_elliptic_curve：椭圆曲线 -------------------- */
    {
        ...
        if (register_ag_preset(
                ...
                "椭圆曲线：由Weierstrass方程 y^2 = x^3 + ax + b 构造椭圆曲线（判别式 != 0）",
                inputs, 2, PRESET_TYPE_PATH,
                "E : y^2 = x^3 + ax + b, \\quad "
                "\\Delta = -16(4a^3 + 27b^2) \\neq 0, \\quad "
    ...
    /* -------------------- ag_sheaf_cohomology：层上同调 -------------------- */
    {
        ...
        if (register_ag_preset(
                ...
                "层上同调：计算层的上同调群 H^i(X, F)，使用Cech上同调或导出函子",
    ...
    /* -------------------- ag_structure_sheaf：结构层 -------------------- */
    {
        ...
        if (register_ag_preset(
                ...
                "结构层：构造代数簇上的结构层 O_X，赋予正则函数层",
                inputs, 1, PRESET_TYPE_MODULE,
    ...
    /* -------------------- ag_tangent_sheaf：切层 -------------------- */
    {
        ...
        if (register_ag_preset(
                ...
                "切层：构造代数簇上的切层 T_X = Der_k(O_X, O_X)",
                inputs, 1, PRESET_TYPE_MODULE,
    ...
    /* -------------------- ag_canonical_divisor：典范除子 -------------------- */
    {
        ...
        if (register_ag_preset(
                ...
                "典范除子：计算代数簇的典范除子 K_X，由微分形式的零点与极点确定",
