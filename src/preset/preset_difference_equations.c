 ...
 * @return false 注册失败
 */
static bool register_diff_preset(
    const char *name,
...
bool preset_difference_equations_register(void)
{
    ...
    /* 线性齐次差分方程 */
    {
        ...
        if (register_diff_preset(
                PRESET_DE_DIFF_LINEAR_HOMOGENEOUS,
                "线性齐次差分方程：求解 a_k y_{n+k} + ... + a_0 y_n = 0 的通解",
    ...
    /* 线性非齐次差分方程 */
    {
        ...
        if (register_diff_preset(
                ...
                "线性非齐次差分方程：求特解与齐次通解之和 y_n = y_h(n) + y_p(n)",
                inputs, 2, PRESET_TYPE_FUNCTION,
    ...
    /* 特征方程法 */
    {
        ...
        if (register_diff_preset(
                ...
                "特征方程法：构造特征方程 a_k r^k + ... + a_0 = 0，根据根的类型确定通解形式",
    ...
    /* 广义Fibonacci数列 */
    {
        ...
        if (register_diff_preset(
                ...
                "广义Fibonacci数列：F_n = aF_{n-1} + bF_{n-2}，给定初始条件求通项公式",
    ...
    /* 线性差分方程组 */
    {
        ...
        if (register_diff_preset(
                ...
                "线性差分方程组：矩阵形式 x_{n+1} = A x_n 求解，x_n = A^n x_0",
                inputs, 2, PRESET_TYPE_FUNCTION,
    ...
    /* 离散稳定性分析 */
    {
        ...
        if (register_diff_preset(
                ...
                "离散稳定性分析：判定差分方程的稳定性（特征根模均<1时渐近稳定）",
    ...
    /* Riccati差分方程 */
    {
        ...
        if (register_diff_preset(
                ...
                "Riccati差分方程：y_{n+1} = (a*y_n + b)/(c*y_n + d)，可化为线性方程组求解",
                inputs, 5, PRESET_TYPE_FUNCTION,
    ...
    /* Logistic映射 */
    {
        ...
        if (register_diff_preset(
                ...
                "Logistic映射：x_{n+1} = rx_n(1-x_n)，分析分岔与混沌行为",
    ...
    /* Mandelbrot迭代 */
    {
        ...
        if (register_diff_preset(
                ...
                "Mandelbrot迭代：z_{n+1} = z_n^2 + c，判定c是否属于Mandelbrot集",
    ...
    /* 离散Lyapunov指数 */
    {
        ...
        if (register_diff_preset(
                ...
                "离散Lyapunov指数：lambda = lim (1/N) sum ln|f'(x_n)|，判定混沌行为",
    ...
    /* Z变换 */
    {
        ...
        if (register_diff_preset(
                ...
                "Z变换：Z{f_n} = sum_{n=0}^{inf} f_n z^{-n}，将时域序列映射到Z域",
                inputs, 1, PRESET_TYPE_FUNCTION,
    ...
    /* Z传递函数 */
    {
        ...
        if (register_diff_preset(
                ...
                "Z传递函数：H(z) = Y(z)/X(z)，由差分方程系数构造传递函数",
                inputs, 2, PRESET_TYPE_FUNCTION,
                "H(z) = \\frac{Y(z)}{X(z)} = "
    ...
    /* Z域频率响应 */
    {
        ...
        if (register_diff_preset(
                ...
                "Z域频率响应：H(e^{j*omega})，计算离散系统的幅频和相频特性",
                inputs, 2, PRESET_TYPE_COMPLEX,
    ...
    /* 递推关系求解 */
    {
        ...
        if (register_diff_preset(
                ...
                "递推关系求解：用生成函数法求解递推关系 a_n = f(a_{n-1}, ..., a_{n-k})",
                inputs, 2, PRESET_TYPE_FUNCTION,
    ...
    /* 组合计数递推 */
    {
        ...
        if (register_diff_preset(
                ...
                "组合计数递推：求解Catalan数、Stirling数、Bell数等经典组合递推",
                inputs, 1, PRESET_TYPE_FUNCTION,
