 ...
 * @return false 注册失败
 */
static bool register_lt_preset(const char *name, const char *description, const PresetType *input_types,
...
bool preset_lie_theory_advanced_register(void) {
    ...
    /* -------------------- lie_bracket：李括号 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_LIE_BRACKET, "李括号：计算李代数中两个元素的括号 [X, Y]", inputs, 3,
                               PRESET_TYPE_VECTOR,
                               "[X, Y] = -[Y, X], \\quad "
    ...
    /* -------------------- lie_algebra_ideal：李代数理想 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_LIE_ALGEBRA_IDEAL, "李代数理想：判定子空间是否为理想（满足 [L, I] ⊆ I）", inputs,
    ...
    /* -------------------- lie_algebra_homomorphism：李代数同态 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_LIE_ALGEBRA_HOMOMORPHISM, "李代数同态：判定映射是否为李代数同态（保持括号）",
    ...
    /* -------------------- derived_algebra：导出代数 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_DERIVED_ALGEBRA, "导出代数：计算导出代数 [L, L]（换位子生成的理想）", inputs, 1,
                               PRESET_TYPE_IDEAL,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_ALGEBRA};
        if (register_lt_preset(PRESET_LIE_ALGEBRA_CENTER, "李代数的中心：计算李代数的中心 Z(L) = {x : [x, y] = 0, ∀y}",
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_ALGEBRA};
        if (register_lt_preset(PRESET_ROOT_SYSTEM, "根系：计算半单李代数的根系 Φ ⊂ h*", inputs, 1, PRESET_TYPE_SET,
    ...
    /* -------------------- simple_roots：单根 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_SIMPLE_ROOTS, "单根：计算根系的基础单根 Δ = {α₁, ..., αₙ}", inputs, 1,
                               PRESET_TYPE_SET,
    ...
    /* -------------------- weyl_group：Weyl群 -------------------- */
    {
        ...
        if (register_lt_preset(
                PRESET_WEYL_GROUP, "Weyl群：计算根系的Weyl群 W = <s_α : α ∈ Δ>", inputs, 1, PRESET_TYPE_GROUP,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_SET};
        if (register_lt_preset(PRESET_CARTAN_MATRIX, "Cartan矩阵：计算单根的Cartan矩阵 A = (a_{ij})", inputs, 1,
                               PRESET_TYPE_MATRIX,
    ...
    /* -------------------- dynkin_diagram：Dynkin图 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_DYNKIN_DIAGRAM, "Dynkin图：构造根系的Dynkin图（分类半单李代数）", inputs, 1,
                               PRESET_TYPE_GRAPH,
                               "\\text{顶点} = \\Delta, \\quad "
    ...
    /* -------------------- irreducible_representation：不可约表示 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_IRREDUCIBLE_REPRESENTATION, "不可约表示：构造最高权为λ的不可约表示 V(λ)", inputs,
                               2, PRESET_TYPE_MODULE,
    ...
    /* -------------------- highest_weight：最高权 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_HIGHEST_WEIGHT, "最高权：计算表示的最高权 λ", inputs, 1, PRESET_TYPE_VECTOR,
    ...
    /* -------------------- weight_space_decomposition：权空间分解 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_MODULE};
        if (register_lt_preset(
                PRESET_WEIGHT_SPACE_DECOMPOSITION, "权空间分解：计算表示的权空间分解 V = ⊕_{μ} V_μ", inputs, 1,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MODULE};
        if (register_lt_preset(PRESET_CHARACTER_FORMULA, "特征标公式：计算表示的特征标（Weyl特征标公式）", inputs, 1,
    ...
    /* -------------------- dimension_formula：维数公式 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_DIMENSION_FORMULA, "维数公式：计算不可约表示的维数（Weyl维数公式）", inputs, 1,
                               PRESET_TYPE_INTEGER,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_ALGEBRA};
        if (register_lt_preset(PRESET_UNIVERSAL_ENVELOPING_ALGEBRA, "泛包络代数：构造李代数的泛包络代数 U(L)", inputs,
                               1, PRESET_TYPE_ALGEBRA,
    ...
    /* -------------------- pbw_basis：PBW基 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_PBW_BASIS, "PBW基：构造Poincaré-Birkhoff-Witt基", inputs, 2, PRESET_TYPE_LIST,
    ...
    /* -------------------- casimir_operator：Casimir算子 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_CASIMIR_OPERATOR, "Casimir算子：构造Casimir算子 Ω = Σ x_i y_i（中心元素）",
    ...
    /* -------------------- infinitesimal_character：中心特征标 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_INFinitESIMAL_CHARACTER, "中心特征标：计算中心 Z(U(L)) 在表示上的特征标", inputs,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_ALGEBRA, PRESET_TYPE_VECTOR};
        if (register_lt_preset(PRESET_VERMA_MODULE, "Verma模：构造Verma模 M(λ) = U(L) ⊗_{U(b)} C_λ", inputs, 2,
                               PRESET_TYPE_MODULE,
    ...
    /* -------------------- lie_algebrization：李代数化 -------------------- */
    {
        ...
        if (register_lt_preset(PRESET_LIE_ALGEBRIZATION, "李代数化：计算李群的李代数 Lie(G) = T_e G", inputs, 1,
                               PRESET_TYPE_ALGEBRA,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_ALGEBRA, PRESET_TYPE_VECTOR};
        if (register_lt_preset(PRESET_EXPONENTIAL_MAP, "指数映射：计算李代数到李群的指数映射 exp: L → G", inputs, 2,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_ALGEBRA};
        if (register_lt_preset(PRESET_ADJOINT_REPRESENTATION, "伴随表示：计算李代数的伴随表示 ad: L → gl(L)", inputs, 1,
                               PRESET_TYPE_FUNCTION,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_ALGEBRA};
        if (register_lt_preset(PRESET_KILLING_FORM, "Killing型：计算李代数的Killing型 κ(x, y) = tr(ad x ∘ ad y)",
    ...
    /* -------------------- semisimple_check：半单判定 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_ALGEBRA};
        if (register_lt_preset(
                PRESET_SEMISIMPLE_CHECK, "半单判定：判定李代数是否半单（Killing型非退化）", inputs, 1,
                PRESET_TYPE_BOOLEAN,
