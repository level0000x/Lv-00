 ...
 * @return false 注册失败
 */
static bool register_measure_preset(
    const char *name,
...
bool preset_measurements_register(void)
{
    ...
    /* 欧几里得距离 */
    {
        ...
        if (register_measure_preset(
                PRESET_DISTANCE_EUCLIDEAN,
                "欧几里得距离：计算两点间的直线距离",
                inputs, 2, PRESET_TYPE_DISTANCE,
    ...
    /* 欧几里得距离平方 */
    {
        PresetType inputs[] = {PRESET_TYPE_POINT, PRESET_TYPE_POINT};
        if (register_measure_preset(
                PRESET_DISTANCE_SQUARED,
                "欧几里得距离平方：避免开方运算，用于距离比较",
    ...
    /* 曼哈顿距离 */
    {
        PresetType inputs[] = {PRESET_TYPE_POINT, PRESET_TYPE_POINT};
        if (register_measure_preset(
                PRESET_DISTANCE_MANHATTAN,
                "曼哈顿距离：两点在坐标轴上的距离之和（L1范数）",
                inputs, 2, PRESET_TYPE_DISTANCE,
    ...
    /* 切比雪夫距离 */
    {
        PresetType inputs[] = {PRESET_TYPE_POINT, PRESET_TYPE_POINT};
        if (register_measure_preset(
                PRESET_DISTANCE_CHEBYSHEV,
                "切比雪夫距离：两点在坐标轴上的最大距离差（L∞范数）",
                inputs, 2, PRESET_TYPE_DISTANCE,
    ...
    /* 点到直线的距离 */
    {
        ...
        if (register_measure_preset(
                PRESET_DISTANCE_POINT_TO_LINE,
                "点到直线的距离：计算点到直线的最短距离",
                inputs, 3, PRESET_TYPE_DISTANCE,
    ...
    /* 点到线段的距离 */
    {
        PresetType inputs[] = {PRESET_TYPE_POINT, PRESET_TYPE_POINT, PRESET_TYPE_POINT};
        if (register_measure_preset(
                PRESET_DISTANCE_POINT_TO_SEGMENT,
                "点到线段的距离：计算点到线段的最短距离",
                inputs, 3, PRESET_TYPE_DISTANCE,
                "d(P, \\overline{AB})",
                "O(1)", true, false)) {
    ...
    /* 三点形成的角度 */
    {
        PresetType inputs[] = {PRESET_TYPE_POINT, PRESET_TYPE_POINT, PRESET_TYPE_POINT};
        if (register_measure_preset(
                PRESET_ANGLE_THREE_POINTS,
                "三点形成的角度：计算以中间点为顶点的角度（弧度）",
                inputs, 3, PRESET_TYPE_ANGLE,
    ...
    /* 两直线夹角 */
    {
        ...
        if (register_measure_preset(
                PRESET_ANGLE_TWO_LINES,
                "两直线夹角：计算两条直线的夹角（弧度）",
                inputs, 4, PRESET_TYPE_ANGLE,
                "\\theta = \\arccos\\left(\\frac{|m_1 - m_2|}{1 + m_1 m_2}\\right)",
                "O(1)", true, false)) {
            success_count++;
        }
    }
    ...
    /* 有向角 */
    {
        PresetType inputs[] = {PRESET_TYPE_POINT, PRESET_TYPE_POINT, PRESET_TYPE_POINT};
        if (register_measure_preset(
                PRESET_DIRECTED_ANGLE,
                "有向角：从射线BA到射线BC的有向角（带符号）",
                inputs, 3, PRESET_TYPE_ANGLE,
                "\\angle(\\vec{BA}, \\vec{BC}) \\in (-\\pi, \\pi]",
                "O(1)", true, false)) {
            success_count++;
        }
    }
    ...
    /* 三角形面积（坐标公式） */
    {
        PresetType inputs[] = {PRESET_TYPE_POINT, PRESET_TYPE_POINT, PRESET_TYPE_POINT};
        if (register_measure_preset(
                PRESET_TRIANGLE_AREA,
                "三角形面积：使用坐标公式计算三角形面积",
                inputs, 3, PRESET_TYPE_AREA,
    ...
    /* 向量叉积（二维） */
    {
        PresetType inputs[] = {PRESET_TYPE_POINT, PRESET_TYPE_POINT, PRESET_TYPE_POINT, PRESET_TYPE_POINT};
        if (register_measure_preset(
                PRESET_VECTOR_CROSS_PRODUCT,
                "向量叉积：计算两个向量的叉积（二维，返回标量）",
                inputs, 4, PRESET_TYPE_SCALAR,
