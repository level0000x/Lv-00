 ...
 * @return false 注册失败
 */
static bool register_game_theory_preset(
    const char *name,
...
bool preset_game_theory_register(void)
{
    ...
    /* -------------------- 占优策略判定 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "占优策略判定：判定博弈中是否存在（严格）占优策略 s_i* > s_i，即对所有对手策略组合均严格更优",
    ...
    /* -------------------- 劣策略剔除 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "\\text{Iterative elimination: } \\forall s_i', \\exists s_i: u_i(s_i, s_{-i}) > u_i(s_i', s_{-i}), \\quad \\forall s_{-i}",
    ...
    /* -------------------- Nash均衡计算 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "Nash均衡计算：计算策略型博弈的所有纯策略Nash均衡，即无人有单方面偏离动机的策略组合",
    ...
    /* -------------------- 混合策略Nash均衡 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "混合策略Nash均衡：计算策略型博弈的混合策略Nash均衡，通过枚举支撑集求解线性方程组",
                inputs, 1, PRESET_TYPE_LIST,
                "\\sigma_i^* \\in \\Delta(S_i): U_i(\\sigma_i^*, \\sigma_{-i}^*) \\ge U_i(\\sigma_i, \\sigma_{-i}^*), \\quad \\forall \\sigma_i \\in \\Delta(S_i)",
    ...
    /* -------------------- 最优响应 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "最优响应：计算给定对手策略组合 s_{-i} 时，玩家 i 的所有最优响应策略 BR_i(s_{-i})",
    ...
    /* -------------------- 合作博弈构造 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "合作博弈构造：由玩家集合N和联盟特征函数 v(S) 构造合作博弈 (N, v)，其中 v(空集) = 0",
                inputs, 2, PRESET_TYPE_STRUCTURE,
                "(N, v), \\quad v: 2^N \\to \\mathbb{R}, \\quad v(\\emptyset) = 0",
                "O(2^{|N|})", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- Shapley值计算 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "Shapley值计算：计算每个玩家 i 的Shapley值 phi_i(v)，基于边际贡献在所有排列上的期望值",
    ...
    /* -------------------- 核心判定 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "核心判定：判定分配方案 x 是否在核心 Core(v) 中，即满足效率性（总量分配）和联盟稳定性（无联盟有偏离动机）",
                inputs, 2, PRESET_TYPE_BOOLEAN,
                "\\text{Core}(v) = \\{x \\in \\mathbb{R}^N : \\sum_{i \\in N} x_i = v(N), \\sum_{i \\in S} x_i \\ge v(S), \\forall S \\subseteq N\\}",
                "O(2^{|N|})", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- Banzhaf权力指数 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "Banzhaf权力指数：计算投票博弈中每个玩家的Banzhaf权力指数，衡量其在所有联盟中的边际贡献（关键玩家次数）",
                inputs, 1, PRESET_TYPE_TUPLE,
                "\\beta_i = \\sum_{S \\subseteq N \\setminus \\{i\\}} [v(S \\cup \\{i\\}) - v(S)], \\quad \\beta_i' = \\beta_i / \\sum_j \\beta_j",
                "O(|N| \\cdot 2^{|N|})", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 分配集计算 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "分配集计算：计算合作博弈的分配集 I(v)，即满足个体理性 x_i >= v({i}) 和效率性 sum(x_i) = v(N) 的所有分配",
                inputs, 1, PRESET_TYPE_SET,
                "I(v) = \\{x \\in \\mathbb{R}^N : x_i \\ge v(\\{i\\}), \\sum_{i \\in N} x_i = v(N)\\}",
                "O(|N|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 展开型博弈构造 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "展开型博弈构造：由博弈树、玩家行动映射和信息集划分构造展开型博弈 Gamma_E",
                inputs, 3, PRESET_TYPE_STRUCTURE,
    ...
    /* -------------------- 子博弈精炼均衡判定 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "子博弈精炼均衡判定：判定策略组合 s* 是否为子博弈精炼Nash均衡，即在每个子博弈中都诱导Nash均衡",
                inputs, 2, PRESET_TYPE_BOOLEAN,
    ...
    /* -------------------- 信息集分析 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "信息集分析：分析博弈中的信息集划分结构，判定完美信息/不完美信息、完美回忆、单节点信息集等性质",
                inputs, 1, PRESET_TYPE_TUPLE,
                "\\mathcal{H}_i = \\{H_{i1}, H_{i2}, \\ldots\\}, \\quad H_{ij} \\subseteq \\mathcal{X}_i, \\quad \\text{完美信息: } |H| = 1, \\forall H \\in \\mathcal{H}",
    ...
    /* -------------------- 极小化极大策略 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                PRESET_GT_MINIMAX,
                "极小化极大策略：计算二人零和博弈的极小化极大策略和博弈值，基于von Neumann minimax定理转化为线性规划求解",
                inputs, 2, PRESET_TYPE_TUPLE,
    ...
    /* -------------------- Pareto最优判定 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                inputs, 2, PRESET_TYPE_BOOLEAN,
                "x \\text{ Pareto最优} \\iff \\not\\exists y: u_i(y) \\ge u_i(x), \\forall i \\text{ 且 } \\exists j: u_j(y) > u_j(x)",
                "O(|N| \\cdot |S|^2)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 进化稳定策略 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "进化稳定策略（ESS）判定：判定策略 s* 是否为进化稳定策略，满足Nash均衡条件和进化稳定性条件",
                inputs, 2, PRESET_TYPE_BOOLEAN,
                "s^* \\text{ 是 ESS} \\iff (1)\\ u(s^*, s^*) \\ge u(s, s^*), \\forall s; \\quad (2)\\ u(s, s^*) = u(s^*, s^*) \\Rightarrow u(s^*, s) > u(s, s)",
                "O(|S|^2)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- Nash讨价还价解 -------------------- */
    {
        ...
        if (register_game_theory_preset(
                ...
                "Nash讨价还价解：在可行集F和争议点d下，最大化Nash乘积 (x_1 - d_1)(x_2 - d_2) 求解最优分配",
                inputs, 2, PRESET_TYPE_TUPLE,
                "(x_1^*, x_2^*) = \\arg\\max_{(x_1, x_2) \\in F,\\ x_i \\ge d_i} (x_1 - d_1)(x_2 - d_2)",
                "O(n) 凸优化", true, false)) {
            success_count++;
        }
    }
