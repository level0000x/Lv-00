...
void csg_node_add_child(CSGNode *parent, CSGNode *child)
{
    ...
    if (!parent->children) {
        ...
        parent->children = (CSGNode **)lv00_calloc(
            (size_t)parent->child_capacity, sizeof(CSGNode *));
    ...
    if (parent->child_count >= parent->child_capacity) {
        ...
        CSGNode **new_arr = (CSGNode **)lv00_realloc(
            parent->children, (size_t)new_cap * sizeof(CSGNode *));
        if (!new_arr) return;
...
void csg_node_init_bbox(CSGNode *node)
{
    ...
    case CSG_NODE_EXTRUDE_ROTATE:
        ...
        for (int i = 1; i < node->child_count; i++) {
            ...
            if (ch->bbox_min[0] < node->bbox_min[0]) node->bbox_min[0] = ch->bbox_min[0];
            if (ch->bbox_min[1] < node->bbox_min[1]) node->bbox_min[1] = ch->bbox_min[1];
            if (ch->bbox_min[2] < node->bbox_min[2]) node->bbox_min[2] = ch->bbox_min[2];
            if (ch->bbox_max[0] > node->bbox_max[0]) node->bbox_max[0] = ch->bbox_max[0];
            if (ch->bbox_max[1] > node->bbox_max[1]) node->bbox_max[1] = ch->bbox_max[1];
            if (ch->bbox_max[2] > node->bbox_max[2]) node->bbox_max[2] = ch->bbox_max[2];
        }
        break;
    }
}
...
static void csg_gen_cube_tris(double w, double h, double d, CSGTriList *out)
{
    ...
    for (int f = 0; f < 6; f++) {
        CSGTriangle tri;
        tri.v[0] = faces[f].v[0]; tri.v[1] = faces[f].v[1]; tri.v[2] = faces[f].v[2];
        tri.normal = csg_tri_normal(&tri); tri.face_id = out->count;
        csg_trilist_append(out, &tri);

        tri.v[0] = faces[f].v[0]; tri.v[1] = faces[f].v[2]; tri.v[2] = faces[f].v[3];
        tri.normal = csg_tri_normal(&tri); tri.face_id = out->count;
        csg_trilist_append(out, &tri);
    }
}
...
static void csg_gen_cylinder_tris(double radius, double height, CSGTriList *out)
{
    ...
    for (int j = 0; j < slices; j++) {
        ...
        CSGTriangle tri;
        tri.v[0] = center_top; tri.v[1] = p1_top; tri.v[2] = p2_top;
        tri.normal = csg_tri_normal(&tri); tri.face_id = out->count;
        csg_trilist_append(out, &tri);

        tri.v[0] = center_bottom; tri.v[1] = p2_bot; tri.v[2] = p1_bot;
        tri.normal = csg_tri_normal(&tri); tri.face_id = out->count;
        csg_trilist_append(out, &tri);
    }
}
 ...
 * @param out     输出：并集结果
 */
static void csg_bsp_union_tri(const CSGTriList *list_a,
                               const CSGTriList *list_b,
                               CSGTriList *out)
{
 ...
 * @param out     输出：差集结果
 */
static void csg_bsp_difference_tri(const CSGTriList *list_a,
                                    const CSGTriList *list_b,
                                    CSGTriList *out)
{
    ...
    for (int i = 0; i < list_a->count; i++) {
        ...
        CSGVec3 center = csg_vec3_scale(
            csg_vec3_add(csg_vec3_add(tri->v[0], tri->v[1]), tri->v[2]),
            1.0 / 3.0);
 ...
 * @param out     输出：交集结果
 */
static void csg_bsp_intersection_tri(const CSGTriList *list_a,
                                      const CSGTriList *list_b,
                                      CSGTriList *out)
{
 ...
 */
static int csg_export_node(const CSGNode *node,
                            char *buf, int buf_size, int written,
                            int indent)
{
    if (!node || !buf || written < 0 || written >= buf_size) return written;

    /* 生成缩进空格 */
    char indent_str[33];
    int  indent_len = indent * 2;
    if (indent_len > 32) indent_len = 32;
    ...
    case CSG_NODE_INTERSECTION: {
        ...
        if (node->kind == CSG_NODE_DIFFERENCE)  op_name = "difference";
        if (node->kind == CSG_NODE_INTERSECTION) op_name = "intersection";

        n = snprintf(buf + written, (size_t)(buf_size - written),
                     "%s%s() {\n", indent_str, op_name);
        if (n > 0) written += n; else if (n < 0) return -1;
    ...
    case CSG_NODE_EXTRUDE_ROTATE: {
        ...
        if (node->kind == CSG_NODE_MINKOWSKI)      op_name = "minkowski";
        if (node->kind == CSG_NODE_EXTRUDE_LINEAR) op_name = "linear_extrude";
        if (node->kind == CSG_NODE_EXTRUDE_ROTATE) op_name = "rotate_extrude";
...
char *csg_export_to_openscad(const CSGNode *root)
{
    ...
    int written = snprintf(buf, (size_t)buf_size,
                           ...
                           "$fn = 64;\n\n");
    if (written < 0) { lv00_free((void **)&buf); return NULL; }
