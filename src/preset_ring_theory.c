 ...
 * @return false 注册失败
 */
static bool register_ring_theory_preset(
    const char *name,
...
bool preset_ring_theory_register(void)
{
    ...
    /* -------------------- 环加法 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "环加法：计算环R中两个元素的和 a + b，加法满足交换律与结合律",
                inputs, 2, PRESET_TYPE_RING,
                "(a, b) \\mapsto a + b \\in R",
                "O(1)", true, true)) {
            success_count++;
        }
    ...
    /* -------------------- 环乘法 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "环乘法：计算环R中两个元素的积 a * b，乘法满足结合律与分配律",
                inputs, 2, PRESET_TYPE_RING,
                "(a, b) \\mapsto a \\cdot b \\in R",
                "O(1)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 加法逆元 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "加法逆元：计算环元素的加法逆元 -a，满足 a + (-a) = 0_R",
                inputs, 1, PRESET_TYPE_RING,
                "a \\mapsto -a, \\quad a + (-a) = 0_R",
                "O(1)", true, true)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 乘法逆元 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                inputs, 1, PRESET_TYPE_RING,
                "a \\mapsto a^{-1}, \\quad a \\cdot a^{-1} = 1_R",
                "O(1)", true, true)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 零元检测 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                inputs, 1, PRESET_TYPE_RING,
                "0_R \\in R, \\quad \\forall a \\in R: a + 0_R = a",
                "O(1)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 单位元检测 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                inputs, 1, PRESET_TYPE_RING,
                "1_R \\in R, \\quad \\forall a \\in R: a \\cdot 1_R = a",
                "O(1)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 理想判定 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "理想判定：验证环R的子集I是否构成理想（加法子群 + 吸收性）",
                inputs, 2, PRESET_TYPE_BOOLEAN,
                "I \\triangleleft R \\Leftrightarrow (I \\le (R,+)) \\land (\\forall r \\in R, a \\in I: ra \\in I \\land ar \\in I)",
                "O(n^2)", false, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 主理想生成 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "主理想生成：由单个元素a生成主理想 <a> = aR = Ra",
                inputs, 1, PRESET_TYPE_IDEAL,
                "\\langle a \\rangle = \\{ra : r \\in R\\} \\quad \\text{（交换环）}",
                "O(1)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 理想和 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "理想和：计算两个理想的和 I + J = {a + b : a in I, b in J}",
    ...
    /* -------------------- 理想交 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "理想交：计算两个理想的交 I ∩ J，是包含于I和J的最大理想",
                inputs, 2, PRESET_TYPE_IDEAL,
                "I \\cap J = \\{a : a \\in I \\land a \\in J\\}",
                "O(n)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 商环构造 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "商环构造：由环R和理想I构造商环 R/I，元素为陪集 a + I",
                inputs, 2, PRESET_TYPE_RING,
                "R/I = \\{a + I : a \\in R\\}, \\quad (a+I)+(b+I)=(a+b)+I, \\; (a+I)(b+I)=(ab)+I",
                "O(n)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 素理想判定 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "素理想判定：判定理想I是否为素理想（R/I是整环）",
                inputs, 2, PRESET_TYPE_BOOLEAN,
                "I \\text{ 素} \\Leftrightarrow R/I \\text{ 是整环} \\Leftrightarrow (ab \\in I \\Rightarrow a \\in I \\lor b \\in I)",
                "O(n^2)", false, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 同态像 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                inputs, 1, PRESET_TYPE_RING,
                "\\text{Im}(f) = \\{f(a) : a \\in R\\}, \\quad \\text{Im}(f) \\le S",
                "O(n)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 环同构判定 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                inputs, 2, PRESET_TYPE_BOOLEAN,
                "R \\cong S \\Leftrightarrow \\exists f: R \\to S \\text{ 双射，且为环同态}",
                "O(n^2)", false, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 欧几里得整环判定 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "欧几里得整环判定：验证整环R是否为欧几里得整环（存在欧几里得范数函数）",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "R \\text{ 是欧几里得整环} \\Leftrightarrow \\exists d: R \\setminus \\{0\\} \\to \\mathbb{N}, \\; \\forall a,b \\neq 0: a = bq + r, \\; d(r) < d(b)",
                "O(n^2)", false, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 主理想整环判定 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "主理想整环判定：验证整环R是否为主理想整环（每个理想都是主理想）",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "R \\text{ 是PID} \\Leftrightarrow \\forall I \\triangleleft R, \\; \\exists a \\in R: I = \\langle a \\rangle",
                "O(n^2)", false, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 唯一分解整环判定 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "唯一分解整环判定：验证整环R是否为唯一分解整环（每个非零非可逆元可唯一分解为不可约元之积）",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "R \\text{ 是UFD} \\Leftrightarrow \\forall a \\neq 0, a \\notin R^*: a = up_1 p_2 \\cdots p_n \\text{（不可约分解唯一）}",
                "O(n^3)", false, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 多项式乘法 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "多项式乘法：计算两个多项式的积 f(x) * g(x)，使用卷积算法",
                inputs, 2, PRESET_TYPE_POLYNOMIAL,
    ...
    /* -------------------- 多项式GCD -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "多项式GCD：使用欧几里得算法计算两个多项式的最大公因式 gcd(f, g)",
                inputs, 2, PRESET_TYPE_POLYNOMIAL,
                "\\gcd(f, g) = d, \\quad d | f \\land d | g \\land (e | f \\land e | g \\Rightarrow e | d)",
                "O(n^2)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 多项式求值 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "多项式求值：计算多项式 f(x) 在点 c 处的值 f(c)，使用秦九韶算法（Horner法则）",
                inputs, 2, PRESET_TYPE_RING,
    ...
    /* -------------------- 多项式不可约判定 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "多项式不可约判定：判定多项式 f(x) 在给定环/域上是否不可约",
                inputs, 2, PRESET_TYPE_BOOLEAN,
    ...
    /* -------------------- 环的特征 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "环的特征：计算环R的特征 char(R)，即满足 n*1_R = 0_R 的最小正整数n",
                inputs, 1, PRESET_TYPE_INTEGER,
                "\\text{char}(R) = \\min\\{n > 0 : n \\cdot 1_R = 0_R\\}",
                "O(1)", false, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 幂零元检测 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "幂零元检测：判定环元素a是否为幂零元，即存在正整数n使得 a^n = 0",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "a \\text{ 幂零} \\Leftrightarrow \\exists n > 0: a^n = 0_R",
                "O(n)", false, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 幂等元检测 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "幂等元检测：判定环元素a是否为幂等元，满足 a^2 = a",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "a \\text{ 幂等} \\Leftrightarrow a^2 = a",
                "O(1)", false, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 单位的群 -------------------- */
    {
        ...
        if (register_ring_theory_preset(
                ...
                "单位的群：计算环R的所有可逆元构成的乘法群 R^*",
                inputs, 1, PRESET_TYPE_SET,
                "R^* = \\{a \\in R : \\exists b \\in R, \\; ab = ba = 1_R\\}",
                "O(n^2)", true, false)) {
            success_count++;
        }
    }
