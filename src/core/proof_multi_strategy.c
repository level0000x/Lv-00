 ...
 */
static bool default_applicability_check(
 ...
 */
static bool area_method_applicability_check(
    const ProofMultiStrategy *mse,
    const ConstraintGraph *graph,
    const Proposition *prop)
{
    ...
    if (prop->name && strstr(prop->name, "area")) return true;
    if (prop->description && strstr(prop->description, "area")) return true;
    return false;
}
 ...
 */
static bool groebner_applicability_check(
    const ProofMultiStrategy *mse,
    const ConstraintGraph *graph,
    const Proposition *prop)
{
 ...
 */
static bool execute_direct_construction(
    ProofMultiStrategy *mse,
    ProofNavigator *nav)
{
    if (!mse || !nav) return false;
    ...
    if (nav->target_prop && nav->target_prop->pattern) {
        /* 执行合一检查 */
        UnifyStatus status = proof_unify(nav->construction,
                                          nav->target_prop, true);
 ...
 * - 生成传统几何风格的证明步骤
 */
static bool execute_area_method(
    ProofMultiStrategy *mse,
    ProofNavigator *nav)
{
    if (!mse || !nav) return false;
    ...
    /* 面积法需要目标命题 */
    if (!nav->target_prop || !nav->construction) return false;
 ...
 * - 通过代数消元验证命题
 */
static bool execute_groebner_basis(
    ProofMultiStrategy *mse,
    ProofNavigator *nav)
{
    if (!mse || !nav) return false;
    ...
    if (step) {
        step->color = PROOF_COLOR_GREEN;
        step->note = _strdup(
            "[Groebner基法] 将几何约束转化为多项式方程组，计算Groebner基");
        proof_navigator_add_step(nav, step);
    }
    ...
    if (nav->engine && nav->construction) {
        ...
        if (dof == 0) {
            ...
            if (solved_step) {
                ...
                solved_step->note = _strdup(
                    "[Groebner基法] 多项式系统完全约束，命题得证");
 ...
 */
static bool execute_vector_method(
    ProofMultiStrategy *mse,
    ProofNavigator *nav)
{
    (void)mse;
 ...
 */
static bool execute_full_angle_method(
    ProofMultiStrategy *mse,
    ProofNavigator *nav)
{
    (void)mse;
 ...
 */
static bool execute_deductive_database(
    ProofMultiStrategy *mse,
    ProofNavigator *nav)
{
    (void)mse;
 ...
 */
static bool execute_coordinate(
    ProofMultiStrategy *mse,
    ProofNavigator *nav)
{
    (void)mse;
 ...
 */
static void fill_default_descriptor(ProofStrategyDescriptor *desc,
...
ProofMultiStrategy *proof_multi_strategy_create(ProofNavigator *nav)
{
    ...
    int default_order[] = {
        PROOF_STRATEGY_DIRECT_CONSTRUCTION,
        PROOF_STRATEGY_AREA_METHOD,
        PROOF_STRATEGY_GROEBNER_BASIS,
        PROOF_STRATEGY_VECTOR_METHOD,
        PROOF_STRATEGY_FULL_ANGLE_METHOD,
        PROOF_STRATEGY_DEDUCTIVE_DATABASE,
...
{
    if (!mse || !descriptor) return false;
    if (descriptor->type < 0 || descriptor->type >= PROOF_STRATEGY_COUNT) return false;
    ...
    if (descriptor->required_axiom_packages && descriptor->axiom_package_count > 0) {
        ...
        target->required_axiom_packages = (char **)calloc(
            descriptor->axiom_package_count, sizeof(char *));
        if (target->required_axiom_packages) {
            for (int i = 0; i < descriptor->axiom_package_count; i++) {
                if (descriptor->required_axiom_packages[i]) {
                    target->required_axiom_packages[i] =
                        _strdup(descriptor->required_axiom_packages[i]);
                }
            }
        }
    }
...
{
    if (!mse) return false;
    if (strategy_type < 0 || strategy_type >= PROOF_STRATEGY_COUNT) return false;

    ProofStrategyDescriptor *desc = &mse->strategies[strategy_type];
    if (desc->status == PROOF_STRATEGY_UNAVAILABLE) return false;

    /* 取消旧激活 */
    if (mse->active_strategy_index >= 0) {
        mse->strategies[mse->active_strategy_index].status =
            PROOF_STRATEGY_AVAILABLE;
    }
...
const ProofStrategyDescriptor *proof_multi_strategy_get_active(
    const ProofMultiStrategy *mse)
{
    if (!mse || mse->active_strategy_index < 0) return NULL;
...
int proof_multi_strategy_evaluate_applicability(
    ...
    int max_count)
{
    if (!mse || !out_applicable_types || max_count <= 0) return 0;
...
{
    if (!mse || mse->active_strategy_index < 0) return false;

    ProofStrategyDescriptor *desc = &mse->strategies[mse->active_strategy_index];
    if (!desc->execute) return false;
...
{
    if (!mse) return PROOF_STRATEGY_COUNT;

    /* 按回退顺序尝试每个策略 */
    for (int i = 0; i < mse->fallback_count; i++) {
        int idx = mse->fallback_order[i];
        if (idx < 0 || idx >= PROOF_STRATEGY_COUNT) continue;
...
bool proof_multi_strategy_pipeline(
    ...
    int pipeline_count)
{
    if (!mse || !pipeline || pipeline_count <= 0) return false;

    for (int i = 0; i < pipeline_count; i++) {
        ProofStrategyType type = pipeline[i];
        if (type < 0 || type >= PROOF_STRATEGY_COUNT) return false;
...
void proof_multi_strategy_set_fallback_order(
    ...
    int count)
{
    if (!mse || !fallback_order || count <= 0) return;

    free(mse->fallback_order);
    mse->fallback_order = (int *)malloc(count * sizeof(int));
    if (!mse->fallback_order) return;
...
{
    if (!mse) {
        if (out_total_attempts) *out_total_attempts = 0;
        if (out_success_count) *out_success_count = 0;
        return;
    }

    if (out_total_attempts) *out_total_attempts = mse->total_attempts;
    if (out_success_count) *out_success_count = mse->success_count;
}
...
{
    switch (type) {
    case PROOF_STRATEGY_DIRECT_CONSTRUCTION: return "直接构造法";
    case PROOF_STRATEGY_AREA_METHOD:         return "面积法";
    case PROOF_STRATEGY_GROEBNER_BASIS:      return "Groebner基法";
    case PROOF_STRATEGY_VECTOR_METHOD:       return "向量法";
    case PROOF_STRATEGY_FULL_ANGLE_METHOD:   return "全角法";
    case PROOF_STRATEGY_DEDUCTIVE_DATABASE:  return "演绎数据库法";
    case PROOF_STRATEGY_COORDINATE:          return "坐标法";
    case PROOF_STRATEGY_ORACLE:              return "Oracle法";
    default:                                 return "未知策略";
    }
}
...
{
    switch (status) {
    case PROOF_STRATEGY_AVAILABLE:   return "可用";
    case PROOF_STRATEGY_UNAVAILABLE: return "不可用";
    case PROOF_STRATEGY_ACTIVE:      return "已激活";
    case PROOF_STRATEGY_COMPLETED:   return "已完成";
    case PROOF_STRATEGY_FAILED:      return "失败";
    default:                          return "未知";
    }
}
