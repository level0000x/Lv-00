    ...
    if (ctx->active_measure && input) {
        ...
        if (new_value) {
            ...
            SymbolicCoord **new_vals =
                lv00_realloc(ctx->measure_values, ctx->measure_value_count * sizeof(SymbolicCoord *));
            if (!new_vals) {
                ctx->measure_value_count--;
                symbolic_coord_destroy(new_value);
                return RECURSION_ERROR;
...
bool measure_system_register_non_symbolic(MeasureSystem *ms, int measure_type_id, NonSymbolicComparator comparator,
    ...
    NonSymbolicMeasureMeta *new_metas =
        lv00_realloc(ms->non_symbolic_metas, new_count * sizeof(NonSymbolicMeasureMeta));
    if (!new_metas)
        return false;
    ...
    for (int i = 0; i < test_count; i++) {
        ...
        if (measure->type == MEASURE_SYMBOLIC) {
            ...
            if (measure->compare_func) {
                 ...
                 */
                results[i].passed = false;
                results[i].error_message = lv00_strdup("Non-symbolic measure requires axiom pack template expansion");
                all_passed = false;
            } else {
                results[i].passed = false;
    ...
    if (axiom_template_name && axiom_template_name[0] != '\0') {
        ...
        if (existing_idx >= 0) {
            ...
            NonSymbolicMeasureValidationMeta *new_metas =
                lv00_realloc(sys->validation_metas, new_count * sizeof(NonSymbolicMeasureValidationMeta));
            if (!new_metas)
                return -1;
