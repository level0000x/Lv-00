...
static GeomNode *graph_alloc_node(ConstraintGraph *graph, GeomType type) {
    ...
    if (graph->node_count >= graph->node_capacity) {
        if (graph->node_capacity > INT_MAX / LV00_ARRAY_GROWTH_FACTOR) { lv00_free((void **)&node); return NULL; } /* would overflow */
        int new_capacity = graph->node_capacity == 0 
            ? LV00_INITIAL_ARRAY_CAPACITY 
            : graph->node_capacity * LV00_ARRAY_GROWTH_FACTOR;
...
static Constraint *graph_alloc_constraint(ConstraintGraph *graph, ConstraintType type) {
    ...
    if (graph->constraint_count >= graph->constraint_capacity) {
        if (graph->constraint_capacity > INT_MAX / LV00_ARRAY_GROWTH_FACTOR) { lv00_free((void **)&con); return NULL; } /* would overflow */
        int new_capacity = graph->constraint_capacity == 0 
            ? LV00_INITIAL_ARRAY_CAPACITY 
...
GeomNode *graph_add_node_with_id(ConstraintGraph *graph, int node_id, GeomType type,
    ...
    if (graph->node_count >= graph->node_capacity) {
        ...
        if (!new_nodes) {
            /* 清理已分配的坐标 */
            for (int i = 0; i < coord_count; i++) {
                if (node->symbolic_coords[i]) symbolic_coord_destroy(node->symbolic_coords[i]);
            }
            lv00_free((void **)&node->symbolic_coords);
            lv00_free((void **)&node);
            return NULL;
...
Constraint *graph_add_constraint_with_id(ConstraintGraph *graph, int constraint_id,
    ...
    if (!graph || !participants || participant_count <= 0) return NULL;
...
static bool constraint_exists(ConstraintGraph *graph, ConstraintType type, int *participants, int count) {
    for (int i = 0; i < graph->constraint_count; i++) {
        ...
        if (c->type != type || c->participant_count != count) continue;
        bool same = true;
...
static bool node_index_ensure_capacity(ConstraintGraph *graph) {
    if (!graph->node_index) {
        ...
        if (!graph->node_index) return false;
...
void graph_node_index_insert(ConstraintGraph *graph, GeomNode *node) {
    if (!node_index_ensure_capacity(graph)) return;
...
static void node_index_remove(ConstraintGraph *graph, int node_id) {
    ...
    if (graph->node_index[idx] == NULL) return;
...
static bool constraint_index_ensure_capacity(ConstraintGraph *graph) {
    if (!graph->constraint_index) {
        ...
        if (!graph->constraint_index) return false;
...
void graph_constraint_index_insert(ConstraintGraph *graph, Constraint *con) {
    if (!constraint_index_ensure_capacity(graph)) return;
...
static void constraint_index_remove(ConstraintGraph *graph, int constraint_id) {
    if (!graph || !graph->constraint_index || graph->constraint_index_capacity == 0) return;
    ...
    while (graph->constraint_index[i] != NULL) {
        if (graph->constraint_index[i]->id == constraint_id) break;
        i = (i + 1) & (capacity - 1);
        if (i == idx) return; /* 未找到 */
    }
    if (graph->constraint_index[i] == NULL) return; /* 未找到 */
...
static bool segments_intersect(const GeomNode *seg_a, const GeomNode *seg_b) {
    ...
    if (seg_a->type != GEOM_LINE_SEGMENT || seg_b->type != GEOM_LINE_SEGMENT) return false;
    if (seg_a->coord_count < 4 || seg_b->coord_count < 4) return false;
    if (!seg_a->symbolic_coords || !seg_b->symbolic_coords) return false;
    ...
    if (fabs(denom) < LV00_EPSILON_DOUBLE) return false; /* 平行或共线 */
 ...
 */
static bool check_incremental_conflict(const ConstraintGraph *graph,
                                        const Constraint *new_constraint) {
    if (!graph || !new_constraint) return false;
    ...
    case BETWEENNESS: {
        ...
        for (int i = 0; i < graph->constraint_count; i++) {
            ...
            if (pid == p1 && p1_lc < 64) p1_lines[p1_lc++] = lid;
            if (pid == p2 && p2_lc < 64) p2_lines[p2_lc++] = lid;
            if (pid == p3 && p3_lc < 64) p3_lines[p3_lc++] = lid;
        }
        /* 如果没有两个点共享一条线，则它们不可能共线 */
        bool any_shared = false;
        for (int i = 0; i < p1_lc && !any_shared; i++) {
            for (int j = 0; j < p2_lc && !any_shared; j++) {
                if (p1_lines[i] == p2_lines[j]) any_shared = true;
            }
        }
        if (!any_shared) {
            for (int i = 0; i < p2_lc && !any_shared; i++) {
                for (int j = 0; j < p3_lc && !any_shared; j++) {
                    if (p2_lines[i] == p3_lines[j]) any_shared = true;
                }
            }
        }
        if (!any_shared) {
            for (int i = 0; i < p1_lc && !any_shared; i++) {
                for (int j = 0; j < p3_lc && !any_shared; j++) {
                    if (p1_lines[i] == p3_lines[j]) any_shared = true;
                }
            }
        }
        if (!any_shared) return true; /* 冲突：不可能共线 */
        break;
    }
    case INTERSECTION: {
...
AddNodeResult graph_add_point(ConstraintGraph *graph, SymbolicCoord **coords, int coord_count) {
    if (!graph) return ADD_NODE_CONFLICT;
    GeomNode *node = graph_alloc_node(graph, GEOM_POINT);
    if (!node) return ADD_NODE_CONFLICT;
    node->symbolic_coords = lv00_malloc(coord_count * sizeof(SymbolicCoord*));
    if (!node->symbolic_coords) {
        graph->node_count--;
...
AddNodeResult graph_add_line_segment(ConstraintGraph *graph, int endpoint1_id, int endpoint2_id) {
    ...
    if (!n1 || !n2) return ADD_NODE_CONFLICT;
    if (n1->type != GEOM_POINT || n2->type != GEOM_POINT) return ADD_NODE_CONFLICT;
    ...
    if (total_coords < 4) total_coords = 4;
...
AddNodeResult graph_add_function_block(ConstraintGraph *graph, int *internal_node_ids, int internal_count,
    ...
    for (int i = 0; i < internal_count; i++) {
        if (!graph_get_node(graph, internal_node_ids[i])) return ADD_NODE_CONFLICT;
    }
    for (int i = 0; i < input_count; i++) {
        GeomNode *n = graph_get_node(graph, input_port_ids[i]);
        if (!n || n->type != GEOM_PORT) return ADD_NODE_CONFLICT;
    }
    ...
    if (!node) return ADD_NODE_CONFLICT;
    node->data.func_block.internal_nodes = internal_count > 0
        ? lv00_malloc(internal_count * sizeof(GeomNode*))
        : NULL;
    ...
    node->data.func_block.input_port_ids = input_count > 0
        ? lv00_malloc(input_count * sizeof(int))
        : NULL;
    ...
    node->data.func_block.output_port_ids = output_count > 0
        ? lv00_malloc(output_count * sizeof(int))
        : NULL;
...
CrossBoundaryConstraint* find_cross_boundary_constraints(ConstraintGraph *graph,
    ...
    for (int i = 0; i < graph->constraint_count; i++) {
        ...
        for (int j = 0; j < c->participant_count; j++) {
            ...
            if (is_internal || is_port) has_internal = true;
...
static bool check_cross_boundary_constraints(ConstraintGraph *graph, GeomNode *func_block) {
    ...
    for (int i = 0; i < graph->constraint_count; i++) {
        Constraint *c = graph->constraints[i];
        if (c->type == CONNECTION) continue; /* 连接约束允许跨边界 */
...
AddConstraintResult graph_add_incidence(ConstraintGraph *graph, int point_id, int line_or_region_id) {
    ...
    if (!point || !target) return ADD_CONSTRAINT_CONFLICT;
    if (point->type != GEOM_POINT) return ADD_CONSTRAINT_CONFLICT;
    if (target->type != GEOM_LINE_SEGMENT && target->type != GEOM_REGION) return ADD_CONSTRAINT_CONFLICT;
    int participants[2] = {point_id, line_or_region_id};
    if (constraint_exists(graph, INCIDENCE, participants, 2)) return ADD_CONSTRAINT_DUPLICATE;
    Constraint *con = graph_alloc_constraint(graph, INCIDENCE);
    if (!con) return ADD_CONSTRAINT_CONFLICT;
    ...
    if (check_incremental_conflict(graph, con)) {
        LOG_WARN("constraint_graph", "检测到 INCIDENCE 约束 %d 的增量冲突 "
                 "(点 %d 在线/区域 %d 上)", con->id, point_id, line_or_region_id);
    }
    if (graph_stream_ctx) {
        char buf[128];
...
AddConstraintResult graph_add_betweenness(ConstraintGraph *graph, int p1_id, int p2_id, int p3_id) {
    ...
    if (!p1 || !p2 || !p3) return ADD_CONSTRAINT_CONFLICT;
    if (p1->type != GEOM_POINT || p2->type != GEOM_POINT || p3->type != GEOM_POINT) {
        return ADD_CONSTRAINT_CONFLICT;
    }
    int participants[3] = {p1_id, p2_id, p3_id};
    if (constraint_exists(graph, BETWEENNESS, participants, 3)) return ADD_CONSTRAINT_DUPLICATE;
    ...
    /* 增量代数冲突检查 */
    if (check_incremental_conflict(graph, con)) {
        LOG_WARN("constraint_graph", "检测到 BETWEENNESS 约束 %d 的增量冲突 "
                 "(点 %d-%d-%d 不共线)", con->id, p1_id, p2_id, p3_id);
    }
    return ADD_CONSTRAINT_OK;
}
...
AddConstraintResult graph_add_intersection(ConstraintGraph *graph, int line1_id, int line2_id, int result_point_id) {
    ...
    if (!l1 || !l2 || !pt) return ADD_CONSTRAINT_CONFLICT;
    if (l1->type != GEOM_LINE_SEGMENT || l2->type != GEOM_LINE_SEGMENT) return ADD_CONSTRAINT_CONFLICT;
    if (pt->type != GEOM_POINT) return ADD_CONSTRAINT_CONFLICT;
    int participants[3] = {line1_id, line2_id, result_point_id};
    if (constraint_exists(graph, INTERSECTION, participants, 3)) return ADD_CONSTRAINT_DUPLICATE;
    ...
    /* 增量代数冲突检查 */
    if (check_incremental_conflict(graph, con)) {
        LOG_WARN("constraint_graph", "检测到 INTERSECTION 约束 %d 的增量冲突 "
                 "(线 %d 和 %d 已在不同点相交)", con->id, line1_id, line2_id);
    }
    return ADD_CONSTRAINT_OK;
}
...
AddConstraintResult graph_add_containment(ConstraintGraph *graph, int inner_id, int outer_id) {
    ...
    if (!inner || !outer) return ADD_CONSTRAINT_CONFLICT;
    if (inner->type != GEOM_POINT && inner->type != GEOM_REGION) return ADD_CONSTRAINT_CONFLICT;
    if (outer->type != GEOM_REGION) return ADD_CONSTRAINT_CONFLICT;
    int participants[2] = {inner_id, outer_id};
    if (constraint_exists(graph, CONTAINMENT, participants, 2)) return ADD_CONSTRAINT_DUPLICATE;
...
AddConstraintResult graph_add_connection(ConstraintGraph *graph, int src_port_id, int dst_port_id) {
    ...
    if (!src || !dst) return ADD_CONSTRAINT_CONFLICT;
    if (src->type != GEOM_PORT || dst->type != GEOM_PORT) return ADD_CONSTRAINT_CONFLICT;
    ...
    if (constraint_exists(graph, CONNECTION, participants, 2)) return ADD_CONSTRAINT_DUPLICATE;
    ...
    for (int i = 0; i < graph->node_count; i++) {
        ...
        if (n->type == GEOM_REGION) {
            for (int j = 0; j < n->data.region.segment_count; j++) {
                if (n->data.region.boundary_segments[j] &&
                    n->data.region.boundary_segments[j]->id == node_id) {
                    return true;
                }
            }
        }
...
RemoveNodeResult graph_remove_node(ConstraintGraph *graph, int node_id) {
    ...
    if (!node) return REMOVE_NODE_NOT_FOUND;
    ...
    if (node->type == GEOM_PORT) {
        for (int i = 0; i < graph->node_count; i++) {
            ...
            if (other->type == GEOM_PORT && other->data.port &&
                other->data.port->connected_to &&
        ...
        if (fb->type == GEOM_FUNCTION_BLOCK && fb->data.func_block.internal_nodes) {
            for (int j = 0; j < fb->data.func_block.internal_node_count; j++) {
                if (fb->data.func_block.internal_nodes[j] &&
                    fb->data.func_block.internal_nodes[j]->id == node_id) {
...
int graph_find_constraints_involving(const ConstraintGraph *graph, int node_id, int *out_indices, int max_results) {
    if (!graph || !out_indices || max_results <= 0) return 0;
    int count = 0;
...
int graph_detect_redundancy(const ConstraintGraph *graph, ConstraintType type, const int *participants, int n_parts) {
    if (!graph || !participants || n_parts <= 0) return -1;
    for (int i = 0; i < graph->constraint_count; i++) {
        Constraint *c = graph->constraints[i];
        if (c->type != type || c->participant_count != n_parts) continue;
        bool same = true;
...
GeomNode *graph_get_node(const ConstraintGraph *graph, int node_id) {
    ...
    if (graph->node_index) {
        unsigned idx = node_id_hash(node_id, graph->node_index_capacity);
        while (graph->node_index[idx] != NULL) {
            if (graph->node_index[idx]->id == node_id) return graph->node_index[idx];
            idx = (idx + 1) & (unsigned)(graph->node_index_capacity - 1);
        }
        return NULL;
    }
...
Constraint *graph_get_constraint(const ConstraintGraph *graph, int constraint_id) {
    ...
    if (graph->constraint_index) {
        ...
        while (graph->constraint_index[idx] != NULL) {
            if (graph->constraint_index[idx]->id == constraint_id) return graph->constraint_index[idx];
            idx = (idx + 1) & (unsigned)(graph->constraint_index_capacity - 1);
        }
        return NULL;
    }
    /* Fallback to linear scan if index not built */
    for (int i = 0; i < graph->constraint_count; i++) {
        if (graph->constraints[i]->id == constraint_id) return graph->constraints[i];
    }
    return NULL;
}
...
int *graph_detect_redundant_constraints(ConstraintGraph *graph, int *out_count) {
    ...
    *out_count = 0;
    if (!graph || graph->constraint_count == 0) return NULL;
    ...
    if (n > 1) {
        ...
        if (entries) {
            ...
            while (i < n) {
                ...
                while (j < n && entries[j].hash == cur_hash) j++;
                ...
                for (int a = i; a < j; a++) {
                    ...
                    for (int b = a + 1; b < j; b++) {
                        ...
                        if (ci->type != cj->type || ci->participant_count != cj->participant_count) continue;
                        bool same = true;
    ...
    if (!point_ids) { lv00_free((void **)&redundant); return redundant; }
    int point_count = 0;
    bool *point_seen = lv00_calloc(graph->node_count, sizeof(bool));
    if (!point_seen) { lv00_free((void **)&point_ids); lv00_free((void **)&redundant); return redundant; }
    
    /* Use a mapping from node id to variable index */
    int max_node_id = 0;
    for (int i = 0; i < graph->node_count; i++) {
        if (graph->nodes[i]->id > max_node_id) max_node_id = graph->nodes[i]->id;
    }
    ...
    if (!node_id_to_var_idx) { lv00_free((void **)&point_seen); lv00_free((void **)&point_ids); lv00_free((void **)&redundant); return redundant; }
    for (int i = 0; i <= max_node_id; i++) node_id_to_var_idx[i] = -1;
    
    /* Collect points involved in INCIDENCE or BETWEENNESS constraints */
    for (int i = 0; i < graph->constraint_count; i++) {
        Constraint *c = graph->constraints[i];
        if (c->type != INCIDENCE && c->type != BETWEENNESS) continue;
        for (int j = 0; j < c->participant_count; j++) {
            int nid = c->participants[j];
            if (nid < 0 || nid > max_node_id) continue;
            GeomNode *n = graph_get_node(graph, nid);
            if (!n || n->type != GEOM_POINT) continue;
    ...
    if (!linear_constraint_indices) { lv00_free((void **)&node_id_to_var_idx); lv00_free((void **)&point_seen); lv00_free((void **)&point_ids); lv00_free((void **)&redundant); return redundant; }
    for (int i = 0; i < graph->constraint_count; i++) {
        Constraint *c = graph->constraints[i];
    ...
    for (int row = 0; row < num_linear; row++) {
        ...
        if (c->type == INCIDENCE && c->participant_count >= 2) {
            ...
            if (pt && seg && seg->type == GEOM_LINE_SEGMENT &&
                seg->coord_count >= 4 && seg->symbolic_coords &&
                ...
                if (seg->symbolic_coords[0]->type == RATIONAL) {
                    mpq_init(ax_q); mpq_set(ax_q, seg->symbolic_coords[0]->data.rational->value); ax_exact = true;
                }
                if (seg->symbolic_coords[1]->type == RATIONAL) {
                    mpq_init(ay_q); mpq_set(ay_q, seg->symbolic_coords[1]->data.rational->value); ay_exact = true;
                }
                if (seg->symbolic_coords[2]->type == RATIONAL) {
                    mpq_init(bx_q); mpq_set(bx_q, seg->symbolic_coords[2]->data.rational->value); bx_exact = true;
                }
                if (seg->symbolic_coords[3]->type == RATIONAL) {
                    mpq_init(by_q); mpq_set(by_q, seg->symbolic_coords[3]->data.rational->value); by_exact = true;
                }
                ...
                if (ax_exact && ay_exact && bx_exact && by_exact) {
                    ...
                    mpq_init(dy_q); mpq_init(dx_q); mpq_init(const_q);
                    mpq_sub(dy_q, by_q, ay_q);
                    mpq_sub(dx_q, bx_q, ax_q);
                    /* const = Ax*dy - Ay*dx */
                    ...
                    mpq_clear(dy_q); mpq_clear(dx_q); mpq_clear(const_q);
                } else {
                    double dx = bx_d - ax_d;
                    double dy = by_d - ay_d;
                ...
                if (ax_exact) mpq_clear(ax_q);
                if (ay_exact) mpq_clear(ay_q);
                if (bx_exact) mpq_clear(bx_q);
                if (by_exact) mpq_clear(by_q);
            }
            ...
            if (p1 && p2 && p3 &&
                p1->coord_count >= 2 && p1->symbolic_coords &&
                p2->coord_count >= 2 && p2->symbolic_coords &&
                p3->coord_count >= 2 && p3->symbolic_coords) {
                ...
                if (p1->symbolic_coords[0]->type == RATIONAL) {
                    mpq_init(p1x_q); mpq_set(p1x_q, p1->symbolic_coords[0]->data.rational->value); p1x_exact = true;
                }
                if (p1->symbolic_coords[1]->type == RATIONAL) {
                    mpq_init(p1y_q); mpq_set(p1y_q, p1->symbolic_coords[1]->data.rational->value); p1y_exact = true;
                }
                if (p3->symbolic_coords[0]->type == RATIONAL) {
                    mpq_init(p3x_q); mpq_set(p3x_q, p3->symbolic_coords[0]->data.rational->value); p3x_exact = true;
                }
                if (p3->symbolic_coords[1]->type == RATIONAL) {
                    mpq_init(p3y_q); mpq_set(p3y_q, p3->symbolic_coords[1]->data.rational->value); p3y_exact = true;
                }
                ...
                if (p1x_exact && p1y_exact && p3x_exact && p3y_exact) {
                    ...
                    mpq_init(dy13_q); mpq_init(dx13_q); mpq_init(const_q);
                    mpq_sub(dy13_q, p3y_q, p1y_q);
                    mpq_sub(dx13_q, p3x_q, p1x_q);
                    mpq_mul(const_q, p1x_q, dy13_q);
                    mpq_t tmp13_q; mpq_init(tmp13_q);
                    mpq_mul(tmp13_q, p1y_q, dx13_q);
                    mpq_sub(const_q, const_q, tmp13_q);
                    mpq_clear(tmp13_q);
                    mpq_set(matrix[row * (num_vars + 1) + num_vars], const_q);
                    mpq_clear(dy13_q); mpq_clear(dx13_q); mpq_clear(const_q);
                } else {
                    double dy13 = p3y - p1y;
                    double dx13 = p3x - p1x;
                ...
                if (p1x_exact) mpq_clear(p1x_q);
                if (p1y_exact) mpq_clear(p1y_q);
                if (p3x_exact) mpq_clear(p3x_q);
                if (p3y_exact) mpq_clear(p3y_q);
            }
        }
    }
    ...
    if (!pivot_row) {
        for (int i = 0; i < num_linear * (num_vars + 1); i++) mpq_clear(matrix[i]);
    ...
    for (int i = 0; i < num_linear; i++) pivot_row[i] = linear_constraint_indices[i];
    ...
    for (int col = 0; col < num_vars && rank < num_linear; col++) {
        ...
        if (pivot < 0) continue; /* All zeros in this column */
        
        /* Swap rows rank and pivot */
        if (pivot != rank) {
            for (int j = 0; j <= num_vars; j++) {
                mpq_swap(matrix[rank * (num_vars + 1) + j],
                         matrix[pivot * (num_vars + 1) + j]);
            }
        ...
        for (int j = col; j <= num_vars; j++) {
            mpq_mul(matrix[rank * (num_vars + 1) + j],
                    matrix[rank * (num_vars + 1) + j], inv_pivot);
        }
        mpq_clear(inv_pivot);
        ...
        for (int row = 0; row < num_linear; row++) {
            ...
            if (mpq_sgn(matrix[row * (num_vars + 1) + col]) == 0) continue;
            ...
            for (int j = col; j <= num_vars; j++) {
                ...
                mpq_sub(matrix[row * (num_vars + 1) + j],
                        matrix[row * (num_vars + 1) + j], tmp);
                mpq_clear(tmp);
            }
            mpq_clear(factor);
        }
    ...
    for (int row = rank; row < num_linear; row++) {
        ...
        if (all_zero) {
            ...
            for (int m = 0; m < *out_count; m++) {
                if (redundant[m] == con_id) { already = true; break; }
            }
            if (!already && *out_count < max_redundant) {
                redundant[*out_count] = con_id;
                (*out_count)++;
                LOG_DEBUG("constraint_graph",
                          "Linear dependency: constraint %d is redundant (Gaussian elimination)",
                          con_id);
            }
        }
    }
    ...
    for (int i = 0; i < rank; i++) {
        for (int j = i + 1; j < rank; j++) {
            ...
            for (int k = 0; k <= num_vars; k++) {
                if (mpq_equal(matrix[i * (num_vars + 1) + k],
                              matrix[j * (num_vars + 1) + k]) == 0) {
                    identical = false;
                    break;
                }
            }
...
static bool constraints_are_independent(Constraint *c1, Constraint *c2) {
    ...
    if (c1->participant_count != c2->participant_count) return true;
...
static bool parse_distance_value(const char *decl, double *out_value) {
    if (!decl || !out_value) return false;
    ...
    if (endptr == eq + 1) return false; /* 未找到有效数字 */
...
static void add_conflict_group(int **conflicts, int *conflict_count, int **conflict_sizes,
                               int *node_ids, int node_count) {
...
static bool has_connection_cycle(ConstraintGraph *graph, int start_port_id,
                                  bool *visited, bool *rec_stack, int *path, int path_len,
                                  int **conflicts, int *conflict_count, int **conflict_sizes,
    ...
    for (int ci = 0; ci < cnt; ci++) {
        ...
        if (next_port >= 0) {
            ...
            if (!visited[next_port]) {
                if (has_connection_cycle(graph, next_port, visited, rec_stack,
                                         path, path_len + 1, conflicts, conflict_count, conflict_sizes,
                                         conn_adj, conn_counts)) {
                    return true;
                }
            }
        }
...
int **graph_detect_conflicts(ConstraintGraph *graph, int *out_conflict_count, int **out_conflict_sizes) {
    ...
    if (!graph || !out_conflict_count || !out_conflict_sizes) {
        ...
        if (out_conflict_sizes) *out_conflict_sizes = NULL;
        return NULL;
    }
    ...
    *out_conflict_sizes = lv00_malloc(max_conflicts * sizeof(int));
        ...
        for (int i = 0; i < graph->constraint_count; i++) {
            ...
            for (int j = 0; j < c->participant_count; j++) {
                ...
                if (adj_counts[nid] < ADJ_MAX_PER_NODE) {
                    ...
                    LOG_DEBUG("constraint_graph",
                              "节点 %d 超出邻接限制 (%d)，约束 %d 被忽略",
                              nid, ADJ_MAX_PER_NODE, i);
                }
                /* 类型特定邻接关系 */
                int *ta = NULL; int *tc = NULL;
                if (c->type == INCIDENCE)    { ta = inc_adj;  tc = inc_counts; }
                if (c->type == CONNECTION)   { ta = conn_adj; tc = conn_counts; }
                if (c->type == INTERSECTION) { ta = int_adj;  tc = int_counts; }
                if (ta && tc && tc[nid] < ADJ_MAX_PER_NODE) {
                    ta[nid * ADJ_MAX_PER_NODE + tc[nid]++] = i;
                } else if (ta && tc) {
                    LOG_DEBUG("constraint_graph",
                              "节点 %d 超出类型特定邻接限制 (%d)，类型 %d",
                              nid, ADJ_MAX_PER_NODE, c->type);
                }
            }
        }
    }
    ...
    for (int i = 0; i < graph->node_count; i++) {
        ...
        if (node->type != GEOM_POINT) continue;
        ...
        if (independent_count > 2) {
            ...
            add_conflict_group(conflicts, out_conflict_count, out_conflict_sizes,
                               conflict_nodes, cn_count);
        }
    }
    ...
    for (int i = 0; i < graph->node_count; i++) {
        ...
        if (seg1->type != GEOM_LINE_SEGMENT) continue;
        ...
        for (int j = i + 1; j < graph->node_count; j++) {
            ...
            if (seg2->type != GEOM_LINE_SEGMENT) continue;
            ...
            if (has_dist1 && has_dist2) {
                ...
                if (seg1->coord_count == 2 && seg2->coord_count == 2 &&
                    seg1->symbolic_coords && seg2->symbolic_coords) {
                    ...
                    for (int ei = 0; ei < 2 && !share_endpoints; ei++) {
                        for (int ej = 0; ej < 2 && !share_endpoints; ej++) {
                            if (seg1->symbolic_coords[ei] && seg2->symbolic_coords[ej]) {
                                if (symbolic_coord_compare(seg1->symbolic_coords[ei],
                                                         seg2->symbolic_coords[ej]) == 0) {
    ...
    for (int i = 0; i < graph->constraint_count; i++) {
        Constraint *c = graph->constraints[i];
        if (c->type != BETWEENNESS || c->participant_count != 3) continue;
        ...
        for (int j = 0; j < graph->node_count; j++) {
            ...
            if (line->type != GEOM_LINE_SEGMENT) continue;
            ...
            for (int mi = 0; mi < mic; mi++) {
                ...
                if (ic->participants[0] == middle_id) { middle_on_line = true; break; }
            }
            ...
            for (int e1i = 0; e1i < e1c; e1i++) {
                ...
                if (ic->participants[0] == end1_id) { end1_on_line = true; break; }
            }
            ...
            for (int e2i = 0; e2i < e2c; e2i++) {
                ...
                if (ic->participants[0] == end2_id) { end2_on_line = true; break; }
            }
        ...
        if (visited && rec_stack && path) {
            for (int i = 0; i < graph->node_count; i++) {
                ...
                if (node->type == GEOM_PORT && node->data.port->type == PORT_OUTPUT) {
                    if (!visited[node->id]) {
                        has_connection_cycle(graph, node->id, visited, rec_stack, path, 0,
                                            conflicts, out_conflict_count, out_conflict_sizes,
                                            conn_adj, conn_counts);
                    }
                }
            }
    ...
    for (int i = 0; i < graph->node_count; i++) {
        ...
        if (point->type != GEOM_POINT) continue;
        ...
        for (int j = 0; j < il_count; j++) {
            ...
            if (!line1 || line1->type != GEOM_LINE_SEGMENT) continue;
            
            for (int k = j + 1; k < il_count; k++) {
                GeomNode *line2 = graph_get_node(graph, incident_lines[k]);
                if (!line2 || line2->type != GEOM_LINE_SEGMENT) continue;
    ...
    if (*out_conflict_count == 0) {
        lv00_free((void **)&adj_lists); lv00_free((void **)&adj_counts);
        lv00_free((void **)&inc_adj); lv00_free((void **)&inc_counts);
        lv00_free((void **)&conn_adj); lv00_free((void **)&conn_counts);
        lv00_free((void **)&int_adj); lv00_free((void **)&int_counts);
        lv00_free((void **)&conflicts);
        lv00_free((void **)&*out_conflict_sizes);
        *out_conflict_sizes = NULL;
        return NULL;
...
bool graph_validate_region_closure(ConstraintGraph *graph, int region_id) {
    ...
    while (visited_count < segment_count) {
        ...
        for (int ep = 0; ep < 2; ep++) {
            if (ep == current_endpoint && prev_segment >= 0) continue; /* Skip the endpoint we came from */
            
            /* Find which segment this endpoint connects to */
            for (int j = 0; j < segment_count; j++) {
                if (j == current_segment || j == prev_segment) continue;
        ...
        if (seg_ids) {
            ...
            for (int i = 0; i < segment_count; i++) {
                for (int j = i + 2; j < segment_count; j++) {
                    ...
                    if (i == 0 && j == segment_count - 1) continue;
                    
                    if (segments_intersect(segments[i], segments[j])) {
                        LOG_WARN("constraint_graph",
                                 "Region %d has self-intersection between segments %d and %d",
...
static const char *trust_color_to_string(TrustColor trust) {
    switch (trust) {
        ...
        case TRUST_BLUE: return "BLUE";
        case TRUST_YELLOW: return "YELLOW";
        case TRUST_ORANGE: return "ORANGE";
        case TRUST_LIGHT_ORANGE: return "LIGHT_ORANGE";
...
char *graph_node_serialize_to_json(const GeomNode *node) {
    ...
    JsonBuf buf;
    if (!json_buf_init(&buf, 1024)) return NULL;
    ...
    switch (node->type) {
        ...
        case GEOM_REGION: {
            ...
            for (int i = 0; i < node->data.region.segment_count; i++) {
                ...
                snprintf(id_str, sizeof(id_str), "%d", 
                         node->data.region.boundary_segments[i]->id);
                json_buf_append(&buf, id_str);
            }
            json_buf_append(&buf, "],");
        ...
        case GEOM_FUNCTION_BLOCK: {
            ...
            for (int i = 0; i < node->data.func_block.internal_node_count; i++) {
                if (i > 0) json_buf_append_char(&buf, ',');
                snprintf(id_str, sizeof(id_str), "%d", 
                         node->data.func_block.internal_nodes[i]->id);
                json_buf_append(&buf, id_str);
            }
            json_buf_append(&buf, "],");
            ...
            for (int i = 0; i < node->data.func_block.input_count; i++) {
                if (i > 0) json_buf_append_char(&buf, ',');
                snprintf(id_str, sizeof(id_str), "%d", 
                         node->data.func_block.input_port_ids[i]);
                json_buf_append(&buf, id_str);
            }
            json_buf_append(&buf, "],");
            ...
            for (int i = 0; i < node->data.func_block.output_count; i++) {
                if (i > 0) json_buf_append_char(&buf, ',');
                snprintf(id_str, sizeof(id_str), "%d", 
                         node->data.func_block.output_port_ids[i]);
                json_buf_append(&buf, id_str);
            }
            json_buf_append(&buf, "],");
            ...
            switch (node->data.func_block.determinism_state) {
                case UNVERIFIED: json_buf_append(&buf, "\"UNVERIFIED\""); break;
                case VERIFIED: json_buf_append(&buf, "\"VERIFIED\""); break;
                case NON_DETERMINISTIC: json_buf_append(&buf, "\"NON_DETERMINISTIC\""); break;
                case PARTIALLY_VERIFIED: json_buf_append(&buf, "\"PARTIALLY_VERIFIED\""); break;
            }
            break;
        }
    }
...
char *graph_constraint_serialize_to_json(const Constraint *constraint) {
    ...
    JsonBuf buf;
    if (!json_buf_init(&buf, 256)) return NULL;
...
static char *json_parser_parse_string(JsonParser *p) {
    if (!json_parser_expect(p, '"')) return NULL;
...
static bool json_parser_parse_int(JsonParser *p, int *out) {
    ...
    if (p->pos == start || (p->pos == start + 1 && negative)) return false;
...
static GeomType string_to_geom_type(const char *str) {
    ...
    if (strcmp(str, "LINE_SEGMENT") == 0) return GEOM_LINE_SEGMENT;
    if (strcmp(str, "REGION") == 0) return GEOM_REGION;
    if (strcmp(str, "PORT") == 0) return GEOM_PORT;
    if (strcmp(str, "FUNCTION_BLOCK") == 0) return GEOM_FUNCTION_BLOCK;
    return GEOM_POINT;
}
        ...
        if (strcmp(key, "nodes") == 0) {
            ...
            while (json_parser_peek(&p) != ']' && json_parser_peek(&p) != '\0') {
                ...
                while (json_parser_peek(&p) != '}' && json_parser_peek(&p) != '\0') {
                    ...
                    if (strcmp(node_key, "id") == 0) {
                        ...
                        if (trust_str) {
                            if (strcmp(trust_str, "GREEN") == 0) trust = TRUST_GREEN;
                            else if (strcmp(trust_str, "BLUE") == 0) trust = TRUST_BLUE;
                            else if (strcmp(trust_str, "YELLOW") == 0) trust = TRUST_YELLOW;
                            else if (strcmp(trust_str, "ORANGE") == 0) trust = TRUST_ORANGE;
                            else if (strcmp(trust_str, "LIGHT_ORANGE") == 0) trust = TRUST_LIGHT_ORANGE;
                            else if (strcmp(trust_str, "AMBER") == 0) trust = TRUST_AMBER;
                            else trust = TRUST_GREEN;
                        ...
                        if (json_parser_expect(&p, '[')) {
                            ...
                            for (int i = 0; i < coord_count && json_parser_peek(&p) != ']' && json_parser_peek(&p) != '\0'; i++) {
                                if (json_parser_peek(&p) == ',') p.pos++;
                                ...
                                if (json_parser_peek(&p) == '{') {
                                    ...
                                    if (coord_key && strcmp(coord_key, "type") == 0) {
                                        ...
                                        if (ct) {
                                            if (strcmp(ct, "RATIONAL") == 0) coord_type = 0;
                                            else if (strcmp(ct, "ALGEBRAIC") == 0) coord_type = 1;
                                            else if (strcmp(ct, "QUADRATIC") == 0) coord_type = 2;
                                            lv00_free((void **)&ct);
                                        }
                                        ...
                                        lv00_free((void **)&coord_key);
                                        json_parser_skip_value(&p);
                                        while (json_parser_peek(&p) != '}') json_parser_skip_value(&p);
                                    }
                                    if (json_parser_peek(&p) == '}') p.pos++;
                                } else {
                                    json_parser_skip_value(&p);
                ...
                if (node) {
                    ...
                    if (node_type == GEOM_REGION && boundary_segs && boundary_seg_count > 0) {
                        ...
                        if (internal_nodes && internal_node_count > 0) {
                            node->data.func_block.internal_nodes = lv00_malloc(internal_node_count * sizeof(GeomNode *));
                        ...
                        if (input_port_ids && input_port_count > 0) {
                            ...
                            if (node->data.func_block.input_port_ids) {
                                memcpy(node->data.func_block.input_port_ids, input_port_ids, input_port_count * sizeof(int));
                            }
                        }
                        if (output_port_ids && output_port_count > 0) {
                            ...
                            if (node->data.func_block.output_port_ids) {
                                memcpy(node->data.func_block.output_port_ids, output_port_ids, output_port_count * sizeof(int));
                            }
                        }
                        node->data.func_block.determinism_state = UNVERIFIED;
                    }
                ...
                if (coords) {
                    for (int i = 0; i < coord_count; i++) {
                        if (coords[i]) symbolic_coord_destroy(coords[i]);
                    }
            ...
            while (json_parser_peek(&p) != ']' && json_parser_peek(&p) != '\0') {
                ...
                if (participants && participant_count > 0) {
                    Constraint *constraint = graph_add_constraint_with_id(graph, constraint_id, constraint_type, participants, participant_count);
                    if (constraint) {
