...
ProbDistribution *prob_dist_create(ProbDistType type,
    ...
    if (param_count > 0 && params) {
        ...
        if (!dist->params) { free(dist); return NULL; }
...
int prob_dist_sample(ProbDistribution *dist,
                      int n_samples,
                      double **out_samples) {
    if (!dist || n_samples <= 0 || !out_samples) return -1;
...
ProbConstraintNode *prob_constraint_create(int node_id,
    ...
    ProbConstraintNode *node = (ProbConstraintNode *)malloc(
        sizeof(ProbConstraintNode));
    if (!node) return NULL;
...
int prob_constraint_sample(ProbConstraintNode *node,
                            int n_samples,
                            double **out_samples) {
    if (!node || n_samples <= 0 || !out_samples) return -1;

    if (!node->coord_dist) {
        /* 鏃犲垎甯冿細杩斿洖 0.0锛堢‘瀹氭€у潗鏍囷級 */
        double *samples = (double *)malloc((size_t)n_samples * sizeof(double));
        if (!samples) return -1;
        for (int i = 0; i < n_samples; i++) samples[i] = 0.0;
        *out_samples = samples;
        return n_samples;
    }
...
bool pctl_evaluate(const ConstraintGraph *graph,
    ...
    if (!graph || !formula || !out_probability) return false;
    ...
    *out_probability = 0.0;
    ...
    case PCTL_EVENTUALLY:
        ...
        *out_probability = (formula->state_predicate &&
            strlen(formula->state_predicate) > 0) ? 1.0 : 0.0;
        break;
    ...
    case PCTL_STEADY_STATE:
        ...
        *out_probability = (graph->node_count > 0) ?
            (1.0 / (double)graph->node_count) : 0.0;
        break;
...
bool prob_constraint_infer(const ConstraintGraph *graph,
                            ...
                            int n,
                            double *out_conf) {
    if (!graph || !constraints || n <= 0 || !out_conf) return false;
