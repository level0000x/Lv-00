 ...
 *
 * @note 内存管理策略：
 *   - GMP 多精度多项式系数数组（poly.coeffs）使用标准 malloc/free，
 *     因为 mpz_poly_clear() 内部调用 free() 释放系数数组。
 ...
 */
static int equation_system_push(EquationSystem *sys, mpz_poly_t poly,
                                  int var_node_id, int coord_index) {
    if (sys->count >= sys->capacity) {
        if (sys->capacity > INT_MAX / 2) return -1;
        /* 使用临时变量计算新容量，避免 realloc 失败时 capacity 已被修改 */
        int new_capacity = sys->capacity == 0 ? SOLVER_DYNARRAY_INIT_CAP : sys->capacity * LV00_ARRAY_GROWTH_FACTOR;
        PolyEquation *new_eqs = lv00_realloc(sys->eqs, new_capacity * sizeof(PolyEquation));
        if (!new_eqs) {
...
static bool coord_to_double(const SymbolicCoord *c, double *out) {
    ...
    switch (c->type) {
        case RATIONAL: {
            ...
            /* 回退到字符串序列化方式 */
            char *s = symbolic_coord_serialize(c);
            if (!s) return false;
            *out = strtod(s, NULL);
            lv00_free((void **)&s);
            return true;
...
static bool coord_to_mpz_scaled(const SymbolicCoord *c, mpz_t result, long scale) {
    ...
    if (c->type == RATIONAL && c->data.rational) {
        /* 精确路径：调用公共辅助函数计算向下取整结果 */
        mpq_srcptr val = c->data.rational->value;
        rational_to_mpz_scaled(val, result, scale);
...
static bool point_coord(GeomNode *pt, int idx, double *out) {
    if (!pt || pt->type != GEOM_POINT || pt->coord_count <= idx) return false;
...
static bool line_from_two_points(GeomNode *p1, GeomNode *p2, LineEquation *out) {
    double x1, y1, x2, y2;
    if (!point_coord(p1, 0, &x1) || !point_coord(p1, 1, &y1)) return false;
    if (!point_coord(p2, 0, &x2) || !point_coord(p2, 1, &y2)) return false;
    /* 方向向量 (dx, dy) */
    double dx = x2 - x1;
    double dy = y2 - y1;
 ...
 */
static void extract_equations_from_constraints(const ConstraintGraph *graph,
                                                EquationSystem *sys) {
    for (int ci = 0; ci < graph->constraint_count; ci++) {
        ...
        case INCIDENCE: {
            ...
            if (line->type == GEOM_LINE_SEGMENT && line->coord_count >= 2) {
                ...
                if (line->coord_count >= 4) {
                    ...
                    mpz_init(lx1_s); mpz_init(ly1_s);
                    mpz_init(lx2_s); mpz_init(ly2_s);
                    ...
                    if (exact) {
                        ...
                        if (!poly.coeffs) { mpz_poly_clear(&poly); mpz_clear(dx_s); mpz_clear(dy_s);
                                           mpz_clear(lx1_s); mpz_clear(ly1_s);
                                           mpz_clear(lx2_s); mpz_clear(ly2_s); continue; }
                        ...
                        if (coord_to_double(line->symbolic_coords[0], &lx1) &&
                            ...
                            poly.degree = 1;
                            poly.coeffs = malloc(2 * sizeof(mpz_t));
                            if (!poly.coeffs) { mpz_poly_clear(&poly); break; }
                            mpz_init(poly.coeffs[1]);
                            mpz_init(poly.coeffs[0]);
                    ...
                    mpz_clear(lx1_s); mpz_clear(ly1_s);
                    mpz_clear(lx2_s); mpz_clear(ly2_s);
                }
            }
            break;
        }
        ...
        case INTERSECTION: {
            ...
            if (c->participant_count < 3) break;
            ...
            if (!line1 || !line2 || !rpt) break;

            long scale = LV00_SOLVER_SCALE_FACTOR;

            /* 尝试精确有理数路径：直接从线段端点坐标计算直线方程 */
            if (line1->type == GEOM_LINE_SEGMENT && line1->coord_count >= 4 &&
                line2->type == GEOM_LINE_SEGMENT && line2->coord_count >= 4) {

                mpz_t l1x1_s, l1y1_s, l1x2_s, l1y2_s;
                mpz_t l2x1_s, l2y1_s, l2x2_s, l2y2_s;
                mpz_init(l1x1_s); mpz_init(l1y1_s); mpz_init(l1x2_s); mpz_init(l1y2_s);
                mpz_init(l2x1_s); mpz_init(l2y1_s); mpz_init(l2x2_s); mpz_init(l2y2_s);
                ...
                if (exact1 && exact2) {
                    ...
                    mpz_init(a1_s); mpz_init(b1_s); mpz_init(c1_s);
                    mpz_init(a2_s); mpz_init(b2_s); mpz_init(c2_s);
                    mpz_init(dx1_s); mpz_init(dy1_s); mpz_init(dx2_s); mpz_init(dy2_s);
                    ...
                    mpz_init(D_s); mpz_init(x_num_s); mpz_init(y_num_s);
                    {
                        mpz_t t1, t2;
                        mpz_init(t1); mpz_init(t2);
                    ...
                    mpz_clear(D_s); mpz_clear(x_num_s); mpz_clear(y_num_s);
                    mpz_clear(a1_s); mpz_clear(b1_s); mpz_clear(c1_s);
                    mpz_clear(a2_s); mpz_clear(b2_s); mpz_clear(c2_s);
                    mpz_clear(dx1_s); mpz_clear(dy1_s); mpz_clear(dx2_s); mpz_clear(dy2_s);
                    mpz_clear(l1x1_s); mpz_clear(l1y1_s); mpz_clear(l1x2_s); mpz_clear(l1y2_s);
                    mpz_clear(l2x1_s); mpz_clear(l2y1_s); mpz_clear(l2x2_s); mpz_clear(l2y2_s);
                    break;
                }
            ...
            /* 回退到 double 近似路径 */
            {
                ...
                if (got1 && got2) {
                    ...
                    mpz_poly_init(&poly);
                    poly.degree = 1;
                    poly.coeffs = malloc(SOLVER_LINEAR_COEFF_COUNT * sizeof(mpz_t));
                    if (!poly.coeffs) { mpz_poly_clear(&poly); break; }
                    mpz_init(poly.coeffs[1]);
                    mpz_init(poly.coeffs[0]);
        ...
        case BETWEENNESS: {
            ...
            if (!p1 || !p2 || !p3) break;
            if (p1->type != GEOM_POINT || p3->type != GEOM_POINT) break;
            ...
            mpz_init(x1_s); mpz_init(y1_s);
            mpz_init(x3_s); mpz_init(y3_s);

            if (p1->symbolic_coords && p3->symbolic_coords &&
                p1->coord_count >= 2 && p3->coord_count >= 2 &&
            ...
            mpz_clear(x1_s); mpz_clear(y1_s);
            mpz_clear(x3_s); mpz_clear(y3_s);
            break;
        }
        ...
        case CONTAINMENT: {
            ...
            /* 当前仅处理点-区域包含 */
            if (inner->type != GEOM_POINT || outer->type != GEOM_REGION) break;
            if (outer->data.region.segment_count <= 0 ||
                !outer->data.region.boundary_segments) break;

            {
                ...
                for (int si = 0; si < seg_count; si++) {
                    ...
                    if (seg->coord_count < 4 || !seg->symbolic_coords) continue;

                    /* 精确有理数路径：获取边界线段端点坐标的缩放整数值 */
                    mpz_t sx1_s, sy1_s, sx2_s, sy2_s;
                    mpz_init(sx1_s); mpz_init(sy1_s);
                    mpz_init(sx2_s); mpz_init(sy2_s);
                    ...
                    mpz_clear(sx1_s); mpz_clear(sy1_s);
                    mpz_clear(sx2_s); mpz_clear(sy2_s);
                }
            }
            break;
        }
        ...
        case CONNECTION: {
            ...
            for (int ni = 0; ni < 2; ni++) {
                ...
                if (!n || !n->numeric_assumption_declaration) continue;
                const char *decl = n->numeric_assumption_declaration;
                const char *prefix = "distance=";
                if (strncmp(decl, prefix, strlen(prefix)) == 0) {
                    dist_val = strtod(decl + strlen(prefix), NULL);
                    dist_node = n;
                    break;
                }
            }
            ...
            /* 两个节点均需要至少 2 个坐标 (x, y) */
            if (nodeA->coord_count < 2 || nodeB->coord_count < 2) break;
            if (!nodeA->symbolic_coords || !nodeB->symbolic_coords) break;
            ...
            mpz_poly_init(&poly);
            poly.degree = 2;
            poly.coeffs = malloc(SOLVER_QUADRATIC_COEFF_COUNT * sizeof(mpz_t));
            if (!poly.coeffs) { mpz_poly_clear(&poly); break; }
    ...
    for (int ni = 0; ni < graph->node_count; ni++) {
        ...
        if (!node || !node->numeric_assumption_declaration) continue;
        ...
        const char *prefix = "distance=";
        if (strncmp(decl, prefix, strlen(prefix)) == 0) {
            dist_sq = strtod(decl + strlen(prefix), NULL);
        ...
        if (node->coord_count >= 4) {
            double x1, y1;
            if (coord_to_double(node->symbolic_coords[0], &x1) &&
                coord_to_double(node->symbolic_coords[1], &y1)) {
                ...
                poly.degree = 2;
            poly.coeffs = malloc(SOLVER_QUADRATIC_COEFF_COUNT * sizeof(mpz_t));
            if (!poly.coeffs) { mpz_poly_clear(&poly); continue; }
            mpz_init(poly.coeffs[2]);
            mpz_init(poly.coeffs[1]);
            mpz_init(poly.coeffs[0]);
...
static VarInfo *build_var_info(EquationSystem *sys, int node_count,
                                int *out_var_count) {
    /* 收集所有不重复的变量节点 id */
    ...
    for (int i = 0; i < sys->count; i++) {
        ...
        for (int j = 0; j < var_count; j++) {
            if (var_ids[j] == vid) { found = true; break; }
        }
        if (!found) var_ids[var_count++] = vid;
    }

    /* 提前返回：如果没有变量，避免 calloc(0, ...) 的未定义行为 */
    if (var_count == 0) {
    ...
    if (info) memset(info, 0, (size_t)var_count * sizeof(VarInfo));
    ...
    for (int i = 0; i < sys->count; i++) {
        ...
        for (int j = 0; j < var_count; j++) {
            if (info[j].node_id == vid) {
                ...
                if (deg > info[j].max_degree) info[j].max_degree = deg;
                break;
            }
        }
    }
...
static bool solve_linear(mpz_poly_t *poly, double *x_out) {
    ...
    if (fabs(a) < LV00_EPSILON_NEWTON) return false;
    *x_out = -b / a;
    return true;
}
...
static bool solve_quadratic(mpz_poly_t *poly, QuadraticRoots *out) {
    ...
    if (fabs(a) < LV00_EPSILON_NEWTON) {
        /* 坍缩为一元一次方程 */
        if (fabs(b) < LV00_EPSILON_NEWTON) return false;
        out->roots[0] = -c / b;
        out->root_count = 1;
        return true;
    }
 ...
 */
static int solve_quadratic_exact(
    const mpz_poly_t *poly,
    SymbolicCoord **solutions,
    int max_solutions) {

    if (!poly || poly->degree != 2 || !solutions || max_solutions <= 0) return 0;
    ...
    if (mpz_cmp_si(a_mpz, 0) == 0) {
        ...
        if (mpz_fits_slong_p(mpq_numref(x_val)) && mpz_fits_ulong_p(mpq_denref(x_val))) {
            solutions[0] = symbolic_coord_create_rational(
                (int64_t)mpz_get_si(mpq_numref(x_val)),
    ...
    if (is_perfect_square) {
        ...
        if (mpz_fits_slong_p(mpq_numref(x1)) && mpz_fits_ulong_p(mpq_denref(x1))) {
            solutions[0] = symbolic_coord_create_rational(
                (int64_t)mpz_get_si(mpq_numref(x1)),
                (uint64_t)mpz_get_ui(mpq_denref(x1)));
        } else {
            /* 超出 int64/uint64 范围，无法精确表示 */
            mpq_clear(x1);
            mpz_clear(num1); mpz_clear(denom);
            mpz_clear(a_mpz); mpz_clear(b_mpz); mpz_clear(c_mpz);
            mpz_clear(D); mpz_clear(sqrt_D);
            return 0;
        }
        mpq_clear(x1);
        ...
        if (max_solutions >= 2) {
            ...
            if (mpz_fits_slong_p(mpq_numref(x2)) && mpz_fits_ulong_p(mpq_denref(x2))) {
                solutions[1] = symbolic_coord_create_rational(
                    (int64_t)mpz_get_si(mpq_numref(x2)),
                    (uint64_t)mpz_get_ui(mpq_denref(x2)));
            } else {
                /* 超出 int64/uint64 范围，解2 无法精确表示，仅返回解1 */
                mpq_clear(x2);
                mpz_clear(num1); mpz_clear(num2); mpz_clear(denom);
                mpz_clear(a_mpz); mpz_clear(b_mpz); mpz_clear(c_mpz);
                mpz_clear(D); mpz_clear(sqrt_D);
                return 1;
            }
    ...
    /* 创建 QUADRATIC 类型的解 */
    Rational *qa = rational_create_from_mpz(
        mpq_numref(rational_part), mpq_denref(rational_part));
    ...
    /* 解1: a + (-sqrt_coeff)*sqrt(n) -- 负的 sqrt 系数 */
    {
        ...
        Rational *qb1 = rational_create_from_mpz(
            neg_num, mpq_denref(sqrt_coeff));
        mpz_clear(neg_num);
    ...
    if (max_solutions >= 2) {
        Rational *qb2 = rational_create_from_mpz(
            mpq_numref(sqrt_coeff), mpq_denref(sqrt_coeff));
 ...
 */
static int solve_cubic_exact(
    const mpz_poly_t *poly,
    SymbolicCoord **solutions,
    int max_solutions) {

    if (!poly || poly->degree != 3 || !solutions || max_solutions <= 0) return 0;
    ...
    /* 检查 a != 0 */
    if (mpz_cmp_si(a_mpz, 0) == 0) {
        mpz_clear(a_mpz); mpz_clear(b_mpz);
        mpz_clear(c_mpz); mpz_clear(d_mpz);
        return 0;
    }
...
static SymbolicCoord *symbolic_coord_from_mpq(const mpq_t val) {
    ...
    if (mpz_fits_slong_p(mpq_numref(canonical)) &&
        mpz_fits_ulong_p(mpq_denref(canonical))) {
        result = symbolic_coord_create_rational(
            (int64_t)mpz_get_si(mpq_numref(canonical)),
 ...
 */
static SymbolicCoord *poly_eval_symbolic(const mpz_poly_t *poly,
                                          const SymbolicCoord *value) {
    if (!poly || !value || poly->degree < 0 || !poly->coeffs) return NULL;
 ...
 */
static PolyEquation *substitute_symbolic_into_equation(
    const PolyEquation *eq,
    int var_index,
    const SymbolicCoord *value) {
 ...
 */
static void substitute_solved_symbolic(EquationSystem *sys, int var_node_id,
                                        int coord_index,
                                        const SymbolicCoord *value) {
    for (int i = 0; i < sys->count; i++) {
        if (sys->eqs[i].var_node_id == var_node_id &&
            sys->eqs[i].coord_index == coord_index) {
            /* 在给定值处符号求值多项式 */
 ...
 */
SymbolicCoord **back_substitute_symbolic(
    PolyEquation **equations, int eq_count,
    int n_vars,
    SymbolicCoord **solved_values,
    int *solved_indices, int solved_count) {
    ...
    while (progress && iteration < max_iterations) {
        ...
        for (int eq_idx = 0; eq_idx < eq_count; eq_idx++) {
            ...
            if (!eq || eq->poly.degree < 0) continue;

            int var_id = eq->var_node_id;
            if (var_id < 0 || var_id >= n_vars) continue;
            if (all_values[var_id] != NULL) continue; /* already solved */
...
bool compute_algebraic_resultant(
 ...
 * @param value       变量的数值
 */
static void substitute_solved(EquationSystem *sys, int var_node_id,
                               int coord_index, double value) {
 ...
 */
static bool try_factor_polynomial(const mpz_poly_t *poly,
                                   mpz_poly_t *factor1,
                                   mpz_poly_t *factor2) {
    ...
    if (mpz_cmp_si(const_term, 0) == 0) {
        ...
        factor1->degree = 1;
        factor1->coeffs = malloc(2 * sizeof(mpz_t));
        if (!factor1->coeffs) {
            mpz_poly_clear(factor1); mpz_poly_clear(factor2);
            mpz_clear(const_term); mpz_clear(lead_coeff);
            return false;
        }
        ...
        factor2->coeffs = malloc((factor2->degree + 1) * sizeof(mpz_t));
        if (!factor2->coeffs) {
            mpz_poly_clear(factor1); mpz_poly_clear(factor2);
            mpz_clear(const_term); mpz_clear(lead_coeff);
            return false;
    ...
    for (long r = -max_root; r <= max_root; r++) {
        ...
        if (is_root) {
            ...
            if (quotient.degree >= 0) {
                ...
                mpz_init_set(quotient.coeffs[quotient.degree],
                             poly->coeffs[poly->degree]);
                for (int i = quotient.degree - 1; i >= 0; i--) {
                    mpz_init(quotient.coeffs[i]);
                    mpz_mul(quotient.coeffs[i], quotient.coeffs[i + 1],
                            poly->coeffs[poly->degree]);
                    mpz_add(quotient.coeffs[i], quotient.coeffs[i],
                            poly->coeffs[i + 1]);
                }
...
static bool check_incompatible_distances(const ConstraintGraph *graph) {
    ...
    for (int i = 0; i < graph->node_count; i++) {
        ...
        double dist_i = -1.0;
        const char *prefix = "distance=";
        if (strncmp(di, prefix, strlen(prefix)) == 0) {
            dist_i = strtod(di + strlen(prefix), NULL);
        } else {
            char *end = NULL;
        ...
        for (int j = i + 1; j < graph->node_count; j++) {
            ...
            for (int k = 0; k < 4; k++) {
                int cmp = symbolic_coord_compare(ni->symbolic_coords[k],
                                                  nj->symbolic_coords[k]);
                if (cmp != 0) { same_endpoints = false; break; }
            }
            if (!same_endpoints) continue;

            const char *dj = nj->numeric_assumption_declaration;
            double dist_j = -1.0;
            if (strncmp(dj, prefix, strlen(prefix)) == 0) {
                dist_j = strtod(dj + strlen(prefix), NULL);
            } else {
                char *end = NULL;
...
static bool check_contradiction_after_substitution(EquationSystem *sys) {
    for (int i = 0; i < sys->count; i++) {
        mpz_poly_t *p = &sys->eqs[i].poly;
        if (p->degree < 0) continue; /* already solved / trivial */
        if (p->degree == 0) {
...
static int constraint_weight(const Constraint *c) {
    switch (c->type) {
        case INCIDENCE:    return 1;  /* one linear equation (point on line) */
        case BETWEENNESS:  return 2;  /* collinearity + ratio bound */
        case INTERSECTION: return 2;  /* point on line1 AND line2 */
        case CONTAINMENT:  return 1;  /* at least one boundary constraint */
        case CONNECTION:   return 1;  /* port connectivity */
        default:           return 1;
    }
}
...
static int count_point_variables(ConstraintGraph *graph, int **out_ids) {
    int count = 0;
    for (int i = 0; i < graph->node_count; i++) {
        if (graph->nodes[i]->type == GEOM_POINT) count++;
    }
    if (out_ids) {
 ...
 */
static void solve_equations_pass(EquationSystem *sys, GroebnerResult *result,
        ...
        for (int i = 0; i < sys->count; i++) {
            ...
            if (p->degree == 1) {
                ...
                if (sol) {
                    ...
                    if (appended) {
                        ...
                        if (solver_stream_ctx) {
                            ...
                            LV00_SAFE_SNPRINTF(_snw_lv, detail, sizeof(detail),
                                     ...
                                     sys->eqs[i].var_node_id,
                                     sys->eqs[i].coord_index, pass, *solved_count);
                            LV00_UNUSED(_snw_lv);
                    ...
                    if (mpz_cmp_si(p->coeffs[1], 0) == 0 &&
                        mpz_cmp_si(p->coeffs[0], 0) != 0) {
                        *no_solution = true;
                    }
                }
            } else if (p->degree == 2) {
                ...
                if (exact_count == 0) {
                    *no_solution = true;
                    ...
                    for (int r = 0; r < exact_count; r++) {
                        if (exact_solutions[r] &&
                            append_solution(result, exact_solutions[r]) == 0) {
                            (*solved_count)++;
                            /* 流式输出: 变量解得（含详细元数据） */
                            if (solver_stream_ctx) {
                                StreamEvent ev;
                                ...
                                LV00_SAFE_SNPRINTF(_snw_qv, detail, sizeof(detail),
                                         ...
                                         "\"pass\":%d,\"total_solved\":%d}",
                                         sys->eqs[i].var_node_id,
                                         sys->eqs[i].coord_index, r,
                                         exact_count, pass, *solved_count);
                                LV00_UNUSED(_snw_qv);
                                ev.detail_json = detail;
                                stream_emit(solver_stream_ctx, &ev);
                            }
                ...
                if (cubic_count == 0) {
                    *no_solution = true;
                    ...
                    for (int r = 0; r < cubic_count; r++) {
                        if (cubic_solutions[r] &&
                            append_solution(result, cubic_solutions[r]) == 0) {
                            (*solved_count)++;
                            /* 流式输出: 变量解得（含详细元数据） */
                            if (solver_stream_ctx) {
                                StreamEvent ev;
...
static int *order_variables_by_dependency(
...
SolverStatus solve_algebraic_system(ConstraintGraph *graph,
                                     const int *dirty_variable_ids,
                                     int dirty_count,
                                     GroebnerResult **out_result) {
    if (!graph || !out_result) {
        debug_log(LOG_LEVEL_ERROR, "solver",
            "solve_algebraic_system: 无效参数 graph=%p out_result=%p",
    ...
    if (!result) {
        debug_log(LOG_LEVEL_ERROR, "solver",
            "solve_algebraic_system: 无法分配 GroebnerResult（大小 %zu 字节）",
    ...
    if (solver_stream_ctx) {
        ...
        char detail[128];
        int _snw_eq;
        LV00_SAFE_SNPRINTF(_snw_eq, detail, sizeof(detail),
                 "{\"equation_count\":%d,\"phase\":\"extraction\"}", sys.count);
        LV00_UNUSED(_snw_eq);
        ev.detail_json = detail;
        stream_emit(solver_stream_ctx, &ev);
    }
     ...
     * 已知值可以尽早代入，减少后续方程的复杂度。 */
    {
        ...
        if (all_var_count > 0) {
            ...
            if (solver_stream_ctx) {
                ...
                char detail[96];
                int _snw_ts;
                LV00_SAFE_SNPRINTF(_snw_ts, detail, sizeof(detail),
                         "{\"phase\":\"topology_sort\",\"var_count\":%d}", all_var_count);
                LV00_UNUSED(_snw_ts);
                ev.detail_json = detail;
                stream_emit(solver_stream_ctx, &ev);
            }
            ...
            int ordered_count = 0;
            int *ordered_ids = order_variables_by_dependency(
                graph, all_var_ids, all_var_count,
                dirty_variable_ids, dirty_count,
                &ordered_count);
            ...
            if (ordered_ids && ordered_count > 0) {
                ...
                for (int i = 0; i < sys.count; i++) priority[i] = INT_MAX;
    ...
    solve_equations_pass(&sys, result, &solved_count, &multiple_solutions,
                         &no_solution, true);
        ...
        if (remaining_before_gb > 0) {
            ...
            if (solver_stream_ctx) {
                ...
                LV00_SAFE_SNPRINTF(_snw_gb_prog, detail, sizeof(detail),
                         "{\"phase\":\"groebner_entry\",\"remaining\":%d,\"solved\":%d}",
                         remaining_before_gb, solved_count);
    ...
    if (solver_stream_ctx) {
        ...
        LV00_SAFE_SNPRINTF(_snw_sum, detail, sizeof(detail),
                 ...
                 "\"overdetermined\":%s,"
                 "\"max_degree\":%d}",
                 solved_count, remaining, multiple_solutions,
                 result->unique ? "true" : "false",
                 result->overdetermined ? "true" : "false",
                 max_degree_global);
...
SolverStatus eliminate_geometry(ConstraintGraph *graph,
                                 ...
                                 int *eliminate_ids,
                                 int elim_count) {
    if (!graph || elim_count <= 0) return SOLVER_OK;
    ...
    if (is_linear) memset(is_linear, 0, (size_t)sys.count * sizeof(bool));
    for (int i = 0; i < sys.count; i++) {
                    ...
                    if (fabs(val) > 9.2e12) {
                        ...
                        if (new_coord) {
                            symbolic_coord_destroy(
                                node->symbolic_coords[sys.eqs[i].coord_index]);
                ...
                if (c->participants[1] == eid) {
                    ...
                    if (p1 && p3 && p1->coord_count >= 2 && p3->coord_count >= 2) {
                        ...
                        if (point_coord(p1, 0, &x1) && point_coord(p1, 1, &y1) &&
                            point_coord(p3, 0, &x3) && point_coord(p3, 1, &y3)) {
                            ...
                            if (target) {
                                ...
                                char buf[256];
                                int _snw;
                                LV00_SAFE_SNPRINTF(_snw, buf, sizeof(buf),
                                         "betweenness:p1=(%.6f,%.6f),p3=(%.6f,%.6f)",
                                         x1, y1, x3, y3);
                                LV00_UNUSED(_snw);
                    ...
                    if (solve_linear(&tmpl_sys.eqs[i].poly, &val)) {
                        ...
                        if (node && node->coord_count > tmpl_sys.eqs[i].coord_index) {
                            if (fabs(val) > 9.2e12) {
                                ...
                                if (new_coord) {
                                    symbolic_coord_destroy(
                                        node->symbolic_coords[tmpl_sys.eqs[i].coord_index]);
    ...
    if (out_of_scope_found) return SOLVER_OUT_OF_SCOPE;
    if (any_eliminated) return SOLVER_OK;
...
SolverStatus analyze_out_of_scope(ConstraintGraph *graph,
                                   int var_id,
                                   char **suggestion) {
    if (!graph || !suggestion) return SOLVER_OUT_OF_SCOPE;
    ...
    if (solver_stream_ctx) {
        ...
        char detail[128];
        int _snw_diag;
        LV00_SAFE_SNPRINTF(_snw_diag, detail, sizeof(detail),
                 "{\"phase\":\"analyze_out_of_scope\",\"var_id\":%d}", var_id);
    ...
    if (solver_stream_ctx) {
        ...
        char detail[128];
        int _snw_found;
        LV00_SAFE_SNPRINTF(_snw_found, detail, sizeof(detail),
                 "{\"degree\":%d,\"var_id\":%d}", target_poly->degree, var_id);
    ...
    if (factored) {
        ...
        *suggestion = lv00_malloc(needed);
        ...
        if (solver_stream_ctx) {
            ...
            char detail[512];
            int _snw_fact;
            LV00_SAFE_SNPRINTF(_snw_fact, detail, sizeof(detail),
        ...
        if (biquadratic) {
            *suggestion = strdup(
            ...
            if (solver_stream_ctx) {
                ...
                ev.detail_json = "{\"diagnosis\":\"biquadratic\",\"resolvable\":true,"
...
int count_degrees_of_freedom(ConstraintGraph *graph, int **out_free_var_ids) {
    if (!graph) {
        if (out_free_var_ids) *out_free_var_ids = NULL;
    ...
    if (eq_per_point) memset(eq_per_point, 0, (size_t)pt_count * sizeof(int));
    bool *point_has_quadratic = lv00_malloc((size_t)pt_count * sizeof(bool));
    if (point_has_quadratic) memset(point_has_quadratic, 0, (size_t)pt_count * sizeof(bool));
        ...
        for (int j = 0; j < c->participant_count; j++) {
            ...
            if (n && n->type == GEOM_POINT) {
                for (int k = 0; k < pt_count; k++) {
                    if (pt_ids[k] == n->id) {
                        if (first_pt_idx < 0) first_pt_idx = k;
                        point_participants++;
                        break;
                    }
                }
    ...
    for (int i = 0; i < graph->node_count; i++) {
        GeomNode *n = graph->nodes[i];
        if (!n || !n->numeric_assumption_declaration) continue;
        if (n->type != GEOM_LINE_SEGMENT) continue;
...
bool check_conflict_equations(const ConstraintGraph *graph) {
    ...
    for (int i = 0; i < sys.count; i++) {
        mpz_poly_t *p = &sys.eqs[i].poly;
        if (p->degree == 1 && mpz_cmp_si(p->coeffs[1], 0) == 0 &&
            mpz_cmp_si(p->coeffs[0], 0) != 0) {
            equation_system_clear(&sys);
            return true;
        }
    ...
    if (max_id > 0) {
        ...
        if (eq_count_per_point) memset(eq_count_per_point, 0, (size_t)(max_id + 1) * sizeof(int));
        for (int i = 0; i < sys.count; i++) {
            int vid = sys.eqs[i].var_node_id;
            if (vid >= 0 && vid <= max_id) {
        ...
        for (int i = 0; i < graph->constraint_count; i++) {
            ...
            for (int j = 0; j < c->participant_count; j++) {
                ...
                if (n && n->type == GEOM_POINT && pid <= max_id) {
                    eq_count_per_point[pid] += constraint_weight(c) /
                                                c->participant_count;
                }
            }
        }
    ...
    for (int i = 0; i < graph->constraint_count; i++) {
        for (int j = i + 1; j < graph->constraint_count; j++) {
            ...
            if (ci->type != cj->type) continue;
            if (ci->participant_count != cj->participant_count) continue;
            ...
            if (same_participants && ci->type == BETWEENNESS &&
                ci->participant_count >= 3) {
...
static void mv_poly_add_term(MVPolynomial *p, const mpz_t coeff,
                              const int *exponents) {
    if (mpz_cmp_si(coeff, 0) == 0) return;
    ...
    if (p->term_count >= p->capacity) {
        ...
        int new_cap = p->capacity == 0 ? SOLVER_DYNARRAY_INIT_CAP : p->capacity;
        if (new_cap > 0 && new_cap > INT_MAX / LV00_ARRAY_GROWTH_FACTOR) {
            lv00_set_error(LV00_ERROR_OUT_OF_MEMORY, "mv_poly_add_term: 容量溢出");
            return;
        }
        new_cap = new_cap == 0 ? SOLVER_DYNARRAY_INIT_CAP : new_cap * LV00_ARRAY_GROWTH_FACTOR;
        p->capacity = new_cap;
...
static int mv_monomial_compare_grlex(const MVMonomial *a, const MVMonomial *b,
                                      int var_count) {
    int deg_a = mv_monomial_total_degree(a, var_count);
    int deg_b = mv_monomial_total_degree(b, var_count);
    if (deg_a != deg_b) return deg_b - deg_a; /* 降序: 高次在前 */
    /* 字典序比较 (从第一个变量开始) */
    for (int v = 0; v < var_count; v++) {
...
static void mv_poly_remove_zeros(MVPolynomial *p) {
    ...
    for (int i = 0; i < p->term_count; i++) {
        if (mpz_cmp_si(p->terms[i].coeff, 0) != 0) {
            if (write != i) p->terms[write] = p->terms[i];
            write++;
        } else {
            lv00_free((void **)&p->terms[i].exponents);
            mpz_clear(p->terms[i].coeff);
...
static const MVMonomial *mv_poly_leading_term(const MVPolynomial *p) {
    if (p->term_count == 0) return NULL;
    return &p->terms[0]; /* 已排序, 第一个即为首项 */
}

/* 获取首项的首单项式指数 (LCM of leading monomials) */
static void mv_monomial_lcm(const MVMonomial *a, const MVMonomial *b,
                             int var_count, int *out_lcm) {
    for (int v = 0; v < var_count; v++) {
        out_lcm[v] = (a->exponents[v] > b->exponents[v])
                     ? a->exponents[v] : b->exponents[v];
    }
}

/* 检查单项式 m 是否能被单项式 d 整除 (即 m 的每个变量幂次 >= d 的) */
static bool mv_monomial_divisible(const MVMonomial *m, const MVMonomial *d,
                                   int var_count) {
    for (int v = 0; v < var_count; v++) {
        if (m->exponents[v] < d->exponents[v]) return false;
    }
    return true;
}

/* 检查单项式 d 能否整除 LCM 单项式 (用 exponents 数组表示) */
static bool mv_monomial_divisible_lcm(const MVMonomial *d,
                                       const int *lcm_exp,
                                       int var_count) {
    for (int v = 0; v < var_count; v++) {
        if (d->exponents[v] > lcm_exp[v]) return false;
    }
    return true;
}

/* 多变量多项式乘以单项式 (in-place: result = p * monomial) */
static void mv_poly_mul_monomial(MVPolynomial *result, const MVPolynomial *p,
                                  const int *mono_exp, const mpz_t mono_coeff,
                                  int var_count) {
...
/* 多变量多项式减法: result = a - b */
static void mv_poly_sub(MVPolynomial *result, const MVPolynomial *a,
                         const MVPolynomial *b) {
    mv_poly_clear(result);
 ...
 * 限制: 只处理全次数 <= 2 的多项式系统。
 */
static void s_polynomial(const MVPolynomial *f, const MVPolynomial *g,
                          int var_count, MVPolynomial *result) {
    mv_poly_init(result, var_count);
    ...
    if (!quo_f_exp) { lv00_free((void **)&lcm_exp); return; }
    int *quo_g_exp = lv00_malloc(var_count * sizeof(int));
    if (!quo_g_exp) { lv00_free((void **)&lcm_exp); lv00_free((void **)&quo_f_exp); return; }
 ...
 * 返回约化后的余式。
 */
static void polynomial_reduce(const MVPolynomial *p, MVPolynomial **G,
                               int g_count, MVPolynomial *remainder) {
    ...
    while (changed && safety < max_iterations) {
        ...
        for (int i = 0; i < g_count; i++) {
            if (mv_poly_is_zero(G[i])) continue;
            ...
            for (int j = 0; j < remainder->term_count && !reduced; j++) {
                if (mv_monomial_divisible(&remainder->terms[j], lt_g,
                                           remainder->var_count)) {
                    /* 计算商单项式 */
                    ...
                    for (int v = 0; v < remainder->var_count; v++) {
                        quo_exp[v] = remainder->terms[j].exponents[v]
                                     - lt_g->exponents[v];
                    }
                    ...
                    mv_poly_mul_monomial(&sub_term, G[i], quo_exp,
                                          remainder->terms[j].coeff,
                                          remainder->var_count);
                    ...
                    for (int k = 0; k < remainder->term_count; k++) {
                        ...
                        mv_poly_add_term(&new_remainder, scaled,
                                          remainder->terms[k].exponents);
                        mpz_clear(scaled);
                    }
                    for (int k = 0; k < sub_term.term_count; k++) {
                        ...
                        mv_poly_add_term(&new_remainder, neg,
                                          sub_term.terms[k].exponents);
                        mpz_clear(neg);
                    }
 ...
 */
static SolverStatus buchberger_groebner(MVPolynomial **F, int f_count,
                                         MVPolynomial ***out_G,
                                         int *out_g_count,
                                         int step_limit) {
    ...
    if (!pair_data) {
        for (int i = 0; i < g_count; i++) { mv_poly_clear(G[i]); lv00_free((void **)&G[i]); }
        lv00_free((void **)&G);
        *out_G = NULL;
        *out_g_count = 0;
        return SOLVER_TIMEOUT;
    ...
    while (changed && steps < step_limit) {
        ...
        for (int i = 0; i < g_count && steps < step_limit; i++) {
            for (int j = i + 1; j < g_count && steps < step_limit; j++) {
                ...
                for (int v = 0; v < var_count; v++) {
                    if (lcm_exp[v] != lt_i->exponents[v]) lcm_is_lt_i = false;
                    if (lcm_exp[v] != lt_j->exponents[v]) lcm_is_lt_j = false;
                }
                lv00_free((void **)&lcm_exp);
                ...
                for (int k = 0; k < g_count && k < i && !skip_by_chain; k++) {
                    if (!pair_data[k * g_capacity + i] ||
                        !pair_data[k * g_capacity + j]) continue;
                    ...
                    if (!lcm_chain) continue;
                    mv_monomial_lcm(lt_i, lt_j, var_count, lcm_chain);
                    bool lt_k_divides_lcm = mv_monomial_divisible_lcm(
                        lt_k, lcm_chain, var_count);
                ...
                if (!mv_poly_is_zero(&remainder)) {
                    ...
                    for (int k = 0; k < remainder.term_count; k++) {
                        int td = mv_monomial_total_degree(&remainder.terms[k],
                                                          var_count);
                        if (td > 4) {
     ...
     *   - 使基中每个多项式的首项互不相同，便于消元 */
    {
        ...
        for (int i = 0; i < g_count; i++) {
            ...
            /* 约化 g_i */
            MVPolynomial remainder;
            polynomial_reduce(G[i], temp_G ? temp_G : NULL,
                              temp_G ? (g_count - 1) : 0, &remainder);
 ...
 */
static MVPolynomial *build_mv_polynomials(EquationSystem *sys,
                                           int **var_id_map,
                                           int **out_coord_map,
                                           int *out_var_count) {
    ...
    if (!cids) { lv00_free((void **)&vids); return NULL; }
    int vcount = 0;
    ...
    *out_var_count = vcount;
    ...
    if (!polys) { lv00_free((void **)&vids); lv00_free((void **)&cids); return NULL; }
    for (int i = 0; i < sys->count; i++) {
        ...
        int var_idx = -1;
        for (int j = 0; j < vcount; j++) {
            if (vids[j] == sys->eqs[i].var_node_id &&
                cids[j] == sys->eqs[i].coord_index) {
                var_idx = j;
                break;
            }
        }
        ...
        for (int d = 0; d <= p->degree; d++) {
            ...
            if (exponents) memset(exponents, 0, (size_t)vcount * sizeof(int));
            exponents[var_idx] = d;
 ...
 * @return 生成的方程数量
 */
static int template_similar_triangles(ConstraintGraph *graph,
                                       EquationSystem *sys) {
    if (!graph || !sys) return 0;
    int added = 0;
    ...
    /* 收集所有线段及其端点 */
    typedef struct { int id; int p1; int p2; } SegInfo;
    ...
    for (int ci = 0; ci < graph->constraint_count; ci++) {
        ...
        for (int s = 0; s < seg_count; s++) {
            if (segs[s].id == seg_id) {
                if (segs[s].p1 < 0) segs[s].p1 = pt_id;
                else if (segs[s].p2 < 0) segs[s].p2 = pt_id;
                break;
            }
        }
    }
    ...
    for (int a = 0; a < pt_count && added < 10; a++) {
        for (int b = a + 1; b < pt_count && added < 10; b++) {
            for (int c = b + 1; c < pt_count && added < 10; c++) {
                ...
                for (int s = 0; s < seg_count; s++) {
                    if ((segs[s].p1 == pa && segs[s].p2 == pb) ||
                        (segs[s].p1 == pb && segs[s].p2 == pa)) has_ab = true;
                    if ((segs[s].p1 == pb && segs[s].p2 == pc) ||
                        (segs[s].p1 == pc && segs[s].p2 == pb)) has_bc = true;
                    if ((segs[s].p1 == pa && segs[s].p2 == pc) ||
                        (segs[s].p1 == pc && segs[s].p2 == pa)) has_ac = true;
                }
                if (!has_ab || !has_bc || !has_ac) continue;
                ...
                bool has_coords = point_coord(nodeA, 0, &xa) &&
                                  point_coord(nodeA, 1, &ya) &&
                                  point_coord(nodeB, 0, &xb) &&
                                  point_coord(nodeB, 1, &yb) &&
                                  point_coord(nodeC, 0, &xc) &&
                                  point_coord(nodeC, 1, &yc);
                ...
                mpz_init(ab2_mpz); mpz_init(bc2_mpz); mpz_init(ac2_mpz);
                 ...
                 * 这确保了三角形的边长比例一致性 */
                /* (仅当存在距离约束时才有意义, 否则跳过) */
                mpz_clear(ab2_mpz); mpz_clear(bc2_mpz); mpz_clear(ac2_mpz);
            }
        }
    }
 ...
 * @return 生成的方程数量
 */
static int template_pythagorean(ConstraintGraph *graph,
                                 EquationSystem *sys) {
    if (!graph || !sys) return 0;
    int added = 0;
    ...
    for (int a = 0; a < pt_count && added < 10; a++) {
        for (int b = a + 1; b < pt_count && added < 10; b++) {
            for (int c = b + 1; c < pt_count && added < 10; c++) {
                ...
                /* 如果 C 处近似直角 (点积接近零) */
                if (fabs(dot) < 1e-6 * (sqrt(cax*cax+cay*cay) *
                                         sqrt(cbx*cbx+cby*cby) + 1.0)) {
                    /* 勾股定理: AB^2 = AC^2 + BC^2
                    ...
                    mpz_init(ab2_x_mpz); mpz_init(ab2_y_mpz);
                    ...
                    mpz_init(ac2_x_mpz); mpz_init(ac2_y_mpz);
                    ...
                    mpz_init(bc2_x_mpz); mpz_init(bc2_y_mpz);
                    ...
                    mpz_init(lhs_mpz); mpz_init(rhs_mpz); mpz_init(diff_mpz);
                    ...
                    mpz_clear(ab2_x_mpz); mpz_clear(ab2_y_mpz);
                    mpz_clear(ac2_x_mpz); mpz_clear(ac2_y_mpz);
                    mpz_clear(bc2_x_mpz); mpz_clear(bc2_y_mpz);
                    mpz_clear(lhs_mpz); mpz_clear(rhs_mpz);
                    mpz_clear(diff_mpz); mpz_clear(threshold);
                }
            }
        }
    }
...
static int template_parallel_cut(const ConstraintGraph *graph, EquationSystem *sys) {
    ...
    for (int i = 0; i < graph->node_count; i++) {
        ...
        if (li->type != GEOM_LINE_SEGMENT || li->coord_count < 4) continue;

        double x1_i, y1_i, x2_i, y2_i;
        if (!coord_to_double(li->symbolic_coords[0], &x1_i) ||
            !coord_to_double(li->symbolic_coords[1], &y1_i) ||
            !coord_to_double(li->symbolic_coords[2], &x2_i) ||
            !coord_to_double(li->symbolic_coords[3], &y2_i)) continue;

        double dx_i = x2_i - x1_i;
        double dy_i = y2_i - y1_i;
        double len_i = sqrt(dx_i * dx_i + dy_i * dy_i);
        if (len_i < LV00_EPSILON_DOUBLE) continue;
        ...
        for (int j = i + 1; j < graph->node_count; j++) {
            ...
            if (lj->type != GEOM_LINE_SEGMENT || lj->coord_count < 4) continue;

            double x1_j, y1_j, x2_j, y2_j;
            if (!coord_to_double(lj->symbolic_coords[0], &x1_j) ||
                !coord_to_double(lj->symbolic_coords[1], &y1_j) ||
                !coord_to_double(lj->symbolic_coords[2], &x2_j) ||
                !coord_to_double(lj->symbolic_coords[3], &y2_j)) continue;

            double dx_j = x2_j - x1_j;
            double dy_j = y2_j - y1_j;
            double len_j = sqrt(dx_j * dx_j + dy_j * dy_j);
            if (len_j < LV00_EPSILON_DOUBLE) continue;
            ...
            for (int ci = 0; ci < graph->constraint_count; ci++) {
                ...
                if (!pt || pt->type != GEOM_POINT) continue;

                int on_line_i = (seg && seg->id == li->id);
                int on_line_j = (seg && seg->id == lj->id);
                if (!on_line_i && !on_line_j) continue;
                ...
                if (on_line_i && pt->coord_count >= 2) {
                    for (int cj = ci + 1; cj < graph->constraint_count; cj++) {
                        ...
                        if (c2->type != INCIDENCE || c2->participant_count < 2) continue;
                        ...
                        if (!pt2 || pt2->type != GEOM_POINT || pt2->coord_count < 2) continue;
                        if (!seg2 || seg2->id != lj->id) continue;
                        ...
                        if (!coord_to_double(pt->symbolic_coords[0], &px) ||
                            !coord_to_double(pt->symbolic_coords[1], &py)) continue;

                        double t_i = ((px - x1_i) * dx_i + (py - y1_i) * dy_i) /
                                    (len_i * len_i);

                        double px2, py2;
                        if (!coord_to_double(pt2->symbolic_coords[0], &px2) ||
                            !coord_to_double(pt2->symbolic_coords[1], &py2)) continue;

                        double t_j = ((px2 - x1_j) * dx_j + (py2 - y1_j) * dy_j) /
                                    (len_j * len_j);
 ...
 * @return 生成的方程数量
 */
static int template_parallel_intercept(ConstraintGraph *graph,
                                        EquationSystem *sys) {
    if (!graph || !sys) return 0;
    int added = 0;
    ...
    /* 计算每条线段的方向向量 */
    for (int s = 0; s < seg_count; s++) {
        if (segs[s].p1 < 0 || segs[s].p2 < 0) continue;
        GeomNode *n1 = find_node(graph, segs[s].p1);
        GeomNode *n2 = find_node(graph, segs[s].p2);
        if (!n1 || !n2) continue;
        double x1, y1, x2, y2;
        if (point_coord(n1, 0, &x1) && point_coord(n1, 1, &y1) &&
            point_coord(n2, 0, &x2) && point_coord(n2, 1, &y2)) {
            segs[s].dx = x2 - x1;
            segs[s].dy = y2 - y1;
        }
    }
            ...
            if (fabs(cross) < 1e-6 * (len_i * len_j + 1.0)) {
                ...
                for (int k = 0; k < seg_count && added < 10; k++) {
                    ...
                    for (int ci = 0; ci < graph->constraint_count; ci++) {
                        ...
                        if (pt == segs[k].p1 && seg == segs[i].id) k1_on_i = true;
                        if (pt == segs[k].p2 && seg == segs[j].id) k2_on_j = true;
                        if (pt == segs[k].p1 && seg == segs[j].id) k1_on_j = true;
                        if (pt == segs[k].p2 && seg == segs[i].id) k2_on_i = true;
                    }
 ...
 * @return 总共生成的额外方程数量
 */
static int apply_geometry_templates(ConstraintGraph *graph,
                                     EquationSystem *sys) {
    if (!graph || !sys) return 0;
    int total = 0;
...
static bool dirty_set_contains(DirtyVariableSet *ds, int var_id) {
    for (int i = 0; i < ds->dirty_count; i++) {
        if (ds->dirty_ids[i] == var_id) return true;
    }
    return false;
}
 ...
 * 则该方程也间接相关。
 */
static void filter_equations_for_dirty(EquationSystem *sys,
                                        DirtyVariableSet *ds,
                                        EquationSystem *filtered) {
    equation_system_init(filtered);

    if (!sys || !ds || ds->dirty_count == 0) return;

    /* 第一遍: 直接筛选涉及脏变量的方程 */
    for (int i = 0; i < sys->count; i++) {
        if (sys->eqs[i].poly.degree < 0) continue;
        if (dirty_set_contains(ds, sys->eqs[i].var_node_id)) {
            equation_system_push(filtered, sys->eqs[i].poly,
                                  sys->eqs[i].var_node_id,
                                  sys->eqs[i].coord_index);
        }
    }
...
static int *order_variables_by_dependency(
    ...
    *out_count = 0;
    if (!graph || var_count == 0) return NULL;

    /* 构建变量 ID 到索引的映射 */
    int *id_to_idx = lv00_malloc(var_count * sizeof(int));
    for (int i = 0; i < var_count; i++) id_to_idx[i] = -1;
    for (int i = 0; i < var_count; i++) {
        /* 线性搜索 var_ids 中是否有重复 */
        bool dup = false;
        for (int j = 0; j < i; j++) {
            if (var_ids[j] == var_ids[i]) { dup = true; break; }
        }
        if (!dup) id_to_idx[i] = i;
    }

    /* 构建邻接矩阵：变量 A 依赖变量 B 当且仅当
     * 存在约束同时涉及 A 和 B，且 B 已有已知值或更少自由度 */
    bool **adj = lv00_malloc((size_t)var_count * sizeof(bool *));
    if (adj) memset(adj, 0, (size_t)var_count * sizeof(bool *));
    for (int i = 0; i < var_count; i++) {
        adj[i] = lv00_malloc((size_t)var_count * sizeof(bool));
        if (adj[i]) memset(adj[i], 0, (size_t)var_count * sizeof(bool));
    }

    /* 统计每个变量的约束参与度 (用于确定依赖方向) */
    int *participation = lv00_malloc((size_t)var_count * sizeof(int));
    if (participation) memset(participation, 0, (size_t)var_count * sizeof(int));
    for (int ci = 0; ci < graph->constraint_count; ci++) {
    ...
    if (in_degree) memset(in_degree, 0, (size_t)var_count * sizeof(int));
    for (int i = 0; i < var_count; i++) {
        for (int j = 0; j < var_count; j++) {
    ...
    if (in_subset) memset(in_subset, 0, (size_t)var_count * sizeof(bool));
    ...
    if (use_subset) {
        ...
        for (int i = 0; i < var_count; i++) in_subset[i] = true;
    }
    ...
    if (visited) memset(visited, 0, (size_t)var_count * sizeof(bool));
    ...
    while (order_count < var_count) {
        ...
        for (int i = 0; i < var_count; i++) {
            ...
            if (participation[i] > best_participation ||
                (participation[i] == best_participation &&
                 var_ids[i] < var_ids[best])) {
                best = i;
        ...
        if (best < 0) {
            ...
            fprintf(stderr, "order_variables_by_dependency: cycle detected in constraint graph, "
    ...
    /* 清理 */
    for (int i = 0; i < var_count; i++) lv00_free((void **)&adj[i]);
    lv00_free((void **)&adj);
 ...
 */
static int *compute_elimination_order(ConstraintGraph *graph,
                                       EquationSystem *sys,
                                       int *out_order_count) {
    *out_order_count = 0;

    if (!graph || !sys) return NULL;
    ...
    if (eq_count) memset(eq_count, 0, (size_t)var_count * sizeof(int));
    for (int i = 0; i < sys->count; i++) {
        if (sys->eqs[i].poly.degree < 0) continue;
        for (int j = 0; j < var_count; j++) {
    ...
    if (constraint_count) memset(constraint_count, 0, (size_t)var_count * sizeof(int));
    for (int ci = 0; ci < graph->constraint_count; ci++) {
        Constraint *c = graph->constraints[ci];
        for (int p = 0; p < c->participant_count; p++) {
            int pid = c->participants[p];
    ...
    while (order_count < var_count) {
        ...
        for (int i = 0; i < var_count; i++) {
            if (visited[i]) continue;
            if (in_degree[i] > 0) continue; /* 还有依赖未消除 */
        ...
        if (best < 0) {
            ...
            fprintf(stderr, "compute_elimination_order: cycle detected in constraint graph, "
                    "%d variables remain unscheduled\n", unvisited);
            for (int i = 0; i < var_count; i++) {
                if (!visited[i]) {
                    order[order_count++] = var_ids[i];
 ...
 */
static void propagate_dependency(const ConstraintGraph *graph,
                                  int var_id,
                                  bool *affected) {
    if (!graph || !affected || var_id < 0) return;
...
GroebnerResult* solver_incremental_solve(ConstraintGraph *graph,
    ...
    if (solver_stream_ctx) {
        ...
        char detail[128];
        int _snw_inc;
        LV00_SAFE_SNPRINTF(_snw_inc, detail, sizeof(detail),
                 "{\"phase\":\"incremental\",\"dirty_count\":%d}", n_dirty_vars);
    ...
    if (affected) memset(affected, 0, (size_t)graph->node_count * sizeof(bool));
    ...
    for (int i = 0; i < graph->node_count; i++) {
        if (affected[i]) {
            affected_ids = lv00_realloc(affected_ids,
    ...
    if (solver_stream_ctx) {
        ...
        char detail[96];
        int _snw_dep;
        LV00_SAFE_SNPRINTF(_snw_dep, detail, sizeof(detail),
                 "{\"phase\":\"dependency_propagation\",\"affected\":%d}", affected_count);
    ...
    if (affected_count == 0 || !affected_ids) {
        ...
        /* 没有受影响的变量，返回空结果 */
        GroebnerResult *result = lv00_malloc(sizeof(GroebnerResult));
        if (result) memset(result, 0, sizeof(GroebnerResult));
        return result;
    }
    ...
    if (solver_stream_ctx) {
        ...
        char detail[96];
        int _snw_filt;
        LV00_SAFE_SNPRINTF(_snw_filt, detail, sizeof(detail),
                 "{\"phase\":\"filter\",\"filtered_eq_count\":%d}", filtered_sys.count);
    ...
    if (!has_oos) {
        solve_equations_pass(&filtered_sys, result, &solved_count,
                             &multiple_solutions, &no_solution, false);
    }
    ...
    if (!no_solution && !has_oos) {
        int remaining = 0;
        for (int i = 0; i < filtered_sys.count; i++) {
            if (filtered_sys.eqs[i].poly.degree >= 0) remaining++;
        }
 ...
 */
int solver_extract_equations_full(const ConstraintGraph *graph,
                                   EquationSystem *out_system) {
    if (!graph || !out_system) return -1;
    ...
    for (int ci = 0; ci < graph->constraint_count; ci++) {
                ...
                if (line->coord_count >= 4) {
                    ...
                    for (int cj = 0; cj < graph->constraint_count; cj++) {
                        ...
                        if (c2->participants[1] == line->id) {
                            GeomNode *candidate = find_node((ConstraintGraph *)graph,
                                                            c2->participants[0]);
                            if (candidate && candidate->type == GEOM_POINT) {
                                if (!ep1) ep1 = candidate;
                                else if (!ep2) ep2 = candidate;
                            }
                        }
                    }
                    if (ep1 && ep2) {
                        got_coords = point_coord(ep1, 0, &lx1) &&
                                     point_coord(ep1, 1, &ly1) &&
                                     point_coord(ep2, 0, &lx2) &&
                                     point_coord(ep2, 1, &ly2);
                    }
                }
        ...
        case CONTAINMENT: {
...

            {
                ...
                for (int si = 0; si < seg_count; si++) {
                     ...
                     */
                    bool seg_is_rational = (seg->symbolic_coords[0] &&
                        seg->symbolic_coords[0]->type == RATIONAL &&
                        seg->symbolic_coords[1] &&
                        seg->symbolic_coords[1]->type == RATIONAL &&
                        seg->symbolic_coords[2] &&
                        seg->symbolic_coords[2]->type == RATIONAL &&
                        seg->symbolic_coords[3] &&
                        seg->symbolic_coords[3]->type == RATIONAL);
                    ...
                    if (seg_is_rational) {
                        ...
                        if (coord_to_mpz_scaled(seg->symbolic_coords[0], sx1_s, scale) &&
                            ...
                            /* 方程1（x-分量）: dy_s * px + (-dy_s*sx1_s + dx_s*sy1_s) = 0 */
                            {
                                ...
                                poly.degree = 1;
                                poly.coeffs = lv00_malloc(2 * sizeof(mpz_t));
                                if (!poly.coeffs) { mpz_poly_clear(&poly); }
                                else {
                                    mpz_init(poly.coeffs[1]);
                                    mpz_init(poly.coeffs[0]);
                                    mpz_set(poly.coeffs[1], dy_s);
                                    ...
                                    mpz_poly_clear(&poly);
                                    count++;
                                    stream_emit_simple(solver_stream_ctx,
                                        STREAM_EVENT_SOLVE_EQUATION_EXTRACTED,
        ...
        case BETWEENNESS: {
             ...
             */
            if (!p1->symbolic_coords || !p3->symbolic_coords) break;

            bool exact_mode = false;
            mpz_t dx13_s, dy13_s, x1_s, y1_s;
            /* 精确有理数路径：检查 p1 和 p3 的四个坐标是否都是 RATIONAL 类型 */
            if (p1->symbolic_coords[0] && p1->symbolic_coords[0]->type == RATIONAL &&
                p1->symbolic_coords[1] && p1->symbolic_coords[1]->type == RATIONAL &&
                p3->symbolic_coords[0] && p3->symbolic_coords[0]->type == RATIONAL &&
                p3->symbolic_coords[1] && p3->symbolic_coords[1]->type == RATIONAL) {
            ...
            if (!exact_mode) {
                ...
                bool ok = point_coord(p1, 0, &x1) && point_coord(p1, 1, &y1) &&
                          point_coord(p3, 0, &x3) && point_coord(p3, 1, &y3);
                if (ok) {
                    double dx13 = x3 - x1;
                    double dy13 = y3 - y1;
...
SolverStatus groebner_basis_compute(EquationSystem *system) {
    if (!system || system->count == 0) return SOLVER_OK;
    ...
    if (!active) {
        for (int i = 0; i < system->count; i++) mv_poly_clear(&mv_polys[i]);
        lv00_free((void **)&mv_polys);
        lv00_free((void **)&var_id_map);
        lv00_free((void **)&coord_map);
        return SOLVER_TIMEOUT;
    ...
    MVPolynomial **G = NULL;
    int g_count = 0;
    SolverStatus status = buchberger_groebner(active, active_count,
                                               &G, &g_count, LV00_SOLVER_BUCHBERGER_STEP_LIMIT);
    ...
    if (solver_stream_ctx) {
        ...
        LV00_SAFE_SNPRINTF(_snw_gc, detail, sizeof(detail),
                 ...
                 (status == SOLVER_OK) ? "ok" :
                 (status == SOLVER_OUT_OF_SCOPE) ? "out_of_scope" : "timeout",
    ...
    if (G) {
        for (int i = 0; i < g_count; i++) {
            ...
            if (best_var < 0 || best_degree < 0) continue;
            ...
            for (int t = 0; t < G[i]->term_count; t++) {
                ...
                if (power <= best_degree) {
                    mpz_add(poly.coeffs[power], poly.coeffs[power],
                            G[i]->terms[t].coeff);
                }
            }
 ...
 */
SolverStatus solver_handle_multiple_solutions(
    const GroebnerResult *result,
    const EquationSystem *system,
    SymbolicCoord ***out_branches,
    int *out_branch_count)
{
    if (!out_branches || !out_branch_count) return SOLVER_TIMEOUT;
    *out_branches = NULL;
    *out_branch_count = 0;
    ...
    struct BranchVariable *branch_vars = lv00_malloc((size_t)max_branch_vars
                                                   * sizeof(struct BranchVariable));
    if (!branch_vars) return SOLVER_TIMEOUT;
    ...
    for (int i = 0; i < system->count; i++) {
        if (system->eqs[i].poly.degree != 2) continue;
        if (mpz_cmp_si(system->eqs[i].poly.coeffs[2], 0) == 0) continue;
        ...
        if (solver_stream_ctx) {
            ...
            LV00_SAFE_SNPRINTF(_snw3, detail, sizeof(detail),
                     ...
                     system->eqs[i].var_node_id, system->eqs[i].coord_index,
                     branch_vars[branch_count].root1, branch_vars[branch_count].root2,
                     discriminant);
            LV00_UNUSED(_snw3);
            ev.detail_json = detail;
            stream_emit(solver_stream_ctx, &ev);
    ...
    SymbolicCoord **branch_coords = lv00_malloc((size_t)total_coords
                                              * sizeof(SymbolicCoord *));
    if (!branch_coords) {
        lv00_free((void **)&branch_vars);
        return SOLVER_TIMEOUT;
    }
    ...
    for (int b = 0; b < total_branches; b++) {
        for (int v = 0; v < branch_count; v++) {
            ...
            double chosen = (b & (1 << v)) ? branch_vars[v].root2
                                           : branch_vars[v].root1;

            /* Create a rational coordinate from the double value. */
            int64_t num_val = (int64_t)(chosen * LV00_SOLVER_SCALE_FACTOR);
            SymbolicCoord *coord = symbolic_coord_create_rational(
                num_val, (uint64_t)LV00_SOLVER_SCALE_FACTOR);
            if (!coord) {
    ...
    for (int b = 0; b < total_branches; b++) {
        ...
        for (int eq = 0; eq < system->count; eq++) {
            ...
            if (system->eqs[eq].poly.degree == 1) {
                ...
                for (int v = 0; v < branch_count; v++) {
                    if (branch_vars[v].var_node_id == system->eqs[eq].var_node_id &&
                        ...
                        branch_val = (b & (1u << v)) ? branch_vars[v].root2
                                                     : branch_vars[v].root1;
                        found = true;
                        break;
                    }
                }
                if (!found) continue; /* Different variable, skip */
                ...
                if (branch_coords && branch_count > 0) {
                    ...
                    for (int v = 0; v < branch_count; v++) {
                        if (branch_vars[v].var_node_id == system->eqs[eq].var_node_id &&
                            branch_vars[v].coord_index == system->eqs[eq].coord_index) {
                            SymbolicCoord *branch_coord =
                                branch_coords[b * branch_count + v];
                            if (branch_coord) {
                                /* 用符号坐标在多项式上求值 */
                                SymbolicCoord *poly_val = poly_eval_symbolic(
                                    &system->eqs[eq].poly, branch_coord);
                                if (poly_val) {
                                    /* 检查求值结果是否为零 */
                                    double eval_d = 0.0;
                ...
                if (!equation_satisfied) {
                    double a_coeff = mpz_get_d(system->eqs[eq].poly.coeffs[1])
                                     / LV00_SOLVER_SCALE_FACTOR;
                    double c_coeff = mpz_get_d(system->eqs[eq].poly.coeffs[0])
                                     / LV00_SOLVER_SCALE_FACTOR;
            ...
            if (system->eqs[eq].poly.degree == 0) {
                double const_val = mpz_get_d(system->eqs[eq].poly.coeffs[0])
                                   / LV00_SOLVER_SCALE_FACTOR;
        ...
        if (solver_stream_ctx && (b % 100 == 0 || b == total_branches - 1)) {
            ...
            char detail[128];
            int _snw4;
            LV00_SAFE_SNPRINTF(_snw4, detail, sizeof(detail),
                     "{\"checked\":%d,\"total\":%d,\"valid\":%d}",
                     b + 1, total_branches, valid_branches);
            LV00_UNUSED(_snw4);
            ev.detail_json = detail;
            stream_emit(solver_stream_ctx, &ev);
        }
    ...
    SymbolicCoord **out_branch_arr = lv00_malloc((size_t)(valid_branches * branch_count)
                                               * sizeof(SymbolicCoord *));
    if (!out_branch_arr) {
        for (int i = 0; i < total_coords; i++) {
            symbolic_coord_destroy(branch_coords[i]);
    ...
    for (int b = 0; b < total_branches; b++) {
        ...
        if (solver_stream_ctx) {
            ...
            char detail[256];
            int pos;
            LV00_SAFE_SNPRINTF(pos, detail, sizeof(detail),
                     "{\"branch\":%d,\"valid\":true,\"values\":[", out_idx);
            for (int v = 0; v < branch_count && pos < (int)sizeof(detail) - 30; v++) {
                char *coord_str = symbolic_coord_serialize(branch_coords[src_base + v]);
                if (coord_str) {
                    int _sn_tmp;
                    LV00_SAFE_SNPRINTF(_sn_tmp, detail + pos, (size_t)(sizeof(detail) - pos - 5),
                                    "%s\"%s\"", (v > 0 ? "," : ""), coord_str);
                    pos += _sn_tmp;
    ...
    if (valid_branches == 0) {
        ...
        *out_branch_count = 0;
        if (solver_stream_ctx) {
            stream_emit_simple(solver_stream_ctx, STREAM_EVENT_ERROR,
                               "多解分支处理: 所有分支均无效", 0);
        }
        return SOLVER_NO_SOLUTION;
    }
    ...
    *out_branch_count = valid_branches;
    ...
    if (valid_branches == 1) {
        if (solver_stream_ctx) {
            stream_emit_simple(solver_stream_ctx, STREAM_EVENT_SOLVE_DONE,
                               "多解分支处理: 过滤后仅剩唯一有效解", valid_branches);
        }
        return SOLVER_UNIQUE;
    }
