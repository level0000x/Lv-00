...
static void cnf_builder_add_lit(CNFBuilder *b, int lit) {
    if (b->clause_buf_len + 1 >= b->clause_buf_size) {
        b->clause_buf_size *= 2;
        b->clause_buffer = (int *)realloc(b->clause_buffer,
            (size_t)b->clause_buf_size * sizeof(int));
    }
...
static char *cnf_builder_to_dimacs(CNFBuilder *b) {
    ...
    for (int ci = 0; ci < b->clause_count; ci++) {
        while (b->clause_buffer[lit_idx] != 0) {
            offset += snprintf(buf + offset, buf_size - offset, "%d ",
                               b->clause_buffer[lit_idx]);
            lit_idx++;
        }
        lit_idx++; /* 璺宠繃缁堟绗?0 */
...
bool approx_count_solutions(const ConstraintGraph *graph,
    ...
    if (out->status_msg) {
        snprintf(out->status_msg, msg_size,
                 ...
                 pac_bound * 100.0,
                 cfg->epsilon, cfg->delta, cnf_vars);
    }
...
bool approx_count_projected(const ConstraintGraph *graph,
    ...
    if (out->status_msg) {
        ...
        if (new_msg) {
            ...
            snprintf(out->status_msg + len, new_size - len,
                     " [projected: %d vars]", proj_count);
        }
    }
    return true;
}
...
double approx_count_get_pac_bound(const PacConfig *cfg,
    ...
    if (res->total_count == 0) return 0.0;
