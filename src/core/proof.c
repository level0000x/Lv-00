 ...
 */
static bool proposition_set_id_list(int **target, int *count_ptr,
                                     const int *ids, int count) {
    if (!target || !count_ptr) return false;
...
bool proposition_set_input_ports(Proposition *prop, int *port_ids, int count) {
    ...
    return proposition_set_id_list(&prop->input_port_ids, &prop->input_count,
                                    port_ids, count);
}
...
bool proposition_set_output_ports(Proposition *prop, int *port_ids, int count) {
    if (!prop) return false;
    return proposition_set_id_list(&prop->output_port_ids, &prop->output_count,
                                    port_ids, count);
}
...
bool proposition_set_preconditions(Proposition *prop, int *region_ids, int count) {
    if (!prop) return false;
    return proposition_set_id_list(&prop->precondition_region_ids, &prop->precondition_count,
                                    region_ids, count);
}
...
bool proposition_set_postconditions(Proposition *prop, int *constraint_ids, int count) {
    if (!prop) return false;
    return proposition_set_id_list(&prop->postcondition_constraint_ids, &prop->postcondition_count,
...
bool proposition_add_sub_proposition(Proposition *parent, Proposition *child) {
    ...
    Proposition **new_arr = lv00_realloc(parent->sub_props,
        (size_t)new_count * sizeof(Proposition*));
    ...
    for (int i = 0; i < copy->node_count; i++) {
        ...
        switch (cn->type) {
            ...
            case GEOM_FUNCTION_BLOCK:
                ...
                break;
            case GEOM_PORT:
                if (cn->data.port && orig->nodes[i]->data.port &&
                    orig->nodes[i]->data.port->connected_to) {
    ...
    if (orig->constraint_count > 0) {
        ...
        for (int i = 0; i < orig->constraint_count; i++) {
            ...
            if (oc->participants && oc->participant_count > 0) {
                ...
                memcpy(cc->participants, oc->participants,
                       oc->participant_count * sizeof(int));
            } else {
...
UnifyStatus proof_unify(
    ...
    if (!construction || !proposition) return UNIFY_STATUS_FAILED;

    ConstraintGraph *pattern = proposition->pattern;

    if (!pattern) return UNIFY_STATUS_FAILED;

    /* 始终深拷贝构造图，避免 unify 过程修改调用者的原图 */
    ConstraintGraph *constr_copy = deep_copy_graph(construction);
    if (!constr_copy) return UNIFY_STATUS_FAILED;
    ...
    if (proposition->input_count > 0 || proposition->output_count > 0) {
        ...
        if (constr_input_count != proposition->input_count ||
            constr_output_count != proposition->output_count) {
            graph_destroy(pattern_copy);
            graph_destroy(constr_copy);
...
static void update_dependent_steps(ProofNavigator *nav, int step_id, int dep_id) {
    if (!nav || dep_id < 0 || dep_id >= nav->step_count) return;
    ...
    int *new_arr = lv00_realloc(dep_step->dependent_step_ids,
        dep_step->dependent_count * sizeof(int));
    if (!new_arr) {
        dep_step->dependent_count--;
        return;
    }
...
bool proof_step_add_dependency(ProofStep *step, int dep_step_id) {
    ...
    int *new_arr = lv00_realloc(step->dependency_step_ids,
...
void proof_step_set_breakpoint(ProofStep *step, bool is_breakpoint) {
    if (step) step->is_breakpoint = is_breakpoint;
}
...
bool proof_navigator_add_step(ProofNavigator *nav, ProofStep *step) {
    if (!nav || !step) return false;

    /* 流式输出：证明步骤添加 */
    if (proof_stream_ctx) {
        char desc_buf[128];
        snprintf(desc_buf, sizeof(desc_buf), "证明步骤添加: %s",
                 proof_step_type_to_string(step->type));
        stream_emit_simple(proof_stream_ctx, STREAM_EVENT_PROOF_STEP_ADDED,
                           desc_buf, nav->step_count);
    }

    step->id = nav->step_count;

    int new_count = nav->step_count + 1;
    ProofStep **new_arr = lv00_realloc(nav->steps,
        (size_t)new_count * sizeof(ProofStep*));
    if (!new_arr) return false;
    ...
    if (step->is_breakpoint) {
        ...
        int *new_bp = lv00_realloc(nav->breakpoint_indices,
            nav->breakpoint_count * sizeof(int));
        if (new_bp) {
...
bool proof_navigator_next(ProofNavigator *nav) {
    if (!nav || nav->current_step >= nav->step_count - 1) return false;
    nav->current_step++;

    if (proof_stream_ctx) {
        stream_emit_simple(proof_stream_ctx, STREAM_EVENT_PROOF_STEP_APPLIED,
                           "执行下一步", nav->current_step);
    }
...
bool proof_navigator_prev(ProofNavigator *nav) {
    if (!nav || nav->current_step <= 0) return false;
    nav->current_step--;
...
bool proof_navigator_goto(ProofNavigator *nav, int step_index) {
    if (!nav || step_index < 0 || step_index >= nav->step_count) return false;
...
bool proof_navigator_next_breakpoint(ProofNavigator *nav) {
    if (!nav || nav->breakpoint_count == 0) return false;
...
ProofColor proof_navigator_compute_final_color(ProofNavigator *nav) {
    if (!nav) return PROOF_COLOR_BLUE_UNEXPLORED;
    ...
    for (int i = 0; i < nav->step_count; i++) {
        ...
        } else if ((step->color == PROOF_COLOR_ORANGE_ORACLE ||
                    step->color == PROOF_COLOR_ORANGE_EX_FALSO) &&
                   final_color != PROOF_COLOR_DARK_ORANGE &&
                   final_color != PROOF_COLOR_AMBER) {
            final_color = step->color;
        } else if (step->color == PROOF_COLOR_YELLOW &&
                   final_color == PROOF_COLOR_GREEN) {
            final_color = PROOF_COLOR_YELLOW;
        } else if (step->color >= PROOF_COLOR_BLUE_UNEXPLORED &&
    ...
    if (proof_stream_ctx) {
        char desc_buf[64];
        snprintf(desc_buf, sizeof(desc_buf), "最终颜色计算: %s",
...
bool proof_dependency_add_sub(ProofDependency *parent, ProofDependency *child) {
    ...
    ProofDependency **new_arr = lv00_realloc(parent->sub_deps,
        (size_t)new_count * sizeof(ProofDependency*));
    if (!new_arr) return false;
...
ProofColor proof_dependency_compute_color(ProofDependency *dep) {
    if (!dep) return PROOF_COLOR_BLUE_UNEXPLORED;
    ...
    for (int i = 0; i < dep->sub_dep_count; i++) {
        ...
        if (sub_color == PROOF_COLOR_DARK_ORANGE) {
            ...
            color = (color == PROOF_COLOR_ORANGE_ORACLE ||
                     color == PROOF_COLOR_ORANGE_EX_FALSO) ?
                    PROOF_COLOR_DARK_ORANGE : PROOF_COLOR_AMBER;
        } else if ((sub_color == PROOF_COLOR_ORANGE_ORACLE ||
                    sub_color == PROOF_COLOR_ORANGE_EX_FALSO) &&
                   color == PROOF_COLOR_AMBER) {
            color = PROOF_COLOR_DARK_ORANGE;
        }
    }
    ...
    if (proof_stream_ctx != NULL && color != old_color) {
        char buf[128];
        snprintf(buf, sizeof(buf), "依赖颜色更新: dep_id=%d -> %s",
            dep->id, proof_color_to_string(color));
...
bool proof_create_ex_falso_block(ConstraintGraph *graph, int *out_block_id) {
    if (!graph || !out_block_id) return false;
    ...
    result = graph_add_function_block(graph, internal_nodes, 2,
                                       input_ports, 1, output_ports, 1);
    if (result != ADD_NODE_OK) return false;
...
bool proof_apply_ex_falso(
    ...
    if (!nav || !bottom_proof || !target_prop) return false;
    ...
    for (int i = 0; i < bottom_proof->node_count; i++) {
        ...
        if (n && n->type == GEOM_FUNCTION_BLOCK &&
            n->trust == TRUST_LIGHT_ORANGE &&
            n->lo_subtype == LIGHT_ORANGE_EXPLOSION) {
            /* 找到爆炸原理块，更新其输出端口类型 */
            if (n->data.func_block.output_port_ids &&
                n->data.func_block.output_count > 0) {
...
bool proof_save_breakpoint(ProofNavigator *nav, int breakpoint_id) {
    ...
    /* 检查断点ID是否有效 */
    if (breakpoint_id < 0) return false;
    ...
    if (proof_stream_ctx != NULL) {
        char buf[128];
        snprintf(buf, sizeof(buf), "断点保存: breakpoint_id=%d, step=%d",
            breakpoint_id, nav->current_step);
        stream_emit_simple(proof_stream_ctx, STREAM_EVENT_INFO, buf, 0);
    }
...
bool proof_restore_breakpoint(ProofNavigator *nav, int breakpoint_id) {
    ...
    if (proof_stream_ctx != NULL) {
        char buf[128];
        snprintf(buf, sizeof(buf), "断点恢复: breakpoint_id=%d, step=%d",
            breakpoint_id, nav->current_step);
        stream_emit_simple(proof_stream_ctx, STREAM_EVENT_INFO, buf, 0);
    }
...
bool proof_export_html(ProofNavigator *nav, const char *filepath) {
    ...
    fprintf(f, "  <div class=\"summary-item\"><strong>最终颜色:</strong>"
            " <span class=\"dot\" style=\"background:%s\"></span> %s</div>\n",
            proof_color_to_html_hex(nav->final_color),
    ...
    /* 统计各颜色步骤数 */
    {
        ...
        for (int i = 0; i < nav->step_count; i++) {
            ...
            if (s && s->color >= 0 && s->color < 10) color_counts[s->color]++;
        }
        ...
        if (color_counts[PROOF_COLOR_YELLOW] > 0 || color_counts[PROOF_COLOR_ORANGE_ORACLE] > 0 || color_counts[PROOF_COLOR_DARK_ORANGE] > 0)
    ...
    for (int i = 0; i < nav->step_count; i++) {
        ...
        switch (step->color) {
            case PROOF_COLOR_GREEN: case PROOF_COLOR_GREEN_VERIFIED: fill_color = "#4CAF50"; break;
            case PROOF_COLOR_BLUE_UNEXPLORED: case PROOF_COLOR_BLUE_RESOURCE:
            case PROOF_COLOR_BLUE_OUT_OF_RANGE: fill_color = "#42A5F5"; break;
            case PROOF_COLOR_YELLOW: fill_color = "#FDD835"; break;
            case PROOF_COLOR_ORANGE_ORACLE: case PROOF_COLOR_ORANGE_EX_FALSO: fill_color = "#FF9800"; break;
            case PROOF_COLOR_AMBER: fill_color = "#FFB300"; break;
            case PROOF_COLOR_DARK_ORANGE: fill_color = "#E65100"; break;
        }
        /* 节点圆 */
        fprintf(f, "    <circle id=\"svgNode%d\" cx=\"%d\" cy=\"70\" r=\"14\" fill=\"%s\" "
                "stroke=\"%s\" stroke-width=\"2\" style=\"cursor:pointer\" "
                "onclick=\"goToStep(%d)\">\n",
                i, cx, fill_color, fill_color, i);
        fprintf(f, "      <title>Step %d: %s (%s)</title>\n",
                step->id, proof_step_type_to_string(step->type), proof_color_to_string(step->color));
        fprintf(f, "    </circle>\n");
        /* 步骤编号文字 */
        fprintf(f, "    <text x=\"%d\" y=\"74\" text-anchor=\"middle\" font-size=\"10\" "
                "fill=\"#fff\" font-weight=\"bold\" style=\"pointer-events:none\">%d</text>\n",
                cx, i + 1);

        /* 步骤类型标签在节点上方 */
        fprintf(f, "    <text x=\"%d\" y=\"54\" text-anchor=\"middle\" font-size=\"8\" "
        ...
        if (i < nav->step_count - 1) {
            int nx = 40 + (i + 1) * 52;
            fprintf(f, "    <line x1=\"%d\" y1=\"70\" x2=\"%d\" y2=\"70\" "
    ...
    for (int i = 0; i < nav->step_count; i++) {
        ...
        switch (step->color) {
            case PROOF_COLOR_GREEN: case PROOF_COLOR_GREEN_VERIFIED: dot_border = "var(--c-green)"; break;
            case PROOF_COLOR_BLUE_UNEXPLORED: case PROOF_COLOR_BLUE_RESOURCE:
            case PROOF_COLOR_BLUE_OUT_OF_RANGE: dot_border = "var(--c-blue)"; break;
            case PROOF_COLOR_YELLOW: dot_border = "var(--c-yellow)"; break;
            case PROOF_COLOR_ORANGE_ORACLE: case PROOF_COLOR_ORANGE_EX_FALSO: dot_border = "var(--c-orange)"; break;
            case PROOF_COLOR_AMBER: dot_border = "var(--c-amber)"; break;
            case PROOF_COLOR_DARK_ORANGE: dot_border = "var(--c-dark-orange)"; break;
        }
        fprintf(f, "<div class=\"step-dot%s\" data-idx=\"%d\" "
                "style=\"border-color:%s\" onclick=\"goToStep(%d)\" "
                "title=\"Step %d: %s (%s)\">%d</div>\n",
                step->is_breakpoint ? " has-breakpoint" : "",
                i, dot_border, i,
                step->id, proof_step_type_to_string(step->type),
                proof_color_to_string(step->color), i + 1);
    }
    fprintf(f, "</div>\n");
    ...
    fprintf(f, "  </div>\n");
    fprintf(f, "  <div class=\"nav-group\">\n");
    fprintf(f, "    <button id=\"btnPlay\" class=\"btn-play\" onclick=\"togglePlay()\" title=\"自动播放\">▶ 播放</button>\n");
    ...
    for (int i = 0; i < nav->step_count; i++) {
        ...
        switch (step->color) {
            case PROOF_COLOR_GREEN:         bar_class="cs-green"; badge_class="bg-green"; break;
            case PROOF_COLOR_BLUE_UNEXPLORED:
            case PROOF_COLOR_BLUE_RESOURCE:
            case PROOF_COLOR_BLUE_OUT_OF_RANGE: bar_class="cs-blue"; badge_class="bg-blue"; break;
            case PROOF_COLOR_GREEN_VERIFIED:    bar_class="cs-green-v"; badge_class="bg-green-v"; break;
            case PROOF_COLOR_YELLOW:            bar_class="cs-yellow"; badge_class="bg-yellow"; break;
            case PROOF_COLOR_ORANGE_ORACLE:
            case PROOF_COLOR_ORANGE_EX_FALSO:   bar_class="cs-orange"; badge_class="bg-orange"; break;
            case PROOF_COLOR_AMBER:             bar_class="cs-amber"; badge_class="bg-amber"; break;
            case PROOF_COLOR_DARK_ORANGE:       bar_class="cs-dark"; badge_class="bg-dark"; break;
        }
        ...
        fprintf(f, "  <h2>Step %d: %s <span class=\"badge %s\">%s</span></h2>\n",
                step->id, proof_step_type_to_string(step->type),
                badge_class, proof_color_to_string(step->color));

        fprintf(f, "  <div class=\"step-row\"><span class=\"label\">类型:</span>"
                "<span class=\"value\">%s</span></div>\n", proof_step_type_to_string(step->type));
        fprintf(f, "  <div class=\"step-row\"><span class=\"label\">步骤 ID:</span>"
                "<span class=\"id-chip\">%d</span>", step->id);
        if (step->node_id >= 0)
            fprintf(f, "<span class=\"label\" style=\"margin-left:8px\">节点:</span>"
                    "<span class=\"id-chip\">%d</span>", step->node_id);
        if (step->constraint_id >= 0)
            fprintf(f, "<span class=\"label\" style=\"margin-left:8px\">约束:</span>"
                    "<span class=\"id-chip\">%d</span>", step->constraint_id);
        if (step->rule_id >= 0)
            fprintf(f, "<span class=\"label\" style=\"margin-left:8px\">规则:</span>"
                    "<span class=\"id-chip\">%d</span>", step->rule_id);
        fprintf(f, "</div>\n");

        fprintf(f, "  <div class=\"step-row\"><span class=\"label\">完成:</span>"
                "<span class=\"value\">%s</span>",
                step->is_completed ? "✅ 是" : "⬜ 否");
        fprintf(f, "<span class=\"label\" style=\"margin-left:12px\">断点:</span>"
                "<span class=\"value\">%s</span>",
                step->is_breakpoint ? "🔴 是" : "⚪ 否");
        /* 时间戳 */
        if (step->timestamp > 0) {
            fprintf(f, "<span class=\"label\" style=\"margin-left:12px\">时间:</span>"
                    "<span class=\"value\">%ld</span>", step->timestamp);
        }
        fprintf(f, "</div>\n");
    ...
    fprintf(f, "<script>\n");
    fprintf(f, "(function(){\n");
    fprintf(f, "  var totalSteps=%d,current=0,panels=[],dots=[],svgNodes=[],isPlaying=false,playTimer=null,playSpeed=1000;\n",
            nav->step_count);
    ...
    fprintf(f, "    if(svgNodes[idx]) svgNodes[idx].scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'});\n");
    fprintf(f, "  }\n");
    fprintf(f, "\n");
    ...
    fprintf(f, "    localStorage.setItem('proof-theme',document.body.classList.contains('dark-theme')?'dark':'light');\n");
    fprintf(f, "  };\n");
    fprintf(f, "  /* 恢复保存的主题 */\n");
    ...
    fprintf(f, "  else {document.getElementById('btnPrev').disabled=true;document.getElementById('btnNext').disabled=true;}\n");
    fprintf(f, "})();\n");
    fprintf(f, "</script>\n");
...
static const char *proof_color_to_html_hex(ProofColor c) {
    switch (c) {
        case PROOF_COLOR_GREEN:              return "#4CAF50";
        case PROOF_COLOR_BLUE_UNEXPLORED:
        case PROOF_COLOR_BLUE_RESOURCE:
        case PROOF_COLOR_BLUE_OUT_OF_RANGE:  return "#42A5F5";
        case PROOF_COLOR_GREEN_VERIFIED:     return "#2E7D32";
        case PROOF_COLOR_YELLOW:             return "#FDD835";
        case PROOF_COLOR_ORANGE_ORACLE:
        case PROOF_COLOR_ORANGE_EX_FALSO:    return "#FF9800";
        case PROOF_COLOR_AMBER:              return "#FFB300";
        case PROOF_COLOR_DARK_ORANGE:        return "#E65100";
...
bool proof_export_coq(ProofNavigator *nav, const char *filepath) {
    ...
    for (int i = 0; i < nav->step_count; i++) {
        ...
        /* 输出步骤注释 */
        if (step->note && step->type != PROOF_STEP_ADD_NODE &&
    ...
    int prop_a_id, int prop_b_id)
{
    ...
    if (nav->equivalence_count >= nav->equivalence_capacity) {
        ...
        PropositionEquivalence *new_arr = lv00_realloc(nav->equivalences,
            new_cap * sizeof(PropositionEquivalence));
        if (!new_arr) return;
    ...
    if (proof_stream_ctx != NULL) {
        char buf[128];
        snprintf(buf, sizeof(buf), "命题等价声明: prop_%d <-> prop_%d",
            prop_a_id, prop_b_id);
        stream_emit_simple(proof_stream_ctx, STREAM_EVENT_INFO, buf, 0);
    }
}
...
int proof_find_equivalent_proposition(
    ...
    int max_count)
{
    if (!nav || !equivalent_ids || max_count <= 0) return 0;
...
static void collect_dependencies(
    const ProofDependency *dep,
    int *dep_ids,
    char **dep_hashes,
    int *count,
    int max_count)
{
    if (!dep || *count >= max_count) return;
...
int proof_validate_dependencies(
    ...
    int max_results)
{
    if (!nav || !results || max_results <= 0) return 0;

    if (!nav->dep_tree) return 0;
...
void proof_set_lemma_view_state(ProofNavigator *nav, int step_id, LemmaViewState state) {
    ...
    if (nav->lemma_view_count >= nav->lemma_view_capacity) {
        ...
        if (!new_ids) return;
        LemmaViewState *new_states = lv00_realloc(nav->lemma_view_states,
            new_cap * sizeof(LemmaViewState));
        if (!new_states) {
...
LemmaViewState proof_get_lemma_view_state(const ProofNavigator *nav, int step_id) {
    if (!nav || step_id < 0) return LEMMA_VIEW_EXPANDED; /* 默认展开 */
...
const char *proof_color_to_string(ProofColor color) {
    switch (color) {
        ...
        case PROOF_COLOR_BLUE_UNEXPLORED: return "Blue (Unexplored)";
        case PROOF_COLOR_BLUE_RESOURCE: return "Blue (Resource Limited)";
        case PROOF_COLOR_BLUE_OUT_OF_RANGE: return "Blue (Out of Range)";
        case PROOF_COLOR_GREEN_VERIFIED: return "Green (Verified Unconstructible)";
        case PROOF_COLOR_YELLOW: return "Yellow (Conditional)";
        case PROOF_COLOR_ORANGE_ORACLE: return "Orange (Oracle)";
        case PROOF_COLOR_ORANGE_EX_FALSO: return "Orange (Ex Falso)";
        case PROOF_COLOR_AMBER: return "Amber (Numeric)";
        case PROOF_COLOR_DARK_ORANGE: return "Dark Orange";
...
const char *proposition_type_to_string(PropositionType type) {
    switch (type) {
        case PROPOSITION_ATOMIC: return "Atomic";
        case PROPOSITION_CONJUNCTION: return "Conjunction";
        case PROPOSITION_DISJUNCTION: return "Disjunction";
        case PROPOSITION_IMPLICATION: return "Implication";
        case PROPOSITION_NEGATION: return "Negation";
        case PROPOSITION_UNIVERSAL: return "Universal";
        case PROPOSITION_EXISTENTIAL: return "Existential";
        case PROPOSITION_BOTTOM: return "Bottom";
        default: return "Unknown";
    }
}
...
const char *proof_step_type_to_string(ProofStepType type) {
    switch (type) {
        case PROOF_STEP_ADD_NODE: return "Add Node";
        case PROOF_STEP_ADD_CONSTRAINT: return "Add Constraint";
        case PROOF_STEP_REWRITE: return "Rewrite";
        case PROOF_STEP_FUNCTION_APP: return "Function Application";
        case PROOF_STEP_PACK_FUNCTION: return "Pack Function";
        case PROOF_STEP_NORMALIZATION: return "Normalization";
        case PROOF_STEP_UNIFY: return "Unify";
        case PROOF_STEP_EX_FALSO: return "Ex Falso";
...
const char *unify_result_to_string(UnifyStatus result) {
    switch (result) {
        case UNIFY_STATUS_OK: return "OK";
        case UNIFY_STATUS_PORT_TYPE_MISMATCH: return "Port Mismatch";
        case UNIFY_STATUS_CONSTRAINT_MISMATCH: return "Constraint Mismatch";
        case UNIFY_STATUS_COORD_MISMATCH: return "Coordinate Mismatch";
        case UNIFY_STATUS_STRUCTURE_MISMATCH: return "Structure Mismatch";
        case UNIFY_STATUS_SCOPE_MISMATCH: return "Scope Mismatch";
        case UNIFY_STATUS_FAILED: return "Error";
        default: return "Unknown";
    }
}
 ...
 */
static int find_concrete_replacement(
    const int *type_var_to_concrete,
    int mapping_count,
    int type_var_node_id)
{
    if (!type_var_to_concrete || mapping_count <= 0) return -1;
        ...
        if (node->type == GEOM_FUNCTION_BLOCK) {
            ...
            for (int j = 0; j < node->data.func_block.input_count; j++) {
                ...
                if (port_node && port_node->type == GEOM_PORT &&
                    port_node->data.port && port_node->data.port->type_region &&
    ...
    int mapping_count)
{
    ...
    if (prop->input_count > 0 && prop->input_port_ids) {
        ...
        memcpy(inst->input_port_ids, prop->input_port_ids,
               prop->input_count * sizeof(int));
    ...
    if (prop->output_count > 0 && prop->output_port_ids) {
        ...
        memcpy(inst->output_port_ids, prop->output_port_ids,
               prop->output_count * sizeof(int));
    ...
    if (prop->precondition_count > 0 && prop->precondition_region_ids) {
        ...
        memcpy(inst->precondition_region_ids, prop->precondition_region_ids,
               prop->precondition_count * sizeof(int));
    ...
    /* ---- 3. 递归深拷贝子命题（也进行实例化） ---- */
    for (int i = 0; i < prop->sub_prop_count; i++) {
        Proposition *sub_inst = proof_instantiate_proposition(
            prop->sub_props[i], type_var_to_concrete, mapping_count);
        if (!sub_inst) {
            proposition_destroy(inst);
            return NULL;
        }
    ...
    if (type_var_to_concrete && mapping_count > 0 && inst->pattern) {
        ...
        for (int i = 0; i < inst->pattern->node_count; i++) {
            ...
            if (node->type == GEOM_PORT && node->data.port) {
                TypeRegion *tr = node->data.port->type_region;
                if (tr && tr->kind == TYPE_KIND_VARIABLE) {
                    int replacement_id = find_concrete_replacement(
                        type_var_to_concrete, mapping_count, tr->variable_id);
                    if (replacement_id >= 0) {
        ...
        for (int i = 0; i < inst->pattern->constraint_count; i++) {
            ...
            if (!c || !c->participants) continue;

            for (int j = 0; j < c->participant_count; j++) {
                int replacement = find_concrete_replacement(
                    type_var_to_concrete, mapping_count, c->participants[j]);
                if (replacement >= 0) {
        ...
        for (int i = 0; i < inst->input_count; i++) {
            int replacement = find_concrete_replacement(
                type_var_to_concrete, mapping_count, inst->input_port_ids[i]);
            if (replacement >= 0) {
        ...
        for (int i = 0; i < inst->output_count; i++) {
            int replacement = find_concrete_replacement(
                type_var_to_concrete, mapping_count, inst->output_port_ids[i]);
            if (replacement >= 0) {
        ...
        for (int i = 0; i < inst->precondition_count; i++) {
            int replacement = find_concrete_replacement(
                type_var_to_concrete, mapping_count, inst->precondition_region_ids[i]);
            if (replacement >= 0) {
        ...
        for (int i = 0; i < inst->postcondition_count; i++) {
            int replacement = find_concrete_replacement(
                type_var_to_concrete, mapping_count, inst->postcondition_constraint_ids[i]);
            if (replacement >= 0) {
        ...
        for (int i = 0; i < inst->pattern->node_count; i++) {
            ...
            for (int j = 0; j < node->data.func_block.internal_node_count; j++) {
                int old_id = node->data.func_block.internal_nodes[j]
                    ? node->data.func_block.internal_nodes[j]->id : -1;
                int replacement = find_concrete_replacement(
                    type_var_to_concrete, mapping_count, old_id);
                if (replacement >= 0) {
            ...
            for (int j = 0; j < node->data.func_block.input_count; j++) {
                int replacement = find_concrete_replacement(
                    type_var_to_concrete, mapping_count,
...
{
    if (!nav || !graph || !info) {
        if (info) info->result = UNCONSTRUCT_ERROR;
    ...
    /* 流式输出 */
    if (proof_stream_ctx) {
        stream_emit_simple(proof_stream_ctx, STREAM_EVENT_PROOF_UNIFY,
                           "不可构造性检查开始", -1);
    }
            ...
            for (int j = 0; j < pkg->unconstructible_count; j++) {
                ...
                if (pattern_match) {
                    ...
                    snprintf(report, sizeof(report),
                        "构造匹配已知的不可构造问题 '%s'（来自公理包 '%s'）",
                        ku->name, pkg->name ? pkg->name : "未知");
    ...
    UnconstructInfo *info)
{
    ...
    UnconstructResult base_result = proof_check_unconstructibility(
        nav, graph, prop, info);
    ...
    /* 提取构造中的代数方程并检查可解性 */
    {
        ...
        if (temp_sys) {
            ...
            solver_extract_equations_full(
                (ConstraintGraph *)graph, temp_sys);
            ...
            for (int i = 0; i < sys_count; i++) {
                ...
                if (poly) {
                    int deg = poly->degree;
                    if (deg == 2) quadratic_count++;
                    else if (deg == 3) cubic_count++;
                    else if (deg == 4) quartic_count++;
                    else if (deg > 4) higher_count++;
                }
            }
            ...
            if (cubic_count > 0 || quartic_count > 0 || higher_count > 0) {
                ...
                if (info->detailed_report) lv00_free((void**)&info->detailed_report);
                info->detailed_report = lv00_strdup(report);
            }
        ...
        for (int pkg_idx = 0; pkg_idx < nav->engine->axiom_package_count; pkg_idx++) {
            ...
            for (int ku_idx = 0; ku_idx < pkg->unconstructible_count; ku_idx++) {
                ...
                if (ku->reduces_to && strlen(ku->reduces_to) > 0) {
                    ...
                    /* 基于构造大小的启发式归约检查 */
                    if (strstr(ku->reduces_to, "trisection") ||
                        strstr(ku->reduces_to, "三等分")) {
                        /* 检查是否可以归约到三等分角 */
                        for (int k = 0; k < graph->constraint_count; k++) {
                            if (graph->constraints[k]->type == BETWEENNESS) {
                                can_reduce = true;
                    ...
                    } else if (strstr(ku->reduces_to, "doubling") ||
                               strstr(ku->reduces_to, "倍立方")) {
                        /* 检查是否可以归约到倍立方 */
                        /* 需要特定比例约束 */
                ...
                if (ku->dependency_count > 0 && ku->dependency_chain) {
                    ...
                    for (int dep_idx = 0; dep_idx < ku->dependency_count; dep_idx++) {
                        if (!ku->dependency_chain[dep_idx]) continue;
    ...
    if (info->result == UNCONSTRUCT_MAYBE_POSSIBLE) {
        info->proof_strategy = info->proof_strategy ?
            info->proof_strategy : "多策略分析未能证明不可构造性";
    }
