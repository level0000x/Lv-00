...
SparseMatrix *sparse_matrix_clone(const SparseMatrix *mat)
{
    ...
    if (mat->nnz > 0) {
        ...
        if (mat->fmt == SPARSE_CSR || mat->fmt == SPARSE_CSC) {
            clone->row_ptr = (int *)calloc((size_t)mat->rows + 1, sizeof(int));
            if (!clone->row_ptr) {
                sparse_matrix_destroy(clone);
                return NULL;
            }
            memcpy(clone->row_ptr, mat->row_ptr,
                   ((size_t)mat->rows + 1) * sizeof(int));
        }

        clone->col_idx = (int *)calloc((size_t)mat->nnz, sizeof(int));
        if (!clone->col_idx) {
            sparse_matrix_destroy(clone);
            return NULL;
        }
        ...
        if (mat->values) {
            clone->values = (double *)calloc((size_t)mat->nnz, sizeof(double));
            if (!clone->values) {
                sparse_matrix_destroy(clone);
                return NULL;
            }
...
void sparse_matrix_print(const SparseMatrix *mat, const char *name)
{
    ...
    const char *fmt_str = (mat->fmt >= 0 && mat->fmt <= 3)
                          ? fmt_names[mat->fmt] : "UNKNOWN";
...
static double add_plus(double a, double b)      { return a + b; }
static double mul_times(double a, double b)     { return a * b; }
static double add_min(double a, double b)       { return fmin(a, b); }
static double add_max(double a, double b)       { return fmax(a, b); }
static double mul_plus(double a, double b)      { return a + b; }
static double add_or(double a, double b)        { return (a != 0.0 || b != 0.0) ? 1.0 : 0.0; }
static double mul_and(double a, double b)       { return (a != 0.0 && b != 0.0) ? 1.0 : 0.0; }
static double add_interval(double a, double b)  { return fmax(a, b); }
static double mul_interval(double a, double b)  { return (a + b); }
...
static void semiring_matvec(const SparseMatrix *A, const Semiring *sr,
                                    ...
                                    int max_iter)
{
    ...
    double *x_new = (double *)calloc((size_t)node_count, sizeof(double));
    if (!x_new) {
                                ...
                                SparseMatrix **mat)
{
    ...
    int *row_nnz = (int *)calloc((size_t)n_rows, sizeof(int));
    if (!row_nnz) {
        return false;
    }
    ...
    csr->nnz = total_nnz;
    csr->row_ptr = (int *)calloc((size_t)n_rows + 1, sizeof(int));
    csr->col_idx = (int *)calloc((size_t)total_nnz, sizeof(int));
    csr->values  = (double *)calloc((size_t)total_nnz, sizeof(double));
    csr->owns_data = true;
                            ...
                            SparseMatrix **C)
{
    ...
    double *work = (double *)calloc((size_t)n, sizeof(double));
    if (!work) {
        return false;
    }

    /* 绗竴閬嶏細缁熻 C 姣忚鐨勯潪闆跺厓绱犳暟 */
    int *c_row_nnz = (int *)calloc((size_t)m, sizeof(int));
    if (!c_row_nnz) {
        free(work);
        return false;
    }
    ...
    csr->row_ptr = (int *)calloc((size_t)m + 1, sizeof(int));
    csr->row_ptr[0] = 0;
    int total_nnz = 0;
    for (int i = 0; i < m; i++) {
                             ...
                             SparseMatrix **out)
{
    ...
    t->nnz = mat->nnz;
    t->row_ptr = (int *)calloc((size_t)n + 1, sizeof(int));
    t->col_idx = (int *)calloc((size_t)mat->nnz, sizeof(int));
    t->values  = NULL;
    t->owns_data = true;
    ...
    if (mat->values) {
        t->values = (double *)calloc((size_t)mat->nnz, sizeof(double));
        if (!t->values) {
            sparse_matrix_destroy(t);
            return false;
        }
    ...
    int *col_counts = (int *)calloc((size_t)n, sizeof(int));
    if (!col_counts) {
        sparse_matrix_destroy(t);
        return false;
    }
    ...
    /* 濉厖杞疆鍚庣殑鍒楃储寮曞拰鍊?*/
    int *offset = (int *)calloc((size_t)n, sizeof(int));
    if (!offset) {
        free(col_counts);
        sparse_matrix_destroy(t);
        return false;
...
bool sparse_cholesky_solve(const SparseMatrix *A,
                           ...
                           double *x)
{
    /* TODO: 闆嗘垚 SuiteSparse CHOLMOD
     * 1. 灏?SparseMatrix 杞崲涓?cholmod_sparse
     * 2. cholmod_analyze 鈫?cholmod_factorize 鈫?cholmod_solve
     * 3. 灏?cholmod_dense 缁撴灉鎷疯礉鍥?x
     */
    if (!A || !b || !x) {
        return false;
    }
    /* 鍗犱綅绗︼細绠€鍗曞洖閫€涓虹瀵嗛珮鏂秷鍏冿紙浠呴檺闈炲父灏忕殑鐭╅樀锛?*/
    if (A->rows != A->cols || A->rows > 10) {
        fprintf(stderr, "[sparse_cholesky_solve] TODO: integrate CHOLMOD. "
                "Matrix too large for placeholder.\n");
        return false;
    }
    /* 灏忓瀷鐭╅樀锛氭嫹璐濅负绋犲瘑鐭╅樀鍚庨珮鏂秷鍏?*/
    int n = A->rows;
...
bool sparse_lu_solve(const SparseMatrix *A,
                     ...
                     double *x)
{
    /* TODO: 闆嗘垚 SuiteSparse UMFPACK
     * 1. 灏?SparseMatrix CSR 鈫?CSC锛圲MFPACK 浣跨敤 CSC锛?     * 2. umfpack_di_symbolic 鈫?umfpack_di_numeric 鈫?umfpack_di_solve
     */
    if (!A || !b || !x) {
        return false;
    }
    ...
    if (A->rows != A->cols || A->rows > 10) {
        fprintf(stderr, "[sparse_lu_solve] TODO: integrate UMFPACK. "
                "Matrix too large for placeholder.\n");
        return false;
    }
...
bool sparse_qr_solve(const SparseMatrix *A,
                     ...
                     double *x)
{
    /* TODO: 闆嗘垚 SuiteSparse SPQR
     * 1. 灏?SparseMatrix 杞崲涓?cholmod_sparse
     * 2. SuiteSparseQR 鍒嗚В骞舵眰瑙?     */
    if (!A || !b || !x) {
        return false;
    }

    if (A->rows > 10 || A->cols > 10) {
        fprintf(stderr, "[sparse_qr_solve] TODO: integrate SPQR. "
                "Matrix too large for placeholder.\n");
        return false;
    }

    /* 鍗犱綅绗︼細瀵逛簬灏忓瀷闂锛岄€€鍖栦负 Cholesky 姹傝В姝ｈ鏂圭▼ A^T A x = A^T b */
                           ...
                           DegreeAnalysis **analysis)
{
    ...
    DegreeAnalysis *da = (DegreeAnalysis *)calloc(1, sizeof(DegreeAnalysis));
    if (!da) {
        return false;
    }
    ...
    da->node_degrees = (int *)calloc((size_t)n_nodes, sizeof(int));
    if (!da->node_degrees) {
        free(da);
        return false;
    }
    ...
    for (int i = 0; i < n_nodes; i++) {
        ...
        total_degree += deg;
        if (deg > da->max_degree) da->max_degree = deg;
        if (deg < da->min_degree && deg > 0) da->min_degree = deg;
        if (deg == 0) da->isolated_count++;
    }
    ...
    da->degree_counts = (int *)calloc((size_t)hist_len, sizeof(int));
    if (!da->degree_counts) {
...
void degree_analysis_free(DegreeAnalysis *analysis)
{
    ...
    free(analysis->node_degrees);
    free(analysis->degree_counts);
    free(analysis);
}
