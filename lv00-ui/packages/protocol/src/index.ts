...
export interface CommandMessage {
  id: number;
  type: 'execute' | 'getState' | 'subscribe' | 'unsubscribe';
  payload: unknown;
}
...
export interface DragState {
  ...
  accumulatedWorldDeltaX: number;
  accumulatedWorldDeltaY: number;
}

// ---- 命中测试 ----

export interface HitResult {
  type: 'NONE' | 'NODE' | 'CONSTRAINT' | 'REGION';
  id: number;
  distance: number;
}
...
function constraintTypeCode(t: ConstraintType): number {
  const map: Record<string, number> = {
    CONSTR_INCIDENCE: 0, CONSTR_BETWEENNESS: 1, CONSTR_INTERSECTION: 2,
    CONSTR_CONTAINMENT: 3, CONSTR_CONNECTION: 4, CONSTR_SEGMENT: 5,
    CONSTR_EQUAL: 6, CONSTR_PERPENDICULAR: 7, CONSTR_PARALLEL: 8, CONSTR_COLLINEAR: 9,
  };
  return map[t] ?? 0;
}
...
function encodeCoords(coords: Coord[]): number[] {
  ...
  for (const c of coords) {
    if (c.type === 'SYM_RATIONAL') {
      result.push(0, c.num, c.den);
    } else if (c.type === 'SYM_QUADRATIC') {
      result.push(2, c.a, c.b, c.n);
    } else {
      result.push(1, c.degree, ...c.coeffs.slice(0, 16));
    }
  }
  return result;
}
...
// Coord 创建工厂
export function coordRational(num: number, den: number = 1): RationalCoord {
  return { type: 'SYM_RATIONAL', num, den };
}

export function coordQuadratic(a: number, b: number, n: number): QuadraticCoord {
  return { type: 'SYM_QUADRATIC', a, b, n };
}

export function coordFromDouble(value: number): RationalCoord {
  const den = 1000000;
  return { type: 'SYM_RATIONAL', num: Math.round(value * den), den };
}
...
export function coordToString(c: Coord): string {
  ...
  if (c.type === 'SYM_QUADRATIC') {
    return `${c.a}+${c.b}√${c.n}`;
  }
  return `poly(deg=${c.degree})`;
}

export function coordToDouble(c: Coord): number {
  if (c.type === 'SYM_RATIONAL') return c.num / c.den;
  if (c.type === 'SYM_QUADRATIC') return c.a + c.b * Math.sqrt(c.n);
  return 0;
}

export function intToRGBA(color: number): string {
