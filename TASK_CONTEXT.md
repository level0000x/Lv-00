...
## 一百二十、批次 PFL 设计参考分析（2026-08-27）

用户提供旧构思《PFL v0.478-dev 完整设计》——"声明即言说"的哲学形式化模板 DSL
（@syntax 句法模式 / @say 言说模板 / @revert 褪回渲染），询问可参考处。

### ① 分析结论（dsl-pfl-reference.md）

- **定位差异**：PFL 是文本渲染 DSL（声明→AST→渲染回文本，双向），
  Lv-00 是几何求解 DSL（声明→AST→IR→约束图，单向）；仅机制层相通。
- **高价值借鉴 R1-R9**：
  - R1 `$name??` 可空 + `$none` 空节点：空态放 **AST 层**而非值层 null，
    直接收敛 S20/S21 从"语义层大改造"降为"2 个新 IR 指令"；
  - R2 `@say`/`@revert` 双向渲染：SVG 导出/ga_codegen 本质是"褪回"，
    记录为远期架构方向（不动现有设施）；
  - R3 `@theory` 命名空间：S17 完整蓝图（子作用域 + 只读导入 + @export + 别名）；
  - R4 三级诊断码体系（错误/警告/信息 55 码 + 修复索引 + 抑制）：新增立项，
    DSL-A 错误处理升级 ~400-600 行；
  - R5 错误恢复点（`;`/`@`/`#!`）：解析器多错误报告 ~150-300 行；
  - R6 严格元数 + 终端函数：印证"IR 层永不引入变参操作码"设计决策（记录）；
  - R7 重复变量 `$x+[分隔符]` + `:each`/`$@index`：并入 S15 结果命名；
  - R8 媒介变量 `$~x` + `@default`：声明式默认替代 S14 with 块（0 新 IR）；
  - R9 推导首参数分组索引：并入 S19（O(N)→O(log N)）。
- **不建议借鉴**：文本捕获（$text/$line）、中文边界、哲学注释、$eval 沙箱、
  褪回深度、模式感知词法、自举语法（与 S16 模板宏路线冲突）。
- **工作量影响**：4600-6700 → 约 5150-7600 行（含 R4/R5 新增 550-900 行）。

### ② 同步修订（dsl-syntax-sugar-design.md v1.1）

- S14/S15/S17/S19/S20/S21 六条按 PFL 参考修订，顶部加 v1.1 修订注记。
...
- 设计深化 / 不执行 / PFL 参考吸收 / 可空 AST 层方案 / 命名空间蓝图 /
  诊断码体系独立立项建议。
- **待用户确认**：D1 R4+R5 是否优先独立实施；D2 @export 与 load 兼容；
  D3 模板化渲染远期方向；D4 AST 层空节点方案采纳。

### 遗留登记

- 无新增遗留（PFL 分析全部为设计留档，未执行）。

## 一百二十一、批次 语法糖/PFL 设计决策定稿（2026-08-27）

用户「可采纳的都采纳看你」——授权由我判断并采纳全部合理设计决策，
设计文档从"深化"升格为"定稿"（仍不执行代码）。

### ① 采纳决策汇总

| 决策 | 结论 |
|---|---|
| 语法糖落地顺序 | 第一批 S1-S8（表达式级，0 新 IR）→ 第二批 S9-S15 → 第三批 S16-S24 逐个立项 |
| 两套 DSL 处置 | **选项 A**：糖加 DSL-A + S12 从 DSL-B 反移植量词/度量；统一留待糖稳定后（选项 C 不做） |
| D1 R4+R5 独立实施 | 采纳：0 批次独立优先（诊断码三级体系 + 错误恢复点，~550-900 行，纯质量升级） |
| D2 @export 与 load 兼容 | 采纳双轨兼容：load 保留全局可见旧语义零改动；新增 import ... use 符号 [as 别名] 显式只读导入 |
| D3 R2 模板化渲染 | 采纳为远期记录：不动 ga_codegen，仅新增第 7 渲染目标时评估 |
| D4 R1 AST 层空节点 | 采纳：S20/S21 用 AST 层空节点 + 2 新 IR（OPTIONAL_BIND/FALLBACK） |

### ② 文档更新（定稿）

- dsl-syntax-sugar-design.md → v1.2 定稿：§6 实施路线加 0 批次（R4+R5）、
  决策定稿表、§7 改为选项 A 决策；
- dsl-pfl-reference.md → v1.1 定稿：§7 决策定稿表（D1-D4 全采纳）。
...
- 设计定稿 / 全部采纳 / 语法糖三批 + 0 批次顺序确定 / 选项 A / D1-D4 采纳。
- **遗留待执行**：0 批次（R4+R5 诊断码/错误恢复）与第一批 S1-S8 的执行
  待用户指示（设计定稿完成，未开始实现）。
...
## 一百二十五、批次 标准统一化第三轮审计（2026-08-27）

用户「再开子代理找其他方面的」——第三轮五路并行审计补充 13 组
"一需求多实现"重复点（总 43 组）。

### ① 第三轮审计结果（L1-L11）

- **引擎生命周期面 L1-L2**：`lvContextState`（context.h，5 态）与 `EngineState`
  （engine.h:196，5 态）**同一副骨架实现两份**——状态集一致、转移图逐位一致、
  状态名映射几乎逐字重复，连"不合并"豁免注释都是复制粘贴的；
  解析→推理→完成进度被建模三次（lvContextState 便捷路径用 / EngineState
  **除定义文件外零生产调用几乎死代码** / lvStageStatus 编排真实主路径）。
- **推理后端面 L3**：真正可用的求解后端只有 GROEBNER；"判定可满足"有
  ~9 条平行路径；注册机制 5+ 套并存（SMT/ATP/EngineScheduler/插件/数值），
  引擎真实分发走 engine_scheduler.c 硬编码静态 VTable **绕开所有注册表**；
  GROEBNER 在 4 处各有一份实例。
- **流/交互协议面 L4**：事件→JSON 字段契约 3 套（C 13 字段权威 vs Python
  14/16 字段漂移副本）；`stream.event` JSON-RPC 序列化器 3 套；WS 服务器
  双栈（C 自实现 RFC6455 vs Python websockets）；interop 21 命令与 L9
  五命令是两套不同协议。
- **内存/日志面 L5-L8**：调试分配器逻辑两份（allocator.c vtable 权威 vs
  lv_malloc_tracked 内联重写零调用）；内存统计两套（扁平实际使用 vs
  按类型生产零调用，README 引用无实现 API）；泄漏检测三路径文档与实现
  脱节；**日志级别 3-4 套词汇表语义方向互相打架**（lv_log.h 用魔法数字
  规避同名冲突）。
- **前端/文档面 L9-L11**：**前端 100% mock 数据与 C 内核零连接**
  （proof_widget.c 为前端声明的 C API 契约成未使用孤岛）；证明/求解器/
  几何/数值文档同主题多篇泛滥（5+5+6+7 篇）；版本号互相打架（v1.1.0 vs
  v3.5.0 vs v5.0.0）；**同一语言语法定义 ≥3 处**（lv_LANGUAGE_SPEC vs
  根 docs/DSL 三篇 vs bootstrap .lv）。

### ② 设计更新（standard-unification-design.md v1.2）

- 新增 §1.11-1.15 现状清单（L1-L11）+ §3.11-3.15 分面方案；
- P0-P4 优先级并入第三轮项（P0 含状态机合并+删 tracked 分配器+删重复文档；
  P1 含进度模型收敛+事件契约单一化+内存统计+日志级别；P3 含推理注册表+
  前端接内核）；
- 新增决策点 F9-F12（状态机合并/推理注册表/流契约/前端接内核）。
...
- 设计深化 / 不执行 / 第三轮审计 13 组 / 总 43 组 / L1-L11 方案 / F9-F12 待确认。
...
## 一百四十二、批次 标准统一化处理原则定稿（2026-08-27，用户指令）

用户说明处理思路：「代码如果需要删需要评审真的没有用了再删」。

### ① 处理原则定稿

- **删除需评审**：代码如需删除，**先评审、确认真的没有用了再删**。
- **默认接线**：所有"接线或删除二选一"的处置项（P0-P4 全部涉及）**默认走接线**
  （激活为真实实现或登记为显式豁免/退役），删除是最后手段。
- **评审前置条件**（满足才可删）：全库 grep 确认零生产调用 + 文档/外部契约无
  引用 + 无测试钉住其行为 + 历史价值评估（示例/数据优先归档而非删除）。
- **评审记录**：评审结论（"确认无用可删"或"保留待接线"）记入 TASK_CONTEXT
  决策登记。
- **与 K5 衔接**：覆盖 K5 已定稿的示例代码处置（归档优先于删除，按 v1.9.3
  既有决策执行）。

### ② 设计更新（standard-unification-design.md）

- §2 统一化原则新增第 6 条「删除需评审（用户 2026-08-27 定稿）」；
- §4 优先级表前新增同注记（执行时必须遵守）。

### ③ 验证

- 纯文档登记，无代码改动；build3 + ctest 288/288 不受影响。

### 决策登记

- 处理原则定稿 / 不执行 / 删除需评审，默认接线，删除是最后手段 /
  **覆盖 P0-P4 全部删除动作 + K5 示例处置（归档优先）**。

## 一百四十三、批次 标准统一化实施前讨论原则定稿（2026-08-27，用户指令）

用户说明处理思路：「任何设计级别实施无论是否和我讨论过以及历史确认过实施前
必须和我讨论后再开始执行」。

### ① 处理原则定稿

- **实施前必须讨论**：任何设计级别实施，**无论是否与用户讨论过、无论历史是否
  已确认，实施前必须先与用户讨论，得到确认后再开始执行**。
- **历史已确认不构成执行许可**：本设计文档 P0-P4 全部实施项、F1-F81 决策点，
  以及此前各轮已定稿的设计决策（DSL 语法糖、K5 示例处置、双 DSL 处置、前端
  留白、PFL 参考等）均不因"历史已确认"而自动获得执行许可。
- **批次实施清单**：每个实施批次开始前，向用户提交实施清单（做什么、改哪些
  文件、预期影响、回归策略），经用户明确同意后才执行。
- **暂停权**：未经讨论确认的设计实施一律不启动；已启动的若用户要求，先暂停
  等待讨论。
- **覆盖范围**：本设计文档全部内容及其关联设计文档（dsl-syntax-sugar-design、
  dsl-pfl-reference、dual-mode-execution-design、L9-scheduler-design 等）的落地执行。

### ② 设计更新（standard-unification-design.md）

- §2 统一化原则新增第 7 条「实施前必须讨论（用户 2026-08-27 定稿）」；
- §4 优先级表前注记补充同原则。
...
- 处理原则定稿 / 不执行 / 实施前必须讨论，历史已确认不构成执行许可 /
  **覆盖 P0-P4 全部实施项 + F1-F81 决策点 + 所有历史定稿设计决策**。
...
## 一百四十七、批次 标准统一化决策确认第一批（2026-08-31，用户确认）

用户「开始讨论和确认吧」→「可以确认」——F1-F96 决策确认启动，
第一批（P0 真实缺陷候选密集组 10+1 个）全部确认。

### ① 确认结果

| 决策点 | 结论 | 备注 |
|---|---|---|
| F53（K27 整数安全） | ✅ 确认 | P0 优先；唯一具体 UB + 解析链防护缺失 |
| F54（K28 递归深度） | ✅ 确认 | P0 优先；文档宣称防护生产裸奔 |
| F55（K29 锁顺序） | ✅ 确认 | P0 优先；2 个真实死锁风险点 |
| F64（K38 时间/时钟） | ✅ 确认 | P0 优先；sledge 超时形同虚设 + solver 超时门恒不触发 |
| F65（K39 协议编解码） | ✅ 确认 | P0 优先；LVZD 头部越界；**LVZD 删除项按红线评审** |
| F69（K43 对象池） | ✅ 确认 | P0 优先；coeff_pool 尺寸失配越界（deg≥4 minimal_poly 可复现） |
| F87（K61 错误通道） | ✅ 确认 | P0 优先；3 个真实缺陷候选 |
| F88（K62 配置持久化） | ✅ 确认 | P0 优先；2 个真实缺陷候选 |
| F91（K65 日志输出） | ✅ 确认 | P0 优先；3 个真实缺陷候选；**删除项按红线评审** |
| F92（K66 结构体布局） | ✅ 确认 | P0 优先；池越界防御 + ABI 错位 + 潜伏越界 |
| F94（K68 平台分支） | ✅ 确认 | **随 K9 批次立项**（非独立 P0） |

### ② 设计文档更新

- §6 决策点 F53/F54/F55/F64/F65/F69/F87/F88/F91/F92 标注
  `→ **✅ 已确认**（2026-08-31 用户，P0 优先）`；F94 标注
  `→ **✅ 已确认**（2026-08-31 用户，随 K9 批次立项）`。
...
- 决策确认第一批 11 个确认 / F53 F54 F55 F64 F65 F69 F87 F88 F91 F92 P0 优先 /
  F94 随 K9 批次 / F65 F91 删除项按红线评审 / 实施前仍按红线提交实施清单。

### 遗留登记

- 待确认：F1-F52 中未确认项 + F56-F63 + F66-F68 + F70-F86 + F89-F90 + F93 +
  F95-F96（第二批起逐批讨论）。

## 一百四十八、批次 标准统一化决策确认第二批（2026-08-31，用户确认）

用户「可以」——第二批（P0 剩余 · 真实缺陷/风险密集组 12 个）全部确认。

### ① 确认结果

| 决策点 | 结论 | 备注 |
|---|---|---|
| F43（K15 全局状态线程安全） | ✅ 确认 | P0 优先；3 个真实竞态 |
| F45（K18 打包/导出） | ✅ 确认 | P0 优先；find_package 必坏 + visibility hidden 失导出 |
| F49（K22 魔法字符串） | ✅ 确认 | P0 优先；已产生真实数值/拼写错误 |
| F60（K34 命令/参数） | ✅ 确认 | P0 优先；2 个疑似真实命令语义缺陷 |
| F61（K35 编码/字符） | ✅ 确认 | P0 优先；2 个真实输出缺陷 |
| F62（K36 调试诊断） | ✅ 确认 | P0 优先；崩溃保护失效 M5；**含删除项走评审** |
| F67（K41 几何判定） | ✅ 确认 | P0 优先；2 个语义错误占位 + 1 验证缺口 |
| F70（K44 配置收敛） | ✅ 确认 | P0 优先；六级默认值漂移 + 107 getter 二选一 |
| F71（K45 API 校验） | ✅ 确认 | P0 优先；误导性诊断链 + 198 行永不运行 |
| F73（K47 树形结构） | ✅ 确认 | P0 优先；2 个 M6 声称-实现脱节 |
| F74（K48 符号表） | ✅ 确认 | P0 优先；量词泄漏风险 |
| F75（K49 依赖获取） | ✅ 确认 | P0 优先；幻影 API 声称 |

### ② 设计文档更新

- §6 决策点 F43/F45/F49/F60/F61/F62/F67/F70/F71/F73/F74/F75 标注
  `→ **✅ 已确认**（2026-08-31 用户，P0 优先）`；F62 追加
  `；含删除项走评审`。
...
- 决策确认第二批 12 个确认 / P0 优先 / F62 含删除项走评审 /
  实施前仍按红线提交实施清单。

### 遗留登记

- 待确认 P0 剩余 19 个：F1/F7/F16/F23/F24/F27/F28/F33/F35/F39/F40/F59/F77/
  F79/F80/F81/F85/F86/F93（第三批）。
- 其余确认型与 P1 待后续批次。

## 一百四十九、批次 标准统一化决策确认第三批（2026-08-31，用户确认）

用户「可以」——第三批（P0 剩余 19 个）全部确认，**P0 全部确认完毕**。

### ① 确认结果

| 决策点 | 结论 | 备注 |
|---|---|---|
| F1（P0 死代码/冗余清理独立批次） | ✅ 确认 | P0 独立批次优先；**删除项走评审** |
| F7（E11 断言参数序） | ✅ 确认 | P0 优先；纯测试面 |
| F16（G1 解析安全接线） | ✅ 确认 | P0 优先；安全面 |
| F23（H5 Python 测试基座） | ✅ 确认 | P0 优先；测试面 |
| F24（I5 层验证机制） | ✅ 确认 | P0 独立立项；先修空转机制 |
| F27（I3 增长逻辑） | ✅ 确认 | P0 优先；纯机械 |
| F28（J1 生命周期统一） | ✅ 确认 | P0 独立立项 |
| F33（K2 快速幂/平方因子） | ✅ 确认 | P0 优先；纯算法 |
| F35（K5 示例面修正） | ✅ 确认 | P0 优先；误导性最强 |
| F39（K10 所有权三态） | ✅ 确认 | P0 优先；泄漏+UB |
| F40（K11 测试设施移出） | ✅ 确认 | P0 优先 |
| F59（K33 文件 IO 闭环） | ✅ 确认 | P0 优先；一次闭环 I4 遗留 |
| F77（K51 迭代器/遍历） | ✅ 确认 | P0 优先；潜伏 UAF+约定漂移 |
| F79（K53 错误码管理） | ✅ 确认 | P0 优先；潜在编译错误 |
| F80（K54 引用计数） | ✅ 确认 | P0 优先；M6+M5 |
| F81（K55 版本/ABI） | ✅ 确认 | P0 优先；承接 K4/K18/K39 |
| F85（K59 模块接口） | ✅ 确认 | P0 优先；K18 失导出证据 |
| F86（K60 线程安全包装） | ✅ 确认 | P0 优先；原子序+UB |
| F93（K67 ctypes 绑定） | ✅ 确认 | P0 优先；指针截断+TypeError |

### ② 设计文档更新

- §6 决策点 F1/F7/F16/F23/F24/F27/F28/F33/F35/F39/F40/F59/F77/F79/F80/F81/
  F85/F86/F93 标注 `→ **✅ 已确认**（2026-08-31 用户，P0 优先）`；
  F1 追加 `；删除项走评审`；F24/F28 为 P0 独立立项。
...
- **P0 决策点全部确认（42 个）**：第一批 11 + 第二批 12 + 第三批 19。
- 剩余 51 个：确认型 ~17（F2-F30 段）+ P1 ~34（F31-F96 段）待第四/五批。

### 遗留登记

- 待第四批：确认型决策（删/接线二选一，F2/F3/F5/F6/F8/F9/F11/F13/F18/F19/F20/
  F25/F29/F30/F70 读路径二选一等）。
- 待第五批：P1 收敛立项（F10/F14/F15/F17/F21/F22/F26/F31/F32/F34/F36/F37/F38/
  F41/F42/F44/F46/F47/F48/F50/F51/F52/F56/F57/F58/F63/F66/F68/F72/F76/F78/F82/
  F83/F84/F89/F90/F95/F96）。

## 一百五十、批次 标准统一化决策确认第四批（2026-08-31，用户确认）

用户「可以」——第四批（确认型决策 14 个）确认，其中 13 个确认 + F5 缓议。

### ① 确认结果

| 决策点 | 结论 | 备注 |
|---|---|---|
| F2（Module 序列化→JSON） | ✅ 确认 | msgpack 删除项走评审 |
| F3（canonical 语义漂移） | ✅ 确认 | **改名**保留导出路径（不删除） |
| F5（Lean 项目合并） | ⏸ 缓议 | P4 项目级批次定 |
| F6（错误码桥接+返回码） | ✅ 确认 | 按原设计纳入 P1/P3 |
| F8（Python DSL 三套归一） | ✅ 确认 | 保留 dsl.py 兼容 re-export |
| F9（EngineState 合并/删除） | ✅ 确认 | 合并；删除项走评审 |
| F11（流事件 JSON 契约） | ✅ 确认 | Python 侧改从 C 契约派生 |
| F13（规范形二选一） | ✅ 确认 | **兑现桥接**；test_expr_canon 回归锚 |
| F18（配置默认值收敛） | ✅ 确认 | 观察 smoke/CI 表现 |
| F19（删 ConfigManager+global_state） | ✅ 确认 | 删除项走评审 |
| F20（后端注册单一化） | ✅ 确认 | 删 VTable 走评审；GROEBNER 验证等价后切 |
| F25（I5 方向修正） | ✅ 确认 | 两处归位 |
| F29（J2 锁抽象单一化） | ✅ 确认 | 旧别名退役走评审 |
| F30（J4 预设单一事实源） | ✅ 确认 | 删除项走评审；preset_func_blocks_compat 保留 |

### ② 设计文档更新

- §6 决策点 F2/F3/F6/F8/F9/F11/F13/F18/F19/F20/F25/F29/F30 标注
  `→ **✅ 已确认**（2026-08-31 用户…）`；F5 标注 `→ **⏸ 缓议**（P4 项目级批次定）`。
...
- 确认型 13 个确认 + F5 缓议 / 全部删除项走评审 /
  实施前仍按红线提交实施清单。

### 遗留登记

- 待第五批（最后一批）：P1 收敛立项 38 个（F10/F14/F15/F17/F21/F22/F26/F31/F32/
  F34/F36/F37/F38/F41/F42/F44/F46/F47/F48/F50/F51/F52/F56/F57/F58/F63/F66/F68/
  F72/F76/F78/F82/F83/F84/F89/F90/F95/F96）+ F5 缓议到 P4 批次。

## 一百五十一、批次 标准统一化决策确认第五批（2026-08-31，用户确认）

用户「可以」——第五批（P1 收敛立项 38 个）全部确认，**F1-F96 全部定案**。

### ① 确认结果

| 决策点 | 结论 |
|---|---|
| F10（L3 推理后端注册表）/ F14（F8 验证入口）/ F15（F9 策略调度） | ✅ P1 立项 |
| F17（G2 缓存层）/ F21（H3 切 numerical_backend）/ F22（H4 公理单源） | ✅ P1 立项 |
| F26（I2 计时基座）/ F31（J5 命名锚定）/ F32（K1 文案规范表） | ✅ P1 立项 |
| F34（K4 版本分层）/ F36（K6 退役机制）/ F37（K7 降级登记）/ F38（K8 基准框架） | ✅ P1 立项 |
| F41（K13 清理机制）/ F42（K14 证明导出）/ F44（K16 头卫生） | ✅ P1 立项 |
| F46（K19 容差表）/ F47（K20 声明检查）/ F48（K21 格式统一） | ✅ P1 立项 |
| F50（K23 序列化注册）/ F51（K25 路径单源）/ F52（K26 错误注入） | ✅ P1 立项 |
| F56（K30 宏约定）/ F57（K31 字符串）/ F58（K32 状态机） | ✅ P1 立项 |
| F63（K37 数学原语）/ F66（K40 构建）/ F68（K42 网络，裁决走评审） | ✅ P1 立项 |
| F72（K46 哈希）/ F76（K50 复杂度）/ F78（K52 复位） | ✅ P1 立项 |
| F82（K56 排序）/ F83（K57 同步）/ F84（K58 缓冲） | ✅ P1 立项 |
| F89（K63 位操作）/ F90（K64 测试设施）/ F95（K69 代码生成器） | ✅ P1 立项 |
| F96（K70-K78 收口归属） | ✅ 按 v1.22 归属逐项确认 |

### ② 设计文档更新

- §6 决策点 F10/F14/F15/F17/F21/F22/F26/F31/F32/F34/F36/F37/F38/F41/F42/F44/
  F46/F47/F48/F50/F51/F52/F56/F57/F58/F63/F66/F68/F72/F76/F78/F82/F83/F84/F89/
  F90/F95/F96 标注 `→ **✅ 已确认**（2026-08-31 用户，P1 立项）`；
  F96 标注 `→ **✅ 已确认**（2026-08-31 用户，按 v1.22 归属逐项确认）`；
  F68 追加 `；裁决项走评审`。
...
- **F1-F96 全部定案**：P0 42 个确认（第一批 11 + 第二批 12 + 第三批 19）+
  确认型 13 个确认 + F5 缓议 + P1 38 个确认 = 93 确认 + 1 缓议（F5）+ 1 待定
  （F4 语言统一 P2 方向，第四批漏列）+ F12 历史已确认（前端留白）。
- 后续：P0 实施讨论（提交第一批实施清单，按红线逐批讨论）。

### 遗留登记

- F4（语言统一 P2 方向）待用户单独确认。
- F5（Lean 合并）缓议到 P4 项目级批次。

## 一百五十二、批次 标准统一化决策确认收口 F4（2026-08-31，用户确认）

用户「可以」——F4（语言统一 P2 方向）确认，**F1-F96 全部定案收口**。

### ① 确认结果

| 决策点 | 结论 | 备注 |
|---|---|---|
| F4（语言统一 P2 推进） | ✅ 确认 | .lv 吸收 dsl_compiler 能力；语法糖第一批 + L11 语法单一事实源 |

### ② 设计文档更新

- §6 决策点 F4 标注 `→ **✅ 已确认**（2026-08-31 用户，P2 推进；语法糖第一批 + L11 语法单一事实源）`。
...
- **F1-F96 全部定案收口**：93 确认 + F5 缓议（P4 批次）+ F12 历史确认 = 96 全部结案。
- 剩余唯一缓议：F5 Lean 项目合并（P4 项目级批次定）。
...
## 批次 177（K61/F87 收尾：错误帧栈读端接线）

### ① 改动

| 提交 | 内容 |
| --- | --- |
| ad7fa800 | **旧式读端接线帧栈**（K61 第 3 个真实缺陷）：lv_get_last_error_code / lv_get_last_error_message 改为帧栈有帧时返回栈顶帧（最新错误，根因优先）；帧栈空时回退旧 TLS（含默认错误描述）。写端桥接已同步（lv_set_error 推帧/lv_clear_error 清帧；push_frame 栈满丢最旧、栈顶恒最新），lv_OK 日志通道迁移（批次 176）后两套状态无分歧——生产 10+ 读端零改动获得 8 帧回溯能力 |
| 34d54655 | test_lv_error_ext.c 新增 test_legacy_reader_wiring：lv_set_error → 帧栈读端同源断言（消息/码/多层取栈顶/清错误回退） |

### ② 验证

- build3 ctest **289/289**（lv_error 专项过）；build_layerval 通过；
- 两提交 push 成功，CI 待验证。

### 决策登记

- K61 三项真实缺陷全部修复：①25 处 lv_set_error(lv_OK) 迁日志（批次 176）；
  ②orchestrator 保根因（批次 176）；③帧栈读端接线（本批）；
- 接线路径：用户决策「接线」——生产读端从帧栈取根因；lv_error_format_chain
  等高级 API 保持（框架能力，生产按需调用）。
...
## 批次 232（数值抽象层设计定稿：形态一 + 强制池化，ND-1..7，文档批）

### 背景

用户提出「外部库引入需要统一抽象层，否则升级会有问题，改动面大也接受」；经两轮
设计讨论（形态一 vs 形态二、迁移节奏、三个遗留点）收敛并批准。设计文档正式化：
`docs/architecture/number-abstraction-layer-design.md`。

### 决策登记（详见设计文档 §7）

| ID | 决策 |
|:---|:---|
| ND-1 | 形态一：统一不透明句柄 `lvNumber`（kind 内部分派；升级现有 lv_number.h） |
| ND-2 | 强制池化：lvNumber 唯一来源数值池（帧池 + 常驻池两级），禁止逐数系统分配 |
| ND-3 | 迁移节奏：抽象层先行 + 分批迁移（0-6 期），每期测试全绿 |
| ND-4 | 双轨 0 期内部合一（lvRational/Rational 存储并入 lvNumber，公共符号薄包装保留） |
| ND-5 | 批量数值唯一形态 = 池内连续段 + 段级回收/晋升；禁止逐系数独立分配 |
| ND-6 | 不建双 ABI 兼容层；ABI 变更随 2.0.0 统一 bump；CI 加 ABI/符号对拍 |
| ND-7 | 元决策：后续未决取舍一律按「最小化后续工程债务」裁决 |

### 变更文件

| 文件 | 变更 |
|:---|:---|
| `docs/architecture/number-abstraction-layer-design.md` | 定稿正式化（ND-1..7 + §0 裁决原则 + §4 六期 + 风险/保真） |

### 验证

- 纯文档批，无代码/构建影响，未跑构建；
- 决策登记与 §0/§4/§7 全文一致（grep 复核无 ⏳ 残留）。
...
## 批次 248（DSL 冻结 + 待执行登记，文档批）

- 用户指示：DSL 代码暂不实施（原「弄一下基础」改为完全不动）；
- 新增 `docs/architecture/dsl-remaining-work.md`：DSL 剩余待执行登记（诊断码/#!suppress/
  loader 汇总、R5 恢复、S3/S4/S6/S9-S24、MPC 文本差异），含重启约定；
- DSL 线从此进入**冻结/待命**，本文档为唯一执行依据；
- P1-P7 立项保持通过（抽象层主线不受 DSL 冻结影响）。

---

## 批次 249（P1 分层执行方案 + 风险前置，文档批）

- 新增 `docs/architecture/p1-s3-execution-plan.md`：
  - P1（S3 quadratic 整簇 + 工具面 opaque）分层切片 P1a-P1f；
  - **风险前置**：粗扫 `->value` 命中 89+ 文件、`->a/->b` 22+ 文件但**大量为通用字段误
    命中**（expr->a / cfg.value 等），不可据此直接改；真读者面须按类型
    （SymbolicCoord.data.* / Quadratic* / lvRational*）逐对象精确建图（P1a）后再动；
