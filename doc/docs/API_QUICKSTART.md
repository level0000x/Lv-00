...
int main(void) {
    ...
    return (result == lv_SOLVE_SUCCESS) ? EXIT_SUCCESS : EXIT_FAILURE;
}
```
...
int main(void) {
    ...
    for (int i = 0; i < num_problems; i++) {
        ...
        if (ok) {
            ...
            EngineSolveResult res = lv_solve(ctx);
            if (res == lv_SOLVE_SUCCESS) {
                printf("求解成功\n");
                success_count++;
            } else {
                printf("求解失败 (code=%d)\n", (int)res);
...
int main(void) {
    if (!lv_context_create()) return 1;

    lvContext *ctx = lv_context_create();
    if (!ctx) { lv_context_destroy(ctx); return 1; }
    ...
    */
    ...
    /* 构建并求解问题... */
    int a = lv_add_point(ctx, 0, 1, 0, 1);
    int b = lv_add_point(ctx, 1, 1, 0, 1);
    int ab = lv_add_line_segment(ctx, a, b);
...
int main(void) {
    ...
    lv_set_memory_limit_ex(1024 * 1024 * 1024);  /* 1 GB */
...
lvContext *create_and_solve_safely(void) {
    ...
    int b = lv_add_point(ctx, 3, 1, 0, 1);
    if (b < 0) {
...
| `lv_ERR_NOT_INIT` | 系统未初始化 | 先调用 lv_context_create() |
...
| 函数类型 | 所有权 | 示例 |
|----------|--------|------|
| `lv_*_create()` | 调用者拥有，负责销毁 | `lv_context_create()` -> `lv_context_destroy()` |
| `lv_*_get_*()` | 借出引用，不可释放 | `lv_config_get_string()` |
| `lv_normalize()` | 调用者拥有返回值 | 必须通过 `normalization_result_free()` 释放 |
| `lv_context_destroy(ctx)` | 释放所有全局资源 | 最后调用 |
...
int main(void) {
    ...
    /* 设置合理的内存上限 */
    lv_set_memory_limit_ex(512 * 1024 * 1024);  /* 512 MB */

    /* 创建大量上下文，观察内存峰值 */
    for (int i = 0; i < 100; i++) {
        lvContext *eng = lv_context_create();
        if (!eng) {
            /* 内存上限触发！ */
            fprintf(stderr, "上下文 %d 创建失败（可能达到内存上限）\n", i);

            MemoryStats stats;
            lv_get_memory_stats_ex(&stats);
            printf("当前: %zu / 峰值: %zu / 上限: %zu\n",
                   stats.current_bytes, stats.peak_bytes,
                   lv_get_memory_limit_ex());
            break;
        }
        lv_context_destroy(eng);
    }
...
static bool session_init(lvSession *s) {
    memset(s, 0, sizeof(*s));
    if (!lv_context_create()) return false;
    s->ctx = lv_context_create();
    return s->ctx != NULL;
}

static void session_destroy(lvSession *s) {
    if (s->ctx) lv_context_destroy(s->ctx);
    lv_context_destroy(ctx);
    memset(s, 0, sizeof(*s));
}
...
static void *worker_thread(void *arg) {
    ...
    int a = lv_add_point(ctx, 0, 1, 0, 1);
    int b = lv_add_point(ctx, 3, 1, 0, 1);
    int c = lv_add_point(ctx, 0, 1, 4, 1);
...
```c
/* ❌ 错误: 多个线程共享同一个引擎 */
lvContext *shared_engine = lv_context_create();
...
| `lv_get_version_info(&info)` | 获取详细版本信息结构体 |
...
| 函数 | 说明 |
|------|------|
| `lv_context_create()` | 全局初始化 |
| `lv_context_destroy(ctx)` | 全局清理 |
| `(ctx != NULL)` | 检查初始化状态 |
| `lv_context_create()` | 创建上下文实例 |
| `lv_context_destroy(e)` | 销毁上下文实例 |
...
| 函数 | 说明 |
|------|------|
| `lv_get_memory_stats_ex(&s)` | 内存统计 |
| `lv_set_memory_limit_ex(n)` | 设置内存上限 |
| `lv_get_memory_limit_ex()` | 获取内存上限 |
...
```
undefined reference to `lv_context_create'
```
...
**解决方案**:
- 确保每个 `lv_context_create()` 都对应 `lv_context_destroy()`
- 确保每个 `lv_normalize()` 返回值都被 `normalization_result_free()` 释放
- 使用 `lv_get_memory_stats_ex()` 监控内存
- 设置内存上限: `lv_set_memory_limit_ex()`
