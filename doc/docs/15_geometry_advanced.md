...
高级几何分析模块扩展了基础约束图之外的几何分析能力，覆盖几何类型定义、变换推理、数据压缩、高维表示、交互几何、演化引擎、事件检测、不变量类型、构造规约和拓扑操作等多个维度。这些模块共同构成 Lv-00 几何系统的上层能力层，为用户提供从静态几何分析到动态几何演化、从精确符号计算到交互式探索的完整工具集。
