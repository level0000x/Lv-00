...
| 来源 | 状态 | 处理方式 |
|------|------|----------|
| engine.h (5层枚举) | 代码实际 | 保留（向后兼容），添加 L6-L10 枚举 |
...
| C02 | 层命名三重不一致 | engine.h, layer_validation.h, TEN_LAYER_INTEGRATION_PLAN.md | 🔴关键 | 以engine.h的L2=Resource/L3=Geometry为正典 |
...
| C07 | Engine/Context状态机重复 | engine.h, context.h | 🟠高 | 保留双状态机但添加迁移注释，标记为过渡期 |
...
| doc/docs/API_QUICKSTART.md | README.md | 创建（基于interop.h和engine.h） |
...
### 10.1 正典状态机（5态）
以 engine.h 的 LV00EngineState 为正典：
...
- [ ] 所有文档引用的文件路径有效
- [ ] engine.h和layer_validation.h的层枚举一致（或明确标注差异）
...
| 层目录命名 | core/src/layer*_*/ | engine.h枚举名 |
| API风格 | README.md §使用示例 | lv00_context_create() |
| 状态机 | engine.h | 5态：IDLE/PARSING/REASONING/COMPLETE/ERROR |
