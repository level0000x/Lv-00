...
typedef enum { GEOM_POINT, GEOM_LINE_SEGMENT, GEOM_REGION, GEOM_PORT, GEOM_FUNCTION_BLOCK } GeomType;
typedef enum { PORT_INPUT, PORT_OUTPUT } PortType;
typedef enum { INCIDENCE = 0, BETWEENNESS = 1, INTERSECTION = 2, CONTAINMENT = 3, CONNECTION = 4 } ConstraintType;
...
struct ConstraintGraph {
    ...
    GeomNode **node_index;
    int node_index_capacity;
    Constraint **constraint_index;
    int constraint_index_capacity;
};
...
typedef struct AxiomPackage {
    ...
    int template_count;
    void *known_unconstructibles;
    int unconstructible_count;
    char *bottom_geometry, *negation_encoding;
    int contradiction_behavior;
    void *expansion_cache;
    int expansion_cache_count, expansion_cache_capacity, max_expansion_depth;
    DependencyRef *dep_refs;
    int dep_ref_count, dep_ref_capacity;
} AxiomPackage;
...
#define CT_COUNT 5
static const char *g_ct_names[CT_COUNT] = {"INCIDENCE", "BETWEENNESS", "INTERSECTION", "CONTAINMENT", "CONNECTION"};
    ...
    for (int i = 0; i < g->constraint_count; i++) {
        ...
        for (int j = 0; j < c->participant_count; j++) {
            ...
            if (!mv_node_exists(g, pid))
                mv_err(r,
                       "node-refs: constraint #%d (%s) refs undeclared "
                       "node %d [participant[%d]]",
                       c->id, c->type < CT_COUNT ? g_ct_names[c->type] : "?", pid, j);
        }
    }
    ...
    for (int i = 0; i < g->node_count; i++) {
        ...
        for (int j = 0; j < ic; j++)
            if (ids && ids[j] >= 0 && !mv_node_exists(g, ids[j]))
                mv_err(r, "node-refs: fb #%d refs undeclared internal node %d", n->id, ids[j]);
        ic = n->data.func_block.input_count;
        ids = n->data.func_block.input_port_ids;
        for (int j = 0; j < ic; j++)
            if (ids && ids[j] >= 0 && !mv_node_exists(g, ids[j]))
                mv_err(r, "node-refs: fb #%d refs undeclared input port %d", n->id, ids[j]);
        ic = n->data.func_block.output_count;
        ids = n->data.func_block.output_port_ids;
        for (int j = 0; j < ic; j++)
            if (ids && ids[j] >= 0 && !mv_node_exists(g, ids[j]))
                mv_err(r, "node-refs: fb #%d refs undeclared output port %d", n->id, ids[j]);
    }
}
...
static void mv_pass2_arity(const ConstraintGraph *g, MvReport *r) {
    ...
    for (int i = 0; i < g->constraint_count; i++) {
        ...
        if (c->participant_count != exp)
            mv_err(r,
                   "arity: constraint #%d (%s) expects %d participants, "
                   "got %d",
                   c->id, g_ct_names[c->type], exp, c->participant_count);
    }
}
...
static void mv_pass3_axiom(const AxiomPackage *pkg, const ConstraintGraph *g, MvReport *r) {
    if (!pkg) {
        mv_warn(r, "axiom: no package, skipping");
        return;
    }
    ...
    if (pkg->templates && pkg->template_count > 0) {
        for (int i = 0; i < pkg->template_count; i++) {
            ...
            if (t->param_desc_count > 0 && t->param_desc_count != t->param_count)
                mv_err(r, "axiom: tmpl '%s' param_count=%d desc_count=%d", t->name ? t->name : "?", t->param_count,
                       t->param_desc_count);
            for (int j = i + 1; j < pkg->template_count; j++)
                if (pkg->templates[j].name && t->name && !strcmp(t->name, pkg->templates[j].name))
                    mv_err(r, "axiom: duplicate template '%s' [%d,%d]", t->name, i, j);
        }
    }
    ...
    if (g && pkg->dep_refs && pkg->dep_ref_count > 0) {
        for (int i = 0; i < pkg->dep_ref_count; i++) {
            ...
            if (dr->dependent_node_id >= 0 && !mv_node_exists(g, dr->dependent_node_id))
                mv_warn(r, "axiom: dep '%s' refs node %d (may be external)", dr->ref_id, dr->dependent_node_id);
        }
    }
}
...
void minimal_verifier_validate(const ConstraintGraph *graph, const AxiomPackage *pkg, MvReport *rpt) {
    mv_rpt_init(rpt);
    if (!graph) {
        mv_err(rpt, "validate: NULL graph");
        return;
    }
...
static bool mv_self_test(void) {
    ...
    {
        MvReport r;
        minimal_verifier_validate(NULL, NULL, &r);
        if (r.passed) {
            printf("  FAIL T1\n");
            ok = false;
            failed++;
    ...
    /* T2: empty graph */
    {
        ...
        memset(&g, 0, sizeof(g));
        MvReport r;
        minimal_verifier_validate(&g, NULL, &r);
        if (!r.passed) {
            printf("  FAIL T2\n");
            ok = false;
            failed++;
    ...
    /* T3: undeclared node reference */
    {
        ...
        if (r.passed) {
            ...
            printf("  OK   T3 (undeclared) [%d errs]\n", r.err_c);
            passed++;
        }
    }
    ...
    /* T5: correct graph */
    {
        ...
        minimal_verifier_validate(&g, NULL, &r);
        if (!r.passed) {
            printf("  FAIL T5 (%d errs)\n", r.err_c);
            ok = false;
            failed++;
        } else {
            printf("  OK   T5 (correct)\n");
            passed++;
        }
    }
    ...
    {
        ConstraintTemplate ts[2];
        memset(ts, 0, sizeof(ts));
        ts[0].name = "dup";
        ts[0].param_count = 2;
        ts[0].param_desc_count = 2;
        ts[1].name = "dup";
        ts[1].param_count = 1;
        ts[1].param_desc_count = 1;
        AxiomPackage p;
        memset(&p, 0, sizeof(p));
        p.templates = ts;
        p.template_count = 2;
        MvReport r;
        minimal_verifier_validate(NULL, &p, &r);
        if (r.passed) {
            printf("  FAIL T6\n");
            ok = false;
            failed++;
        } else {
            printf("  OK   T6 (dup tmpl) [%d errs]\n", r.err_c);
            passed++;
        }
    }
