...
| `ctx.py` | Engine类C委托方法（从ctx.py迁移） | 拆分迁移 |
...
| Week 4 | 迁移引擎绑定 | `ctx.py`, `groebner.py`, `sparse_la.py` |
...
| R10 | `ctx.py` | L578-613 | `Engine.unify_detailed()` | 注释说明C函数已移除，Python侧模拟诊断丢失精度；硬编码中文字符串 | `layer4_reasoning/proof_engine_enhanced.c` | 2人天 |
...
| R17 | `ctx.py` | L819-863 | `Engine.solve_with_retry()`/`Engine.safe_solve()` | Python侧重试每次跨越FFI边界；C层可实现更高效的指数退避 | `layer4_reasoning/ctx.c` | 1人天 |
...
| H5 | `prop_verifier.h` | 2个文件 | `layer4_reasoning/ctx.c`, `stream_context_util.c` | 命题验证器 | 1人天 |
...
| 周次 | 任务 | 产出 |
|------|------|------|
| Week 10 | 迁移基础绑定 | `_ctypes_binding.py`, `native_types.py`, `ctx.py` |
...
class BatchCallOptimizer:
    ...
    def __init__(self, ctx, flush_threshold: int = 10):
        self._engine = ctx
