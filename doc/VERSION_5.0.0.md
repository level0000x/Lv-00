...
每层编译为 `OBJECT` 库，通过 `target_link_libraries` 控制依赖方向，最终聚合为 `lv00_static` 静态库（及可选的 `lv00_shared` 共享库）。
...
- `LV00_EXCLUDE_BROKEN_PRESETS` 排除了部分有深层依赖问题的预设模块（如微分几何、泛函分析）
