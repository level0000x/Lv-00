...
> 输入：**二十一轮共一百零五路子代理并行审计**（第一轮：DSL/序列化/导出/证书证明/存储配置；
...
7. **实施前必须讨论（用户 2026-08-27 定稿）**：**任何设计级别实施，无论是否与用户讨论过、无论历史是否已确认，实施前必须先与用户讨论，得到确认后再开始执行**。即：
   - 本设计文档记录的 P0-P4 全部实施项、F1-F81 决策点，以及此前各轮已定稿的设计决策（DSL 语法糖、K5 示例处置、双 DSL 处置、前端留白、PFL 参考等），**均不因"历史已确认"而自动获得执行许可**；
...
> 实施（本设计 P0-P4 全部实施项、F1-F81 决策点，以及所有历史已定稿的设计决策），
...
*附：本设计基于二十一轮一百零五路子代理审计（每轮 5 路 ×21 = 138 组重复点），
