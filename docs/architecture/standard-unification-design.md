...
8. **可配置的限制一律走配置，禁止硬编码（用户 2026-08-31 定稿）**：凡是"可配置的"限制（深度/上限/超时/阈值/大小等），**一律读 lvConfig 运行时配置（或 config.h X-macro 编译期可调），禁止在代码里硬编码字面量**。即：
   - 任何限制类常量，若语义上"不同部署/不同输入可能需要不同值"，必须走配置（lvConfig 字段 + X-macro 默认值 + lv.config.json 可调），禁止 `#define XXX 256` 散落或代码内联 `if (depth > 256)`；
   - 编译期尺寸常量（影响 sizeof/数组维度，如 lv_CONFIG_POOL_*）保留宏形式（不可运行时配置），但归入 config.h 权威区并加注释说明不可运行时配置的原因；
   - 新增限制一律先问"是否需要可配"：需要 → lvConfig；不需要但可能变 → 仍建议配置默认值；确属恒定语义（如 uint64 上限）才允许常量；
   - 现有硬编码限制（256/4096/10000/30000/1000/128 等散落字面量）在实施时逐步收敛到配置或权威宏（depth_limits.h / config.h），不得新增散落字面量；
   - 本原则覆盖 K28 深度限制接线（AST 深度默认 256 必须读 parser_max_ast_depth 配置而非硬编码）、F27 增长上限、K75 资源配额、K38 超时等全部限制类实施。
9. **最优优先，不考虑复杂度/难度/重写范围（用户 2026-08-31 定稿）**：方案选型与实施时，**以"最优解"为唯一标准，不因实现复杂、难度大、需重写范围广而退而求其次**。即：
   - 多个候选方案之间取舍时，选**正确性/架构最优**者（如并发修复选原子快照而非加锁折中、收敛选单一权威而非兼容别名），复杂度与改动范围不是否决理由；
   - 既有代码若与最优方案冲突（需大范围重写/迁移），**照重写**，不保留"够用但非最优"的中间态；
   - "最优"定义以本设计文档锚定的权威/健康基线为准（单一事实源、无竞态、可配置、无死机制），与原则 1/2/3/8 一致；
   - 实施批次内的范围评估以"达成最优解所需"为准，不因工作量而缩小到次优；必要时分阶段实施但每阶段都朝最优解推进，不得停在次优态；
   - 本原则覆盖全部 P0-P4 实施、F1-F96 决策落地与后续新增设计。
