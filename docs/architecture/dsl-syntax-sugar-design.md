...
> - 两套 DSL 处置：选项 A（糖加 DSL-A + 从 DSL-B 反移植量词/度量）；
...
| 两套 DSL 处置 | **选项 A**：糖加 DSL-A + 从 DSL-B 反移植量词/度量（S12），先收窄差距，统一待糖稳定后 |
| D1 R4+R5 独立实施 | **采纳**：0 批次优先，纯质量升级 |
...
## 7. 两套 DSL 分裂的处置建议（决策：选项 A）

现状：DSL-A（22 关键字）与 DSL-B（45+ 关键字）互不引用，语法糖默认加 DSL-A 会加剧分裂。

**决策（v1.2 定稿，选项 A）**：
- 语法糖只加 DSL-A，但 S12 量词/度量补齐从 DSL-B 语义反向移植，先收窄能力差距。
- 统一（选项 C 立即合并）留到 DSL-A 语法糖稳定后评估——与"十层架构不可动"冲突，**不做**。
- 选项 B 双轨映射表作为语法糖文档附录可选产出，不承诺维护。
...
*附：本设计已定稿（v1.2），全部决策采纳；代码执行待用户另行指示。
0 批次（R4+R5 诊断码/错误恢复）为独立优先项。*
