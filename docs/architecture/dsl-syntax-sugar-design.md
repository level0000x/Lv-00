# Lv-00 DSL 语法糖设计（四路研究汇总 v1.2 定稿）

> 状态：设计定稿（2026-08-27），**仅设计不执行**
> 前置文档：dsl-syntax-baseline.md v1.1（三格式图景：lv 家族 .lv/.lvz + 独立 dsl_compiler）
> 输入：四路子代理研究（命令式 / 函数式 / 领域 DSL / 证明语言与宏系统）
>
> **v1.1 修订（PFL 参考，2026-08-27）**：用户旧构思《PFL v0.478-dev 完整设计》
> 已单独分析（见 dsl-pfl-reference.md）。受影响条目：
> S14 → 改用声明式 `default`（0 新 IR，替代 with 块）；
> S15 → 补充迭代命名（$@index 对应 i）；
> S17 → 采用 PFL @theory 蓝图（只读导入 + @export + 别名）；
> S19 → 补充首参数分组索引（O(log N)）；
> S20/S21 → 改用 AST 层空节点方案（2 新 IR：OPTIONAL_BIND/FALLBACK）；
> 新增 R4 诊断码体系 + R5 错误恢复（独立立项，+550-900 行）。
>
> **v1.2 决策定稿（2026-08-27）**：用户授权"可采纳的都采纳"，全部设计决策
> 确认如下（详见 §6 决策定稿）：
> - 落地顺序：第一批 S1-S8 → 第二批 S9-S15 → 第三批 S16-S24 逐个立项；
> - 语法糖落点（v1.3 修正见下）：以用户书写的 **lv 家族**（.lv/.lvz）为主，
>   dsl_compiler 不优先加糖；
> - R4 诊断码 + R5 错误恢复：独立优先批次（D1 采纳）；
> - S17 命名空间：@export 与旧 load 双轨兼容（D2 采纳，方案见 §6）；
> - R2 模板化渲染：记录远期方向，不动 ga_codegen（D3 采纳）；
> - R1 AST 层空节点方案：S20/S21 正式采用（D4 采纳）。
>
> **v1.3 修正（2026-08-27，用户澄清"用户写的是 lvz 和 lv 文件"）**：
> 基线文档经代码核查修正为**三格式图景**（见 dsl-syntax-baseline.md v1.1）：
> - 用户书写的语言是 **lv 家族**：`.lv` 规约（lv_loader）+ `.lvz` 公理/预设
>   （module_lvz/axiom_pkg，共享 lexer_shared 词法），共 289 个文件；
> - **dsl_compiler 不是用户书写语言**：引擎内部"字符串→约束图"通道
>   （lv_prove / L7 GEOMETRY 阶段 / L9 EXPORT/VISUALIZE），无专属文件格式；
> - **决策修正**：语法糖落点从"DSL-A"改为 **lv 家族**；S1-S24 候选按
>   lv 家族语法重新对齐（desugar 目标从 dsl_compiler 的 30 IR 改为
>   lv_loader/module_lvz 的现有语义）；dsl_compiler 的 22 关键字简写
>   （midpoint(A,B)/constraint{} 等）反向移植到 lv 家族作为参考。
>   本设计文档正文保留原 DSL-A 对齐示例作参考，最终落点以本节为准。
...
## 6. 实施路线（三批，决策已定稿，执行待用户另行指示）

### 第一批：表达式级（S1-S8）——约 900-1100 行（**已定稿，优先执行**）
...
复用现有约束 IR（0 新 op，S14 例外 +1；经 v1.1 修订 S14 改声明式 default 后为 **0 新 op**）。
覆盖痛点：约束自然化（S9/S10）、分步证明（S11）、量词能力补齐（S12）、批量构造（S15）。

### 第三批：系统级（S16-S24）——约 2500-4000 行（逐个立项）
依赖架构决策（宏展开时机、命名空间、nullable 语义、证明项导出接 L9）。
S17 命名空间采用 R3 蓝图；S20/S21 采用 R1 AST 层空节点方案。
逐个立项，不做则留档。

### 0 批次（独立优先项，R4+R5）：诊断码体系 + 错误恢复——约 550-900 行
**纯质量升级，不依赖任何语法糖**，符合"不留工程债务"路线：
- R4 三级诊断码（错误/警告/信息 + 触发条件 + 修复方向索引 + #!suppress 抑制）；
- R5 错误恢复点（`;`/`@`/`#!` 恢复点，一次编译多错误报告）。
**决策**：作为独立批次优先实施（D1 采纳），可与第一批并行推进。

### 决策定稿（v1.2 + v1.3 修正）

| 决策点 | 结论 |
|---|---|
| 落地顺序 | 第一批 S1-S8 → 第二批 S9-S15 → 第三批逐个立项；R4+R5 独立优先 |
| 语法糖落点（**v1.3 修正**） | 以用户书写的 **lv 家族**（.lv/.lvz）为主；dsl_compiler 不优先加糖，其 22 关键字简写反向移植到 lv 家族作参考 |
| D1 R4+R5 独立实施 | **采纳**：0 批次优先，纯质量升级（诊断码体系落点：lv_loader 与 dsl_compiler 均可挂） |
| D2 @export 与 load 兼容 | **采纳双轨兼容**：`load "x"` 保留全局可见旧语义（向后兼容零改动）；新增 `import "x" use 符号 [as 别名]` 显式只读导入走 @export/@theory 作用域；旧脚本零改动，新脚本推荐 import |
| D3 R2 模板化渲染 | **采纳为远期方向记录**：不动 ga_codegen；仅当新增第 7 渲染目标时评估模板化增量方案 |
| D4 R1 AST 层空节点 | **采纳**：S20/S21 正式采用 AST 层空节点 + 2 新 IR（OPTIONAL_BIND/FALLBACK），工作量下调 |

---

## 7. 语法体系分裂的处置建议（v1.3 修正：lv 家族 vs dsl_compiler）

现状（基线 v1.1 修正）：用户书写的 **lv 家族**（.lv 规约 140 文件 + .lvz
公理/预设 149 文件，共享词法）与引擎内部通道 **dsl_compiler**（22 关键字，
自建词法，非用户书写语言）互不引用。

**决策（v1.3 修正，替换原"选项 A"）**：
- **语法糖加在用户书写的 lv 家族**（.lv / .lvz）；S1-S24 候选按 lv 家族
  语法对齐，desugar 目标为 lv_loader/module_lvz 现有语义。
- **dsl_compiler 不优先加糖**：它是内部通道（lv_prove / L7 GEOMETRY /
  L9 EXPORT/VISUALIZE 的输入字符串格式），"太空"的观感不影响用户；
  其已验证的 22 关键字简写（midpoint(A,B)、constraint{} 等）**反向移植**
  到 lv 家族作参考，避免"内部通道比用户语言顺口"的反常。
- **统一三套**（把 dsl_compiler 并入 lv 家族或反之）留待语法糖稳定后评估，
  与"十层架构不可动"冲突的重构不做。
- R4 诊断码 + R5 错误恢复（0 批次）：lv_loader 与 dsl_compiler 均可挂，
  先做 lv_loader（用户语言）侧。
...
> **v1.3 注**：行数估算原按 DSL-A 为落点；落点改为 lv 家族后，S1-S24 的
> 具体 desugar 目标需按 lv_loader/module_lvz 现有语义重新对齐，总量级
> 相当（lv_loader 现 4534 行 + module_lvz 1340 行，加糖空间与 DSL-A 同量级）。

对比基线：原 DSL-A 现实现约 3000 行（lexer+parse+IR+load）；lv 家族现有
约 5900 行（lv_loader 4534 + module_lvz 1340），语法糖全量落地后总量
仍在单模块可控范围（最大 .c 现 <2000 行，语法糖按模块拆分后单文件仍 <1500 行）。
...
*附：本设计已定稿（v1.2 + v1.3 修正），全部决策采纳；代码执行待用户另行指示。
0 批次（R4+R5 诊断码/错误恢复）为独立优先项。v1.3 修正语法糖落点：
以用户书写的 lv 家族（.lv/.lvz）为主，dsl_compiler 不优先加糖。*
