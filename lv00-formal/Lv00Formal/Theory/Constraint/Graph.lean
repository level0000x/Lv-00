...
/-- 归一化函数接口：后续对应 C 内核 normalization / equivalence_merge。 -/
constant normalize : ConstraintGraph → ConstraintGraph

/-- 归一化幂等性是 Lv-00 自有体系的关键元理论目标。 -/
def NormalizationIdempotent : Prop :=
  ∀ g : ConstraintGraph, normalize (normalize g) = normalize g

/-- 归一化保持良构性。 -/
def NormalizationPreservesWellFormedness : Prop :=
  ∀ g : ConstraintGraph, WellFormedGraph g → WellFormedGraph (normalize g)

/-- 约束图状态判定接口。 -/
constant checkStatus : ConstraintGraph → ConstraintStatus
