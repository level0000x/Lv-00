...
int64_t proof_tptp_export(lvEngine *ctx, int64_t proof_id, char *buf, int64_t buf_size) {
    ...
    char *tptp_text = atp_encode_constraint_graph(
        graph, ATP_FORMAT_TPTP_FOF, "lv_proof_export", true, NULL);
...
/** 仿射变换(6参数矩阵) */
int64_t preset_affine(lvEngine *ctx, int64_t obj_id,
...
/** 更新编辑器中的节点位置（重置并重新执行） */
int64_t visual_editor_update(lvEngine *ctx, int64_t editor_id,
...
int64_t meta_verify_report(lvEngine *ctx, int64_t *out_overall_pass) {
    ...
    int passed = 1;
    if (lv_conflict_detect_quick(graph)) passed = 0;
...
int64_t upper_func_block_preset_instantiate(lvEngine *ctx, const char *name,
...
int64_t upper_func_block_preset_compose(lvEngine *ctx, const char *name_a, const char *name_b,
        const char *new_name) {
...
int64_t func_block_preset_batch(lvEngine *ctx, const char **names, int64_t count,
        int64_t *out_ids) {
    (void)ctx;
...
int64_t func_block_preset_validate(lvEngine *ctx, const char *name,
        int64_t *input_ids, int64_t input_count) {
    if (!name) return 0;
    PresetEntry *entry = func_block_registry_find(name);
    /* 预设不存在 = 验证失败 */
...
int64_t func_block_preset_bindings(lvEngine *ctx, int64_t instance_id,
...
int64_t func_block_preset_search(lvEngine *ctx, const char *query,
        char *buf, int64_t buf_size) {
    (void)ctx;
    if (!buf || buf_size <= 0) return -1;
    if (!query || query[0] == '\0') {
...
int64_t func_block_preset_register(lvEngine *ctx, const char *name,
 ...
 * 返回成功导出的格式数量。
 */
int64_t lv_upper_export_all(lvEngine *ctx, int64_t proof_id,
