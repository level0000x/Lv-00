    ...
    switch (a->type) {
        ...
        case EXPR_TYPE_FUNCTION:
            return (a->data.function.func_name && b->data.function.func_name &&
                    ...
                    lv_expr_structurally_equal(a->data.function.argument,
...
static bool lv_ineq_structurally_equal(const lvInequality *a, const lvInequality *b) {
    ...
    return (a->type == b->type &&
            lv_expr_structurally_equal(a->left, b->left) &&
            lv_expr_structurally_equal(a->right, b->right));
}

/** 创建带错误消息的证明结构 */
static lvInequalityProof *lv_ineq_make_proof(lvInequality *target,
...
bool lv_ineq_system_add(lvInequalitySystem *sys, lvInequality *ineq) {
    ...
    if (sys->count >= sys->capacity) {
        ...
        lvInequality **new_arr = (lvInequality **) lv_realloc(
            sys->inequalities, (size_t) new_cap * sizeof(lvInequality *));
        if (!new_arr)
            return false;
...
bool lv_ineq_system_add_var_constraint(lvInequalitySystem *sys,
                                          lvExpr *var,
        ...
        for (uint32_t i = 0; i < sys->count; i++) {
            ...
            if (c->left == ineq->left && c->right == ineq->right) {
                ...
                if (c->type == ineq->type ||
                    ...
                    /* 创建简单证明 */
                    if (proof) {
                        lvInequalityProof *p = (lvInequalityProof *) lv_calloc(
                            1, sizeof(lvInequalityProof));
                        if (p) {
                            p->target = ineq;
                            p->status = INEQ_STATUS_PROVED;
                            p->step_count = 1;
                            p->step_capacity = 1;
                            p->steps = (lvInequalityStep *) lv_calloc(
                                1, sizeof(lvInequalityStep));
                            if (p->steps) {
...
lvInequalityStatus lv_ineq_prove_with_method(lvInequality *ineq,
                                                  lvInequalityMethod method,
                                                  const lvInequalitySystem *sys,
                                                  lvInequalityProof **proof) {
    if (!ineq)
    ...
    if (sys && sys->count > 0) {
        ...
        for (uint32_t i = 0; i < sys->count && !found_in_system; i++) {
            if (sys->inequalities[i] &&
                lv_expr_structurally_equal(sys->inequalities[i]->left, ineq->left)) {
    ...
    switch (method) {
        ...
        case INEQ_METHOD_AM_GM:
            ...
            if (proof) {
                *proof = lv_ineq_make_proof(ineq, INEQ_STATUS_UNKNOWN,
 ...
 * - 创建算术平均和几何平均的约束不等式
 */
bool lv_ineq_am_gm(lvExpr **expressions, uint32_t count,
                     lvExpr **out_lower_bound, lvExpr **out_upper_bound) {
 ...
 */
bool lv_ineq_cauchy_schwarz(lvExpr **a, lvExpr **b, uint32_t count,
    ...
    *out_ineq = NULL;
    ...
    if (!a_sq) { lv_expr_free(two); return false; }
    for (uint32_t i = 0; i < count; i++) {
 ...
 */
bool lv_ineq_rearrangement(lvExpr **a, lvExpr **b, uint32_t count,
                              lvExpr **out_min, lvExpr **out_max) {
    if (!a || !b || count == 0)
        return false;
    ...
    if (out_max) {
        ...
        for (uint32_t i = 0; i < count; i++) {
            ...
            if (!same_prods[i]) { lv_free((void **) &same_prods); return false; }
        }
    ...
    if (out_min) {
        ...
        for (uint32_t i = 0; i < count; i++) {
            ...
            if (!rev_prods[i]) { lv_free((void **) &rev_prods); return false; }
        }
 ...
 * 其中 a, b, c >= 0, r >= 0
 */
bool lv_ineq_schur(lvExpr *a, lvExpr *b, lvExpr *c, uint32_t r,
                     lvInequality **out_ineq) {
    if (!a || !b || !c)
        return false;
 ...
 */
bool lv_ineq_jensen(const char *func, lvExpr **points, mpq_t *weights,
                       uint32_t count, bool is_convex, lvInequality **out_ineq) {
    ...
    *out_ineq = NULL;
    ...
    if (weights) {
        ...
        for (uint32_t i = 0; i < count; i++) {
            ...
            if (!w_x[i]) { lv_expr_free(w_expr); goto cleanup_jensen; }

            /* f(x_i) */
            lvExpr *fx = lv_expr_function(func, points[i]);
            if (!fx) { lv_expr_free(w_expr); goto cleanup_jensen; }
            /* 需要另一个权重表达式副本 */
            lvExpr *w_expr2 = lv_expr_create_rational_mpq(weights[i]);
            if (!w_expr2) { lv_expr_free(w_expr); lv_expr_free(fx); goto cleanup_jensen; }
...
cleanup_jensen:
        ...
        if (!sum) { lv_expr_free(inv_n); return false; }
        weighted_sum = lv_expr_mul(sum, inv_n);
        if (!weighted_sum) { lv_expr_free(sum); lv_expr_free(inv_n); return false; }

        /* 等权函数和 = (f(x_1) + ... + f(x_n)) / n */
        fx_arr = (lvExpr **) lv_malloc((size_t) count * sizeof(lvExpr *));
        if (!fx_arr) { lv_expr_free(weighted_sum); lv_expr_free(sum); lv_expr_free(inv_n); return false; }
        for (uint32_t i = 0; i < count; i++) {
        ...
        lv_free((void **) &fx_arr);
        if (!fx_sum) { lv_expr_free(weighted_sum); lv_expr_free(sum); lv_expr_free(inv_n); return false; }

        inv_n2 = lv_expr_create_rational(1, count);
        if (!inv_n2) { lv_expr_free(fx_sum); lv_expr_free(weighted_sum); lv_expr_free(sum); lv_expr_free(inv_n); return false; }
        weighted_func = lv_expr_mul(fx_sum, inv_n2);
        if (!weighted_func) { lv_expr_free(inv_n2); lv_expr_free(fx_sum); lv_expr_free(weighted_sum); lv_expr_free(sum); lv_expr_free(inv_n); return false; }
    }
 ...
 * 产生三条不等式
 */
uint32_t lv_ineq_triangle(lvExpr *a, lvExpr *b, lvExpr *c,
                             lvInequality **out_inequalities, uint32_t max_count) {
 ...
 * a <= b, b < c => a < c
 */
bool lv_ineq_transitive(lvInequality **ineqs, uint32_t count,
                          lvInequality **out_result) {
 ...
 * a < c, b < d => a + b < c + d
 */
bool lv_ineq_merge(lvInequality **ineqs, uint32_t count,
                     lvInequality **out_result) {
    if (!ineqs || count < 2 || !out_result)
        return false;
...
static bool expr_is_cross_term(const lvExpr *e,
                                 lvExpr **out_a, lvExpr **out_b) {
    ...
    if (out_a) *out_a = (lvExpr *) factor_a;
    if (out_b) *out_b = (lvExpr *) factor_b;
    return true;
}
    ...
    *out_sos = NULL;
    ...
    if (poly->type == EXPR_TYPE_POWER && poly->data.power.exponent) {
        if (poly->data.power.exponent->type == EXPR_TYPE_RATIONAL) {
            int64_t exp_val = 0;
            if (lv_expr_get_integer(poly->data.power.exponent, &exp_val) &&
                exp_val == 2) {
                lvSOSDecomposition *sos = (lvSOSDecomposition *)
                    lv_calloc(1, sizeof(lvSOSDecomposition));
                if (!sos) return false;
                sos->squares = (lvExpr **) lv_malloc(sizeof(lvExpr *));
                if (!sos->squares) { lv_free((void **) &sos); return false; }
    ...
    if (poly->type == EXPR_TYPE_SUM && poly->data.composite.count > 0) {
        ...
        lvExpr **square_bases = (lvExpr **) lv_calloc(
            (size_t) max_squares, sizeof(lvExpr *));
                ...
                for (uint32_t ai = 0; ai < sq_count; ai++) {
                    for (uint32_t bi = ai + 1; bi < sq_count; bi++) {
                        if ((lv_expr_structurally_equal(square_bases[ai], cross_a) &&
                            ...
                            square_bases[ai] = lv_expr_add(
                                lv_expr_copy(square_bases[ai]),
                                lv_expr_copy(square_bases[bi]));
        ...
        if (sq_count > 0) {
            ...
            sos->squares = (lvExpr **) lv_malloc(
                (size_t) sq_count * sizeof(lvExpr *));
            if (!sos->squares) {
                lv_free((void **) &sos);
                lv_free((void **) &square_bases);
                lv_free((void **) &consumed);
        ...
        /* 没有找到任何平方项 — 整个和式无法识别 */
        lvSOSDecomposition *sos_fail = (lvSOSDecomposition *)
            lv_calloc(1, sizeof(lvSOSDecomposition));
        if (sos_fail) {
    ...
    if (poly->type == EXPR_TYPE_PRODUCT && poly->data.composite.count == 2) {
        ...
        if (a && b &&
            lv_expr_structurally_equal(a, b)) {
            lvSOSDecomposition *sos = (lvSOSDecomposition *)
                lv_calloc(1, sizeof(lvSOSDecomposition));
            if (!sos) return false;
            sos->squares = (lvExpr **) lv_malloc(sizeof(lvExpr *));
 ...
 */
bool lv_ineq_triangle_area(lvExpr *a, lvExpr *b, lvExpr *c,
                              lvExpr *area, lvInequality **out_ineq) {
    if (!a || !b || !c || !area)
        return false;
 ...
 */
bool lv_ineq_weitzenbock(lvExpr *a, lvExpr *b, lvExpr *c,
                            lvExpr *area, lvInequality **out_ineq) {
    if (!a || !b || !c || !area)
        return false;
 ...
 * PA + PB + PC >= 2(p + q + r)
 */
bool lv_ineq_erdos_mordell(lvExpr *pa, lvExpr *pb, lvExpr *pc,
                              lvExpr *p, lvExpr *q, lvExpr *r,
                              lvInequality **out_ineq) {
    if (!pa || !pb || !pc || !p || !q || !r)
        return false;
