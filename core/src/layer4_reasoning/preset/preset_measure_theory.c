...
bool preset_measure_theory_register(void) {
    ...
    LV_PRESET_REGISTER(success_count,
            PRESET_MT_SIGMA_ALGEBRA, "σ代数构造：由集合X和子集族C生成最小σ-代数 σ(C)", 2, PRESET_TYPE_SET,
            "\\sigma(\\mathcal{C}) = \\bigcap\\{"
            "\\Sigma : \\mathcal{C} \\subseteq \\Sigma, \\Sigma \\text{ 为 } \\sigma\\text{-代数}\\}",
            "O(\\infty)", false, false,
            PRESET_TYPE_SET, PRESET_TYPE_SET);

    /* Borel σ-代数 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_BOREL_ALGEBRA, "Borel σ-代数：由拓扑空间所有开集生成Borel σ-代数 B(X)",
            1, PRESET_TYPE_SET, "\\mathcal{B}(X) = \\sigma(\\{U \\subseteq X : U \\text{ 开集}\\})",
            "O(\\infty)", false, false,
            PRESET_TYPE_TOPOLOGY);

    /* 测度空间 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_MEASURE_SPACE, "测度空间：构造测度空间 (X, Σ, μ) 满足 μ(∅) = 0", 3,
            PRESET_TYPE_SPACE,
            "(X, \\Sigma, \\mu),\\quad \\mu: \\Sigma \\to [0,+\\infty],\\quad \\mu(\\emptyset)=0",
            "O(1)", true, false,
            PRESET_TYPE_SET, PRESET_TYPE_SET, PRESET_TYPE_FUNCTION);

    /* 零测集判定 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_NULL_SET, "零测集判定：判定集合 N 是否满足 μ(N) = 0", 2,
            PRESET_TYPE_BOOLEAN, "\\mu(N) = 0 \\Rightarrow N \\text{ 为零测集}", "O(1)", false,
            false,
            PRESET_TYPE_SET, PRESET_TYPE_FUNCTION);
    ...
    /* Lebesgue测度 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_LEBESGUE_MEASURE, "Lebesgue测度：在 R^n 上构造Lebesgue测度 m", 1,
    ...
    /* 计数测度 */
    LV_PRESET_REGISTER(success_count,
            PRESET_MT_COUNTING_MEASURE, "计数测度：对任意集合赋予其元素个数", 1, PRESET_TYPE_FUNCTION,
            "\\#(A) = \\begin{cases} n & A \\text{ 有限}, |A|=n \\\\ +\\infty & A \\text{ 无穷} \\end{cases}",
            "O(1)", true, false,
            PRESET_TYPE_SET);

    /* Dirac测度 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_DIRAC_MEASURE, "Dirac测度：在点 x0 处的集中测度 δ_{x0}", 1,
            PRESET_TYPE_FUNCTION,
    ...
    /* 外测度 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_OUTER_MEASURE, "外测度：计算集合A的外测度 μ*(A)，由可测集覆盖的下确界定义",
    ...
    /* 可测函数判定 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_MEASURABLE_FUNCTION, "可测函数判定：f 可测 ⇔ ∀B ∈ B(R), f^{-1}(B) ∈ Σ",
            2, PRESET_TYPE_BOOLEAN,
            "f \\text{ 可测} \\Leftrightarrow "
            "\\forall B\\in\\mathcal{B}(\\mathbb{R}): f^{-1}(B)\\in\\Sigma",
            "O(\\infty)", false, false,
            PRESET_TYPE_FUNCTION, PRESET_TYPE_SET);

    /* 简单函数逼近 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_SIMPLE_FUNCTION, "简单函数逼近：用简单函数序列 s_n 逼近非负可测函数 f",
    ...
    /* Lebesgue积分 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_LEBESGUE_INTEGRAL,
            "Lebesgue积分：计算 f 在 A 上关于 μ 的Lebesgue积分 ∫_A f dμ", 3,
            PRESET_TYPE_SCALAR, "\\int_A f\\,d\\mu = \\int_A f^+\\,d\\mu - \\int_A f^-\\,d\\mu",
            "O(\\infty)", true, false,
            PRESET_TYPE_FUNCTION, PRESET_TYPE_SET, PRESET_TYPE_FUNCTION);

    /* L^p空间范数 */
    LV_PRESET_REGISTER(success_count,
            PRESET_MT_LP_NORM, "L^p空间范数：计算 f 的 L^p 范数 ||f||_p", 3, PRESET_TYPE_SCALAR,
            "\\|f\\|_p = \\left(\\int |f|^p\\,d\\mu\\right)^{1/p},\\quad p \\geq 1", "O(\\infty)", true, false,
            PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_FUNCTION);
    ...
    /* 单调收敛定理 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_MONOTONE_CONVERGENCE,
            "单调收敛定理：若 0≤f1≤f2≤... 且 fn→f a.e.，则 lim∫fn dμ = ∫f dμ", 2,
            PRESET_TYPE_SCALAR,
            "0\\leq f_1\\leq f_2\\leq\\cdots,\\ f_n\\to f \\text{ a.e.}"
            "\\Rightarrow \\lim\\int f_n\\,d\\mu = \\int f\\,d\\mu",
            "O(\\infty)", true, false,
            PRESET_TYPE_SEQUENCE, PRESET_TYPE_FUNCTION);

    /* Fatou引理 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_FATOU_LEMMA, "Fatou引理：∫liminf fn dμ ≤ liminf ∫fn dμ", 2,
            PRESET_TYPE_BOOLEAN,
    ...
    /* 控制收敛定理 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_DOMINATED_CONVERGENCE,
            "控制收敛定理：若 fn→f a.e. 且 |fn|≤g∈L^1，则lim∫fn dμ=∫f dμ", 3,
            PRESET_TYPE_SCALAR,
            "f_n\\to f\\text{ a.e.},\\ |f_n|\\leq g\\in L^1"
            "\\Rightarrow \\lim\\int f_n\\,d\\mu = \\int f\\,d\\mu",
            "O(\\infty)", true, false,
            PRESET_TYPE_SEQUENCE, PRESET_TYPE_FUNCTION, PRESET_TYPE_FUNCTION);

    /* 几乎处处收敛 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_ALMOST_EVERYWHERE, "几乎处处收敛：判定 fn 是否几乎处处收敛于 f", 3,
    ...
    /* 乘积测度 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_PRODUCT_MEASURE, "乘积测度：构造两个σ-有限测度空间的乘积测度", 2,
            PRESET_TYPE_SPACE, "(\\mu\\times\\nu)(A\\times B) = \\mu(A)\\cdot\\nu(B)", "O(1)", true,
            false,
            PRESET_TYPE_SPACE, PRESET_TYPE_SPACE);

    /* Fubini定理 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_FUBINI_THEOREM, "Fubini定理：若 f∈L^1(μ×ν)，则重积分可交换次序", 3,
            PRESET_TYPE_SCALAR,
    ...
    /* 绝对连续性 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_ABSOLUTE_CONTINUITY, "绝对连续性：ν≪μ ⇔ μ(A)=0 ⇒ ν(A)=0", 2,
            PRESET_TYPE_BOOLEAN, "\\nu\\ll\\mu \\Leftrightarrow \\mu(A)=0\\Rightarrow\\nu(A)=0",
            "O(\\infty)", false, false,
            PRESET_TYPE_FUNCTION, PRESET_TYPE_FUNCTION);

    /* Radon-Nikodym导数 */
    LV_PRESET_REGISTER(success_count, PRESET_MT_RADON_NIKODYM, "Radon-Nikodym导数：当 ν≪μ 时，dν/dμ 满足 ν(A)=∫_A(dν/dμ)dμ",
            2, PRESET_TYPE_FUNCTION,
