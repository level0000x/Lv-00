 ...
 * @brief 注册单个测度论预设
 */
static bool register_mt_preset(
    const char *name,
...
bool preset_measure_theory_register(void)
{
    ...
    /* Borel σ-代数 */
    {
        ...
        if (register_mt_preset(
                ...
                "Borel σ-代数：由拓扑空间所有开集生成Borel σ-代数 B(X)",
                inputs, 1, PRESET_TYPE_SET,
                "\\mathcal{B}(X) = \\sigma(\\{U \\subseteq X : U \\text{ 开集}\\})",
                "O(\\infty)", false, false)) {
            success_count++;
        }
    }
    ...
    /* 测度空间 */
    {
        ...
        if (register_mt_preset(
                ...
                "测度空间：构造测度空间 (X, Σ, μ) 满足 μ(∅) = 0",
                inputs, 3, PRESET_TYPE_SPACE,
    ...
    /* 零测集判定 */
    {
        ...
        if (register_mt_preset(
                PRESET_MT_NULL_SET,
                "零测集判定：判定集合 N 是否满足 μ(N) = 0",
                inputs, 2, PRESET_TYPE_BOOLEAN,
                "\\mu(N) = 0 \\Rightarrow N \\text{ 为零测集}",
                "O(1)", false, false)) {
            success_count++;
        }
    }
    ...
    /* Lebesgue测度 */
    {
        ...
        if (register_mt_preset(
                ...
                "Lebesgue测度：在 R^n 上构造Lebesgue测度 m",
    ...
    /* 外测度 */
    {
        ...
        if (register_mt_preset(
                ...
                "外测度：计算集合A的外测度 μ*(A)，由可测集覆盖的下确界定义",
    ...
    /* Lebesgue积分 */
    {
        ...
        if (register_mt_preset(
                ...
                "Lebesgue积分：计算 f 在 A 上关于 μ 的Lebesgue积分 ∫_A f dμ",
    ...
    /* 单调收敛定理 */
    {
        ...
        if (register_mt_preset(
                ...
                "单调收敛定理：若 0≤f1≤f2≤... 且 fn→f a.e.，则 lim∫fn dμ = ∫f dμ",
                inputs, 2, PRESET_TYPE_SCALAR,
    ...
    /* Fatou引理 */
    {
        ...
        if (register_mt_preset(
                ...
                "Fatou引理：∫liminf fn dμ ≤ liminf ∫fn dμ",
                inputs, 2, PRESET_TYPE_BOOLEAN,
    ...
    /* 控制收敛定理 */
    {
        ...
        if (register_mt_preset(
                ...
                "控制收敛定理：若 fn→f a.e. 且 |fn|≤g∈L^1，则lim∫fn dμ=∫f dμ",
                inputs, 3, PRESET_TYPE_SCALAR,
    ...
    /* Fubini定理 */
    {
        ...
        if (register_mt_preset(
                ...
                "Fubini定理：若 f∈L^1(μ×ν)，则重积分可交换次序",
                inputs, 3, PRESET_TYPE_SCALAR,
    ...
    /* Radon-Nikodym导数 */
    {
        ...
        if (register_mt_preset(
                ...
                "Radon-Nikodym导数：当 ν≪μ 时，dν/dμ 满足 ν(A)=∫_A(dν/dμ)dμ",
                inputs, 2, PRESET_TYPE_FUNCTION,
