...
bool preset_functional_analysis_register(void) {
    ...
    /* 范数判定 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_NORM_CHECK, "范数判定：验证函数 ||·|| 是否满足正定性、齐次性、三角不等式",
            2, PRESET_TYPE_BOOLEAN,
            "\\|x\\|=0\\Leftrightarrow x=0,"
            "\\ \\|\\alpha x\\|=|\\alpha|\\|x\\|,"
            "\\ \\|x+y\\|\\leq\\|x\\|+\\|y\\|",
            "O(\\infty)", false, false,
            PRESET_TYPE_FUNCTION, PRESET_TYPE_SPACE);

    /* Banach空间判定 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_BANACH_SPACE_CHECK, "Banach空间判定：验证赋范空间是否完备（Cauchy列收敛）",
    ...
    /* 内积判定 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_INNER_PRODUCT_CHECK, "内积判定：验证二元函数是否满足内积公理", 2,
            PRESET_TYPE_BOOLEAN,
    ...
    /* Hilbert空间判定 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_HILBERT_SPACE_CHECK, "Hilbert空间判定：验证内积空间是否关于诱导范数完备",
            1, PRESET_TYPE_BOOLEAN,
    ...
    /* 有界性判定 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_BOUNDED_OPERATOR, "有界性判定：T:X→Y 线性，∃M>0: ||Tx||≤M||x||", 3,
    ...
    LV_PRESET_REGISTER(success_count,
            PRESET_FA_COMPACT_OPERATOR, "紧算子判定：有界集在T下的像的闭包为紧集", 2, PRESET_TYPE_BOOLEAN,
            "\\forall B\\subset X\\text{ 有界}: \\overline{T(B)}\\text{ 为紧集}", "O(\\infty)", false, false,
            PRESET_TYPE_FUNCTION, PRESET_TYPE_SPACE);

    /* 谱分析 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_SPECTRAL_ANALYSIS, "谱分析：分析有界线性算子T的谱集组成", 1,
            PRESET_TYPE_LIST,
    ...
    /* Hahn-Banach定理 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_HAHN_BANACH, "Hahn-Banach定理：将子空间上的有界线性泛函保范延拓到全空间",
            3, PRESET_TYPE_FUNCTION, "\\exists F\\in X^*: F|_M = f,\\ \\|F\\| = \\|f\\|",
            "O(\\infty)", true, false,
            PRESET_TYPE_FUNCTION, PRESET_TYPE_SPACE, PRESET_TYPE_SPACE);

    /* 开映射定理 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_OPEN_MAPPING, "开映射定理：Banach空间之间的满射有界线性算子将开集映射为开集",
            2, PRESET_TYPE_SET,
    ...
    /* 闭图像定理 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_CLOSED_GRAPH, "闭图像定理：图像为闭集的线性算子必有界", 3,
    ...
    /* 一致有界原理 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_UNIFORM_BOUNDEDNESS, "一致有界原理：逐点有界的算子族一致有界", 2,
            PRESET_TYPE_BOOLEAN,
            "\\sup_\\alpha\\|T_\\alpha x\\|<\\infty\\ (\\forall x)"
            "\\Rightarrow\\sup_\\alpha\\|T_\\alpha\\|<\\infty",
            "O(\\infty)", false, false,
            PRESET_TYPE_LIST, PRESET_TYPE_SPACE);
    ...
    /* 弱收敛 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_WEAK_CONVERGENCE, "弱收敛：x_n ⇀ x ⇔ ∀f∈X*: f(x_n)→f(x)", 3,
            PRESET_TYPE_BOOLEAN,
            "x_n\\rightharpoonup x \\Leftrightarrow "
            "\\forall f\\in X^*: \\lim f(x_n)=f(x)",
            "O(\\infty)", false, false,
            PRESET_TYPE_SEQUENCE, PRESET_TYPE_VECTOR, PRESET_TYPE_SPACE);

    /* 弱*收敛 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_WEAK_STAR_CONVERGENCE, "弱*收敛：f_n ⇀* f ⇔ ∀x∈X: f_n(x)→f(x)", 3,
            PRESET_TYPE_BOOLEAN,
            "f_n\\rightharpoonup^* f \\Leftrightarrow "
            "\\forall x\\in X: \\lim f_n(x)=f(x)",
            "O(\\infty)", false, false,
            PRESET_TYPE_SEQUENCE, PRESET_TYPE_FUNCTION, PRESET_TYPE_SPACE);
    ...
    /* 对偶空间 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_DUAL_SPACE, "对偶空间：X* = L(X,F)，所有有界线性泛函构成的对偶空间", 1,
            PRESET_TYPE_SPACE, "X^* = \\{f:X\\to\\mathbb{F}: f\\text{ 线性有界}\\}", "O(1)", true,
            false,
            PRESET_TYPE_SPACE);

    /* 伴随算子 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_ADJOINT_OPERATOR, "伴随算子：⟨Tx,y⟩ = ⟨x,T*y⟩，T*为T的Hilbert伴随", 1,
            PRESET_TYPE_FUNCTION, "\\langle Tx, y\\rangle = \\langle x, T^*y\\rangle", "O(\\infty)",
            true, false,
            PRESET_TYPE_FUNCTION);

    /* 自伴算子 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_SELF_ADJOINT, "自伴算子判定：T = T*，即 ⟨Tx,y⟩ = ⟨x,Ty⟩", 1,
            PRESET_TYPE_BOOLEAN,
    ...
    /* 正交投影 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_ORTHOGONAL_PROJECTION, "正交投影：计算 x 在闭子空间 M 上的正交投影 P_M x",
            2, PRESET_TYPE_VECTOR, "x - P_M x \\perp M,\\quad P_M x \\in M", "O(n)", true,
            false,
            PRESET_TYPE_VECTOR, PRESET_TYPE_SPACE);

    /* Gram-Schmidt正交化 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_GRAM_SCHMIDT, "Gram-Schmidt正交化：将线性无关向量组正交规范化", 1,
            PRESET_TYPE_LIST,
    ...
    /* Riesz表示定理 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_RIESZ_REPRESENTATION,
            "Riesz表示定理：∃!y_f∈H: f(x) = ⟨x,y_f⟩, ||f|| = ||y_f||", 2, PRESET_TYPE_VECTOR,
    ...
    /* Banach不动点定理 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_FIXED_POINT, "Banach不动点定理：完备度量空间上的压缩映射存在唯一不动点",
    ...
    /* 压缩映射判定 */
    LV_PRESET_REGISTER(success_count, PRESET_FA_CONTRACTION_MAPPING, "压缩映射判定：∃k∈(0,1): d(Tx,Ty) ≤ k·d(x,y)", 2,
