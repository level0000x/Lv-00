 ...
 * @return false 注册失败
 */
static bool register_linalg_preset(const char *name, const char *description, const PresetType *input_types,
                                   int input_count, PresetType output_type, const char *math_def,
                                   const char *complexity, bool is_constructive, bool is_reversible) {
    return preset_blocks_register_simple(name, description, PRESET_CATEGORY_LINEAR_ALGEBRA, input_types, input_count,
...
bool preset_linear_algebra_register(void) {
    ...
    /* -------------------- 矩阵创建 -------------------- */
    {
        ...
        if (register_linalg_preset(PRESET_LINALG_MATRIX_CREATE, "矩阵创建：根据行数和列数创建 m x n 零矩阵", inputs, 2,
    ...
    /* -------------------- 矩阵加法 -------------------- */
    {
        ...
        if (register_linalg_preset(PRESET_LINALG_MATRIX_ADD, "矩阵加法：计算两个同阶矩阵的和 C = A + B，对应元素相加",
                                   inputs, 2, PRESET_TYPE_MATRIX, "C_{ij} = A_{ij} + B_{ij}", "O(n^2)", true, true)) {
            success_count++;
        }
    }
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX, PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_MATRIX_SUBTRACT,
                                   "矩阵减法：计算两个同阶矩阵的差 C = A - B，对应元素相减", inputs, 2,
    ...
    /* -------------------- 矩阵乘法 -------------------- */
    {
        ...
        if (register_linalg_preset(
                PRESET_LINALG_MATRIX_MULTIPLY, "矩阵乘法：计算两个矩阵的乘积 C = A * B，行与列的点积", inputs, 2,
    ...
    /* -------------------- 矩阵标量乘法 -------------------- */
    {
        ...
        if (register_linalg_preset(PRESET_LINALG_MATRIX_SCALE, "矩阵标量乘法：计算标量与矩阵的乘积 B = k * A", inputs,
                                   2, PRESET_TYPE_MATRIX, "B_{ij} = k \\cdot A_{ij}", "O(n^2)", true, true)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 矩阵转置 -------------------- */
    {
        ...
        if (register_linalg_preset(PRESET_LINALG_MATRIX_TRANSPOSE, "矩阵转置：将矩阵的行与列互换，得到转置矩阵 A^T",
                                   inputs, 1, PRESET_TYPE_MATRIX, "(A^T)_{ij} = A_{ji}", "O(n^2)", true, true)) {
            success_count++;
        }
    }
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_MATRIX_TRACE, "矩阵迹：计算方阵主对角线元素之和 tr(A) = sum(a_ii)",
                                   inputs, 1, PRESET_TYPE_SCALAR, "\\text{tr}(A) = \\sum_{i=1}^{n} a_{ii}", "O(n)",
                                   false, false)) {
            success_count++;
        }
    }
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_MATRIX_NEGATE, "矩阵取负：计算矩阵的负矩阵 B = -A，每个元素取反",
                                   inputs, 1, PRESET_TYPE_MATRIX, "B_{ij} = -A_{ij}", "O(n^2)", true, true)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 2x2行列式 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(
                PRESET_LINALG_DETERMINANT_2X2, "2x2行列式：det(A) = ad - bc", inputs, 1, PRESET_TYPE_SCALAR,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_DETERMINANT_3X3, "3x3行列式：按第一行展开计算 det(A)", inputs, 1,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_DETERMINANT_N, "n阶行列式：使用LU分解或余子式展开计算方阵的行列式",
    ...
    /* -------------------- 2x2逆矩阵 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(
                PRESET_LINALG_INVERSE_2X2, "2x2逆矩阵：A^{-1} = (1/det(A)) * adj(A)", inputs, 1, PRESET_TYPE_MATRIX,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_INVERSE_3X3, "3x3逆矩阵：使用伴随矩阵法计算 A^{-1} = adj(A)/det(A)",
                                   inputs, 1, PRESET_TYPE_MATRIX, "A^{-1} = \\frac{\\text{adj}(A)}{\\det(A)}", "O(1)",
                                   true, true)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 伴随矩阵 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(
                PRESET_LINALG_ADJUGATE, "伴随矩阵：计算方阵的伴随矩阵 adj(A)，元素为代数余子式的转置", inputs, 1,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_LU_DECOMPOSITION,
                                   "LU分解：将方阵分解为下三角矩阵 L 和上三角矩阵 U 的乘积 A = LU", inputs, 1,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_QR_DECOMPOSITION,
                                   "QR分解：将矩阵分解为正交矩阵 Q 和上三角矩阵 R 的乘积 A = QR", inputs, 1,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_CHOLESKY, "Cholesky分解：将正定对称矩阵分解为 A = L * L^T", inputs, 1,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_EIGENVALUES_2X2, "2x2矩阵特征值：求解特征方程 det(A - lambda*I) = 0",
    ...
    /* -------------------- 2x2特征向量 -------------------- */
    {
        ...
        if (register_linalg_preset(PRESET_LINALG_EIGENVECTORS_2X2, "2x2矩阵特征向量：计算对应于给定特征值的特征向量",
                                   inputs, 2, PRESET_TYPE_LIST, "Av = \\lambda v, \\quad v \\neq 0", "O(1)", true,
                                   false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 向量空间判定 -------------------- */
    {
        ...
        if (register_linalg_preset(PRESET_LINALG_VECTOR_SPACE_TEST,
                                   "向量空间判定：验证集合在给定加法和标量乘法下是否满足向量空间公理", inputs, 3,
                                   PRESET_TYPE_BOOLEAN,
    ...
    /* -------------------- 线性无关判定 -------------------- */
    {
        ...
        if (register_linalg_preset(PRESET_LINALG_LINEAR_INDEPENDENCE,
                                   "线性无关判定：验证向量组 {v1,...,vk} 是否线性无关", inputs, 1, PRESET_TYPE_BOOLEAN,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_LIST};
        if (register_linalg_preset(PRESET_LINALG_SPAN,
                                   "生成空间：计算向量组 {v1,...,vk} 的线性生成空间 span{v1,...,vk}", inputs, 1,
                                   PRESET_TYPE_SET,
    ...
    /* -------------------- 基 -------------------- */
    {
        ...
        if (register_linalg_preset(PRESET_LINALG_BASIS, "基：计算向量空间的一组基（极大线性无关组）", inputs, 1,
                                   PRESET_TYPE_LIST,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_SET};
        if (register_linalg_preset(PRESET_LINALG_DIMENSION, "维数：计算向量空间的维数 dim(V)（基中元素个数）", inputs,
    ...
    /* -------------------- 线性映射判定 -------------------- */
    {
        ...
        if (register_linalg_preset(PRESET_LINALG_LINEAR_MAP_TEST,
                                   "线性映射判定：验证映射 T 是否满足 T(u+v) = T(u)+T(v) 和 T(cv) = cT(v)", inputs, 1,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_KERNEL, "核空间：计算线性映射的核 ker(T) = {v : T(v) = 0} 的基",
                                   inputs, 1, PRESET_TYPE_LIST, "\\ker(T) = \\{v \\in V : T(v) = 0\\}", "O(n^3)", true,
                                   false)) {
            success_count++;
        }
    }
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_IMAGE, "像：计算线性映射的像空间 Im(T) = {T(v) : v in V} 的基", inputs,
                                   1, PRESET_TYPE_LIST,
                                   "\\text{Im}(T) = \\{T(v) : v \\in V\\} = "
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_linalg_preset(PRESET_LINALG_RANK_NULLITY, "秩-零化度定理：验证 rank(T) + nullity(T) = dim(V)",
                                   inputs, 1, PRESET_TYPE_BOOLEAN, "\\text{rank}(T) + \\text{nullity}(T) = \\dim(V)",
                                   "O(n^3)", false, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 内积 -------------------- */
    {
        ...
        if (register_linalg_preset(PRESET_LINALG_INNER_PRODUCT, "内积：计算内积空间中两个向量的内积 <u, v>", inputs, 2,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_LIST};
        if (register_linalg_preset(PRESET_LINALG_GRAM_SCHMIDT, "Gram-Schmidt正交化：将线性无关向量组转化为正交向量组",
                                   inputs, 1, PRESET_TYPE_LIST,
    ...
    /* -------------------- 正交补 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_SET};
        if (register_linalg_preset(
                PRESET_LINALG_ORTHOGONAL_COMPLEMENT, "正交补：计算子空间 W 的正交补 W^perp", inputs, 1, PRESET_TYPE_SET,
    ...
    /* -------------------- 子空间投影 -------------------- */
    {
        ...
        if (register_linalg_preset(PRESET_LINALG_PROJECTION, "子空间投影：计算向量 v 在子空间 W 上的正交投影 proj_W(v)",
