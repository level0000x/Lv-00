 ...
 * @return false 注册失败
 */
static bool register_graph_theory_preset(
    const char *name,
...
bool preset_graph_theory_register(void)
{
    ...
    /* -------------------- 邻接矩阵 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "计算图的邻接矩阵 A(G)，其中 A_{ij} 表示顶点 i 与 j 之间的边数",
    ...
    /* -------------------- 度序列 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "d(G) = (d_1, d_2, \\ldots, d_n), d_1 \\ge d_2 \\ge \\cdots \\ge d_n",
                "O(|V| + |E|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 子图判定 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "判定图 H 是否是图 G 的子图（V(H) ⊆ V(G), E(H) ⊆ E(G)）",
                inputs, 2, PRESET_TYPE_BOOLEAN,
                "H \\le G \\Leftrightarrow V(H) \\subseteq V(G) \\wedge E(H) \\subseteq E(G)",
    ...
    /* -------------------- 补图 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "计算图的补图 \\bar{G}，其中 \bar{G} 包含 G 中不存在的所有边",
                inputs, 1, PRESET_TYPE_GRAPH,
                "\\bar{G} = (V, \\bar{E}), \\bar{E} = \\{uv : u,v \\in V, uv \\notin E, u \\ne v\\}",
                "O(|V|^2)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 连通分量 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "计算图的所有连通分量（基于BFS/DFS遍历）",
                inputs, 1, PRESET_TYPE_LIST,
                "G = C_1 \\cup C_2 \\cup \\cdots \\cup C_k, C_i \\text{ 连通}",
                "O(|V| + |E|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 强连通分量 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                PRESET_GRAPH_SCC,
                "计算有向图的所有强连通分量（Kosaraju算法）",
                inputs, 1, PRESET_TYPE_LIST,
                "\\text{SCC}(G) = \\{C_1, C_2, \\ldots, C_k\\}, C_i \\text{ 强连通}",
                "O(|V| + |E|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 割点检测 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "检测图中的所有割点（关节点），删除后增加连通分量数",
                inputs, 1, PRESET_TYPE_SET,
                "v \\text{ 是割点} \\Leftrightarrow c(G - v) > c(G)",
                "O(|V| + |E|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 桥检测 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "检测图中的所有桥（割边），删除后增加连通分量数",
                inputs, 1, PRESET_TYPE_SET,
                "e \\text{ 是桥} \\Leftrightarrow c(G - e) > c(G)",
                "O(|V| + |E|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 最短路径 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "Dijkstra单源最短路径算法，计算从源点到目标点的最短路径",
    ...
    /* -------------------- 欧拉路径判定 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "判定图是否存在欧拉路径或欧拉回路（经过每条边恰好一次）",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "G \\text{ 有欧拉回路} \\Leftrightarrow G \\text{ 连通且所有顶点度为偶数}",
                "O(|V| + |E|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 色数 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "计算图的色数 chi(G)，即顶点着色所需的最少颜色数",
                inputs, 1, PRESET_TYPE_INTEGER,
                "\\chi(G) = \\min\\{k : \\exists \\text{ 合法的 } k \\text{-着色}\\}",
    ...
    /* -------------------- 平面图判定 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "判定图是否是平面图（可在平面上绘制且边不相交）",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "G \\text{ 平面} \\Leftrightarrow |E| \\le 3|V| - 6 \\text{（|V| \\ge 3 时必要条件）}",
                "O(|V|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 四色定理验证 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "验证四色定理对给定平面图成立（构造4-着色方案）",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "\\chi(G) \\le 4, \\forall \\text{ 平面图 } G",
                "O(|V|^2)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 最大匹配 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "计算图的最大匹配（基于增广路算法）",
                inputs, 1, PRESET_TYPE_SET,
                "M^* = \\arg\\max_{M \\subseteq E} |M|, M \\text{ 中边两两不邻}",
                "O(|V| \\cdot |E|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 完美匹配判定 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "判定图是否存在完美匹配（每个顶点恰好被一条匹配边覆盖）",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "G \\text{ 有完美匹配} \\Leftrightarrow |M^*| = |V|/2",
                "O(|V| \\cdot |E|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 独立集 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "计算图的最大独立集（集合中任意两顶点不相邻）",
                inputs, 1, PRESET_TYPE_SET,
                "\\alpha(G) = \\max\\{|S| : S \\subseteq V, \\forall u,v \\in S, uv \\notin E\\}",
    ...
    /* -------------------- 支配集 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "计算图的最小支配集（每个顶点要么在集合中，要么与集合中顶点相邻）",
                inputs, 1, PRESET_TYPE_SET,
    ...
    /* -------------------- 完全图 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "构造 n 阶完全图 K_n（每对顶点之间都有边相连）",
                inputs, 1, PRESET_TYPE_GRAPH,
                "K_n = (V, E), |V| = n, |E| = \\binom{n}{2}",
                "O(n^2)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 二部图判定 -------------------- */
    {
        ...
        if (register_graph_theory_preset(
                ...
                "判定图是否是二部图（顶点集可划分为两个独立集）",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "G \\text{ 二部} \\Leftrightarrow \\chi(G) \\le 2 \\Leftrightarrow G \\text{ 无奇数环}",
                "O(|V| + |E|)", true, false)) {
            success_count++;
        }
    }
