...
bool preset_arithmetic_geometry_register(void) {
    ...
    /* Weierstrass 标准形式 */
    {
        ...
        if (register_ag_preset(PRESET_AG_WEIERSTRASS_FORM,
                               "Weierstrass 标准形式：y² = x³ + ax + b，判别式 Δ = -16(4a³ + 27b²) ≠ 0", inputs, 3,
    ...
    /* 椭圆曲线群运算 */
    {
        ...
        if (register_ag_preset(PRESET_AG_ELLIPTIC_ADD, "椭圆曲线群运算：P + Q，弦切法定义加法", inputs, 3,
                               PRESET_TYPE_TUPLE,
                               "P + Q = R, \\quad "
    ...
    /* 椭圆曲线点加倍 */
    {
        ...
        if (register_ag_preset(PRESET_AG_ELLIPTIC_DOUBLE, "椭圆曲线点加倍：2P，切线法定义", inputs, 2,
                               PRESET_TYPE_TUPLE,
                               "2P = R, \\quad "
    ...
    /* 椭圆曲线标量乘法 */
    {
        ...
        if (register_ag_preset(PRESET_AG_ELLIPTIC_SCALAR, "椭圆曲线标量乘法：nP = P + P + ... + P（n次）", inputs, 3,
                               PRESET_TYPE_TUPLE,
    ...
    /* 椭圆曲线判别式与 j 不变量 */
    {
        ...
        if (register_ag_preset(PRESET_AG_ELLIPTIC_INVARIANTS, "椭圆曲线判别式 Δ 与 j-不变量：曲线的同构类由 j 唯一确定",
    ...
    /* 椭圆曲线 torsion 点 */
    {
        ...
        if (register_ag_preset(PRESET_AG_ELLIPTIC_TORSION,
                               "椭圆曲线 torsion 点：E[n] = {P : nP = O}，Mazur定理限制可能结构", inputs, 2,
                               PRESET_TYPE_SET,
    ...
    /* 模群作用 */
    {
        ...
        if (register_ag_preset(
                PRESET_AG_MODULAR_GROUP, "模群 SL₂(ℤ) 作用：τ ↦ (aτ+b)/(cτ+d)", inputs, 2, PRESET_TYPE_SCALAR,
    ...
    /* Eisenstein 级数 */
    {
        ...
        if (register_ag_preset(PRESET_AG_EISENSTEIN_SERIES, "Eisenstein 级数：G_k(τ) = Σ' (mτ+n)^(-k)", inputs, 2,
                               PRESET_TYPE_SCALAR,
    ...
    /* 模判别式 Δ */
    {
        ...
        if (register_ag_preset(PRESET_AG_MODULAR_DISCRIMINANT, "模判别式 Δ(τ) = qΠ(1-qⁿ)²⁴，q = exp(2πiτ)", inputs, 1,
                               PRESET_TYPE_SCALAR,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_SCALAR};
        if (register_ag_preset(PRESET_AG_J_INVARIANT, "模 j-不变量：j(τ) = 1728G₄³/(G₄³ - G₆²)", inputs, 1,
                               PRESET_TYPE_SCALAR,
    ...
    /* Pell 方程求解 */
    {
        ...
        if (register_ag_preset(PRESET_AG_PELL_EQUATION, "Pell 方程：x² - Dy² = 1，连分数法求最小解", inputs, 1,
                               PRESET_TYPE_TUPLE,
    ...
    /* Thue 方程 */
    {
        ...
        if (register_ag_preset(PRESET_AG_THUE_EQUATION, "Thue 方程：F(x,y) = m，有限解定理", inputs, 2,
                               PRESET_TYPE_LIST,
                               "F(x,y) = m, \\quad "
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_INTEGER};
        if (register_ag_preset(PRESET_AG_MORDELL_EQUATION, "Mordell 方程：y² = x³ + k，椭圆曲线特殊情形", inputs, 1,
                               PRESET_TYPE_LIST,
    ...
    /* Fermat 方程判定 */
    {
        ...
        if (register_ag_preset(PRESET_AG_FERMAT_EQUATION, "Fermat 方程：xⁿ + yⁿ = zⁿ，n > 2 时无非平凡整数解（Wiles）",
    ...
    /* 代数整数环 */
    {
        ...
        if (register_ag_preset(PRESET_AG_INTEGER_RING, "代数整数环 O_K：K 中所有代数整数的集合", inputs, 1,
                               PRESET_TYPE_RING,
    ...
    /* 理想类群 */
    {
        ...
        if (register_ag_preset(PRESET_AG_CLASS_GROUP, "理想类群 Cl(K)：衡量主理想整环的偏离程度", inputs, 1,
                               PRESET_TYPE_GROUP,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_RING};
        if (register_ag_preset(PRESET_AG_UNIT_GROUP, "单位群 O_K^×：Dirichlet单位定理描述其结构", inputs, 1,
                               PRESET_TYPE_GROUP,
    ...
    /* Dedekind zeta 函数 */
    {
        ...
        if (register_ag_preset(PRESET_AG_DEDEKIND_ZETA, "Dedekind zeta 函数：ζ_K(s) = ∏(1 - N(p)^(-s))^(-1)", inputs, 2,
                               PRESET_TYPE_SCALAR,
    ...
    /* p-adic 赋值 */
    {
        ...
        if (register_ag_preset(PRESET_AG_PADIC_VALUATION, "p-adic 赋值：v_p(n) = n 中 p 的最高幂次", inputs, 2,
                               PRESET_TYPE_INTEGER,
    ...
    /* p-adic 范数 */
    {
        ...
        if (register_ag_preset(PRESET_AG_PADIC_NORM, "p-adic 范数：|x|_p = p^(-v_p(x))", inputs, 2, PRESET_TYPE_SCALAR,
                               "|x|_p = p^{-v_p(x)}, \\quad "
    ...
    /* Hensel 引理 */
    {
        ...
        if (register_ag_preset(PRESET_AG_HENSEL_LEMMA, "Hensel 引理：模 p 解可提升为 p-adic 解", inputs, 3,
                               PRESET_TYPE_BOOLEAN,
    ...
    /* p-adic 数域扩张 */
    {
        ...
        if (register_ag_preset(PRESET_AG_PADIC_EXTENSION, "p-adic 数域扩张：Q_p 的有限扩张", inputs, 2,
                               PRESET_TYPE_FIELD,
    ...
    /* 有理点高度 */
    {
        ...
        if (register_ag_preset(PRESET_AG_HEIGHT_FUNCTION, "有理点高度：H(P) = max(|x|, |y|)，P = (x:y:z) 既约", inputs,
    ...
    /* Mordell-Weil 定理 */
    {
        ...
        if (register_ag_preset(PRESET_AG_MORDELL_WEIL, "Mordell-Weil 定理：椭圆曲线有理点群有限生成", inputs, 1,
                               PRESET_TYPE_TUPLE,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_EQUATION, PRESET_TYPE_INTEGER};
        if (register_ag_preset(PRESET_AG_FALTINGS, "Faltings 定理（Mordell 猜想）：亏格 ≥ 2 的曲线有有限多有理点",
                               inputs, 2, PRESET_TYPE_BOOLEAN,
    ...
    /* Chabauty 方法 */
    {
        ...
        if (register_ag_preset(PRESET_AG_CHABAUTY, "Chabauty 方法：用 p-adic 积分计算有理点上界", inputs, 2,
                               PRESET_TYPE_INTEGER,
