 ...
 * @return false 注册失败
 */
static bool register_optimization_preset(const char *name, const char *description, const PresetType *input_types,
...
bool preset_optimization_register(void) {
    ...
    /* -------------------- 梯度下降法 -------------------- */
    {
        ...
        if (register_optimization_preset(
                PRESET_OPT_GRADIENT_DESCENT, "梯度下降法：沿目标函数的负梯度方向迭代搜索极小值点", inputs, 2,
                PRESET_TYPE_TUPLE, "x_{k+1} = x_k - \\alpha_k \\nabla f(x_k)", "O(n \\cdot k)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 牛顿法 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_TUPLE};
        if (register_optimization_preset(
                PRESET_OPT_NEWTON_METHOD, "牛顿法：利用目标函数的二阶导数（Hessian矩阵）构造二次近似模型进行迭代",
                inputs, 2, PRESET_TYPE_TUPLE, "x_{k+1} = x_k - [\\nabla^2 f(x_k)]^{-1} \\nabla f(x_k)",
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_TUPLE};
        if (register_optimization_preset(PRESET_OPT_CONJUGATE_GRADIENT,
                                         "共轭梯度法：在共轭方向上依次进行一维搜索，适用于大规模无约束优化问题", inputs,
                                         2, PRESET_TYPE_TUPLE,
    ...
    /* -------------------- 拟牛顿法 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_TUPLE};
        if (register_optimization_preset(
                PRESET_OPT_QUASI_NEWTON, "拟牛顿法（BFGS/DFP）：通过梯度差分近似Hessian矩阵的逆，避免直接计算二阶导数",
    ...
    /* -------------------- 拉格朗日乘子法 -------------------- */
    {
        ...
        if (register_optimization_preset(
                PRESET_OPT_LAGRANGE_MULTIPLIER, "拉格朗日乘子法：引入拉格朗日乘子将等式约束优化问题转化为无约束问题",
                inputs, 2, PRESET_TYPE_TUPLE, "\\mathcal{L}(x, \\lambda) = f(x) + \\sum_{i=1}^{m} \\lambda_i h_i(x)",
                "O(n^3)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- KKT条件 -------------------- */
    {
        ...
        if (register_optimization_preset(
                PRESET_OPT_KKT_CONDITIONS, "KKT条件：约束优化问题局部最优解的必要条件（Karush-Kuhn-Tucker条件）",
    ...
    /* -------------------- 罚函数法 -------------------- */
    {
        ...
        if (register_optimization_preset(PRESET_OPT_PENALTY_METHOD,
    ...
    /* -------------------- 障碍函数法 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_LIST, PRESET_TYPE_SCALAR};
        if (register_optimization_preset(
                PRESET_OPT_BARRIER_METHOD, "障碍函数法：在可行域内部添加障碍项，使迭代点始终保持在可行域内并逼近最优解",
                inputs, 3, PRESET_TYPE_FUNCTION, "B(x, \\mu) = f(x) - \\mu \\sum_{i=1}^{m} \\ln(-g_i(x))",
                "O(n \\cdot k)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 单纯形法 -------------------- */
    {
        ...
        if (register_optimization_preset(
                PRESET_OPT_SIMPLEX, "单纯形法：在可行域的顶点之间移动，逐步改进目标函数值，求解线性规划问题", inputs, 3,
                PRESET_TYPE_TUPLE, "\\min c^T x \\quad \\text{s.t.} \\quad Ax = b, \\quad x \\ge 0", "O(2^n) 最坏情况",
                true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 内点法 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_MATRIX, PRESET_TYPE_TUPLE};
        if (register_optimization_preset(
                PRESET_OPT_INTERIOR_POINT, "内点法：通过障碍函数在可行域内部逼近最优解，具有多项式时间复杂度", inputs,
                3, PRESET_TYPE_TUPLE,
                "\\min c^T x - \\mu \\sum_{i} \\ln x_i \\quad \\text{s.t.} \\quad Ax = b, \\quad x > 0", "O(n^{3.5} L)",
                true, false)) {
            success_count++;
        }
    }
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_MATRIX, PRESET_TYPE_TUPLE};
        if (register_optimization_preset(PRESET_OPT_DUAL_SIMPLEX,
                                         "对偶单纯形法：基于对偶理论，在保持对偶可行性的前提下恢复原始可行性", inputs,
                                         3, PRESET_TYPE_TUPLE, "\\max b^T y \\quad \\text{s.t.} \\quad A^T y \\le c",
    ...
    /* -------------------- 凸性检验 -------------------- */
    {
        ...
        if (register_optimization_preset(
                PRESET_OPT_CONVEXITY_TEST, "凸性检验：判断目标函数是否为凸函数（Hessian矩阵半正定）", inputs, 1,
                PRESET_TYPE_BOOLEAN,
    ...
    /* -------------------- 凸梯度法 -------------------- */
    {
        ...
        if (register_optimization_preset(PRESET_OPT_CVX_GRADIENT,
                                         "凸梯度法：针对凸函数的梯度下降优化，保证收敛到全局最优解", inputs, 3,
                                         PRESET_TYPE_TUPLE,
    ...
    /* -------------------- 近端梯度法 -------------------- */
    {
        ...
        if (register_optimization_preset(
                ...
                "近端梯度法：处理复合目标函数 min f(x)+g(x)，其中f光滑、g非光滑但可计算近端算子", inputs, 3,
                PRESET_TYPE_TUPLE,
    ...
    /* -------------------- ADMM -------------------- */
    {
        ...
        if (register_optimization_preset(
                PRESET_OPT_ADMM, "交替方向乘子法（ADMM）：将全局优化问题分解为局部子问题，适用于分布式凸优化", inputs,
                3, PRESET_TYPE_TUPLE, "\\min f(x) + g(z) \\quad \\text{s.t.} \\quad Ax + Bz = c", "O(n \\cdot k)", true,
                false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 欧拉-拉格朗日方程 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION};
        if (register_optimization_preset(
                PRESET_OPT_EULER_LAGRANGE, "欧拉-拉格朗日方程：泛函极值问题的必要条件，将变分问题转化为微分方程",
                inputs, 1, PRESET_TYPE_EQUATION,
                "\\frac{\\partial L}{\\partial y} - \\frac{d}{dx}\\frac{\\partial L}{\\partial y'} = 0", "O(n)", true,
                false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 变分法 -------------------- */
    {
        ...
        if (register_optimization_preset(
                PRESET_OPT_CALCULUS_OF_VARIATIONS, "变分法：求解泛函极值问题的数学框架，寻找使泛函取极值的函数", inputs,
                3, PRESET_TYPE_FUNCTION, "J[y] = \\int_a^b L(x, y(x), y'(x)) \\, dx \\to \\min", "O(n)", true, false)) {
            success_count++;
        }
    }
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_LIST, PRESET_TYPE_LIST};
        if (register_optimization_preset(PRESET_OPT_DUAL_PROBLEM, "对偶问题：构造原优化问题的Lagrange对偶问题", inputs,
                                         3, PRESET_TYPE_FUNCTION,
                                         "g(\\lambda, \\mu) = \\inf_x \\mathcal{L}(x, \\lambda, \\mu) = \\inf_x "
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_LIST, PRESET_TYPE_LIST};
        if (register_optimization_preset(PRESET_OPT_STRONG_DUALITY_TEST,
                                         "强对偶性检验：验证原问题最优值与对偶问题最优值是否相等（对偶间隙为零）",
                                         inputs, 3, PRESET_TYPE_BOOLEAN,
                                         "p^* = d^* \\quad \\text{其中} \\quad p^* = \\inf_x f_0(x), \\quad d^* = "
    ...
    /* -------------------- 弱对偶性检验 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_LIST, PRESET_TYPE_LIST};
        if (register_optimization_preset(
                PRESET_OPT_WEAK_DUALITY_TEST, "弱对偶性检验：验证对偶间隙是否非负（d* <= p* 恒成立）", inputs, 3,
                PRESET_TYPE_BOOLEAN, "d^* \\le p^* \\quad \\text{（对偶间隙} \\quad p^* - d^* \\ge 0 \\text{）}",
                "O(n^3)", true, false)) {
            success_count++;
        }
    }
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_TUPLE, PRESET_TYPE_SCALAR};
        if (register_optimization_preset(PRESET_OPT_SIMULATED_ANNEALING,
                                         "模拟退火：受物理退火过程启发的概率性全局优化算法，通过接受劣解来跳出局部最优",
                                         inputs, 3, PRESET_TYPE_TUPLE,
                                         "P(\\text{接受}) = \\begin{cases} 1 & \\Delta E < 0 \\\\ \\exp(-\\Delta E / "
    ...
    /* -------------------- 遗传算法 -------------------- */
    {
        ...
        if (register_optimization_preset(
                ...
                "遗传算法：基于自然选择和遗传机制的全局优化元启发式算法，通过选择、交叉、变异操作进化种群", inputs, 3,
                PRESET_TYPE_TUPLE,
