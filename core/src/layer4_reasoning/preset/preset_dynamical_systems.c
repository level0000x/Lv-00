 ...
 * @return false 注册失败
 */
static bool register_ds_preset(const char *name, const char *description, const PresetType *input_types,
...
bool preset_dynamical_systems_register(void) {
    ...
    /* Lyapunov 直接法稳定性判定 */
    {
        ...
        if (register_ds_preset(PRESET_DS_LYAPUNOV_DIRECT,
                               "Lyapunov 直接法：若存在正定函数 V 使 dV/dt ≤ 0，则平衡点稳定", inputs, 3,
                               PRESET_TYPE_BOOLEAN,
    ...
    /* 渐近稳定性判定（线性化） */
    {
        ...
        if (register_ds_preset(PRESET_DS_ASYMPTOTIC_LINEAR, "线性化渐近稳定性：若 Jacobian 特征值实部全负，则渐近稳定",
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR};
        if (register_ds_preset(PRESET_DS_EXPONENTIAL_STABILITY, "指数稳定性：存在 α,β > 0 使 ||x(t)|| ≤ β||x₀||e^(-αt)",
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR};
        if (register_ds_preset(PRESET_DS_CENTER_MANIFOLD, "中心流形约化：将高维系统降维到中心特征空间", inputs, 2,
    ...
    /* 鞍点分岔判定 */
    {
        ...
        if (register_ds_preset(PRESET_DS_SADDLE_NODE, "鞍点分岔（Saddle-Node）：平衡点随参数消失/产生", inputs, 3,
                               PRESET_TYPE_BOOLEAN,
                               "\\dot{x} = \\mu - x^2, \\quad "
                               "\\mu < 0: \\text{无平衡点}, \\quad "
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        if (register_ds_preset(PRESET_DS_TRANSCRITICAL, "跨临界分岔（Transcritical）：两个平衡点交换稳定性", inputs, 3,
                               PRESET_TYPE_BOOLEAN,
                               "\\dot{x} = \\mu x - x^2, \\quad "
    ...
    /* Pitchfork 分岔判定 */
    {
        ...
        if (register_ds_preset(PRESET_DS_PITCHFORK, "Pitchfork 分岔：对称系统中平衡点的对称性破缺", inputs, 4,
                               PRESET_TYPE_BOOLEAN,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        if (register_ds_preset(PRESET_DS_HOPF, "Hopf 分岔：平衡点失稳产生极限环", inputs, 3, PRESET_TYPE_BOOLEAN,
    ...
    /* 分岔图计算 */
    {
        ...
        if (register_ds_preset(PRESET_DS_BIFURCATION_DIAGRAM, "分岔图计算：绘制平衡点/周期解随参数变化的曲线", inputs,
                               4, PRESET_TYPE_FUNCTION,
                               "(x^*, \\mu): f(x^*, \\mu) = 0, \\quad "
                               "\\text{稳定: 实线}, \\quad "
                               "\\text{不稳定: 虚线}",
                               "O(n²)", true, false)) {
    ...
    /* Poincaré 映射构造 */
    {
        ...
        if (register_ds_preset(PRESET_DS_POINCARE_MAP, "Poincaré 映射：周期轨道的截面映射", inputs, 3,
                               PRESET_TYPE_FUNCTION,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR};
        if (register_ds_preset(PRESET_DS_LIMIT_CYCLE_STABILITY, "极限环稳定性：通过 Poincaré 映射的 Jacobian 判定",
                               inputs, 2, PRESET_TYPE_BOOLEAN,
    ...
    /* 周期解求解（谐波平衡） */
    {
        ...
        if (register_ds_preset(PRESET_DS_HARMONIC_BALANCE, "谐波平衡法：设 x(t) = Σaₙcos(nωt) + bₙsin(nωt) 求周期解",
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR};
        if (register_ds_preset(PRESET_DS_FLOQUET_MULTIPLIERS, "Floquet 乘子：周期系数线性系统的特征指数", inputs, 2,
                               PRESET_TYPE_LIST,
    ...
    /* Lyapunov 指数计算 */
    {
        ...
        if (register_ds_preset(PRESET_DS_LYAPUNOV_EXPONENTS, "Lyapunov 指数：刻画相邻轨道的指数分离率", inputs, 3,
                               PRESET_TYPE_LIST,
    ...
    /* 混沌判定（Devaney 定义） */
    {
        ...
        if (register_ds_preset(
                PRESET_DS_CHAOS_DEVANEY, "Devaney 混沌定义：初值敏感、拓扑传递、周期点稠密", inputs, 2,
                PRESET_TYPE_BOOLEAN,
    ...
    /* Lorenz 吸引子 */
    {
        ...
        if (register_ds_preset(PRESET_DS_LORENZ_ATTRACTOR, "Lorenz 吸引子：ẋ = σ(y-x), ẏ = x(ρ-z)-y, ż = xy-βz", inputs,
    ...
    /* Henon 映射 */
    {
        ...
        if (register_ds_preset(PRESET_DS_HENON_MAP, "Henon 映射：x_{n+1} = 1 - ax_n² + y_n, y_{n+1} = bx_n", inputs, 2,
                               PRESET_TYPE_FUNCTION,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_INTEGER};
        if (register_ds_preset(PRESET_DS_STABLE_MANIFOLD, "稳定流形 W^s(x*)：所有渐近趋近平衡点的轨道集合", inputs, 3,
                               PRESET_TYPE_SURFACE,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_INTEGER};
        if (register_ds_preset(PRESET_DS_UNSTABLE_MANIFOLD, "不稳定流形 W^u(x*)：所有远离平衡点的轨道集合", inputs, 3,
                               PRESET_TYPE_SURFACE,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        if (register_ds_preset(PRESET_DS_INERTIAL_MANIFOLD, "惯性流形：有限维不变流形，吸引所有轨道", inputs, 3,
                               PRESET_TYPE_SURFACE,
    ...
    /* 平均法 */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        if (register_ds_preset(
                PRESET_DS_METHOD_AVG, "平均法：对快速振荡系统进行时间平均", inputs, 3, PRESET_TYPE_FUNCTION,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_INTEGER};
        if (register_ds_preset(PRESET_DS_MULTIPLE_SCALES, "多尺度方法：引入多个时间尺度 T₀, T₁, T₂, ...", inputs, 3,
                               PRESET_TYPE_FUNCTION,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        if (register_ds_preset(PRESET_DS_SINGULAR_PERTURBATION, "奇异摄动法：处理边界层问题", inputs, 3,
                               PRESET_TYPE_FUNCTION,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR};
        if (register_ds_preset(PRESET_DS_WKBJ, "WKBJ 近似：大参数渐近展开", inputs, 2, PRESET_TYPE_FUNCTION,
