...
/** 分析学模块预设函数块总数 */
#define ANALYSIS_PRESET_COUNT 48
 ...
 * @brief 分析学模块预设注册宏
 *
 * 封装单个分析学预设的注册调用，自动累加 success_count，
 * 并在失败时记录错误日志。
 */
#define REGISTER_ANALYSIS(name, desc, inputs, input_count, output, math_def, complexity, constructive, reversible) \
    do {                                                                                                        \
        if (register_analysis_preset((name), (desc), (inputs), (input_count), (output), (math_def), (complexity), \
                                     (constructive), (reversible))) {                                            \
            success_count++;                                                                                    \
        } else {                                                                                                \
            PRESET_ERROR_LOG("分析预设注册失败: %s", (name));                                                   \
        }                                                                                                       \
    } while (0)
...
bool preset_analysis_register(void) {
    ...
    /* -------------------- 数列极限 -------------------- */
    {
        ...
        REGISTER_ANALYSIS(PRESET_SEQUENCE_LIMIT, "计算数列的极限 lim(n→∞) a_n", inputs, 1, PRESET_TYPE_LIMIT,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR};
        REGISTER_ANALYSIS(PRESET_LEFT_LIMIT, "计算函数在某点的左极限 lim(x→a⁻) f(x)", inputs, 2,
                                     PRESET_TYPE_LIMIT, "\\lim_{x \\to a^-} f(x) = L", "O(∞)", true, false);
    }
    /* -------------------- 右极限 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR};
        REGISTER_ANALYSIS(PRESET_RIGHT_LIMIT, "计算函数在某点的右极限 lim(x→a⁺) f(x)", inputs, 2,
                                     PRESET_TYPE_LIMIT, "\\lim_{x \\to a^+} f(x) = L", "O(∞)", true, false);
    }
    /* -------------------- 上极限 -------------------- */
    {
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_SEQUENCE};
        REGISTER_ANALYSIS(PRESET_LIMIT_EXISTS_TEST, "判定数列极限是否存在", inputs, 1, PRESET_TYPE_BOOLEAN,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR};
        REGISTER_ANALYSIS(PRESET_CONTINUITY_TEST, "判定函数在某点是否连续", inputs, 2, PRESET_TYPE_BOOLEAN,
    ...
    /* -------------------- 一致连续判定 -------------------- */
    {
        ...
        REGISTER_ANALYSIS(PRESET_UNIFORM_CONTINUITY_TEST, "判定函数在区间上是否一致连续", inputs, 2,
                                     PRESET_TYPE_BOOLEAN,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR};
        REGISTER_ANALYSIS(PRESET_DISCONTINUITY_CLASSIFY, "对函数的间断点进行分类（可去、跳跃、无穷、振荡）",
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_REGION};
        REGISTER_ANALYSIS(PRESET_LIPSCHITZ_TEST, "判定函数是否 Lipschitz 连续", inputs, 2,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR};
        REGISTER_ANALYSIS(PRESET_DERIVATIVE, "计算函数在某点的导数 f'(a)", inputs, 2, PRESET_TYPE_DERIVATIVE,
                                     "f'(a) = \\lim_{h \\to 0} \\frac{f(a+h) - f(a)}{h}", "O(∞)", true, false);
    }
    /* -------------------- 高阶导数 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_INTEGER};
        REGISTER_ANALYSIS(PRESET_HIGHER_DERIVATIVE, "计算函数的 n 阶导数 f^(n)(x)", inputs, 2,
    ...
    /* -------------------- 梯度 -------------------- */
    {
        ...
        REGISTER_ANALYSIS(PRESET_GRADIENT, "计算标量场的梯度 ∇f", inputs, 1, PRESET_TYPE_VECTOR,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION};
        REGISTER_ANALYSIS(PRESET_LAPLACIAN, "计算标量场的拉普拉斯算子 Δf", inputs, 1, PRESET_TYPE_SCALAR,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION};
        REGISTER_ANALYSIS(PRESET_INDEFINITE_INTEGRAL, "计算函数的不定积分 ∫f(x)dx", inputs, 1,
    ...
    /* -------------------- 定积分 -------------------- */
    {
        ...
        REGISTER_ANALYSIS(PRESET_DEFINITE_INTEGRAL, "计算定积分 ∫ₐᵇ f(x)dx", inputs, 3, PRESET_TYPE_SCALAR,
    ...
    /* -------------------- 重积分 -------------------- */
    {
        ...
        REGISTER_ANALYSIS(PRESET_MULTIPLE_INTEGRAL, "计算重积分 ∫∫...∫_D f dV", inputs, 3,
    ...
    /* -------------------- 曲线积分 -------------------- */
    {
        ...
        REGISTER_ANALYSIS(PRESET_LINE_INTEGRAL, "计算曲线积分 ∫_C f ds 或 ∫_C F·dr", inputs, 2,
                                     PRESET_TYPE_SCALAR,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_REGION};
        REGISTER_ANALYSIS(PRESET_INTEGRABILITY_TEST, "判定函数在区间上是否黎曼可积", inputs, 2,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_SEQUENCE};
        REGISTER_ANALYSIS(PRESET_SERIES_CONVERGENCE_TEST, "判定数项级数是否收敛（比较、比值、根值判别法）",
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_SEQUENCE};
        REGISTER_ANALYSIS(PRESET_POWER_SERIES_RADIUS, "计算幂级数的收敛半径 R", inputs, 1,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_SEQUENCE};
        REGISTER_ANALYSIS(PRESET_SERIES_SUM, "计算收敛级数的和", inputs, 1, PRESET_TYPE_SCALAR,
                                     "\\sum_{n=1}^{\\infty} a_n = \\lim_{N \\to \\infty} S_N", "O(∞)", true, false);
    }
    /* -------------------- 傅里叶级数 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR};
        REGISTER_ANALYSIS(PRESET_FOURIER_SERIES, "计算函数的傅里叶级数展开", inputs, 2, PRESET_TYPE_SEQUENCE,
    ...
    /* -------------------- L^p 范数 -------------------- */
    {
        ...
        REGISTER_ANALYSIS(PRESET_LP_NORM, "计算函数的 L^p 范数", inputs, 3, PRESET_TYPE_SCALAR,
                                     "\\|f\\|_p = \\left(\\int_D |f|^p \\, dx\\right)^{1/p}", "O(∞)", true, false);
    }
    /* -------------------- 一致范数 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_REGION};
        REGISTER_ANALYSIS(PRESET_SUP_NORM, "计算函数的一致范数（上确界范数）", inputs, 2, PRESET_TYPE_SCALAR,
                                     "\\|f\\|_{\\infty} = \\sup_{x \\in D} |f(x)|", "O(∞)", true, false);
    }
    /* -------------------- 完备化 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_SPACE};
        REGISTER_ANALYSIS(PRESET_COMPLETION, "构造度量空间的完备化", inputs, 1, PRESET_TYPE_SPACE,
    ...
    /* -------------------- 度量空间判定 -------------------- */
    {
        ...
        REGISTER_ANALYSIS(PRESET_METRIC_SPACE_TEST, "判定二元函数是否构成度量", inputs, 2,
                                     PRESET_TYPE_BOOLEAN,
    ...
    /* -------------------- 柯西序列判定 -------------------- */
    {
        ...
        REGISTER_ANALYSIS(PRESET_CAUCHY_SEQUENCE_TEST, "判定序列是否是柯西序列", inputs, 2,
                                     PRESET_TYPE_BOOLEAN,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_SPACE};
        REGISTER_ANALYSIS(PRESET_COMPLETE_SPACE_TEST, "判定度量空间是否完备", inputs, 1, PRESET_TYPE_BOOLEAN,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR};
        REGISTER_ANALYSIS(PRESET_CONTRACTION_MAPPING, "判定映射是否是压缩映射", inputs, 2,
                                     PRESET_TYPE_BOOLEAN,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION};
        REGISTER_ANALYSIS(PRESET_FIXED_POINT_THEOREM, "应用 Banach 不动点定理求不动点", inputs, 1,
    ...
    /* -------------------- Γ 函数 -------------------- */
    {
        ...
        REGISTER_ANALYSIS(PRESET_GAMMA_FUNCTION, "计算 Γ 函数值", inputs, 1, PRESET_TYPE_SCALAR,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_SCALAR};
        REGISTER_ANALYSIS(PRESET_ZETA_FUNCTION, "计算黎曼 ζ 函数值", inputs, 1, PRESET_TYPE_SCALAR,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_SCALAR};
        REGISTER_ANALYSIS(PRESET_ERROR_FUNCTION, "计算误差函数 erf(x)", inputs, 1, PRESET_TYPE_SCALAR,
    ...
    /* -------------------- 方向导数 -------------------- */
    {
        ...
        REGISTER_ANALYSIS(PRESET_DIRECTIONAL_DERIVATIVE, "计算函数在某点沿某方向的方向导数", inputs, 3,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        REGISTER_ANALYSIS(PRESET_MEAN_VALUE_THEOREM, "应用积分中值定理求中值点", inputs, 3,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION};
        REGISTER_ANALYSIS(PRESET_INFINITE_LIMIT, "计算函数在无穷远处的极限", inputs, 1, PRESET_TYPE_LIMIT,
