...
bool preset_field_theory_register(void) {
     ...
     */
    LV_PRESET_REGISTER(success_count, "field_add", "域加法：计算域F中两个元素的和 a + b", 2, PRESET_TYPE_FIELD, "(a, b) \\mapsto a + b \\in F", "O(1)", true, true, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     */
    LV_PRESET_REGISTER(success_count, PRESET_FIELD_MULTIPLY, "域乘法：计算域F中两个元素的积 a * b", 2, PRESET_TYPE_FIELD, "(a, b) \\mapsto a \\cdot b \\in F", "O(1)", true, true, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     */
    LV_PRESET_REGISTER(success_count, "field_inverse", "乘法逆元：计算域中非零元素的乘法逆元 a^{-1}，满足 a * a^{-1} = 1", 1, PRESET_TYPE_FIELD, "a \\mapsto a^{-1}, \\quad a \\cdot a^{-1} = 1_F, \\; a \\neq 0", "O(1)", true, true, PRESET_TYPE_FIELD);
     ...
     */
    LV_PRESET_REGISTER(success_count, PRESET_FIELD_DIVIDE, "域除法：计算域F中两个元素的商 a / b（b != 0）", 2, PRESET_TYPE_FIELD, "(a, b) \\mapsto a \\cdot b^{-1} \\in F, \\quad b \\neq 0", "O(1)", true, true, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_FIELD_CHARACTERISTIC, "域特征：计算域F的特征 char(F)，即满足 n*1=0 的最小正整数n（若不存在则为0）", 1, PRESET_TYPE_INTEGER, "\\text{char}(F) = \\min\\{n > 0 : n \\cdot 1_F = 0\\}, \\quad \\text{char}(F) \\in \\{0, p \\; "
                       "| \\; p \\text{ 是素数}\\}", "O(1)", false, false, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_FIELD_SUBFIELD_CHECK, "子域判定：判定域F的子集K是否构成子域（含0,1，四则运算封闭）", 2, PRESET_TYPE_BOOLEAN, "K \\le F \\Leftrightarrow 0, 1 \\in K \\land (\\forall a,b \\in K: a \\pm b, ab \\in K) \\land "
                       "(\\forall a \\neq 0 \\in K: a^{-1} \\in K)", "O(n^2)", false, false, PRESET_TYPE_FIELD, PRESET_TYPE_SET);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_FIELD_EXTENSION_CHECK, "域扩张判定：判定E是否为F的域扩张 E/F（F是E的子域）", 2, PRESET_TYPE_BOOLEAN, "E/F \\text{ 是域扩张} \\Leftrightarrow F \\le E", "O(n^2)", false, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_FIELD_PRIME_SUBFIELD, "素子域：获取域F的素子域（char=p时为F_p，char=0时为Q）", 1, PRESET_TYPE_FIELD, "\\text{PrimeSubfield}(F) = \\begin{cases} \\mathbb{F}_p & \\text{char}(F) = p \\neq 0 \\\\ "
                       "\\mathbb{Q} & \\text{char}(F) = 0 \\end{cases}", "O(1)", true, false, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_EXTENSION_DEGREE, "扩张次数：计算域扩张 E/F 的次数 [E:F]（E作为F上向量空间的维数）", 2, PRESET_TYPE_INTEGER, "[E : F] = \\dim_F(E)", "O(n)", false, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_SIMPLE_EXTENSION, "简单扩张：由域F和元素alpha构造简单扩张 F(alpha)", 2, PRESET_TYPE_FIELD, "F(\\alpha) = F[\\alpha] \\text{（当 } \\alpha \\text{ 是代数元时）}", "O(n)", true, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, "algebraic_extension", "代数扩张判定：判定域扩张 E/F 是否为代数扩张（E中每个元素都是F上的代数元）", 2, PRESET_TYPE_BOOLEAN, "E/F \\text{ 是代数扩张} \\Leftrightarrow \\forall \\alpha \\in E, \\; \\exists f \\in F[x]: "
                       "f(\\alpha) = 0", "O(n^2)", false, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_TRANSCENDENTAL_EXTENSION, "超越扩张判定：判定域扩张 E/F 是否为超越扩张（E中存在F上的超越元）", 2, PRESET_TYPE_BOOLEAN, "E/F \\text{ 是超越扩张} \\Leftrightarrow \\exists \\alpha \\in E, \\; \\forall f \\in F[x] "
                       "\\setminus \\{0\\}: f(\\alpha) \\neq 0", "O(n^2)", false, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_FINITE_EXTENSION, "有限扩张判定：判定域扩张 E/F 是否为有限扩张（[E:F] < infinity）", 2, PRESET_TYPE_BOOLEAN, "E/F \\text{ 是有限扩张} \\Leftrightarrow [E : F] < \\infty", "O(n)", false, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_ALGEBRAIC_ELEMENT_CHECK, "代数元判定：判定域扩张中的元素alpha是否为F上的代数元", 2, PRESET_TYPE_BOOLEAN, "\\alpha \\text{ 是代数元} \\Leftrightarrow \\exists f \\in F[x] \\setminus \\{0\\}: f(\\alpha) = 0", "O(n^2)", false, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_MINIMAL_POLYNOMIAL, "极小多项式：计算代数元alpha在F上的极小多项式（首一最低次零化多项式）", 2, PRESET_TYPE_POLYNOMIAL, "\\text{minpoly}_F(\\alpha) = \\text{F[x]中首一的最低次多项式 } f, \\; f(\\alpha) = 0", "O(n^2)", true, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_FIELD_TOWER, "域塔定理：验证域塔 F subset K subset E 的次数关系 [E:F] = [E:K]*[K:F]", 3, PRESET_TYPE_INTEGER, "[E : F] = [E : K] \\cdot [K : F]", "O(n)", true, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_PRIMITIVE_ELEMENT, "本原元定理：对有限可分扩张 E/F 构造本原元 theta 使得 E = F(theta)", 2, PRESET_TYPE_FIELD, "E = F(\\theta), \\quad \\text{当 } E/F \\text{ 是有限可分扩张时}", "O(n^2)", true, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_NORMAL_EXTENSION, "正规扩张判定：判定域扩张 E/F 是否为正规扩张（某个多项式族的分裂域）", 2, PRESET_TYPE_BOOLEAN, "E/F \\text{ 是正规扩张} \\Leftrightarrow E \\text{ 是 } F[x] \\text{ 中某个多项式集合的分裂域}", "O(n^3)", false, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_GALOIS_GROUP, "伽罗瓦群：计算域扩张 E/F 的伽罗瓦群 Gal(E/F)（所有F-自同构组成的群）", 2, PRESET_TYPE_GROUP, "\\text{Gal}(E/F) = \\{\\sigma : E \\to E \\; | \\; \\sigma|_F = \\text{id}_F\\}", "O(n!)", true, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_GALOIS_GROUP_ORDER, "伽罗瓦群阶：计算伽罗瓦群 Gal(E/F) 的阶 |Gal(E/F)|", 2, PRESET_TYPE_INTEGER, "|\\text{Gal}(E/F)| \\le [E : F], \\quad \\text{等号成立当且仅当 } E/F \\text{ 是伽罗瓦扩张}", "O(n!)", false, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_FIXED_FIELD, "不动域：计算伽罗瓦群G的不动域 E^G = {x in E | sigma(x)=x, 对所有sigma in G}", 2, PRESET_TYPE_FIELD, "E^G = \\{x \\in E : \\sigma(x) = x, \\; \\forall \\sigma \\in G\\}", "O(n^2)", true, false, PRESET_TYPE_FIELD, PRESET_TYPE_GROUP);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_GALOIS_CORRESPONDENCE, "伽罗瓦对应：建立伽罗瓦群的子群与中间域之间的双射对应关系", 2, PRESET_TYPE_BOOLEAN, "\\{\\text{Gal}(E/F) \\text{ 的子群}\\} \\xleftrightarrow{1-1} \\{F \\subseteq K \\subseteq E "
                       "\\text{ 的中间域}\\}", "O(n!)", false, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_GALOIS_CHECK, "伽罗瓦扩张判定：判定 E/F 是否为伽罗瓦扩张（正规且可分）", 2, PRESET_TYPE_BOOLEAN, "E/F \\text{ 是伽罗瓦扩张} \\Leftrightarrow E/F \\text{ 正规且可分} \\Leftrightarrow "
                       "|\\text{Gal}(E/F)| = [E:F]", "O(n^3)", false, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_SEPARABLE_EXTENSION, "可分扩张判定：判定域扩张 E/F 是否为可分扩张（每个元素的最小多项式无重根）", 2, PRESET_TYPE_BOOLEAN, "E/F \\text{ 是可分扩张} \\Leftrightarrow \\forall \\alpha \\in E, \\; "
                       "\\text{minpoly}_F(\\alpha) \\text{ 无重根}", "O(n^2)", false, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_SPLITTING_FIELD, "分裂域构造：构造多项式 f(x) 在域F上的分裂域（包含f(x)所有根的最小扩域）", 2, PRESET_TYPE_FIELD, "E = F(\\alpha_1, \\ldots, \\alpha_n), \\quad f(x) = (x - \\alpha_1) \\cdots (x - \\alpha_n) \\in E[x]", "O(n!)", true, false, PRESET_TYPE_FIELD, PRESET_TYPE_POLYNOMIAL);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_CYCLOTOMIC_FIELD, "分圆域：构造n次分圆域 Q(zeta_n)，即添加n次本原单位根的扩域", 1, PRESET_TYPE_FIELD, "\\mathbb{Q}(\\zeta_n) \\text{，其中 } \\zeta_n = e^{2\\pi i / n}, \\; \\zeta_n^n = 1", "O(n \\log n)", true, false, PRESET_TYPE_INTEGER);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_FINITE_FIELD_CONSTRUCT, "有限域构造：构造有限域 GF(p^n)，其中p为素数，n为正整数", 2, PRESET_TYPE_FIELD, "\\text{GF}(p^n) = \\mathbb{F}_p[x] / \\langle f(x) \\rangle, \\quad f \\text{ 是 } "
                       "\\mathbb{F}_p \\text{ 上 } n \\text{ 次不可约多项式}", "O(n \\log p)", true, false, PRESET_TYPE_INTEGER, PRESET_TYPE_INTEGER);
     ...
     */
    LV_PRESET_REGISTER(success_count, PRESET_FROBENIUS_AUTOMORPHISM, "Frobenius自同构：计算有限域上的Frobenius映射 x -> x^p", 2, PRESET_TYPE_FIELD, "\\varphi_p(x) = x^p, \\quad \\varphi_p \\in \\text{Gal}(\\mathbb{F}_{p^n}/\\mathbb{F}_p)", "O(\\log p)", true, true, PRESET_TYPE_FIELD, PRESET_TYPE_INTEGER);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_FIELD_EMBEDDING, "域嵌入：构造域扩张 E/F 的F-嵌入（保持F不动的域同态）", 2, PRESET_TYPE_LIST, "\\text{Hom}_F(E, L) = \\{\\sigma : E \\to L \\; | \\; \\sigma|_F = \\text{id}_F\\}", "O(n^2)", true, false, PRESET_TYPE_FIELD, PRESET_TYPE_FIELD);
     ...
     * @reversible false
     */
    LV_PRESET_REGISTER(success_count, PRESET_ALGEBRAIC_CLOSURE, "代数闭包：构造域F的代数闭包（F的代数扩张且是代数闭域）", 1, PRESET_TYPE_FIELD, "\\overline{F} \\text{ 满足：} \\overline{F}/F \\text{ 是代数扩张，且 } \\overline{F} \\text{ 是代数闭域}", "O(n!)", true, false, PRESET_TYPE_FIELD);
