 ...
 * @brief 注册单个统计学预设
 */
static bool register_stat_preset(const char *name, const char *description, const PresetType *input_types,
...
bool preset_statistics_register(void) {
    ...
    /* 算术平均值 */
    {
        ...
        if (register_stat_preset(PRESET_STAT_MEAN, "算术平均值：mu = (1/n) Σ x_i", inputs, 1, PRESET_TYPE_SCALAR,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_LIST};
        if (register_stat_preset(PRESET_STAT_MEDIAN, "中位数：排序后位于中间位置的值", inputs, 1, PRESET_TYPE_SCALAR,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_LIST};
        if (register_stat_preset(PRESET_STAT_MODE, "众数：出现频率最高的值 M_o = argmax f(x)", inputs, 1,
    ...
    /* 分位数 */
    {
        ...
        if (register_stat_preset(PRESET_STAT_QUANTILE, "分位数：计算第 p 百分位数 P_p", inputs, 2, PRESET_TYPE_SCALAR,
    ...
    /* 正态分布 */
    {
        ...
        if (register_stat_preset(PRESET_STAT_NORMAL_DIST, "正态分布：计算 N(mu, sigma^2) 的概率密度函数值", inputs, 3,
    ...
    /* t分布 */
    {
        ...
        if (register_stat_preset(PRESET_STAT_T_DIST, "t分布：计算 t(nu) 的概率密度函数值", inputs, 2,
                                 PRESET_TYPE_PROBABILITY,
                                 "f(t) = \\frac{\\Gamma((\\nu+1)/2)}{\\sqrt{\\nu\\pi}\\Gamma(\\nu/2)}"
                                 "\\left(1+\\frac{t^2}{\\nu}\\right)^{-(\\nu+1)/2}",
                                 "O(n)", true, false)) {
    ...
    /* F分布 */
    {
        ...
        if (register_stat_preset(PRESET_STAT_F_DIST, "F分布：计算 F(d1, d2) 的概率密度函数值", inputs, 3,
    ...
    /* Z检验 */
    {
        ...
        if (register_stat_preset(PRESET_STAT_Z_TEST, "Z检验：大样本均值检验，Z = (x_bar - mu0) / (sigma / sqrt(n))",
    ...
    /* t检验 */
    {
        ...
        if (register_stat_preset(
                PRESET_STAT_T_TEST, "t检验：小样本均值检验（sigma未知），t = (x_bar - mu0) / (S / sqrt(n))", inputs, 2,
    ...
    /* F检验 */
    {
        ...
        if (register_stat_preset(PRESET_STAT_F_TEST, "F检验：两总体方差齐性检验，F = S1^2 / S2^2", inputs, 2,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_LIST};
        if (register_stat_preset(PRESET_STAT_ANOVA, "ANOVA（单因素方差分析）：检验 k 组均值是否相等", inputs, 1,
                                 PRESET_TYPE_BOOLEAN,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_LIST, PRESET_TYPE_LIST};
        if (register_stat_preset(PRESET_STAT_WILCOXON, "Wilcoxon秩和检验：非参数两独立样本位置检验", inputs, 2,
                                 PRESET_TYPE_BOOLEAN,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_LIST};
        if (register_stat_preset(PRESET_STAT_KRUSKAL_WALLIS, "Kruskal-Wallis检验：非参数多独立样本位置检验", inputs, 1,
                                 PRESET_TYPE_BOOLEAN,
    ...
    /* 多元线性回归 */
    {
        ...
        if (register_stat_preset(
                ...
                inputs, 2, PRESET_TYPE_LIST, "\\hat{\\beta} = (X^T X)^{-1} X^T y", "O(p^3 + np^2)", true, false)) {
            success_count++;
        }
    }
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX, PRESET_TYPE_LIST};
        if (register_stat_preset(PRESET_STAT_LOGISTIC_REGRESSION, "逻辑回归：P(Y=1|X) = 1/(1+e^{-Xβ})，极大似然估计",
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_LIST, PRESET_TYPE_LIST};
        if (register_stat_preset(PRESET_STAT_R_SQUARED, "决定系数 R^2：衡量回归模型的拟合优度", inputs, 2,
                                 PRESET_TYPE_SCALAR,
    ...
    /* 先验分布 */
    {
        ...
        if (register_stat_preset(PRESET_STAT_PRIOR_DISTRIBUTION, "先验分布：设定参数的先验分布 pi(theta)", inputs, 2,
    ...
    /* 后验分布 */
    {
        ...
        if (register_stat_preset(PRESET_STAT_POSTERIOR_DISTRIBUTION,
                                 "后验分布：由贝叶斯定理 pi(theta|X) ∝ L(X|theta)·pi(theta)", inputs, 3,
    ...
    /* 贝叶斯因子 */
    {
        ...
        if (register_stat_preset(PRESET_STAT_BAYES_FACTOR, "贝叶斯因子：B01 = P(X|H0)/P(X|H1)，衡量两个假设的证据强度",
                                 inputs, 3, PRESET_TYPE_SCALAR, "B_{01} = \\frac{P(X|H_0)}{P(X|H_1)}", "O(\\infty)",
                                 true, false)) {
            success_count++;
        }
    }
    ...
    /* Bootstrap重抽样 */
    {
        ...
        if (register_stat_preset(PRESET_STAT_BOOTSTRAP, "Bootstrap重抽样：B次有放回重抽样，估计统计量的抽样分布",
                                 inputs, 3, PRESET_TYPE_LIST, "\\hat{\\theta}_b^* = T(X_b^*),\\quad b = 1,\\ldots,B",
                                 "O(B n)", true, false)) {
            success_count++;
        }
    }
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_LIST};
        if (register_stat_preset(PRESET_STAT_VARIANCE, "方差：sigma^2 = (1/n) Σ (x_i - x_bar)^2", inputs, 1,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_LIST, PRESET_TYPE_SCALAR};
        if (register_stat_preset(PRESET_STAT_CONFIDENCE_INTERVAL, "置信区间：构建参数在置信水平 1-alpha 下的置信区间",
                                 inputs, 2, PRESET_TYPE_TUPLE, "P(L \\leq \\theta \\leq U) = 1 - \\alpha", "O(n)", true,
                                 false)) {
            success_count++;
        }
    }
