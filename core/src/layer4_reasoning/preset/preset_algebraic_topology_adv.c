 ...
 * @return false 注册失败
 */
static bool register_at_adv_preset(
    ...
    bool is_constructive, bool is_reversible)
{
    return preset_blocks_register_simple(
        name, description, PRESET_CATEGORY_TOPOLOGY,
...
int preset_algebraic_topology_adv_register(void)
{
    int success_count = 0;
     ...
     */
    {
        ...
        REGISTER_AT_ADV(PRESET_AT_FUNDAMENTAL_GROUP,
            "基本群 π₁(X, x₀)：计算拓扑空间X在基点x₀处的基本群，由闭路径同伦类构成，反映空间的一维洞结构",
            inputs, 2, PRESET_TYPE_GROUP,
            "\\pi_1(X, x_0) = \\{ [\\gamma] : \\gamma: [0,1] \\to X, \\gamma(0) = \\gamma(1) = x_0 \\} / \\sim",
            "O(∞)", false, false);
    }
     ...
     */
    {
        ...
        REGISTER_AT_ADV(PRESET_AT_COVERING_SPACE,
            "覆盖空间：验证覆盖映射 p: Ỹ → Y，局部同胚且满足离散纤维条件",
     ...
     */
    {
        ...
        REGISTER_AT_ADV(PRESET_AT_UNIVERSAL_COVERING,
            "万有覆盖空间：构造拓扑空间X的万有覆盖空间，单连通且覆盖变换群同构于基本群",
     ...
     */
    {
        ...
        REGISTER_AT_ADV(PRESET_AT_HOMOLOGY_GROUP,
            "同调群 Hₙ(X)：计算拓扑空间X的第n维奇异同调群，反映n维洞结构",
            inputs, 2, PRESET_TYPE_GROUP,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_TOPOLOGY, PRESET_TYPE_INTEGER};
        REGISTER_AT_ADV(PRESET_AT_COHOMOLOGY_GROUP,
            "上同调群 Hⁿ(X)：计算拓扑空间X的上同调群，对偶于同调群，具有杯积环结构",
            inputs, 2, PRESET_TYPE_GROUP,
     ...
     */
    {
        ...
        REGISTER_AT_ADV(PRESET_AT_MAYER_VIETORIS_SEQUENCE,
            "Mayer-Vietoris序列：利用空间分解 X = A ∪ B 导出同调群的长正合序列",
     ...
     */
    {
        ...
        REGISTER_AT_ADV(PRESET_AT_EXACT_SEQUENCE,
            "正合序列：验证群同态序列是否满足 im(f_i) = ker(f_{i+1}) 的正合条件",
            inputs, 1, PRESET_TYPE_BOOLEAN,
            "\\operatorname{im}(f_i) = \\ker(f_{i+1}), \\quad \\forall i",
            "O(∞)", false, false);
    }
     ...
     */
    {
        ...
        REGISTER_AT_ADV(PRESET_AT_EULER_CHARACTERISTIC,
            "Euler特征数 χ(X)：计算拓扑空间X的Euler特征数，各维Betti数的交错和",
            inputs, 1, PRESET_TYPE_INTEGER,
            "\\chi(X) = \\sum_{n=0}^{\\infty} (-1)^n \\operatorname{rk}(H_n(X))",
            "O(∞)", true, false);
    }
