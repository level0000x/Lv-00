 ...
 * @return false 注册失败
 */
static bool register_numerical_preset(
    const char *name,
...
int preset_numerical_register(void)
{
    ...
    /* -------------------- 二分法 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "二分法求方程 f(x)=0 的根，要求 f 在区间 [a,b] 上连续且变号",
    ...
    /* -------------------- 不动点迭代 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "不动点迭代法：将 f(x)=0 转化为 x=g(x) 进行迭代求解",
    ...
    /* -------------------- Simpson公式 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "复化Simpson公式求定积分 ∫[a,b] f(x)dx",
                inputs, 4, PRESET_TYPE_SCALAR,
                "S_n = \\frac{h}{3}\\left[f(a) + 4\\sum_{\\text{odd}} f(x_i) + 2\\sum_{\\text{even}} f(x_i) + f(b)\\right]",
                "O(n)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 高斯求积 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "高斯求积公式：利用正交多项式的零点作为求积节点",
                inputs, 4, PRESET_TYPE_SCALAR,
                "\\int_a^b f(x)\\,dx \\approx \\sum_{i=1}^{n} A_i \\, f(x_i), \\quad x_i \\text{ 为 Legendre 多项式零点}",
                "O(n)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- Romberg积分 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "Romberg积分：基于Richardson外推的逐次加速积分方法",
                inputs, 4, PRESET_TYPE_SCALAR,
    ...
    /* -------------------- 前向差分 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "前向差分公式近似计算函数在 x 处的一阶导数",
                inputs, 3, PRESET_TYPE_SCALAR,
                "f'(x) \\approx \\frac{f(x+h) - f(x)}{h}, \\quad \\text{误差 } O(h)",
                "O(1)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- Richardson外推 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "Richardson外推法：通过逐步减半步长并外推消除低阶误差项",
                inputs, 3, PRESET_TYPE_SCALAR,
                "D_{k,m} = \\frac{4^m D_{k,m-1} - D_{k-1,m-1}}{4^m - 1}",
                "O(n^2)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- Lagrange插值 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "Lagrange插值：给定 n+1 个数据点构造 Lagrange 插值多项式并求值",
                inputs, 3, PRESET_TYPE_SCALAR,
    ...
    /* -------------------- Newton插值 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "Newton差商插值：利用差商表构造 Newton 插值多项式并求值",
                inputs, 3, PRESET_TYPE_SCALAR,
                "N_n(x) = f[x_0] + \\sum_{i=1}^{n} f[x_0, \\ldots, x_i] \\prod_{j=0}^{i-1} (x - x_j)",
                "O(n^2)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 多项式拟合 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "多项式拟合：用最小二乘法拟合 n 次多项式",
                inputs, 3, PRESET_TYPE_LIST,
                "\\min_{\\{a_k\\}} \\sum_{j=1}^{m} \\left| y_j - \\sum_{k=0}^{n} a_k x_j^k \\right|^2",
                "O(mn^2)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- Euler方法 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "显式Euler方法求解初值问题 y'=f(x,y), y(x0)=y0",
                inputs, 5, PRESET_TYPE_LIST,
                "y_{n+1} = y_n + h \\, f(x_n, y_n), \\quad h = \\frac{x_{end} - x_0}{N}",
                "O(N)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 四阶Runge-Kutta方法 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "经典四阶Runge-Kutta方法求解初值问题 y'=f(x,y), y(x0)=y0",
                inputs, 5, PRESET_TYPE_LIST,
    ...
    /* -------------------- Adams多步方法 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "Adams-Bashforth四步显式方法求解初值问题 y'=f(x,y)",
                inputs, 5, PRESET_TYPE_LIST,
    ...
    /* -------------------- 高斯消元法 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "Ax = b \\Rightarrow Ux = c \\Rightarrow x = U^{-1}c",
                "O(n^3)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- LU分解 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "A = LU, \\quad L \\text{ 为单位下三角矩阵}, \\quad U \\text{ 为上三角矩阵}",
                "O(n^3)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 迭代求解法 -------------------- */
    {
        ...
        if (register_numerical_preset(
                ...
                "迭代法（Jacobi/Gauss-Seidel）求解线性方程组 Ax=b",
