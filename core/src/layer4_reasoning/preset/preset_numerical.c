 ...
 * @return false 注册失败
 */
static bool register_numerical_preset(const char *name, const char *description, const PresetType *input_types,
                                      int input_count, PresetType output_type, const char *math_def,
                                      const char *complexity, bool is_constructive, bool is_reversible) {
    return preset_blocks_register_simple(name, description, PRESET_CATEGORY_NUMERICAL, input_types, input_count,
...
bool preset_numerical_register(void) {
    ...
    /* -------------------- 二分法 -------------------- */
    {
        ...
        if (register_numerical_preset(
                PRESET_NUMERICAL_BISECTION, "二分法求方程 f(x)=0 的根，要求 f 在区间 [a,b] 上连续且变号", inputs, 3,
    ...
    /* -------------------- 牛顿法 -------------------- */
    {
        ...
        if (register_numerical_preset(PRESET_NUMERICAL_NEWTON, "牛顿法（切线法）求方程 f(x)=0 的根，需要函数及其导数",
                                      inputs, 3, PRESET_TYPE_SCALAR, "x_{n+1} = x_n - \\frac{f(x_n)}{f'(x_n)}",
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        if (register_numerical_preset(PRESET_NUMERICAL_SECANT, "割线法求方程 f(x)=0 的根，用差商代替导数", inputs, 3,
                                      PRESET_TYPE_SCALAR,
    ...
    /* -------------------- 不动点迭代 -------------------- */
    {
        ...
        if (register_numerical_preset(PRESET_NUMERICAL_FIXED_POINT,
                                      "不动点迭代法：将 f(x)=0 转化为 x=g(x) 进行迭代求解", inputs, 2,
    ...
    /* -------------------- 梯形公式 -------------------- */
    {
        ...
        if (register_numerical_preset(
                PRESET_NUMERICAL_TRAPEZOID, "复化梯形公式求定积分 ∫[a,b] f(x)dx", inputs, 4, PRESET_TYPE_SCALAR,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR, PRESET_TYPE_INTEGER};
        if (register_numerical_preset(PRESET_NUMERICAL_SIMPSON, "复化Simpson公式求定积分 ∫[a,b] f(x)dx", inputs, 4,
                                      PRESET_TYPE_SCALAR,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR, PRESET_TYPE_INTEGER};
        if (register_numerical_preset(PRESET_NUMERICAL_GAUSS_QUADRATURE,
                                      "高斯求积公式：利用正交多项式的零点作为求积节点", inputs, 4, PRESET_TYPE_SCALAR,
    ...
    /* -------------------- Romberg积分 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR, PRESET_TYPE_INTEGER};
        if (register_numerical_preset(
                PRESET_NUMERICAL_ROMBERG, "Romberg积分：基于Richardson外推的逐次加速积分方法", inputs, 4,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        if (register_numerical_preset(PRESET_NUMERICAL_FORWARD_DIFF, "前向差分公式近似计算函数在 x 处的一阶导数",
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        if (register_numerical_preset(PRESET_NUMERICAL_CENTRAL_DIFF, "中心差分公式近似计算函数在 x 处的一阶导数",
                                      inputs, 3, PRESET_TYPE_SCALAR,
    ...
    /* -------------------- Richardson外推 -------------------- */
    {
        ...
        if (register_numerical_preset(
                PRESET_NUMERICAL_RICHARDSON, "Richardson外推法：通过逐步减半步长并外推消除低阶误差项", inputs, 3,
    ...
    /* -------------------- Newton插值 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_LIST, PRESET_TYPE_LIST, PRESET_TYPE_SCALAR};
        if (register_numerical_preset(
                PRESET_NUMERICAL_NEWTON_INTERP, "Newton差商插值：利用差商表构造 Newton 插值多项式并求值", inputs, 3,
                PRESET_TYPE_SCALAR,
    ...
    /* -------------------- 样条插值 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_LIST, PRESET_TYPE_LIST, PRESET_TYPE_SCALAR};
        if (register_numerical_preset(
                PRESET_NUMERICAL_SPLINE_INTERP, "三次样条插值：构造分段三次多项式，保证节点处二阶连续", inputs, 3,
                PRESET_TYPE_SCALAR,
    ...
    /* -------------------- 最小二乘法 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_LIST, PRESET_TYPE_LIST, PRESET_TYPE_FUNCTION};
        if (register_numerical_preset(
                PRESET_NUMERICAL_LEAST_SQUARES, "最小二乘拟合：给定数据点和基函数族，求最小二乘逼近", inputs, 3,
                PRESET_TYPE_LIST,
    ...
    /* -------------------- 多项式拟合 -------------------- */
    {
        ...
        if (register_numerical_preset(
                PRESET_NUMERICAL_POLYNOMIAL_FIT, "多项式拟合：用最小二乘法拟合 n 次多项式", inputs, 3, PRESET_TYPE_LIST,
    ...
    /* -------------------- Euler方法 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR,
                               PRESET_TYPE_INTEGER};
        if (register_numerical_preset(
                PRESET_NUMERICAL_EULER, "显式Euler方法求解初值问题 y'=f(x,y), y(x0)=y0", inputs, 5, PRESET_TYPE_LIST,
    ...
    /* -------------------- 四阶Runge-Kutta方法 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR,
                               PRESET_TYPE_INTEGER};
        if (register_numerical_preset(PRESET_NUMERICAL_RK4, "经典四阶Runge-Kutta方法求解初值问题 y'=f(x,y), y(x0)=y0",
    ...
    /* -------------------- Adams多步方法 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR,
                               PRESET_TYPE_INTEGER};
        if (register_numerical_preset(PRESET_NUMERICAL_ADAMS, "Adams-Bashforth四步显式方法求解初值问题 y'=f(x,y)",
                                      inputs, 5, PRESET_TYPE_LIST,
    ...
    /* -------------------- 高斯消元法 -------------------- */
    {
        ...
        if (register_numerical_preset(PRESET_NUMERICAL_GAUSS_ELIMINATION, "高斯消元法（列主元）求解线性方程组 Ax=b",
    ...
    /* -------------------- LU分解 -------------------- */
    {
        ...
        if (register_numerical_preset(
                PRESET_NUMERICAL_LU_DECOMPOSITION, "LU分解：将矩阵 A 分解为下三角矩阵 L 和上三角矩阵 U 的乘积", inputs,
                1, PRESET_TYPE_TUPLE, "A = LU, \\quad L \\text{ 为单位下三角矩阵}, \\quad U \\text{ 为上三角矩阵}",
                "O(n^3)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 迭代求解法 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX, PRESET_TYPE_LIST, PRESET_TYPE_SCALAR, PRESET_TYPE_INTEGER};
        if (register_numerical_preset(PRESET_NUMERICAL_ITERATIVE_SOLVE,
                                      "迭代法（Jacobi/Gauss-Seidel）求解线性方程组 Ax=b", inputs, 4, PRESET_TYPE_LIST,
    ...
    /* -------------------- 特征值计算 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_numerical_preset(
                PRESET_NUMERICAL_EIGENVALUES, "QR算法计算矩阵的全部特征值", inputs, 1, PRESET_TYPE_LIST,
    ...
    /* -------------------- 奇异值分解 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_MATRIX};
        if (register_numerical_preset(
                PRESET_NUMERICAL_SVD, "奇异值分解：将矩阵分解为 A = UΣV^T", inputs, 1, PRESET_TYPE_TUPLE,
    ...
    /* 返回是否所有预设都注册成功 */
    /* lv00_log_info("数值分析预设注册完成，共 %d 个预设", success_count) */
