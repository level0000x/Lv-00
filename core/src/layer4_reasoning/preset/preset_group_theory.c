 ...
 * @return false 注册失败
 */
static bool register_group_theory_preset(
    const char *name,
...
bool preset_group_theory_register(void)
{
    ...
    /* -------------------- 群运算 -------------------- */
    {
        ...
        if (register_group_theory_preset(
                ...
                "群运算：计算群中两个元素的乘积 a * b",
                inputs, 3, PRESET_TYPE_GROUP_ELEMENT,
                "a \\cdot b \\in G",
                "O(1)", true, false)) {
            success_count++;
        }
    ...
    /* -------------------- 生成子群 -------------------- */
    {
        ...
        if (register_group_theory_preset(
                ...
                "由元素集合生成的子群 <S>",
                inputs, 2, PRESET_TYPE_SUBGROUP,
                "\\langle S \\rangle = \\{s_1^{e_1} \\cdots s_k^{e_k} : s_i \\in S\\}",
                "O(|G|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 正规子群判定 -------------------- */
    {
        ...
        if (register_group_theory_preset(
                ...
                "判定子群是否是正规子群",
                inputs, 2, PRESET_TYPE_BOOLEAN,
                "H \\triangleleft G \\Leftrightarrow gHg^{-1} = H, \\forall g \\in G",
                "O(|G|·|H|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 群同构判定 -------------------- */
    {
        ...
        if (register_group_theory_preset(
                ...
                "判定两个群是否同构",
                inputs, 2, PRESET_TYPE_BOOLEAN,
                "G \\cong H \\Leftrightarrow \\exists \\varphi: G \\to H \\text{ 双射同态}",
                "O(|G|!)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 内自同构群 -------------------- */
    {
        ...
        if (register_group_theory_preset(
                ...
                "计算群的内自同构群 Inn(G)",
                inputs, 1, PRESET_TYPE_GROUP,
                "\\text{Inn}(G) = \\{\\varphi_g : x \\mapsto gxg^{-1}\\}",
                "O(|G|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 阿贝尔群判定 -------------------- */
    {
        ...
        if (register_group_theory_preset(
                ...
                "判定群是否是阿贝尔群（交换群）",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "G \\text{ 阿贝尔} \\Leftrightarrow ab = ba, \\forall a,b \\in G",
                "O(|G|²)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 可解群判定 -------------------- */
    {
        ...
        if (register_group_theory_preset(
                ...
                "判定群是否是可解群",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "G \\text{ 可解} \\Leftrightarrow \\exists k: G^{(k)} = \\{e\\}",
                "O(|G|² k)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 幂零群判定 -------------------- */
    {
        ...
        if (register_group_theory_preset(
                ...
                "判定群是否是幂零群",
                inputs, 1, PRESET_TYPE_BOOLEAN,
                "G \\text{ 幂零} \\Leftrightarrow \\exists k: G_k = \\{e\\}",
                "O(|G|² k)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 上中心列 -------------------- */
    {
        ...
        if (register_group_theory_preset(
                ...
                "计算群的上中心列",
                inputs, 1, PRESET_TYPE_SEQUENCE,
                "Z_0(G) = \\{e\\}, Z_{i+1}(G)/Z_i(G) = Z(G/Z_i(G))",
                "O(|G|^2 k)", true, false)) {
            success_count++;
        }
    }
...
static __attribute__((unused)) char** get_group_theory_names(void)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (!result[i]) {
            for (int j = 0; j < i; j++) { void *tmp = result[j]; lv_free(&tmp); }
            { void *tmp = result; lv_free(&tmp); }
            return NULL;
        }
    }
    result[count] = NULL;
