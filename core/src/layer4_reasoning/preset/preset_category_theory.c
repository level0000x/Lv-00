 ...
 * @return false 注册失败
 */
static bool register_category_theory_preset(
    const char *name,
...
int preset_category_theory_register(void)
{
    ...
    /* -------------------- 态射复合 -------------------- */
    {
        ...
        if (register_category_theory_preset(
                ...
                "态射复合：计算两个可复合态射的复合 g o f，要求 cod(f) = dom(g)",
                inputs, 2, PRESET_TYPE_FUNCTION,
                "(g \\circ f)(x) = g(f(x)), \\quad \\text{要求 } \\mathrm{cod}(f) = \\mathrm{dom}(g)",
                "O(1)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 推出 -------------------- */
    {
        ...
        if (register_category_theory_preset(
                PRESET_CAT_PUSHOUT,
                "推出：构造余交换方框的推出 A +_C B，满足泛性质",
                inputs, 2, PRESET_TYPE_SET,
                "A \\amalg_C B \\text{ 满足泛性质}",
                "O(|A| + |B|)", true, false)) {
            success_count++;
        }
    }
    ...
    /* 返回是否所有预设都注册成功 */
    /* lv00_log_info("范畴论预设注册完成，共 %d 个预设", success_count) */
