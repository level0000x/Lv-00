...
static CNFBuilder *encode_constraint_graph(const ConstraintGraph *graph) {
    ...
    if (!node_var) { cnf_destroy(cnf); return NULL; }
    ...
    for (int i = 0; i < graph->constraint_count; i++) {
        ...
        if (found < 2) { free(p_indices); continue; }
        ...
        switch (c->type) {
            ...
            default:
                /* 其他类型：确保至少一个参与者为真 */
                {
                    ...
                    if (lit) {
                        for (int k = 0; k < found; k++) lit[k] = node_var[p_indices[k]];
...
static XORConstraint *xor_generate(int num_vars, uint64_t seed) {
    ...
    if (!xc->vars) { free(xc); return NULL; }
...
static CNFBuilder *xor_to_cnf(const XORConstraint *xc, CNFBuilder *base) {
    ...
    if (xc->count > 10) {
        ...
        if (!aux) { cnf_destroy(result); return NULL; }
...
static void dpll_solve(DPLLSolver *s, CNFBuilder *cnf) {
    if (s->solution_count >= s->max_solutions) return;

    for (int i = 1; i <= s->var_count; i++) s->assign[i] = 0;

    int status = dpll_unit_propagate(s, cnf);
    if (status == 0) { s->satisfiable = 0; return; }

    /* 选第一个未赋值变量 */
    int var = 0;
    for (int i = 1; i <= s->var_count; i++) {
        if (s->assign[i] == 0) { var = i; break; }
    }

    if (var == 0) {
        /* 所有变量已赋值 → 找到一个解 */
        s->solution_count++;
        if (s->solution_count == 1) s->satisfiable = 1;
        return;
    }
...
static int count_solutions(CNFBuilder *cnf, int max_count) {
    if (!cnf || cnf->var_count == 0) return 1; /* 空 CNF：恒真 */
...
bool approx_count_solutions(const ConstraintGraph *graph,
    ...
    if (valid_estimates > 0) {
        ...
        if (out->cell_sol_count == 0) out->cell_sol_count = 1;
...
char *approx_count_to_sat(const ConstraintGraph *graph, int *out_cnf_vars) {
    ...
    if (out_cnf_vars) *out_cnf_vars = cnf->var_count;
