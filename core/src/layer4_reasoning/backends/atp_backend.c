...
#undef LV_ATP_LANG_ROW
    ...
    int n = lv_snprintf(buf + offset, (size_t) remaining,
    ...
    if (format == ATP_FORMAT_TPTP_TFF && remaining > 128) {
        n = lv_snprintf(buf + offset, (size_t) remaining,
    ...
    for (int i = 0; i < node_count && remaining > 64; i++) {
        ...
        n = lv_snprintf(buf + offset, (size_t) remaining, "%s(p%d_decl, axiom, point(p%d)).\n", lang, nid, nid);
        if (n > 0 && n < remaining) {
            offset += n;
            remaining -= n;
        }
    ...
    if (include_proof_goal && remaining > 128) {
        if (target_prop) {
            ...
            n = lv_snprintf(buf + offset, (size_t) remaining, "%s(%s, conjecture, $false).\n", lang, prop_name);
        } else {
            n = lv_snprintf(buf + offset, (size_t) remaining, "%s(goal, conjecture, $false).\n", lang);
        }
        if (n > 0 && n < remaining) {
            offset += n;
            remaining -= n;
...
int atp_solver_solve(ATPBackendSolver *solver, ATPResultInfo *result) {
    ...
    if (!solver->is_initialized) {
        ...
        lv_snprintf(result->error_message, sizeof(result->error_message), "ATP solver not initialized");
        return (int) lv_ERROR_NOT_INITIALIZED;
    }
    ...
    if (!solver->has_problem) {
        ...
        lv_snprintf(result->error_message, sizeof(result->error_message), "No problem loaded");
    ...
    if (!exe_name) {
        ...
        lv_snprintf(result->error_message, sizeof(result->error_message), "Unknown ATP backend type");
        return (int) lv_OK;
    }
    ...
    if (!atp_check_executable(exe_name)) {
        ...
        lv_snprintf(result->error_message, sizeof(result->error_message),
...
#undef LV_ATP_MODE_ROW
    ...
    lv_snprintf(extra_args, sizeof(extra_args), "%s -t %d --proof tptp", mode, (int) solver->config.timeout_seconds);

    if (solver->config.custom_options) {
        size_t len = strlen(extra_args);
        lv_snprintf(extra_args + len, sizeof(extra_args) - len, " %s", solver->config.custom_options);
    }
    ...
    if (rc != (int) lv_OK) {
        ...
        lv_snprintf(result->error_message, sizeof(result->error_message), "Failed to execute ATP subprocess: error %d",
                 rc);
        return (int) lv_OK;
    }
...
int atp_solver_solve_graph(ATPBackendSolver *solver, const ConstraintGraph *graph, ATPInputFormat format,
    ...
    if (!tptp) {
        ...
        lv_snprintf(result->error_message, sizeof(result->error_message), "TPTP encoding failed");
        return (int) lv_ERROR_OUT_OF_MEMORY;
    }
    ...
    if (rc != (int) lv_OK) {
        ...
        result->result = ATP_RESULT_ERROR;
        result->error_code = rc;
        lv_snprintf(result->error_message, sizeof(result->error_message), "Failed to load TPTP encoding");
        return rc;
    }
...
int atp_auto_solve(const ConstraintGraph *graph, const ATPConfig *config, ATPResultInfo *result) {
    ...
    result->result = ATP_RESULT_UNKNOWN;
    result->backend = ATP_BACKEND_COUNT;
    lv_snprintf(result->error_message, sizeof(result->error_message), "No ATP backend available; returning UNKNOWN");
