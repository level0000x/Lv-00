...
int ideal_intersection(lvRingRegistry *registry, int ideal_id_a, int ideal_id_b) {
    ...
    lvIdeal *result_ideal = ideal_alloc_locked(ia->ring_id, ia->generator_count + ib->generator_count, NULL);
    if (!result_ideal) {
        goto cleanup;
    }
...
int ideal_quotient(lvRingRegistry *registry, int ideal_id_a, int ideal_id_b, const char *result_label) {
    ...
    lvIdeal *result_ideal = ideal_alloc_locked(ia->ring_id, ia->generator_count, result_label);
    if (!result_ideal) {
        goto cleanup;
    }
