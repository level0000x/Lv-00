 ...
 *
 * 线程安全设计：
 *   - 所有静态状态使用线程局部存储（LV00_THREAD_LOCAL）
 *   - 引擎实例本身不含共享可变状态
 *   - 节点 ID 分配使用原子递增
 *
 * 内存管理：
 *   - 统一使用 lv00_malloc/lv00_calloc/lv00_realloc/lv00_free
 *   - 所有分配失败路径均通过 lv00_set_error 报告错误
 *   - 树结构的销毁采用递归释放，防止内存泄漏
 *
...
/** 原子递增节点 ID（线程安全） */
static LV00_THREAD_LOCAL uint32_t g_trace_node_id_counter = 1;
...
static inline bool _visit_check(uint32_t *visited_map, uint32_t map_size,
    ...
    while (visited_map[idx] != 0) {
        if (visited_map[idx] == node_id) return 0;
        idx = (idx + 1) % map_size;
    }
    return -1;
}
...
static StringBuffer *string_buffer_create(void) {
    StringBuffer *buf = (StringBuffer *)lv00_malloc(sizeof(StringBuffer));
    if (!buf) return NULL;
    buf->capacity = EXPORT_BUFFER_INITIAL_SIZE;
    buf->data = (char *)lv00_malloc(buf->capacity);
    if (!buf->data) {
        lv00_free((void **)&buf);
        return NULL;
    }
...
static bool string_buffer_append(StringBuffer *buf, const char *fmt, ...) {
    ...
    while (buf->length + (size_t)needed + 1 >= buf->capacity) {
        if (buf->capacity >= EXPORT_BUFFER_MAX_SIZE) return -1;
        size_t new_cap = buf->capacity * 2;
        if (new_cap > EXPORT_BUFFER_MAX_SIZE) new_cap = EXPORT_BUFFER_MAX_SIZE;
        char *new_data = (char *)lv00_realloc(buf->data, new_cap);
 ...
 */
Lv00ProofTraceNode *lv00_trace_node_create(Lv00TraceNodeType type, const char *label) {
    Lv00ProofTraceNode *node = (Lv00ProofTraceNode *)lv00_calloc(1, sizeof(Lv00ProofTraceNode));
    if (!node) {
        lv00_set_error(LV00_ERROR_ALLOCATION_FAILED,
                       "无法分配溯源节点内存");
        return NULL;
    }
    ...
    node->children = (Lv00ProofTraceNode **)lv00_calloc(
        node->child_capacity, sizeof(Lv00ProofTraceNode *));
    if (!node->children) {
        lv00_free((void **)&node);
        lv00_set_error(LV00_ERROR_ALLOCATION_FAILED,
                       "无法分配溯源节点子节点数组");
 ...
 */
void lv00_trace_node_destroy(Lv00ProofTraceNode *node) {
    if (!node) return;

    /* 递归销毁所有子节点 */
    for (uint32_t i = 0; i < node->child_count; i++) {
        lv00_trace_node_destroy(node->children[i]);
    }
    ...
    if (node->dependency_ids) {
        lv00_free((void **)&node->dependency_ids);
    }
 ...
 */
int lv00_trace_node_add_child(Lv00ProofTraceNode *parent,
                                Lv00ProofTraceNode *child) {
    if (!parent || !child) {
    ...
    if (parent->child_count >= parent->child_capacity) {
        ...
        Lv00ProofTraceNode **new_children = (Lv00ProofTraceNode **)lv00_realloc(
            parent->children, new_cap * sizeof(Lv00ProofTraceNode *));
        if (!new_children) {
            lv00_set_error(LV00_ERROR_ALLOCATION_FAILED,
                           "扩容子节点数组失败");
            return -1;
 ...
 * @param status 新状态
 */
void lv00_trace_node_set_status(Lv00ProofTraceNode *node,
                                 Lv00TraceNodeStatus status) {
    if (!node) return;
 ...
 * @return 计算后的信任颜色
 */
TrustColor lv00_trace_node_compute_color(Lv00ProofTraceNode *node) {
    ...
    switch (node->type) {
        ...
        case TRACE_NODE_GOAL: {
            ...
            for (uint32_t i = 0; i < node->child_count; i++) {
                TrustColor child_color = lv00_trace_node_compute_color(node->children[i]);
 ...
 */
Lv00ProofTraceTree *lv00_trace_tree_create(Proposition *root_prop) {
    Lv00ProofTraceTree *tree = (Lv00ProofTraceTree *)lv00_calloc(1, sizeof(Lv00ProofTraceTree));
    if (!tree) {
        lv00_set_error(LV00_ERROR_ALLOCATION_FAILED,
                       "无法分配溯源树内存");
        return NULL;
    ...
    tree->all_nodes = (Lv00ProofTraceNode **)lv00_calloc(
        tree->node_capacity, sizeof(Lv00ProofTraceNode *));
    if (!tree->all_nodes) {
        lv00_free((void **)&tree);
        lv00_set_error(LV00_ERROR_ALLOCATION_FAILED,
                       "无法分配溯源树节点数组");
    ...
    tree->root = lv00_trace_node_create(TRACE_NODE_GOAL, root_label);
    if (!tree->root) {
 ...
 */
void lv00_trace_tree_destroy(Lv00ProofTraceTree *tree) {
    if (!tree) return;

    /* 递归销毁根节点（会递归销毁所有子节点） */
    if (tree->root) {
        lv00_trace_node_destroy(tree->root);
    }
 ...
 * @return 是否成功
 */
static bool trace_tree_register_node(Lv00ProofTraceTree *tree,
                                      Lv00ProofTraceNode *node) {
    if (!tree || !node) return -1;

    if (tree->node_count >= tree->node_capacity) {
        uint32_t new_cap = tree->node_capacity * 2;
        Lv00ProofTraceNode **new_nodes = (Lv00ProofTraceNode **)lv00_realloc(
            tree->all_nodes, new_cap * sizeof(Lv00ProofTraceNode *));
 ...
 * @param tree 溯源树
 */
static void trace_tree_update_stats(Lv00ProofTraceTree *tree) {
    if (!tree) return;
    ...
    for (uint32_t i = 0; i < tree->node_count; i++) {
        Lv00ProofTraceNode *node = tree->all_nodes[i];
 ...
 */
uint32_t lv00_trace_tree_find_path(const Lv00ProofTraceTree *tree,
    ...
    while (top < tree->node_count) {
        Lv00ProofTraceNode *current = stack[top].node;
        ...
        for (uint32_t i = 0; i < current->child_count; i++) {
            ...
            if (!VISIT_CHECK(child->id)) {
                ...
                if (child->id == to_id) {
                    ...
                    memcpy(out_path, path, copy_len * sizeof(Lv00ProofTraceNode *));
                    result = copy_len;
                    goto cleanup;
                }
 ...
 */
int lv00_trace_tree_export_dot(const Lv00ProofTraceTree *tree, const char *path) {
    if (!tree || !path) {
        lv00_set_error(LV00_ERROR_NULL_POINTER,
                       "导出 DOT 时参数为 NULL");
        return -1;
    ...
    for (uint32_t i = 0; i < tree->node_count; i++) {
        ...
        for (uint32_t j = 0; j < node->child_count; j++) {
            Lv00ProofTraceNode *child = node->children[j];
            fprintf(fp, "    n%d -> n%d;\n", node->id, child->id);
        }
    }
 ...
 *
 * @param tree 溯源树
 * @return JSON 字符串（调用者需用 lv00_free 释放），失败返回 NULL
 */
char *lv00_trace_tree_to_json(const Lv00ProofTraceTree *tree) {
    if (!tree) {
        lv00_set_error(LV00_ERROR_NULL_POINTER,
                       "导出 JSON 时溯源树为 NULL");
        return NULL;
 ...
 */
Lv00ContradictionPath *lv00_contradiction_path_create(void) {
    Lv00ContradictionPath *path = (Lv00ContradictionPath *)lv00_calloc(
        1, sizeof(Lv00ContradictionPath));
    if (!path) {
        lv00_set_error(LV00_ERROR_ALLOCATION_FAILED,
                       "无法分配矛盾路径内存");
        return NULL;
    ...
    path->nodes = (Lv00ContradictionPathNode *)lv00_calloc(
        path->node_capacity, sizeof(Lv00ContradictionPathNode));
    if (!path->nodes) {
        lv00_free((void **)&path);
        lv00_set_error(LV00_ERROR_ALLOCATION_FAILED,
                       "无法分配矛盾路径节点数组");
 ...
 */
void lv00_contradiction_path_destroy(Lv00ContradictionPath *path) {
    if (!path) return;

    if (path->nodes) {
        lv00_free((void **)&path->nodes);
    }
 ...
 */
uint32_t lv00_contradiction_path_add_node(Lv00ContradictionPath *path,
    ...
    if (path->node_count >= path->node_capacity) {
        ...
        Lv00ContradictionPathNode *new_nodes =
            (Lv00ContradictionPathNode *)lv00_realloc(
                path->nodes, new_cap * sizeof(Lv00ContradictionPathNode));
        if (!new_nodes) {
            lv00_set_error(LV00_ERROR_ALLOCATION_FAILED,
                           "扩容矛盾路径节点数组失败");
            return (uint32_t)-1;
    ...
    Lv00ContradictionPathNode *node = &path->nodes[idx];
 ...
 */
int lv00_detect_contradiction(const ConstraintGraph *graph,
 ...
 */
int lv00_contradiction_path_validate(Lv00ContradictionPath *path) {
    if (!path) {
        lv00_set_error(LV00_ERROR_NULL_POINTER,
                       "验证矛盾路径时参数为 NULL");
        return -1;
    ...
    if (path->node_count == 0) {
        lv00_set_error(LV00_ERROR_PROOF_INCOMPLETE,
                       "矛盾路径为空");
        return -1;
    }
    ...
    if (!path->nodes[0].is_assumption) {
        lv00_set_error(LV00_ERROR_PROOF_INVALID,
                       "矛盾路径的第一个节点不是假设");
        return -1;
    }
 ...
 */
int lv00_engine_proof_by_contradiction(Lv00ProofEngine *engine,
    ...
    *out_path = NULL;
    ...
    /* 创建矛盾路径 */
    Lv00ContradictionPath *path = lv00_contradiction_path_create();
    if (!path) return -1;

    /* 创建溯源树 */
    Lv00ProofTraceTree *tree = lv00_trace_tree_create(NULL);
    if (!tree) {
        lv00_contradiction_path_destroy(path);
        return -1;
    }
    ...
    lv00_contradiction_path_add_node(path, assumption_stmt,
                                      "反证法初始假设", true);

    /* 创建假设节点 */
    Lv00ProofTraceNode *hyp_node = lv00_trace_node_create(
        TRACE_NODE_HYPOTHESIS, "Negation Hypothesis");
    if (hyp_node) {
        lv00_trace_node_add_child(tree->root, hyp_node);
    ...
    while (step < max_steps && !found_contradiction) {
        ...
        if (lv00_detect_contradiction(engine->graph, engine->navigator,
            ...
            /* 添加矛盾节点到溯源树 */
            Lv00ProofTraceNode *contra_node = lv00_trace_node_create(
                TRACE_NODE_CONTRADICTION, "Contradiction Found");
            if (contra_node) {
                safe_strncpy(contra_node->description, cdesc,
                             sizeof(contra_node->description));
                lv00_trace_node_set_status(contra_node, TRACE_STATUS_PROVED);
                if (hyp_node) {
                    lv00_trace_node_add_child(hyp_node, contra_node);
                } else {
                    lv00_trace_node_add_child(tree->root, contra_node);
                }
        ...
        if (engine->rule_library && engine->graph) {
            Lv00RuleMatch **matches = (Lv00RuleMatch **)lv00_malloc(
                16 * sizeof(Lv00RuleMatch *));
            if (matches) {
                uint32_t match_count = lv00_rule_find_matches(
                    engine->rule_library, engine->graph, engine->navigator,
                    matches, 16);

                if (match_count > 0) {
                    /* 应用第一个匹配的规则 */
                    ProofStep **new_steps = (ProofStep **)lv00_malloc(
                        8 * sizeof(ProofStep *));
                    if (new_steps) {
                        uint32_t step_count = lv00_rule_apply_match(
                        ...
                        if (step_count > 0) {
                            ...
                            lv00_contradiction_path_add_node(
                                path, step_stmt,
                                matches[0]->rule ?
                            ...
                            /* 创建推导节点 */
                            Lv00ProofTraceNode *deriv_node =
                                lv00_trace_node_create(
                                    TRACE_NODE_DERIVATION,
                                    matches[0]->rule ?
                                        matches[0]->rule->name : "Derivation");
                            if (deriv_node) {
                                deriv_node->rule = matches[0]->rule;
                                lv00_trace_node_set_status(
                                    deriv_node, TRACE_STATUS_EXPLORING);
                                if (hyp_node) {
                                    lv00_trace_node_add_child(hyp_node, deriv_node);
                                } else {
                                    lv00_trace_node_add_child(tree->root, deriv_node);
                                }
                ...
                for (uint32_t m = 0; m < match_count; m++) {
                    lv00_rule_match_destroy(matches[m]);
                }
    ...
    if (found_contradiction) {
        ...
        if (tree->root) {
            lv00_trace_node_set_status(tree->root, TRACE_STATUS_PROVED);
        }
    } else {
        tree->is_complete = false;
        tree->final_color = TRUST_BLUE;
        if (tree->root) {
            lv00_trace_node_set_status(tree->root, TRACE_STATUS_BLOCKED);
        }
    }
    ...
    /* 验证路径 */
    if (found_contradiction) {
        lv00_contradiction_path_validate(path);
    }
 ...
 * 引擎创建后需要通过 lv00_proof_engine_set_rule_library 设置规则库。
 *
 * @param config 引擎配置（可为 NULL，使用默认值）
 * @return 新引擎实例，失败返回 NULL
 */
Lv00ProofEngine *lv00_proof_engine_create(const Lv00ProofEngineConfig *config) {
    Lv00ProofEngine *engine = (Lv00ProofEngine *)lv00_calloc(
        1, sizeof(Lv00ProofEngine));
    if (!engine) {
        lv00_set_error(LV00_ERROR_ALLOCATION_FAILED,
                       "无法分配证明引擎内存");
        return NULL;
 ...
 */
void lv00_proof_engine_destroy(Lv00ProofEngine *engine) {
    if (!engine) return;

    /* 释放当前溯源树 */
    if (engine->current_trace) {
        lv00_trace_tree_destroy(engine->current_trace);
        engine->current_trace = NULL;
    }

    /* 释放缓存（如果有） */
    if (engine->proof_cache) {
        lv00_free((void **)&engine->proof_cache);
    }
 ...
 */
void lv00_proof_engine_set_rule_library(Lv00ProofEngine *engine,
 ...
 * 引擎最多支持 LV00_PROOF_MAX_STRATEGIES 个策略。
 *
 * @param engine   引擎实例
 * @param strategy 策略描述
 * @return true 注册成功，false 引擎已满或参数无效
 */
int lv00_proof_engine_register_strategy(Lv00ProofEngine *engine,
    ...
    if (engine->strategy_count >= LV00_PROOF_MAX_STRATEGIES) {
        lv00_set_error(LV00_ERROR_RESOURCE_EXHAUSTED,
                       "策略数量已达上限 (%d)", LV00_PROOF_MAX_STRATEGIES);
        return -1;
    }
    ...
    if (insert_pos < engine->strategy_count) {
        memmove(&engine->strategies[insert_pos + 1],
                ...
                (engine->strategy_count - insert_pos) * sizeof(Lv00ProofStrategy));
    }
 ...
 * @return 策略中文名称字符串
 */
static const char *get_strategy_name_zh(Lv00StrategyType type) {
 ...
 * @return 是否成功
 */
static bool execute_strategy_direct(Lv00ProofEngine *engine,
                                     const Proposition *goal,
                                     Lv00ProofTraceTree *tree) {
    if (!engine || !goal || !tree) return -1;
    ...
    while (step < max_steps && !success) {
        ...
        if (engine->graph) {
            ...
            if (unify_result == UNIFY_STATUS_OK) {
                ...
                if (proved_node) {
                    lv00_trace_node_set_status(proved_node, TRACE_STATUS_PROVED);
                    lv00_trace_node_add_child(tree->root, proved_node);
            ...
            if (matches) {
                ...
                for (uint32_t m = 0; m < match_count && !success; m++) {
                    ProofStep **new_steps = (ProofStep **)lv00_malloc(
                        4 * sizeof(ProofStep *));
                    if (new_steps) {
                        uint32_t sc = lv00_rule_apply_match(
 ...
 * @return 是否成功
 */
static bool execute_strategy_contrapositive(Lv00ProofEngine *engine,
                                             const Proposition *goal,
                                             Lv00ProofTraceTree *tree) {
    if (!engine || !goal || !tree) return -1;
    ...
    if (contra_node) {
        safe_strncpy(contra_node->description,
                     ...
                     sizeof(contra_node->description));
        lv00_trace_node_set_status(contra_node, TRACE_STATUS_EXPLORING);
        lv00_trace_node_add_child(tree->root, contra_node);
        trace_tree_register_node(tree, contra_node);
    }
 ...
 * @return 是否成功
 */
static bool execute_strategy_induction(Lv00ProofEngine *engine,
                                        const Proposition *goal,
                                        Lv00ProofTraceTree *tree) {
    if (!engine || !goal || !tree) return -1;

    /* 基础步节点 */
    Lv00ProofTraceNode *base_node = lv00_trace_node_create(
    ...
    if (base_node) {
        ...
        lv00_trace_node_add_child(tree->root, base_node);
        trace_tree_register_node(tree, base_node);
    }

    /* 归纳假设节点 */
    Lv00ProofTraceNode *ih_node = lv00_trace_node_create(
        TRACE_NODE_HYPOTHESIS, "Inductive Hypothesis");
    if (ih_node) {
        safe_strncpy(ih_node->description,
                     "归纳假设：假设命题在 n=k 时成立",
                     sizeof(ih_node->description));
        lv00_trace_node_set_status(ih_node, TRACE_STATUS_EXPLORING);
        lv00_trace_node_add_child(tree->root, ih_node);
        trace_tree_register_node(tree, ih_node);
    }

    /* 归纳步节点 */
    Lv00ProofTraceNode *step_node = lv00_trace_node_create(
    ...
    if (step_node) {
        ...
        lv00_trace_node_add_child(tree->root, step_node);
 ...
 * @return 是否成功
 */
static bool execute_strategy_cases(Lv00ProofEngine *engine,
                                    const Proposition *goal,
                                    Lv00ProofTraceTree *tree) {
    if (!engine || !goal || !tree) return -1;
    ...
    for (int c = 0; c < num_cases && c < (int)LV00_PROOF_MAX_BRANCHES; c++) {
        char case_label[128];
        snprintf(case_label, sizeof(case_label), "Case %d", c + 1);

        Lv00ProofTraceNode *case_node = lv00_trace_node_create(
        ...
        if (case_success) {
            lv00_trace_node_set_status(case_node, TRACE_STATUS_PROVED);
        } else {
            lv00_trace_node_set_status(case_node, TRACE_STATUS_BLOCKED);
            all_proved = false;
        }

        lv00_trace_node_add_child(tree->root, case_node);
 ...
 * @return 是否成功
 */
static bool execute_strategy_construction(Lv00ProofEngine *engine,
                                           const Proposition *goal,
                                           Lv00ProofTraceTree *tree) {
    if (!engine || !goal || !tree) return -1;
    ...
    if (engine->rule_library && engine->graph) {
        /* 查找构造规则 */
        Lv00Rule **construct_rules = (Lv00Rule **)lv00_malloc(
            16 * sizeof(Lv00Rule *));
        if (construct_rules) {
            uint32_t rule_count = lv00_rule_library_get_by_type(
                engine->rule_library, RULE_TYPE_CONSTRUCTOR,
                construct_rules, 16);

            for (uint32_t r = 0; r < rule_count && !success; r++) {
                if (lv00_rule_is_applicable(construct_rules[r],
                                             engine->graph,
                                             engine->navigator)) {
                    Lv00RuleMatch match;
            ...
            lv00_free((void **)&construct_rules);
        }
    }

    if (success) {
        lv00_trace_node_set_status(construct_node, TRACE_STATUS_PROVED);
    } else {
        lv00_trace_node_set_status(construct_node, TRACE_STATUS_BLOCKED);
    }

    lv00_trace_node_add_child(tree->root, construct_node);
 ...
 * @return 是否成功
 */
static bool execute_strategy_unfolding(Lv00ProofEngine *engine,
                                        const Proposition *goal,
                                        Lv00ProofTraceTree *tree) {
    if (!engine || !goal || !tree) return -1;
    ...
    if (engine->rule_library) {
        Lv00Rule **def_rules = (Lv00Rule **)lv00_malloc(
            16 * sizeof(Lv00Rule *));
        if (def_rules) {
            uint32_t rule_count = lv00_rule_library_get_by_type(
            ...
            for (uint32_t r = 0; r < rule_count; r++) {
                Lv00ProofTraceNode *unfold_node = lv00_trace_node_create(
                ...
                if (unfold_node) {
                    ...
                    lv00_trace_node_set_status(unfold_node, TRACE_STATUS_PROVED);
                    lv00_trace_node_add_child(tree->root, unfold_node);
                    trace_tree_register_node(tree, unfold_node);
                }
            }

            lv00_free((void **)&def_rules);
        }
    }
 ...
 * @return 是否成功
 */
static bool execute_strategy_backward(Lv00ProofEngine *engine,
                                       const Proposition *goal,
                                       Lv00ProofTraceTree *tree) {
    if (!engine || !goal || !tree) return -1;

    /* 创建逆向推理起始节点 */
    Lv00ProofTraceNode *back_node = lv00_trace_node_create(
        TRACE_NODE_GOAL, "Backward Analysis");
    if (back_node) {
        safe_strncpy(back_node->description,
                     "从目标出发，逆向分析所需前提",
                     sizeof(back_node->description));
        lv00_trace_node_set_status(back_node, TRACE_STATUS_EXPLORING);
        lv00_trace_node_add_child(tree->root, back_node);
    ...
    if (goal->sub_props) {
        for (int i = 0; i < goal->sub_prop_count; i++) {
            ...
            Lv00ProofTraceNode *sub_goal = lv00_trace_node_create(
                TRACE_NODE_GOAL, sub->name ? sub->name : "Sub-goal");
            if (sub_goal) {
                sub_goal->proposition = sub;
                lv00_trace_node_set_status(sub_goal, TRACE_STATUS_EXPLORING);
                lv00_trace_node_add_child(tree->root, sub_goal);
 ...
 * @return 是否成功
 */
static bool execute_strategy_forward(Lv00ProofEngine *engine,
                                      const Proposition *goal,
                                      Lv00ProofTraceTree *tree) {
    if (!engine || !goal || !tree) return -1;
    ...
    while (step < max_steps) {
        ...
        if (engine->graph) {
            ...
            if (result == UNIFY_STATUS_OK) {
                ...
                if (final_node) {
                    lv00_trace_node_set_status(final_node, TRACE_STATUS_PROVED);
                    lv00_trace_node_add_child(tree->root, final_node);
        ...
        if (engine->rule_library && engine->graph) {
            Lv00RuleMatch **matches = (Lv00RuleMatch **)lv00_malloc(
                8 * sizeof(Lv00RuleMatch *));
            if (matches) {
                uint32_t mc = lv00_rule_find_matches(
                    engine->rule_library, engine->graph, engine->navigator,
                    matches, 8);
                ...
                if (mc > 0) {
                    ProofStep **new_steps = (ProofStep **)lv00_malloc(
                        4 * sizeof(ProofStep *));
                    if (new_steps) {
                        lv00_rule_apply_match(
                            matches[0], engine->graph, engine->navigator,
                            new_steps, 4);

                        Lv00ProofTraceNode *fwd_node = lv00_trace_node_create(
                            TRACE_NODE_DERIVATION,
                            matches[0]->rule ? matches[0]->rule->name : "Forward Step");
                        if (fwd_node) {
                            fwd_node->rule = matches[0]->rule;
                            lv00_trace_node_set_status(fwd_node, TRACE_STATUS_EXPLORING);
                            lv00_trace_node_add_child(tree->root, fwd_node);
 ...
 * @return 是否成功
 */
static bool execute_strategy_hybrid(Lv00ProofEngine *engine,
                                     const Proposition *goal,
                                     Lv00ProofTraceTree *tree) {
    if (!engine || !goal || !tree) return -1;

    /* 阶段 1: 逆向分析 */
    Lv00ProofTraceNode *analysis_node = lv00_trace_node_create(
        TRACE_NODE_DERIVATION, "Phase 1: Backward Analysis");
    if (analysis_node) {
        safe_strncpy(analysis_node->description,
                     "混合策略阶段1：逆向分析目标结构",
                     sizeof(analysis_node->description));
        lv00_trace_node_set_status(analysis_node, TRACE_STATUS_EXPLORING);
        lv00_trace_node_add_child(tree->root, analysis_node);
    ...
    if (forward_ok) {
        if (analysis_node) {
            lv00_trace_node_set_status(analysis_node, TRACE_STATUS_PROVED);
        }
        return 0;
    }
    ...
    if (contra_path) {
        lv00_contradiction_path_destroy(contra_path);
    }
 ...
 * @return 是否成功
 */
static bool dispatch_strategy(Lv00ProofEngine *engine,
                               const Proposition *goal,
                               Lv00StrategyType type,
                               Lv00ProofTraceTree *tree) {
    switch (type) {
 ...
 */
int lv00_proof_engine_prove_with_strategy(Lv00ProofEngine *engine,
                                            const Proposition *goal,
    ...
    *out_trace = NULL;
    ...
    /* 创建溯源树 */
    Lv00ProofTraceTree *tree = lv00_trace_tree_create((Proposition *)goal);
    if (!tree) return -1;
 ...
 * @return true 证明成功，false 证明失败
 */
int lv00_proof_engine_prove(Lv00ProofEngine *engine,
                              const Proposition *goal,
                              ConstraintGraph *graph,
                              Lv00ProofTraceTree **out_trace) {
    if (!engine || !goal || !out_trace) {
    ...
    if (engine->strategy_count > 0) {
        return lv00_proof_engine_prove_with_strategy(
            engine, goal, graph,
 ...
 */
int lv00_proof_engine_auto_prove(Lv00ProofEngine *engine,
                                   const Proposition *goal,
                                   ConstraintGraph *graph,
                                   Lv00ProofTraceTree **out_trace,
    ...
    *out_trace = NULL;
    ...
    if (engine->strategy_count > 0) {
        for (uint32_t i = 0; i < engine->strategy_count; i++) {
            Lv00ProofTraceTree *trace = NULL;
            bool success = lv00_proof_engine_prove_with_strategy(
                engine, goal, graph,
            ...
            /* 释放失败的溯源树 */
            if (trace) {
                lv00_trace_tree_destroy(trace);
            }
        }
    } else {
        /* 没有注册策略，尝试所有内置策略 */
        static const Lv00StrategyType builtin_strategies[] = {
            STRATEGY_DIRECT,
 ...
 */
void lv00_proof_engine_get_stats(const Lv00ProofEngine *engine,
                                  uint64_t *out_total,
 ...
 * @return 验证结果
 */
Lv00VerifyResult lv00_verify_proof(const Lv00ProofTraceTree *trace,
                                    char *out_error) {
    if (!trace) {
        if (out_error) {
    ...
    for (uint32_t i = 0; i < trace->node_count; i++) {
        Lv00ProofTraceNode *node = trace->all_nodes[i];
    ...
    /* 检查信任颜色传播 */
    TrustColor computed = lv00_trace_node_compute_color(trace->root);
 ...
 * @return 验证结果
 */
Lv00VerifyResult lv00_verify_proof_step(const ProofStep *step,
 ...
 * @return true 节点是冗余的
 */
static bool is_redundant_node(const Lv00ProofTraceNode *node) {
    if (!node) return -1;
    ...
    if (node->type == TRACE_NODE_DERIVATION && node->child_count == 1) {
        Lv00ProofTraceNode *child = node->children[0];
 ...
 */
int lv00_optimize_proof(const Lv00ProofTraceTree *trace,
    ...
    *out_optimized = NULL;
    ...
    /* 创建新的溯源树 */
    Lv00ProofTraceTree *optimized = lv00_trace_tree_create(
    ...
    for (uint32_t i = 0; i < trace->node_count; i++) {
        Lv00ProofTraceNode *src_node = trace->all_nodes[i];
        ...
        /* 创建新节点并复制属性 */
        Lv00ProofTraceNode *new_node = lv00_trace_node_create(
        ...
        /* 添加到优化后的树 */
        lv00_trace_node_add_child(optimized->root, new_node);
...
uint32_t lv00_simplify_proof(Lv00ProofTraceTree *trace) {
    ...
    for (uint32_t i = 0; i < trace->node_count; i++) {
        ...
        if (is_redundant_node(node)) {
            lv00_trace_node_set_status(node, TRACE_STATUS_BLOCKED);
            removed++;
        }
    }
    ...
    /* 重新计算信任颜色 */
    if (trace->root) {
        lv00_trace_node_compute_color(trace->root);
 ...
 * @return 自然语言文本（调用者需用 lv00_free 释放），失败返回 NULL
 */
char *lv00_proof_to_natural_language(const Lv00ProofTraceTree *trace,
 ...
 *
 * @param trace 溯源树
 * @return LaTeX 文本（调用者需用 lv00_free 释放），失败返回 NULL
 */
char *lv00_proof_to_latex(const Lv00ProofTraceTree *trace) {
    if (!trace) {
        lv00_set_error(LV00_ERROR_NULL_POINTER,
                       "导出 LaTeX 时溯源树为 NULL");
        return NULL;
 ...
 *
 * @param trace 溯源树
 * @return Coq 脚本（调用者需用 lv00_free 释放），失败返回 NULL
 */
char *lv00_proof_to_coq(const Lv00ProofTraceTree *trace) {
    if (!trace) {
        lv00_set_error(LV00_ERROR_NULL_POINTER,
                       "导出 Coq 时溯源树为 NULL");
        return NULL;
 ...
 *
 * @param trace 溯源树
 * @return Isar 脚本（调用者需用 lv00_free 释放），失败返回 NULL
 */
char *lv00_proof_to_isar(const Lv00ProofTraceTree *trace) {
    if (!trace) {
        lv00_set_error(LV00_ERROR_NULL_POINTER,
                       "导出 Isar 时溯源树为 NULL");
        return NULL;
