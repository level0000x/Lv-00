...
static const ConflictDetectorConfig g_default_config = {.enable_basic_checks = true,
...
ConflictReport *lv_conflict_report_create(void) {
    ...
    report->conflicts = (ConflictRecord *) lv_malloc(sizeof(ConflictRecord) * report->capacity);
...
/* 添加矛盾记录 */
static bool conflict_report_add(ConflictReport *report, ConflictType type, ConflictSeverity severity,
                                const char *description, const char *suggestion) {
    if (!conflict_report_ensure_capacity(report))
        return false;
...
const char *lv_conflict_type_name(ConflictType type) {
    switch (type) {
        case CONFLICT_POINT_POSITION:
            return "PointPositionConflict";
        case CONFLICT_DISTANCE_MISMATCH:
            return "DistanceMismatch";
        case CONFLICT_ANGLE_MISMATCH:
            return "AngleMismatch";
        case CONFLICT_COLLINEAR_VS_ANGLE:
            return "CollinearVsAngle";
        case CONFLICT_PERPENDICULAR_VS_PARALLEL:
            return "PerpendicularVsParallel";
        case CONFLICT_CONTAINMENT_VS_SEPARATION:
            return "ContainmentVsSeparation";
        case CONFLICT_INTERSECTION_VS_PARALLEL:
            return "IntersectionVsParallel";
        case CONFLICT_TRANSITIVE_EQUALITY:
            return "TransitiveEquality";
        case CONFLICT_TRANSITIVE_ORDER:
            return "TransitiveOrder";
        case CONFLICT_CYCLIC_DEPENDENCY:
            return "CyclicDependency";
        case CONFLICT_ALGEBRAIC_NO_SOLUTION:
            return "AlgebraicNoSolution";
        case CONFLICT_ALGEBRAIC_OVERCONSTRAINED:
            return "AlgebraicOverconstrained";
        case CONFLICT_ALGEBRAIC_SINGULAR:
            return "AlgebraicSingular";
...
static int detect_structural_constraint_conflicts(const ConstraintGraph *graph, const ConflictDetectorConfig *config,
    ...
    for (int i = 0; i < graph->constraint_count; i++) {
        ...
        if (expected < 0) {
            report_constraint_conflict(report, constraint, CONFLICT_UNKNOWN, CONFLICT_SEVERITY_ERROR,
                                       "约束类型未知，无法进行语义校验。",
        ...
        if (constraint->participant_count != expected ||
            ...
            report_constraint_conflict(report, constraint, CONFLICT_UNKNOWN, CONFLICT_SEVERITY_CRITICAL,
                                       "约束参与者数量或参与者数组无效。",
                                       "按约束类型重新创建约束：INCIDENCE/CONTAINMENT/CONNECTION "
                                       "需要2个参与者，BETWEENNESS/INTERSECTION 需要3个参与者。");
            continue;
        }
 ...
 */
static int detect_point_position_conflicts(const ConstraintGraph *graph, const ConflictDetectorConfig *config,
                                           ConflictReport *report) {
    if (!graph || !report)
        return lv_ERROR_NULL_POINTER;
    ...
    for (int i = 0; i < graph->node_count; i++) {
        ...
        for (int ci = 0; ci < c_count; ci++) {
            ...
            if (cons->type == CONSTRAINT_COINCIDENT && cons->participant_count >= 2) {
                ...
                if (other && other->type == GEOM_POINT && other->symbolic_coords && other->coord_count >= 2) {
                    ...
                    if (cx != 0 || cy != 0) {
                        ...
                        report_constraint_conflict(report, cons, CONFLICT_POINT_POSITION, CONFLICT_SEVERITY_ERROR, desc,
                                                   "修正点的坐标或移除重合约束。");
                    }
                }
            }
            ...
            if (cons->type == INCIDENCE && cons->participant_count >= 2) {
                ...
                if (line_node && line_node->type == GEOM_LINE_SEGMENT && line_node->symbolic_coords &&
                    line_node->coord_count >= 4) {
                    double px = symbolic_coord_to_double(x_coord);
                    double py = symbolic_coord_to_double(y_coord);
 ...
 */
static int detect_distance_conflicts(const ConstraintGraph *graph, const ConflictDetectorConfig *config,
                                     ConflictReport *report) {
    if (!graph || !report)
        return lv_ERROR_NULL_POINTER;
    ...
    for (int i = 0; i < graph->constraint_count; i++) {
        ...
        if (!c1 || !c1->is_active || c1->type != CONSTRAINT_DISTANCE)
            continue;
        ...
        for (int j = i + 1; j < graph->constraint_count; j++) {
            ...
            if (!c2 || !c2->is_active || c2->type != CONSTRAINT_DISTANCE)
                continue;
            ...
            if (diff > eps) {
                ...
                report_constraint_conflict(report, c1, CONFLICT_DISTANCE_MISMATCH, CONFLICT_SEVERITY_ERROR, desc,
 ...
 * @brief 快速基础检测
 */
static int detect_basic_conflicts(const ConstraintGraph *graph, const ConflictDetectorConfig *config,
                                  ConflictReport *report) {
    int err;
 ...
 */
static int check_constraint_pair_conflict(const ConstraintGraph *graph, Constraint *c1, Constraint *c2,
                                          const ConflictDetectorConfig *config, ConflictReport *report) {
    if (!c1 || !c2 || !c1->is_active || !c2->is_active)
        return 0;
    ...
    if (c1->type == CONSTRAINT_ANGLE && c2->type == CONSTRAINT_ANGLE && c1->participant_count >= 2 &&
        ...
        if (same_pair) {
            ...
            if (diff > angle_eps && fabs(diff - PI) > angle_eps) {
                ...
                report_constraint_conflict(report, c1, CONFLICT_ANGLE_MISMATCH, CONFLICT_SEVERITY_ERROR, desc,
    ...
    if ((c1->type == CONSTRAINT_PARALLEL && c2->type == CONSTRAINT_PERPENDICULAR) ||
        ...
        if (c1->participant_count >= 2 && c2->participant_count >= 2) {
            ...
            if (same_pair) {
                ...
                report_constraint_conflict(report, c1, CONFLICT_PERPENDICULAR_VS_PARALLEL, CONFLICT_SEVERITY_CRITICAL,
                                           desc, "移除平行约束或垂直约束，两条线段不能同时平行和垂直。");
            }
        }
    }
    ...
    if ((c1->type == CONSTRAINT_HORIZONTAL && c2->type == CONSTRAINT_VERTICAL) ||
        ...
        if (c1->participant_count >= 1 && c2->participant_count >= 1) {
            ...
            if (line1 == line2) {
                ...
                report_constraint_conflict(report, c1, CONFLICT_PERPENDICULAR_VS_PARALLEL, CONFLICT_SEVERITY_CRITICAL,
                                           desc, "移除水平约束或垂直约束，一条线段不能同时水平和垂直。");
            }
        }
    }
    ...
    if ((c1->type == INTERSECTION && c2->type == CONSTRAINT_PARALLEL) ||
        ...
        if (c1->participant_count >= 2 && c2->participant_count >= 2) {
            ...
            if (same_pair) {
                ...
                report_constraint_conflict(report, c1, CONFLICT_INTERSECTION_VS_PARALLEL, CONFLICT_SEVERITY_CRITICAL,
                                           desc, "移除相交约束或平行约束，两条线段不能同时相交和平行。");
            }
        }
    }
 ...
 * @brief 检测所有约束组合冲突
 */
static int detect_combination_conflicts(const ConstraintGraph *graph, const ConflictDetectorConfig *config,
                                        ConflictReport *report) {
    if (!graph || !report)
        return lv_ERROR_NULL_POINTER;
    ...
    for (int i = 0; i < graph->constraint_count; i++) {
        for (int j = i + 1; j < graph->constraint_count; j++) {
            int err =
                check_constraint_pair_conflict(graph, graph->constraints[i], graph->constraints[j], config, report);
            if (err != 0)
                return err;
        }
    }
 ...
 */
static int detect_transitive_equality_conflicts(const ConstraintGraph *graph, const ConflictDetectorConfig *config,
                                                ConflictReport *report) {
    if (!graph || !report)
        return lv_ERROR_NULL_POINTER;
...
#define UF_UNION(a, b)            \
    ...
    for (int i = 0; i < graph->constraint_count; i++) {
        Constraint *cons = graph->constraints[i];
        if (!cons || !cons->is_active || cons->type != CONSTRAINT_COINCIDENT)
            continue;
        if (cons->participant_count < 2 || !cons->participants)
            continue;
    ...
    for (int i = 0; i < graph->constraint_count; i++) {
        ...
        /* 如果距离值接近 0，不算矛盾 */
        if (fabs(cons->numeric_value) <= dist_eps)
            continue;
        ...
        if (ra == rb) {
            ...
            report_constraint_conflict(report, cons, CONFLICT_TRANSITIVE_EQUALITY, CONFLICT_SEVERITY_ERROR, desc,
 ...
 * @brief 传递闭包检测
 */
static int detect_transitive_conflicts(const ConstraintGraph *graph, const ConflictDetectorConfig *config,
                                       ConflictReport *report) {
    int err;
...
int lv_conflict_detect_all(const ConstraintGraph *graph, const ConflictDetectorConfig *config, ConflictReport *report) {
    if (!graph || !report)
        return lv_ERROR_NULL_POINTER;
    ...
    /* 2. 组合检测 */
    if (cfg->enable_combination_checks && (cfg->max_conflicts == 0 || report->conflict_count < cfg->max_conflicts)) {
    ...
    /* 3. 传递检测 */
    if (cfg->enable_transitive_checks && (cfg->max_conflicts == 0 || report->conflict_count < cfg->max_conflicts)) {
    ...
    /* 4. 代数检测（较耗时，最后执行） */
    if (cfg->enable_algebraic_checks && (cfg->max_conflicts == 0 || report->conflict_count < cfg->max_conflicts)) {
        ...
        if (eq_count > 0) {
            ...
            if (pair_count > (long long) INT_MAX) {
                ...
                int *pair_constraint_count = (int *) lv_calloc(sizeof(int), (size_t) pair_count);
                if (pair_constraint_count) {
                    for (int i = 0; i < graph->constraint_count; i++) {
                        ...
                        if (a >= 0 && b >= 0 && a < graph->node_count && b < graph->node_count) {
...
int lv_conflict_detect_for_node(const ConstraintGraph *graph, int node_id, ConflictReport *report) {
    if (!graph || !report)
        return lv_ERROR_NULL_POINTER;
    ...
    for (int i = 0; i < c_count; i++) {
        ...
        for (int j = i + 1; j < c_count; j++) {
            ...
            if (!cj || !cj->is_active)
                continue;
            check_constraint_pair_conflict(graph, ci, cj, &g_default_config, report);
        }
    }
...
int lv_conflict_detect_for_constraint(const ConstraintGraph *graph, int constraint_id, ConflictReport *report) {
    if (!graph || !report)
        return lv_ERROR_NULL_POINTER;
...
void lv_conflict_report_print(const ConflictReport *report, void *output, bool verbose) {
    if (!report)
        return;
    ...
    fprintf(out, "Critical: %s, Error: %s, Warning: %s\n", report->has_critical ? "YES" : "no",
            report->has_error ? "YES" : "no", report->has_warning ? "YES" : "no");
    ...
    for (int i = 0; i < report->conflict_count; i++) {
        ...
        fprintf(out, "\n[%d] %s: %s\n", i + 1, lv_conflict_severity_name(rec->severity),
...
int lv_conflict_report_to_json(const ConflictReport *report, char *buffer, size_t buffer_size) {
...
ConflictType lv_conflict_get_worst_type(const ConflictReport *report) {
    if (!report || report->conflict_count == 0)
