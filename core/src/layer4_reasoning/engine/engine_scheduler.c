 ...
 * ============================================================ */
static bool rule_matches(const RoutingRule *rule, const GraphFeatures *features,
                         const EngineScheduler *scheduler) {
 ...
 * ============================================================ */
static int extract_assignments_from_graph(const ConstraintGraph *graph,
                                          SMTVariableAssignment **out_assignments,
                                          int *out_count) {
    if (!graph || !out_assignments || !out_count) return -1;
    ...
    for (int i = 0; i < graph->node_count && idx < max_assignments; i++) {
        ...
        for (int ci = 0; ci < node->coord_count && idx < max_assignments; ci++) {
            ...
            snprintf(assignments[idx].var_name, SMT_VAR_NAME_MAX_LEN,
                     "p%d_%s", node->id, (ci == 0) ? "x" : "y");
            ...
            assignments[idx].value.rational.approx_value =
                sc->cache_valid ? sc->cached_value : 0.0;
...
EngineScheduler *scheduler_create(void) {
    EngineScheduler *sched = (EngineScheduler *)
        lv_calloc(1, sizeof(EngineScheduler));
    ...
    entry->priority = 100;
    lv_strncpy(entry->description, "Built-in Groebner basis solver",
...
int scheduler_register_backend(EngineScheduler *scheduler,
                                SolverBackendType type, int priority,
    ...
    for (int i = 0; i < scheduler->backend_count; i++) {
        if (scheduler->backends[i].type == type) {
            ...
            if (description) {
                lv_strncpy(scheduler->backends[i].description, description,
                           sizeof(scheduler->backends[i].description));
            }
            if (detect_func) {
    ...
    entry->available = (detect_func != NULL) ||
                       (type == GROEBNER); /* GROEBNER 始终可用 */
    scheduler->backend_count++;
...
int scheduler_unregister_backend(EngineScheduler *scheduler, SolverBackendType type) {
    ...
    for (int i = 0; i < scheduler->backend_count; i++) {
        if (scheduler->backends[i].type == type) {
            ...
            scheduler->backend_count--;
            memset(&scheduler->backends[scheduler->backend_count], 0,
                   sizeof(SchedulerBackendEntry));
            return 0;
        }
    }
    return -1; /* 未找到 */
...
int scheduler_list_available_backends(const EngineScheduler *scheduler,
    ...
    if (!scheduler || !out_types || max_count <= 0) return 0;
 ...
 * 后端选择与可用性
 * ============================================================ */
bool scheduler_is_backend_available(const EngineScheduler *scheduler,
...
void scheduler_set_backend_available(EngineScheduler *scheduler,
                                      SolverBackendType type, bool available) {
    if (!scheduler) return;
...
int scheduler_add_routing_rule(EngineScheduler *scheduler, RoutingRule *rule) {
    if (!scheduler || !rule) return -1;
    if (scheduler->routing_rule_count >= SCHEDULER_MAX_ROUTING_RULES) return -1;
...
int scheduler_remove_routing_rule(EngineScheduler *scheduler, const char *name) {
    ...
    for (int i = 0; i < scheduler->routing_rule_count; i++) {
        if (strcmp(scheduler->routing_rules[i].name, name) == 0) {
            ...
            scheduler->routing_rule_count--;
            memset(&scheduler->routing_rules[scheduler->routing_rule_count], 0,
                   sizeof(RoutingRule));
            return 0;
        }
    }
...
const char *scheduler_feature_summary(const GraphFeatures *features) {
    if (!features) return "NULL features";
    ...
    snprintf(summary, sizeof(summary),
             ...
             features->nonlinear_ratio,
             features->estimated_equation_count,
             features->estimated_degree_max);
    return summary;
}
...
SolverBackendType scheduler_select_backend(const EngineScheduler *scheduler,
        ...
        if (rule_matches(&sorted_rules[i], &features, scheduler)) {
            if (out_reason && reason_size > 0) {
                snprintf(out_reason, reason_size,
                         "Rule '%s' matched (priority=%d), backend=%d",
                         sorted_rules[i].name,
                         sorted_rules[i].priority,
                         (int)sorted_rules[i].target_backend);
            }
    ...
    if (out_reason && reason_size > 0) {
        snprintf(out_reason, reason_size,
                 "No matching rule, using default backend=%d",
...
int scheduler_solve_with_backend(EngineScheduler *scheduler,
    ...
    if (!scheduler || !graph || !out_result) return -1;
    ...
    case GROEBNER: {
        ...
        SolverStatus status = solve_algebraic_system(
            nc_graph, NULL, 0, &groebner_result);

        out_result->sat_result = solver_status_to_sat(status);

        /* 从图中提取赋值 */
        if (out_result->sat_result == SMT_RESULT_SAT) {
            extract_assignments_from_graph(graph,
                &out_result->assignments,
                &out_result->assignment_count);
    ...
    case SMT_SINGULAR:
        ...
        snprintf(out_result->error_message, sizeof(out_result->error_message),
                 ...
                 smtsolver_backend_type_name(backend_type));
        break;
    ...
    default:
        ...
        out_result->error_code = SMT_ERROR_BACKEND_UNAVAILABLE;
        snprintf(out_result->error_message, sizeof(out_result->error_message),
                 "Unknown backend type %d", (int)backend_type);
        break;
    }
...
int scheduler_solve(EngineScheduler *scheduler,
    ...
    /* 自动选择后端 */
    char reason[128];
    SolverBackendType backend = scheduler_select_backend(
        scheduler, graph, reason, sizeof(reason));

    int rc = scheduler_solve_with_backend(scheduler, graph, backend, out_result);

    /* 如果失败且启用了回退，尝试回退链 */
    if (rc != 0 && scheduler->enable_fallback &&
        out_result->sat_result == SMT_RESULT_ERROR) {
        for (int i = 0; i < scheduler->fallback_depth; i++) {
            SolverBackendType fb = scheduler->fallback_chain[i];
            if (fb == backend) continue; /* 跳过已经失败的后端 */

            SMTSolverResult fb_result;
            smtsolver_result_init(&fb_result);

            int fb_rc = scheduler_solve_with_backend(
                scheduler, graph, fb, &fb_result);
...
int scheduler_diagnose(const EngineScheduler *scheduler, char *buf, size_t buf_size) {
    if (!scheduler || !buf || buf_size == 0) return -1;
    ...
    rc = snprintf(buf + pos, buf_size - (size_t)pos,
                  ...
                  scheduler->enable_fallback ? "enabled" : "disabled",
                  scheduler->fallback_depth,
                  scheduler->auto_create ? "yes" : "no",
                  scheduler->backend_count);
    if (rc > 0) pos += rc;

    for (int i = 0; i < scheduler->backend_count; i++) {
        rc = snprintf(buf + pos, buf_size - (size_t)pos,
                      "  [%d] type=%d (%s) available=%s priority=%d desc=%s\n",
                      i,
                      (int)scheduler->backends[i].type,
                      smtsolver_backend_type_name(scheduler->backends[i].type),
                      scheduler->backends[i].available ? "yes" : "no",
                      scheduler->backends[i].priority,
    ...
    for (int i = 0; i < scheduler->routing_rule_count; i++) {
        rc = snprintf(buf + pos, buf_size - (size_t)pos,
                      ...
                      i,
                      scheduler->routing_rules[i].name,
                      scheduler->routing_rules[i].priority,
                      scheduler->routing_rules[i].enabled ? "yes" : "no",
                      scheduler->routing_rules[i].condition_count,
    ...
    rc = snprintf(buf + pos, buf_size - (size_t)pos,
                  ...
                  (long long)scheduler->stats.total_solve_time_us,
                  (long long)scheduler->stats.max_solve_time_us,
...
GroebnerResult *scheduler_convert_smt_to_groebner(
    ...
    if (!smt_result || !graph) return NULL;

    GroebnerResult *result = (GroebnerResult *)
        lv_calloc(1, sizeof(GroebnerResult));
    if (!result) return NULL;

    if (smt_result->sat_result != SMT_RESULT_SAT ||
        smt_result->assignment_count <= 0) {
        /* 无解或无赋值 */
    ...
    result->solutions = (SymbolicCoord **)
        lv_calloc((size_t)smt_result->assignment_count, sizeof(SymbolicCoord *));
    if (!result->solutions) {
        lv_free((void **)&result);
        return NULL;
    }
    ...
    for (int i = 0; i < smt_result->assignment_count; i++) {
        ...
        /* 使用缓存的值创建有理数坐标 */
        Rational *rat = rational_create(
            assign->value.rational.numerator,
            (uint64_t)assign->value.rational.denominator);
        if (!rat) continue;

        SymbolicCoord *sc = (SymbolicCoord *)
            lv_calloc(1, sizeof(SymbolicCoord));
        if (!sc) {
...
int lv_engine_schedule(const char *task_name, int priority) {
    ...
    if (!g_default_scheduler) {
        g_default_scheduler = scheduler_create();
        if (!g_default_scheduler) return -1;
    }
    ...
    if (!g_compat_engine) return -1;
    if (!g_compat_engine->main_graph) return 0;

    SMTSolverResult result;
    smtsolver_result_init(&result);

    if (strcmp(task_name, "solve") == 0) {
        scheduler_solve(g_default_scheduler,
                        g_compat_engine->main_graph, &result);
...
bool lv_engine_execute_pending(void) {
    if (!g_compat_engine || !g_default_scheduler) return false;
    if (!g_compat_engine->main_graph) return false;
