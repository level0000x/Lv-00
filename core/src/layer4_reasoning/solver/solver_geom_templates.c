...
static int template_similar_triangles(ConstraintGraph *graph, EquationSystem *sys) {
    ...
    if (!segs) { lv_free((void **) &point_ids); return 0; }
    int seg_count = 0;
    ...
    for (int a = 0; a < pt_count && added < 10; a++) {
        for (int b = a + 1; b < pt_count && added < 10; b++) {
            for (int c = b + 1; c < pt_count && added < 10; c++) {
                ...
                for (int s = 0; s < seg_count; s++) {
                    if ((segs[s].p1 == pa && segs[s].p2 == pb) || (segs[s].p1 == pb && segs[s].p2 == pa)) has_ab = true;
                    if ((segs[s].p1 == pb && segs[s].p2 == pc) || (segs[s].p1 == pc && segs[s].p2 == pb)) has_bc = true;
                    if ((segs[s].p1 == pa && segs[s].p2 == pc) || (segs[s].p1 == pc && segs[s].p2 == pa)) has_ac = true;
                }
                ...
                if (ab_len < lv_EPSILON_DOUBLE || bc_len < lv_EPSILON_DOUBLE || ac_len < lv_EPSILON_DOUBLE) continue;
        ...
        for (int b = a + 1; b < pt_count && added < 10; b++) {
            for (int c = b + 1; c < pt_count && added < 10; c++) {
                ...
                if (fabs(dot) < 1e-6 * (sqrt(cax * cax + cay * cay) * sqrt(cbx * cbx + cby * cby) + 1.0)) {
                    ...
                    mpz_t threshold; mpz_init(threshold);
...
static int template_parallel_cut(const ConstraintGraph *graph, EquationSystem *sys) {
    ...
    for (int i = 0; i < graph->node_count; i++) {
        ...
        if (len_i < lv_EPSILON_DOUBLE) continue;
        ...
        for (int j = i + 1; j < graph->node_count; j++) {
            ...
            if (len_j < lv_EPSILON_DOUBLE) continue;
            ...
            for (int ci = 0; ci < graph->constraint_count; ci++) {
                ...
                if (on_line_i && pt->coord_count >= 2) {
                    for (int cj = ci + 1; cj < graph->constraint_count; cj++) {
                        ...
                        mpz_init(poly.coeffs[1]); mpz_init(poly.coeffs[0]);
...
static int template_parallel_intercept(ConstraintGraph *graph, EquationSystem *sys) {
    ...
    typedef struct { int id; int p1, p2; double dx, dy; } SegInfo;
    SegInfo *segs = lv_calloc((size_t) graph->node_count, sizeof(SegInfo));
    if (!segs) return 0;
    int seg_count = 0;
    ...
    for (int i = 0; i < graph->node_count; i++) {
        ...
        if (n && n->type == GEOM_LINE_SEGMENT && n->coord_count >= 4) {
            segs[seg_count].id = n->id;
            segs[seg_count].p1 = -1; segs[seg_count].p2 = -1;
            segs[seg_count].dx = 0; segs[seg_count].dy = 0;
            seg_count++;
        }
    }
    ...
    for (int s = 0; s < seg_count; s++) {
        ...
        if (point_coord(n1, 0, &x1) && point_coord(n1, 1, &y1) && point_coord(n2, 0, &x2) && point_coord(n2, 1, &y2)) {
            segs[s].dx = x2 - x1; segs[s].dy = y2 - y1;
        }
    }

    for (int i = 0; i < seg_count && added < 10; i++) {
        if (segs[i].p1 < 0 || fabs(segs[i].dx) + fabs(segs[i].dy) < lv_EPSILON_DOUBLE) continue;
        for (int j = i + 1; j < seg_count && added < 10; j++) {
            if (segs[j].p1 < 0 || fabs(segs[j].dx) + fabs(segs[j].dy) < lv_EPSILON_DOUBLE) continue;
