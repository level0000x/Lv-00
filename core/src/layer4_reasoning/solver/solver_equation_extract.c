...
/* ================================================================== */
/*  VTable: ConstraintType -> 方程提取 handler                          */
/* ================================================================== */

typedef int (*ConstraintExtractEquationFunc)(const ConstraintGraph *graph, EquationSystem *sys, const Constraint *c);

/* ---- INCIDENCE: 点在线段上 ---- */
static int extract_incidence_equation(const ConstraintGraph *graph, EquationSystem *sys, const Constraint *c) {
...
/* ---- INTERSECTION: 两线相交 ---- */
static int extract_intersection_equation(const ConstraintGraph *graph, EquationSystem *sys, const Constraint *c) {
    ...
    if (got1 && got2) {
        ...
        mpz_poly_clear(&poly);
        stream_emit_simple(solver_stream_ctx, STREAM_EVENT_SOLVE_EQUATION_EXTRACTED,
                           "提取方程: 交点约束 (y)", sys->eqs.count);
    }
    return 0;
}

/* ---- CONTAINMENT: 点包含在区域内 ---- */
static int extract_containment_equation(const ConstraintGraph *graph, EquationSystem *sys, const Constraint *c) {
...
/* ---- BETWEENNESS: 点介于两点之间 ---- */
static int extract_betweenness_equation(const ConstraintGraph *graph, EquationSystem *sys, const Constraint *c) {
    if (c->participant_count < 3)
        return 0;
...
/* ---- ANGLE: 角度约束（占位，不产生方程） ---- */
static int extract_angle_equation(const ConstraintGraph *graph, EquationSystem *sys, const Constraint *c) {
    (void)graph;
    (void)sys;
    (void)c;
    return 0;
...
/* ---- CONNECTION: 距离约束连接 ---- */
static int extract_connection_equation(const ConstraintGraph *graph, EquationSystem *sys, const Constraint *c) {
    if (c->participant_count < 2)
        return 0;
...
/* ---- 静态查找表 ---- */
static const ConstraintExtractEquationFunc kConstraintEquationExtractOps[] = {
    [INCIDENCE]    = extract_incidence_equation,
    [INTERSECTION] = extract_intersection_equation,
    [BETWEENNESS]  = extract_betweenness_equation,
    [CONTAINMENT]  = extract_containment_equation,
    [ANGLE]        = extract_angle_equation,
    [CONNECTION]   = extract_connection_equation,
};
...
int solver_extract_equations_full(const ConstraintGraph *graph, EquationSystem *out_system) {
    ...
    for (int ci = 0; ci < graph->constraint_count; ci++) {
        ...
        int type = c->type;
        if (type >= 0 && type < (int)(sizeof(kConstraintEquationExtractOps) / sizeof(kConstraintEquationExtractOps[0])) && kConstraintEquationExtractOps[type]) {
            if (kConstraintEquationExtractOps[type](graph, out_system, c) != 0)
                goto push_error;
        } else {
