...
NormalizationResult *graph_normalize(ConstraintGraph *graph, bool scope_aware) {
    ...
    if (normalization_stream_ctx && phase2 > 0) {
        ...
        stream_emit_simple(normalization_stream_ctx, STREAM_EVENT_NORMALIZE_MERGE, desc, 2);
    }
    /* 将阶段2的合并记录到结果数组中 */
    ...
    if (normalization_stream_ctx && phase3 > 0) {
        ...
        stream_emit_simple(normalization_stream_ctx, STREAM_EVENT_NORMALIZE_MERGE, desc, 3);
    }
    /* 将阶段3的合并记录到结果数组中 */
