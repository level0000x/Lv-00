 ...
 * @brief 上浮操作（插入后维护堆性质）
 */
static void pq_sift_up(Lv00ProofPriority *pq, int idx)
{
    while (idx > 0) {
        int parent = (idx - 1) / 2;
 ...
 * @brief 下沉操作（删除后维护堆性质）
 */
static void pq_sift_down(Lv00ProofPriority *pq, int idx)
{
    while (1) {
        int largest = idx;
        int left  = 2 * idx + 1;
 ...
 */
static int pq_ensure_capacity(Lv00ProofPriority *pq)
{
    int new_cap;
    PQEntry *new_entries;
...
{
    Lv00ProofPriority *pq;

    pq = (Lv00ProofPriority *)calloc(1, sizeof(Lv00ProofPriority));
    if (!pq) return NULL;
