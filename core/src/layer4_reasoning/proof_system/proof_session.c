...
static const lvStrToEnumEntry s_session_status_to_string_entries[] = {
    {"ACTIVE", SESSION_STATUS_ACTIVE},
    {"COMPLETE", SESSION_STATUS_COMPLETE},
    {"ABANDONED", SESSION_STATUS_ABANDONED},
    {"ERROR", SESSION_STATUS_ERROR},
};

const char *session_status_to_string(lvSessionStatus status) {
    return lv_enum_to_str(s_session_status_to_string_entries, lv_ARRAY_SIZE(s_session_status_to_string_entries), (int) status, "UNKNOWN");
}

/** @brief step_result_to_string 名称表（按枚举值升序） */
static const lvStrToEnumEntry s_step_result_to_string_entries[] = {
    {"ACCEPTED", STEP_RESULT_ACCEPTED},
    {"REJECTED", STEP_RESULT_REJECTED},
    {"GOAL_CHANGED", STEP_RESULT_GOAL_CHANGED},
    {"PROVED", STEP_RESULT_PROVED},
    {"ERROR", STEP_RESULT_ERROR},
};

const char *step_result_to_string(lvStepResult result) {
    return lv_enum_to_str(s_step_result_to_string_entries, lv_ARRAY_SIZE(s_step_result_to_string_entries), (int) result, "UNKNOWN");
}
