...
bool axiom_package_add_known_unconstructible(AxiomPackage *pkg, KnownUnconstructible *item) {
    ...
    KnownUnconstructible *new_arr =
        lv00_realloc(pkg->known_unconstructibles, (pkg->unconstructible_count + 1) * sizeof(KnownUnconstructible));
    if (!new_arr)
        return false;
...
KnownUnconstructible *axiom_package_lookup_unconstructible(AxiomPackage *pkg, const char *name) {
    ...
    for (int i = 0; i < pkg->unconstructible_count; i++) {
        if (pkg->known_unconstructibles[i].name && strcmp(pkg->known_unconstructibles[i].name, name) == 0) {
...
bool axiom_package_register_template(AxiomPackage *pkg, ConstraintTemplate *tmpl) {
    ...
    ConstraintTemplate *new_arr = lv00_realloc(pkg->templates, (pkg->template_count + 1) * sizeof(ConstraintTemplate));
    if (!new_arr)
        return false;
...
ConstraintTemplate *axiom_package_get_template(AxiomPackage *pkg, const char *name) {
    ...
    for (int i = 0; i < pkg->template_count; i++) {
        if (pkg->templates[i].name && strcmp(pkg->templates[i].name, name) == 0) {
...
static bool parser_expect(Parser *p, TokenType type) {
    if (p->current.type != type) {
        lv00_set_error(LV00_ERROR_PARSE, "解析错误 (行 %d, 列 %d): 期望 %d, 得到 %d", p->current.line, p->current.col,
                       type, p->current.type);
        p->has_error = true;
        return false;
    }
...
static bool parse_unconstructible(Parser *p, AxiomPackage *pkg) {
    ...
    while (p->current.type != TOK_RBRACE && p->current.type != TOK_EOF && !p->has_error) {
        ...
        if (strcmp(prop, "reduces_to") == 0) {
            ...
            /* 添加到依赖链 */
            char **new_deps = lv00_realloc(uc.dependency_chain, (uc.dependency_count + 1) * sizeof(char *));
            if (new_deps) {
...
static bool parse_package_body(Parser *p, AxiomPackage *pkg) {
    while (p->current.type != TOK_RBRACE && p->current.type != TOK_EOF && !p->has_error) {
        ...
        if (strcmp(keyword, "template") == 0) {
            ...
            lv00_set_error(LV00_ERROR_PARSE, "解析错误 (行 %d): 未知的关键字 '%s'", p->current.line, keyword);
            p->has_error = true;
            break;
        }
    }
...
bool axiom_package_validate_dependencies(AxiomPackage *pkg, AxiomPackage **loaded_packages, int package_count) {
    ...
    for (int i = 0; i < pkg->unconstructible_count; i++) {
        ...
        if (uc->reduces_to && uc->reduces_to[0] != '\0') {
            ...
            if (!target) {
                lv00_set_error(LV00_ERROR_NOT_FOUND, "依赖验证失败: 问题 '%s' 的 reduces_to '%s' 未找到", uc->name,
                               uc->reduces_to);
                all_valid = false;
                /* 继续检查其他问题 */
            }
    ...
    for (int i = 0; i < factory_count; i++) {
        ...
        if (passed == factory_tests[i].expected_pass) {
            ...
            if (result.failed < total) {
                ...
                snprintf(msg, sizeof(msg), "[FACTORY] '%s': expected %s, got %s", factory_tests[i].name,
                         factory_tests[i].expected_pass ? "pass" : "fail", passed ? "pass" : "fail");
    ...
    for (int i = 0; i < user_count; i++) {
        ...
        if (passed == user_tests[i].expected_pass) {
            ...
            if (result.failed < total) {
                result.failed++;
                char msg[AXIOM_TEST_MSG_BUF_SIZE];
                snprintf(msg, sizeof(msg), "[USER] '%s': expected %s, got %s", user_tests[i].name,
                         user_tests[i].expected_pass ? "pass" : "fail", passed ? "pass" : "fail");
                result.failure_messages[result.failed - 1] = lv00_strdup_safe(msg);
            }
        }
    }
...
bool axiom_package_store_expansion_cache(AxiomPackage *pkg, const char *template_name, SymbolicCoord **params,
    ...
    if (pkg->expansion_cache_count >= pkg->expansion_cache_capacity) {
        ...
        TemplateExpansionCache *new_arr = lv00_realloc(pkg->expansion_cache, new_cap * sizeof(TemplateExpansionCache));
        if (!new_arr)
            return false;
...
int axiom_package_register_dependency_ref(AxiomPackage *pkg, const char *ref_id, const char *content_hash,
    ...
    if (pkg->dep_ref_count >= pkg->dep_ref_capacity) {
        ...
        DependencyRef *new_arr = lv00_realloc(pkg->dep_refs, (size_t) new_cap * sizeof(DependencyRef));
        if (!new_arr)
            return -2;
