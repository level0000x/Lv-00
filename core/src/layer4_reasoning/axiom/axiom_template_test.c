...
int axiom_template_test_run(AxiomPackage *pkg, TemplateTestCase **test_cases, int count, int *out_passed,
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (elapsed >= (uint64_t) timeout_ms) {
            ...
            for (int j = i; j < count; j++) {
                ...
                if (fail_count < count && test_cases[j]) {
                    char msg[128];
                    int _sn_ret;
                    lv_SAFE_SNPRINTF(_sn_ret, msg, sizeof(msg), "[SKIPPED] '%s': total time exceeded (%d ms limit)",
        ...
        if (!tmpl) {
            if (fail_count < count) {
                char msg[128];
                int _sn_ret;
                lv_SAFE_SNPRINTF(_sn_ret, msg, sizeof(msg), "[ERROR] Template '%s' not found", tc->template_name);
                (void) _sn_ret;
                failures[fail_count] = lv_strdup_safe(msg);
                if (failures[fail_count])
                    fail_count++;
        ...
        if (!tmpl->expand) {
            if (fail_count < count) {
                char msg[128];
                int _sn_ret;
                lv_SAFE_SNPRINTF(_sn_ret, msg, sizeof(msg), "[ERROR] Template '%s' has no expand function",
                                 tc->template_name);
                (void) _sn_ret;
                failures[fail_count] = lv_strdup_safe(msg);
                if (failures[fail_count])
        ...
        if (step_count > step_limit) {
            ...
            if (fail_count < count) {
                char msg[256];
                int _sn_ret;
                lv_SAFE_SNPRINTF(_sn_ret, msg, sizeof(msg), "[TIMEOUT] '%s': step count %d exceeds limit %d (nodes=%d)",
                                 tc->template_name, step_count, step_limit, expanded->node_count);
                (void) _sn_ret;
                failures[fail_count] = lv_strdup_safe(msg);
                if (failures[fail_count])
                    fail_count++;
        ...
        if (actual_pass == tc->expected_result) {
            ...
            if (fail_count < count) {
                ...
                lv_SAFE_SNPRINTF(_sn_ret, msg, sizeof(msg),
                                 "[FAIL] '%s': expected %s, got %s (nodes=%d, constraints=%d)", tc->template_name,
                                 tc->expected_result ? "pass" : "fail", actual_pass ? "pass" : "fail",
                                 expanded->node_count, expanded->constraint_count);
                (void) _sn_ret;
                failures[fail_count] = lv_strdup_safe(msg);
                if (failures[fail_count])
                    fail_count++;
    ...
    *out_failures = failures;
    ...
    if (timed_out > 0 || skipped > 0) {
        LOG_WARN("axiom_template_test", "Smoke test completed with %d timed out, %d skipped (of %d total)", timed_out,
                 skipped, count);
    }
...
bool axiom_template_verify_normal_form(AxiomPackage *pkg, const char *template_name) {
    ...
    if (!tmpl) {
        LOG_ERROR("axiom_template_test", "Template '%s' not found for normal form verification", template_name);
        return false;
    }
    ...
    if (tmpl->normal_form.constraint_type_count <= 0 && tmpl->normal_form.node_type_count <= 0) {
        LOG_INFO("axiom_template_test", "Template '%s' has no normal form defined, skipping", template_name);
        return true;
    }
    ...
    for (int i = 0; i < tmpl->normal_form.constraint_type_count && pos < 240; i++) {
        /* 追加约束类型编号 */
        int _n;
        lv_SAFE_SNPRINTF(_n, &canonical_buf[pos], (size_t) (256 - pos), "%d(",
                         tmpl->normal_form.expected_constraint_types[i]);
        pos += _n;
        ...
        for (int j = 0; j < tmpl->normal_form.node_type_count && pos < 240; j++) {
            ...
            lv_SAFE_SNPRINTF(_n, &canonical_buf[pos], (size_t) (256 - pos), "%d",
                             tmpl->normal_form.expected_node_types[j]);
            pos += _n;
        }
        if (pos < 255) {
            canonical_buf[pos++] = ')';
