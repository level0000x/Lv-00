...
/** 添加剪枝操作到记录 */
static bool add_pruning_operation(PruningRecord *record,
                                   const PruningOperation *op) {
...
MetaProofResult meta_prove_direct_contradiction(MetaProofContext *ctx,
    ...
    /* 获取节点的所有约束 */
    int constraint_count = constraint_graph_get_constraints_for_node(
        ctx->graph, node_id, NULL, 0);
    ...
    constraint_graph_get_constraints_for_node(
        ctx->graph, node_id, constraint_ids, constraint_count);
...
MetaProofResult meta_prove_pruning(MetaProofContext *ctx,
    ...
    /* L1: 直接矛盾 */
    if (ctx->enable_l1) {
        MetaProofResult result = meta_prove_direct_contradiction(
            ctx, node_id, candidate, NULL);
    ...
    /* L2: 传播矛盾 */
    if (ctx->enable_l2 && ctx->prop_ctx) {
        MetaProofResult result = meta_prove_propagation_contradiction(
            ctx, node_id, candidate);
        if (result == META_PROVE_VALID) {
            return result;
        }
    ...
    /* L3: 代数排除 */
    if (ctx->enable_l3) {
        MetaProofResult result = meta_prove_algebraic_exclusion(
            ctx, node_id, candidate);
        if (result == META_PROVE_VALID) {
            return result;
        }
...
void meta_proof_record_pruning(MetaProofContext *ctx,
                                ...
                                TrustColor trust) {
    if (!ctx || !ctx->record || !removed || count <= 0) return;
