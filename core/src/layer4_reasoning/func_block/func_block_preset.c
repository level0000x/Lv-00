...
static struct {
    ...
    .count = 0, .initialized = false, .next_preset_id = LV00_PRESET_ID_OFFSET /**< 预设ID起始偏移 */
};
...
const char *func_block_preset_param_type_string(PresetParamType type) {
    switch (type) {
        ...
        case PARAM_TYPE_VECTOR:
            return "向量";
        case PARAM_TYPE_SCALAR:
            return "标量";
        case PARAM_TYPE_BOOLEAN:
            return "布尔值";
        case PARAM_TYPE_CURVE:
            return "曲线";
        case PARAM_TYPE_SURFACE:
            return "曲面";
        case PARAM_TYPE_ANY:
            return "任意类型";
...
const char *func_block_preset_complexity_string(PresetComplexity complexity) {
    switch (complexity) {
        case COMPLEXITY_O1:
            return "O(1) - 常数时间";
        case COMPLEXITY_OLOGN:
            return "O(log n) - 对数时间";
        case COMPLEXITY_ON:
            return "O(n) - 线性时间";
        case COMPLEXITY_ONLOGN:
            return "O(n log n) - 线性对数";
        case COMPLEXITY_ON2:
            return "O(n²) - 平方时间";
        case COMPLEXITY_ON3:
            return "O(n³) - 立方时间";
        case COMPLEXITY_UNKNOWN:
            return "未知";
        default:
            return "未知";
    }
