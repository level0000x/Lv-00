...
bool preset_query(const PresetQueryCriteria *criteria, PresetQueryResult **out_result) {
    ...
    const char **candidate_names = (const char **) lv00_malloc((size_t) max_candidates * sizeof(const char *));
    if (!candidate_names) {
        unlock_library();
    ...
    if (match_count > 0) {
        result->names = (const char **) lv00_malloc((size_t) match_count * sizeof(const char *));
        if (!result->names) {
...
bool preset_instantiate(const char *name, const int *input_nodes, int input_count,
    ...
    if (entry->template_fb) {
        ...
        if (instance->output_count > 0 && entry->template_fb->output_port_ids) {
            instance->output_node_ids = (int *) lv00_malloc((size_t) instance->output_count * sizeof(int));
...
int preset_instantiate_batch(const char **names, const int **input_nodes_array, const int *input_counts, int count,
    ...
    /* 分配实例数组 */
    PresetInstanceHandle *instances =
        (PresetInstanceHandle *) lv00_malloc((size_t) count * sizeof(PresetInstanceHandle));
    if (!instances) {
        set_error("内存分配失败");
        return 0;
    }
    ...
    *out_is_valid = false;
    ...
    if (!inst->func_block) {
        if (out_error_message) {
            *out_error_message = lv00_strdup_safe("验证失败: 实例没有关联的函数块");
        }
        return true;
    }
    ...
    if (inst->func_block->input_count < 0) {
        if (out_error_message) {
            *out_error_message = lv00_strdup_safe("验证失败: 输入端口数量无效");
        }
        return true;
    }
...
bool preset_library_init(void) {
    ...
    g_library.hash_table =
        (InternalPresetEntry **) lv00_calloc(g_library.hash_table_size, sizeof(InternalPresetEntry *));
...
bool preset_register_custom(const PresetMetadata *metadata, const FuncBlock *template_fb,
    ...
    /* 创建新条目 */
    InternalPresetEntry *entry = (InternalPresetEntry *) lv00_malloc(sizeof(InternalPresetEntry));
    if (entry == NULL) {
        unlock_library();
        set_error("内存分配失败");
        return false;
    ...
    entry->metadata.description = metadata->description ? lv00_strdup(metadata->description) : NULL;
    entry->metadata.mathematical_def = metadata->mathematical_def ? lv00_strdup(metadata->mathematical_def) : NULL;
