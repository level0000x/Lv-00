 ...
 */
static PresetHashTable g_hash_table = {.buckets = NULL, .bucket_count = 0, .node_count = 0, .dirty = true};
...
static bool hash_remove_entry(const char *name) {
    if (g_hash_table.bucket_count == 0 || !g_hash_table.buckets)
        return false;
...
static bool hash_rebuild(void) {
    ...
    g_hash_table.buckets = (HashNode **) lv00_malloc((size_t) bucket_count * sizeof(HashNode *));
    if (!g_hash_table.buckets) {
        g_hash_table.bucket_count = 0;
        g_hash_table.dirty = true;
        return false;
    ...
    for (int i = 0; i < entry_count; i++) {
        if (!g_registry.entries[i].name)
            continue;
        if (!hash_insert_entry(g_registry.entries[i].name, &g_registry.entries[i])) {
            /* 插入失败，销毁哈希表并返回错误 */
            hash_destroy();
            return false;
        }
...
static bool ensure_registry_capacity(void) {
    ...
    PresetEntry *new_entries = lv00_realloc(g_registry.entries, (size_t) new_capacity * sizeof(PresetEntry));
    if (!new_entries) {
...
bool func_block_register(const char *name, const char *description, PresetCategory category, FuncBlock *fb) {
     ...
     */
    bool result = add_preset_entry_ex(name, description, category, fb, true, true);
    if (result) {
        /* 【哈希表延迟更新】注册成功后仅设置脏标志，
