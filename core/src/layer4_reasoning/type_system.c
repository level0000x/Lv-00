 ...
 *
 * @par 设计要点
 * - 宇宙层级类型论：类型按层级组织（V_0 基础层 -> V_1 幂集层 -> V_2+ 高阶层）
 * - 支持 9 种类型种类，涵盖几何基础类型和高阶构造类型
 * - 类型推断采用规则表驱动策略，支持用户注册自定义推理规则
 * - 非良基模式检测防止无限类型递归
 * - 等价检查结合结构等价和类型变量统一
 *
 * @par 依赖关系
 * - 上层: 被 proof.c（端口类型检查）、engine.c（类型验证）调用
 * - 下层: 依赖 lv00_utils.c（内存管理）、lv00_internal.h（内部常量）
 * - 同层: 与 rewrite.c 协作进行类型等价的重写路径检查，与 stream.c 协作事件输出
 */
