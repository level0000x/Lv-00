...
static bool scope_push(LambdaScope *scope, int port_id) {
    ...
    if (scope->depth >= scope->capacity) {
        if (scope->capacity > INT_MAX / 2) return false;
...
static int scope_lookup(const LambdaScope *scope, int index) {
    if (!scope || index < 0 || index >= scope->depth) return -1;
...
static bool node_is_formal_param(const ConstraintGraph *graph, int node_id) {
    ...
    if (!node || node->type != GEOM_PORT) return false;
 ...
 * @return true 找到连接
 */
static bool find_connection_target(const ConstraintGraph *graph, int node_id,
                                    int direction, int *out_target)
{
    if (!graph || !out_target) return false;
    *out_target = -1;
 ...
 * @return true 编译成功
 */
static bool lambda_to_graph_internal(LvLambdaTerm *term, ConstraintGraph *graph,
                                      LambdaScope *scope, int depth,
                                      int *out_node_id)
{
    if (!term || !graph || !scope || !out_node_id) return false;
    *out_node_id = -1;
    ...
    case LV_LAMBDA_VAR: {
        ...
        if (port_id < 0) {
            LOG_ERROR("lambda_to_graph", "自由的 De Bruijn 索引 %d（作用域深度 %d）",
                      index, scope->depth);
            return false;
        }
        *out_node_id = port_id;
    ...
    case LV_LAMBDA_ABS: {
        ...
        nr = graph_add_function_block(graph, internal_ids, internal_count,
                                       input_ids, 1, output_ids, 1);
        lv_free((void**)&internal_ids);
        ...
        LOG_DEBUG("lambda_to_graph", "编译 ABS: fb=%d, input=%d, output=%d, internals=%d",
                  fb_id, input_port_id, output_port_id, internal_count);
    ...
    case LV_LAMBDA_APP: {
        ...
        if (left_node->type == GEOM_FUNCTION_BLOCK) {
            ...
            if (left_node->data.func_block.output_count > 0 &&
                left_node->data.func_block.output_port_ids) {
        ...
        if (right_node->type == GEOM_FUNCTION_BLOCK) {
            /* 实参为函数块时，取其输入端口 */
            if (right_node->data.func_block.input_count > 0 &&
                right_node->data.func_block.input_port_ids) {
        ...
        if (left_output >= 0 && right_input >= 0) {
            ...
            if (cr != ADD_CONSTRAINT_OK && cr != ADD_CONSTRAINT_DUPLICATE) {
                LOG_WARN("lambda_to_graph", "APP 连接 %d→%d 失败 (result=%d)",
                         left_output, right_input, (int)cr);
            }
        }

        LOG_DEBUG("lambda_to_graph", "编译 APP: left=%d, right=%d, conn=%d→%d",
                  left_node_id, right_node_id, left_output, right_input);
...
bool lambda_to_graph(LvLambdaTerm *term, ConstraintGraph *graph, int *out_node_id) {
    if (!term || !graph || !out_node_id) return false;
 ...
 */
static LvLambdaTerm *graph_to_lambda_internal(ConstraintGraph *graph, int node_id,
                                               int *binder_port_ids, int binder_count,
                                               int binder_capacity)
{
    ...
    case GEOM_FUNCTION_BLOCK: {
        if (node->data.func_block.input_count < 1 ||
        ...
        if (node->data.func_block.output_count > 0 &&
        ...
        LOG_DEBUG("graph_to_lambda", "函数块 %d: input=%d, output=%d, body_root=%d",
                  node_id, input_port_id, output_port_id, body_root_id);

        /* 递归重建 body */
        LvLambdaTerm *body = graph_to_lambda_internal(graph, body_root_id,
                                                       new_binders, binder_count + 1,
                                                       new_cap);
     ...
     * ============================================================ */
    default:
        LOG_WARN("graph_to_lambda", "不支持的节点类型 %d (id=%d)，尝试作为 VAR",
                 (int)node->type, node_id);
        /* 兜底：创建变量项 */
...
LvLambdaTerm *graph_to_lambda(ConstraintGraph *graph, int node_id) {
    ...
    LvLambdaTerm *result = graph_to_lambda_internal(graph, node_id,
                                                     binder_port_ids, binder_count,
                                                     binder_capacity);
