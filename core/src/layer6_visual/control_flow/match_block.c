...
lvMatchBlock *lv_match_block_create(int case_count) {
    ...
    if (!block)
        lv_RETURN_ERROR_NULL(lv_ERROR_ALLOCATION_FAILED, "failed to allocate match block");
    block->input_port = -1;
    block->output_port = -1;
    if (case_count > 0) {
        ...
        if (!block->cases) {
            lv_free((void **) &block);
            lv_RETURN_ERROR_NULL(lv_ERROR_ALLOCATION_FAILED, "failed to allocate cases array");
        }
