...
void lv_effect_tracker_destroy(lvEffectTracker *tracker) {
    ...
    if (tracker->current_effect) lv_effect_annotation_destroy(tracker->current_effect);
    lv_free((void **)&tracker);
}
...
void lv_effect_tracker_record(lvEffectTracker *tracker, lvEffectType effect,
    ...
    if (tracker->entry_count >= tracker->entry_capacity) {
        ...
        lvEffectLogEntry *new_entries = (lvEffectLogEntry *)lv_realloc(tracker->entries, (size_t)new_cap * sizeof(lvEffectLogEntry));
...
lvEffectAnnotation *lv_effect_compose(const lvEffectAnnotation *a,
    ...
    if (count == 0) { lv_free((void **)&result); return NULL; }
