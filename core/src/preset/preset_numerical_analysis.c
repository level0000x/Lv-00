...
/** 数值分析模块预设函数块总数 */
#define NUMERICAL_ANALYSIS_PRESET_COUNT 25
 ...
 * @return false 注册失败
 */
static bool register_numerical_analysis_preset(const char *name, const char *description, const PresetType *input_types,
...
bool preset_numerical_analysis_register(void) {
    ...
    /* -------------------- 梯形法则 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(
                ...
                "梯形法则数值积分：∫ₐᵇ f(x)dx ≈ h/2·[f(a) + 2Σf(xᵢ) + f(b)]，h = (b-a)/n", inputs, 4,
                PRESET_TYPE_SCALAR,
    ...
    /* -------------------- 辛普森法则 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(
                ...
                "辛普森法则数值积分：∫ₐᵇ f(x)dx ≈ h/3·[f(a) + 4Σf(x₂ᵢ₋₁) + 2Σf(x₂ᵢ) + f(b)]", inputs, 4,
                PRESET_TYPE_SCALAR,
    ...
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR, PRESET_TYPE_INTEGER};
        if (register_numerical_analysis_preset(PRESET_NUMERICAL_INTEGRAL_GAUSS,
    ...
    /* -------------------- 龙贝格积分 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR, PRESET_TYPE_INTEGER};
        if (register_numerical_analysis_preset(
                PRESET_NUMERICAL_INTEGRAL_ROMBERG, "龙贝格积分：通过理查森外推递推加速梯形序列，达到高精度积分", inputs,
                4, PRESET_TYPE_SCALAR, "R_{m,k} = \\frac{4^k R_{m,k-1} - R_{m-1,k-1}}{4^k - 1}", "O(n²)", true,
                false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 二分法求根 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(
                PRESET_ROOT_BISECTION, "二分法求根：在区间 [a,b] 中迭代缩小区间，收敛精度 |b-a| < ε", inputs, 4,
    ...
    /* -------------------- 牛顿法求根 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(PRESET_ROOT_NEWTON, "牛顿法求根：xₙ₊₁ = xₙ - f(xₙ)/f'(xₙ)，二阶收敛速度",
                                               inputs, 4, PRESET_TYPE_SCALAR, "x_{n+1} = x_n - \\frac{f(x_n)}{f'(x_n)}",
    ...
    /* -------------------- 割线法求根 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        if (register_numerical_analysis_preset(
                PRESET_ROOT_SECANT, "割线法求根：xₙ₊₁ = xₙ - f(xₙ)·(xₙ - xₙ₋₁)/(f(xₙ) - f(xₙ₋₁))，超线性收敛", inputs,
                4, PRESET_TYPE_SCALAR, "x_{n+1} = x_n - f(x_n) \\cdot \\frac{x_n - x_{n-1}}{f(x_n) - f(x_{n-1})}",
                "O(log²(1/ε))", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 不动点迭代 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(
                PRESET_ROOT_FIXED_POINT, "不动点迭代：xₙ₊₁ = g(xₙ)，当 |g'(x*)| < 1 时收敛到不动点 x*", inputs, 3,
    ...
    /* -------------------- 非线性方程组牛顿法 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(
                PRESET_SYSTEM_NEWTON, "非线性方程组牛顿法：Xₙ₊₁ = Xₙ - J⁻¹(Xₙ)·F(Xₙ)，J 为雅可比矩阵", inputs, 3,
                PRESET_TYPE_TUPLE,
    ...
    /* -------------------- 拉格朗日插值 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_LIST, PRESET_TYPE_LIST, PRESET_TYPE_SCALAR};
        if (register_numerical_analysis_preset(
                PRESET_INTERPOLATION_LAGRANGE, "拉格朗日插值：L(x) = Σ yᵢ·Π(x - xⱼ)/(xᵢ - xⱼ)，j≠i", inputs, 3,
                PRESET_TYPE_SCALAR,
    ...
    /* -------------------- 牛顿均差插值 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(
                ...
                "牛顿均差插值：N(x) = f[x₀] + f[x₀,x₁](x-x₀) + f[x₀,x₁,x₂](x-x₀)(x-x₁) + ...", inputs, 3,
    ...
    /* -------------------- 线性样条插值 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(
                ...
                inputs, 3, PRESET_TYPE_SCALAR, "S_i(x) = y_i + \\frac{y_{i+1} - y_i}{x_{i+1} - x_i}(x - x_i)", "O(n)",
                true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 三次样条插值 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(
                ...
                inputs, 3, PRESET_TYPE_SCALAR, "S_i(x) = a_i + b_i(x-x_i) + c_i(x-x_i)^2 + d_i(x-x_i)^3", "O(n)", true,
                false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 切比雪夫插值 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(
                ...
                "切比雪夫插值：在切比雪夫节点 xₖ = cos((2k+1)π/(2n)) 上插值，最小化龙格现象", inputs, 4,
    ...
    /* -------------------- 前向差分 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        if (register_numerical_analysis_preset(
                PRESET_DIFFERENTIATION_FORWARD, "前向差分近似导数：f'(x) ≈ (f(x+h) - f(x)) / h，一阶精度 O(h)", inputs,
                3, PRESET_TYPE_SCALAR, "f'(x) \\approx \\frac{f(x+h) - f(x)}{h} + O(h)", "O(1)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 后向差分 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        if (register_numerical_analysis_preset(
                PRESET_DIFFERENTIATION_BACKWARD, "后向差分近似导数：f'(x) ≈ (f(x) - f(x-h)) / h，一阶精度 O(h)", inputs,
                3, PRESET_TYPE_SCALAR, "f'(x) \\approx \\frac{f(x) - f(x-h)}{h} + O(h)", "O(1)", true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 中心差分 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR};
        if (register_numerical_analysis_preset(
                PRESET_DIFFERENTIATION_CENTRAL, "中心差分近似导数：f'(x) ≈ (f(x+h) - f(x-h)) / (2h)，二阶精度 O(h²)",
                inputs, 3, PRESET_TYPE_SCALAR, "f'(x) \\approx \\frac{f(x+h) - f(x-h)}{2h} + O(h^2)", "O(1)", true,
                false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 理查森外推 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(
                ...
                inputs, 4, PRESET_TYPE_SCALAR, "D(h) = \\frac{2^{2n} D(h/2) - D(h)}{2^{2n} - 1} + O(h^{2n+2})", "O(n)",
                true, false)) {
            success_count++;
        }
    }
    ...
    /* -------------------- 欧拉方法 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR,
                               PRESET_TYPE_INTEGER};
        if (register_numerical_analysis_preset(
                PRESET_ODE_EULER, "欧拉方法求解ODE：yₙ₊₁ = yₙ + h·f(xₙ, yₙ)，一阶精度 O(h)", inputs, 5,
    ...
    /* -------------------- 四阶龙格-库塔方法 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR,
                               PRESET_TYPE_INTEGER};
        if (register_numerical_analysis_preset(
                PRESET_ODE_RK4, "四阶龙格-库塔方法：yₙ₊₁ = yₙ + h/6·(k₁ + 2k₂ + 2k₃ + k₄)，四阶精度 O(h⁴)", inputs, 5,
    ...
    /* -------------------- 自适应步长方法 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_FUNCTION, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR,
                               PRESET_TYPE_SCALAR};
        if (register_numerical_analysis_preset(
                PRESET_ODE_ADAPTIVE, "自适应步长方法（RK45）：根据局部误差估计自动调整步长，平衡精度与效率", inputs, 5,
                PRESET_TYPE_LIST,
    ...
    /* -------------------- ODE方程组求解 -------------------- */
    {
        PresetType inputs[] = {PRESET_TYPE_LIST, PRESET_TYPE_LIST, PRESET_TYPE_SCALAR, PRESET_TYPE_SCALAR,
                               PRESET_TYPE_INTEGER};
        if (register_numerical_analysis_preset(
                PRESET_ODE_SYSTEM, "ODE方程组求解：使用RK4方法同时求解多个耦合的一阶常微分方程", inputs, 5,
                PRESET_TYPE_LIST,
    ...
    /* -------------------- LU分解 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(
                ...
                "LU分解：将矩阵 A 分解为下三角矩阵 L 和上三角矩阵 U，A = LU，用于高效求解线性方程组", inputs, 1,
                PRESET_TYPE_TUPLE,
    ...
    /* -------------------- 特征值计算 -------------------- */
    {
        ...
        if (register_numerical_analysis_preset(
                PRESET_MATRIX_EIGENVALUES, "特征值计算：使用QR算法迭代求解矩阵的所有特征值，Av = λv", inputs, 2,
