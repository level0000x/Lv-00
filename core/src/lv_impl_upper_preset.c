...
/** 获取预设的逆预设名称（委托 func_block_preset_get_inverse；无逆名返回 NULL） */
const char *func_block_preset_inverse_name(lvEngine *ctx, const char *name) {
    (void) ctx;
    if (!name)
        return NULL;
    return func_block_preset_get_inverse(name);
}
...
/** 获取注册时间戳（PresetEntry 无 registration_time 字段，显式不支持而非模拟固定值） */
int64_t func_block_preset_registration_time(lvEngine *ctx, const char *name) {
    ...
    if (!entry)
        return -1; /* 未找到 */
    lv_RETURN_ERROR(lv_ERROR_UNSUPPORTED, "func_block_preset_registration_time: no registration_time field");
}
