...
static const ResultMessageEntry g_result_table[] = {
    {lv_RESULT_SUCCESS, "求解成功", "所有约束已满足，几何构造一致"},

    {lv_RESULT_NO_SOLUTION, "无解", "约束系统矛盾，无法找到满足所有约束的几何构造"},

    {lv_RESULT_INFINITE_SOLUTIONS, "无穷多解", "约束不足，存在无穷多个满足条件的几何构造"},

    {lv_RESULT_TIMEOUT, "求解超时", "求解过程未能在规定时间内完成，请尝试简化问题或增加超时时间"},

    {lv_RESULT_CANCELLED, "操作已取消", "求解过程被用户或系统取消"},

    {lv_RESULT_NUMERIC_ERROR, "数值计算错误", "求解过程中遇到数值不稳定或精度溢出"},

    {lv_RESULT_MEMORY_ERROR, "内存错误", "系统内存不足，无法完成求解"},

    {lv_RESULT_GRAPH_ERROR, "约束图错误", "约束图结构异常，可能包含损坏的节点或约束"},

    {lv_RESULT_NORMALIZE_SUCCESS, "归一化成功", "约束图已成功归一化，同构结构已合并"},

    {lv_RESULT_NORMALIZE_PARTIAL, "归一化部分完成", "约束图部分归一化，某些子图可能需要手动处理"},

    {lv_RESULT_REWRITE_SUCCESS, "重写成功", "所有匹配的重写规则已成功应用"},

    {lv_RESULT_REWRITE_NO_MATCH, "无匹配规则", "当前约束图中未找到可匹配的重写规则"},

    {lv_RESULT_PROOF_VERIFIED, "证明已验证", "命题证明已通过验证，结论成立"},

    {lv_RESULT_PROOF_FAILED, "证明失败", "命题证明验证失败，可能存在推理错误"},

    {lv_RESULT_PROOF_INCOMPLETE, "证明不完整", "证明存在未完成的分支或未解决的子目标"},
};
