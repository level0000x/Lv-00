...
const char *lv_status_message(int code) {
     ...
     * 未收录判定使用 error_codes.h 的显式查询接口 lv_error_is_unknown()，
     * 不再与 "未知错误码" 占位符字面量耦合。 */
    const char *msg = lv_error_string((lvErrorCode) code);
    return lv_error_is_unknown((lvErrorCode) code) ? "未知状态码" : msg;
}
 ...
 * @brief 状态码类别区间映射表
 *
 * 区间定义与类别键来自 error_codes.h 的 LV_ERROR_CATEGORY_RANGES_X
 * （粗粒度类别区间的单一事实来源），此处仅作宏展开，不再手写区间。
 * 类别长名经 LV_EC_CAT_LONG(键) 从 LV_ERROR_CATEGORIES_X 单点派生，与
 * LV_ERROR_CODES_X 第 5 字段（每码类别短名）共用同一类别键，无需手工同步。
 * "警告" 区间（负码 INT_MIN..-1）是状态码模块特有语义，不属于错误码体系，
 * 单独保留于此。
 */
#define LV_X_STATUS_RANGE_ITEM(min, max, category) {min, max, LV_EC_CAT_LONG(category)},
static const struct {
    int min;
    int max;
    const char *category;
...
const char *lv_status_name(int code) {
    /* 委托给统一错误码系统的规范错误表；未收录的状态码保持原有回退文本。
     * 未收录判定使用 error_codes.h 的显式查询接口 lv_error_is_unknown()，
     * 不再与 "UNKNOWN_ERROR" 占位符字面量耦合。 */
    const char *name = lv_error_name((lvErrorCode) code);
    return lv_error_is_unknown((lvErrorCode) code) ? "lv_UNKNOWN" : name;
}
