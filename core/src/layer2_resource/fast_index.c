 ...
 *   - 均匀网格标准语义：全局统一单元大小 = 插入包围盒 max(w,h) 的中位数
 *     （单元大小必须全局一致才能用 Cantor 配对哈希定位网格单元）
 *
 ...
 * 键为 int64_t + void* 值）：
 *   以"i64 键 = Cantor 配对值（位模式转 int64）、值 = 格内 CellEntry 链表头"
 *   的方案重新核实，仍不迁移：
 *     a. 键可无损承载：Cantor 配对 uint64 → int64_t 位模式，lv_hashtable_i64
 *        按位模式判等（测试已覆盖大 uint64 键），键唯一性问题已消除。
 *     b. 值承载多值链：值 = 链表头指针可存多值，但链表仍需表外手写维护
 *        （CellEntry 的分配/头插/释放），且 destroy 需 foreach 遍历释放每条
 *        链 —— 与 lv_fast_index_destroy 现有释放路径等量，纯多一层间接。
 *     c. 查询语义差异：原 query 直接返回 cell_hash 桶内全部条目（含哈希
 *        碰撞落同桶的其它格条目，作为"候选超集"交给调用者做精确检查）；
 *        迁移后 get 只返回精确键所在格的条目，返回集合变化，破坏既有
 *        "候选超集"行为（对外行为一致性要求下不可接受）。
 *     d. 扩容语义：i64 形态自动扩容按负载因子 0.75（原实现 capacity/4 且
 *        上限 4096），键 = 配对值不变、重哈希正确，但容量策略与内存峰值
 *        行为不同，且理由 b/c 已足以否决。
 * 结论：i64 形态消除了"键宽"障碍，但"多值链表外维护 + 查询碰撞超集语义
 * 改变 + 零性能收益"三项理由依旧成立，强行收敛收益为负，故保留。
 *
