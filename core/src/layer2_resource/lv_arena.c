 ...
 */
static void *arena_alloc_impl(lvArena *arena, size_t size) {
    if (!arena || size == 0) return NULL;

    size_t aligned = align_up(size, LV_ARENA_ALIGNMENT);

    /* 检查当前块是否有足够空间 */
    if (!arena->head || arena->head->used + aligned > arena->head->capacity) {
        /* 计算新块大小：翻倍策略 */
...
void *lv_arena_alloc(lvArena *arena, size_t size) {
    if (!arena) return NULL;
    lv_arena_lock(arena);
    void *ptr = arena_alloc_impl(arena, size);
...
char *lv_arena_strdup(lvArena *arena, const char *str) {
    ...
    lv_arena_lock(arena);
    char *ptr = (char *)arena_alloc_impl(arena, len + 1);
    if (ptr) {
