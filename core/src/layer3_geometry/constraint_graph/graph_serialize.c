...
char *graph_node_serialize_to_json(const GeomNode *node) {
    ...
    switch (node->type) {
        ...
        case GEOM_LINE_SEGMENT: {
            ...
            if (half < 2)
                half = 2;
            lv_json_buf_append_raw(&buf, "\"endpoint1_start\":0,");
            lv_json_buf_append_raw(&buf, "\"endpoint2_start\":");
            snprintf(id_str, sizeof(id_str), "%d", half);
            lv_json_buf_append_raw(&buf, id_str);
            lv_json_buf_append_raw(&buf, ",");
            lv_json_buf_append_raw(&buf, "\"coord_count\":");
        ...
        case GEOM_REGION: {
            lv_json_buf_append_raw(&buf, "\"boundary_segments\":[");
            ...
            lv_json_buf_append_raw(&buf, "],");
            lv_json_buf_append_raw(&buf, "\"segment_count\":");
        ...
        case GEOM_PORT: {
            if (node->data.port) {
                lv_json_buf_append_raw(&buf, "\"port_type\":\"");
                lv_json_buf_append_raw(&buf, node->data.port->type == PORT_INPUT ? "INPUT" : "OUTPUT");
                lv_json_buf_append_raw(&buf, "\",");
                lv_json_buf_append_raw(&buf, "\"is_formal_param\":");
                lv_json_buf_append_raw(&buf, node->data.port->is_formal_param ? "true" : "false");
                lv_json_buf_append_raw(&buf, ",");
                lv_json_buf_append_raw(&buf, "\"is_polymorphic\":");
                lv_json_buf_append_raw(&buf, node->data.port->is_polymorphic ? "true" : "false");
            }
            break;
        }

        case GEOM_FUNCTION_BLOCK: {
            lv_json_buf_append_raw(&buf, "\"internal_nodes\":[");
            ...
            lv_json_buf_append_raw(&buf, "\"determinism_state\":");
            switch (node->data.func_block.determinism_state) {
                case UNVERIFIED:
                    lv_json_buf_append_raw(&buf, "\"UNVERIFIED\"");
                    break;
                case VERIFIED:
                    lv_json_buf_append_raw(&buf, "\"VERIFIED\"");
                    break;
                case NON_DETERMINISTIC:
                    lv_json_buf_append_raw(&buf, "\"NON_DETERMINISTIC\"");
                    break;
                case PARTIALLY_VERIFIED:
                    lv_json_buf_append_raw(&buf, "\"PARTIALLY_VERIFIED\"");
                    break;
            }
            break;
        }
