 ...
 */
int geo_point_on_segment(double px, double py,
                         double x1, double y1,
                         double x2, double y2)
{
    /* 叉积判断共线 */
    double cross = (px - x1) * (y2 - y1) - (py - y1) * (x2 - x1);
    if (fabs(cross) > GEO_EPSILON) return 0;
 ...
 * @return 有符号面积的两倍
 */
double geo_signed_area_2x(double x1, double y1,
                          double x2, double y2,
                          double x3, double y3)
{
 ...
 */
int geo_point_in_triangle(double px, double py,
                          double x1, double y1,
                          double x2, double y2,
                          double x3, double y3)
{
    ...
    if (has_neg && has_pos) return -1; /* 外部 */
 ...
 */
int geo_segments_intersect(double ax1, double ay1, double ax2, double ay2,
                           double bx1, double by1, double bx2, double by2)
{
    ...
    /* 一般相交 */
    if (((d1 > GEO_EPSILON && d2 < -GEO_EPSILON) ||
         (d1 < -GEO_EPSILON && d2 > GEO_EPSILON)) &&
        ((d3 > GEO_EPSILON && d4 < -GEO_EPSILON) ||
         (d3 < -GEO_EPSILON && d4 > GEO_EPSILON))) {
        return 1;
    }

    /* 退化情况：检查端点是否在另一条线段上 */
    if (fabs(d1) < GEO_EPSILON && geo_point_on_segment(ax1, ay1, bx1, by1, bx2, by2)) return 1;
    if (fabs(d2) < GEO_EPSILON && geo_point_on_segment(ax2, ay2, bx1, by1, bx2, by2)) return 1;
    if (fabs(d3) < GEO_EPSILON && geo_point_on_segment(bx1, by1, ax1, ay1, ax2, ay2)) return 1;
    if (fabs(d4) < GEO_EPSILON && geo_point_on_segment(bx2, by2, ax1, ay1, ax2, ay2)) return 1;
