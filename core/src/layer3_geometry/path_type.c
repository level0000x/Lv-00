...
int path_to_equality(lvPathSystem *sys, int path_id, ConstraintGraph **out_equality) {
    ...
    for (int i = 0; i < chain_len; i++) {
        ...
        /* 使用节点 ID 生成区分性的符号坐标 */
        SymbolicCoord *coord_x;
        SymbolicCoord *coord_y;
        if (!symbolic_coord_pair_create_rational(nid * lv_RATIONAL_SCALE_LOW + 1, lv_RATIONAL_SCALE_LOW,
                                                 nid * lv_RATIONAL_SCALE_LOW + 2, lv_RATIONAL_SCALE_LOW, &coord_x,
                                                 &coord_y)) {
            graph_destroy(eq);
            lv_RETURN_ERROR(lv_ERROR_OUT_OF_MEMORY, "path_to_equality: 创建符号坐标失败");
        }
