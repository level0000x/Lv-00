...
bool ga_three_points_collinear(const Lv00MultiVector *p1,
    ...
    double cross_norm_sq = cx * cx + cy * cy + cz * cz;
    return cross_norm_sq < 1e-20;
}
...
bool ga_four_points_coplanar(const Lv00MultiVector *p1,
    ...
    /* 提取笛卡尔坐标，使用标量三重积判断共面
     * 四点共面当且仅当 (P2-P1) . ((P3-P1) x (P4-P1)) = 0 */
    ...
    /* 标量三重积 (P2-P1) . ((P3-P1) x (P4-P1)) */
    double triple = ax * cross_x + ay * cross_y + az * cross_z;

    return fabs(triple) < 1e-10;
}
