...
static double eval_b_div(const FormulaNode *node, double x, double y) {
            ...
            return (fabs(r) > lv_EPSILON_SUPERTINY) ? l / r : 0.0;
        }
...
static double eval_u_tan(const FormulaNode *node, double x, double y) {
            ...
            double rem = fmod(v + lv_HALF_PI, lv_PI);
            if (lv_is_zero(rem, lv_EPSILON_DOUBLE) || lv_is_equal(rem, lv_PI, lv_EPSILON_DOUBLE)) {
                return 0.0;
            }
            return tan(v);
        }
