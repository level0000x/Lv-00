...
bool lv00_expr_canonicalize(Lv00ExprCanonical *expr) {
    ...
    int n = expr->term_count;
    int vc = expr->var_count;

    /* 第一步：按哈希分组，合并同类项 */
    TermHashIdx *hash_indices = (TermHashIdx *) lv00_malloc((size_t) n * sizeof(TermHashIdx));
    if (!hash_indices)
        return false;

    for (int i = 0; i < n; i++) {
        hash_indices[i].hash = term_hash(expr->terms[i].exponents, vc);
        hash_indices[i].idx = i;
        hash_indices[i].merged = false;
    }

    /* 按哈希排序 */
    qsort(hash_indices, (size_t) n, sizeof(TermHashIdx), cmp_hashidx);
    ...
    while (i < n) {
        if (hash_indices[i].merged) {
            i++;
            continue;
        }

        int group_start = i;
        uint64_t group_hash = hash_indices[i].hash;

        /* 找到同哈希组本 */
        while (i + 1 < n && hash_indices[i + 1].hash == group_hash)
            i++;

        /* 在同组中查找精确匹配的指数 */
        int base_idx = hash_indices[group_start].idx;
        Lv00Rational *sum = lv00_rational_clone(expr->terms[base_idx].coeff);

        for (int j = group_start + 1; j <= i && j < n; j++) {
            int other_idx = hash_indices[j].idx;
            if (exponents_equal(expr->terms[base_idx].exponents,
                                expr->terms[other_idx].exponents, vc)) {
                /* 匹配：合并系数 */
                lv00_rational_add_inplace(sum, expr->terms[other_idx].coeff);
                hash_indices[j].merged = true;
            }
        }

        /* 非零系数：保留此项 */
        if (!lv00_rational_is_zero(sum)) {
            lv00_rational_destroy(&expr->terms[base_idx].coeff);
            expr->terms[base_idx].coeff = sum;

            /* 将保留项移到 write_idx */
            if (write_idx != base_idx) {
                Lv00ExprTerm tmp = expr->terms[write_idx];
                expr->terms[write_idx] = expr->terms[base_idx];
                expr->terms[base_idx] = tmp;
            }
            write_idx++;
        } else {
    ...
    for (int j = write_idx; j < n; j++) {
        if (expr->terms[j].coeff) {
            lv00_rational_destroy(&expr->terms[j].coeff);
            expr->terms[j].coeff = NULL;
        }
    }

    expr->term_count = write_idx;
    lv00_free((void **) &hash_indices);
    ...
    /* 第二步：按次数降序 + 字典序排列 */
    /* 建立指针数组用于排序 */
    Lv00ExprTerm **sorted = (Lv00ExprTerm **) lv00_malloc((size_t) expr->term_count * sizeof(Lv00ExprTerm *));
    if (!sorted)
        return false;

    for (int j = 0; j < expr->term_count; j++) {
        sorted[j] = &expr->terms[j];
    }

    qsort(sorted, (size_t) expr->term_count, sizeof(Lv00ExprTerm *), cmp_term_canonical);

    /* 将排序后的项原位重排 */
    Lv00ExprTerm *temp = (Lv00ExprTerm *) lv00_malloc((size_t) expr->term_count * sizeof(Lv00ExprTerm));
    if (!temp) {
    ...
    memcpy(expr->terms, temp, (size_t) expr->term_count * sizeof(Lv00ExprTerm));
    ...
    if (expr->term_count > 0 && expr->terms[0].coeff) {
        int sgn = lv00_rational_sgn(expr->terms[0].coeff);
        if (sgn < 0) {
            /* 将所有项乘以 -1 */
            for (int j = 0; j < expr->term_count; j++) {
                lv00_rational_neg_inplace(expr->terms[j].coeff);
            }
        }
    }
...
char *lv00_expr_canonical_to_string(const Lv00ExprCanonical *expr) {
    ...
    size_t buf_cap = 256;
    size_t buf_len = 0;
    char *buf = (char *) lv00_malloc(buf_cap);
    if (!buf)
        return NULL;
    buf[0] = '\0';
