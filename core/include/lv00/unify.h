 ...
 *
 * 使用示例：
 LV00_PUBLIC_API *   UnifyStatus s = unify_construction_with_proposition(construction, proposition);
 *   if (s != UNIFY_STATUS_OK) {
 *       UnifyFailureInfo info;
 LV00_PUBLIC_API *       unify_construction_with_proposition_detailed(construction, pattern, &info);
 *   }
 *
 * ======================================================================== */
 ...
 */
LV00_PUBLIC_API void unify_set_stream_context(StreamContext *ctx);
 ...
 */
LV00_PUBLIC_API UnifyStatus unify_construction_with_proposition(const ConstraintGraph *construction, const ConstraintGraph *proposition);
 ...
 * @return 合一状态
 */
LV00_PUBLIC_API UnifyStatus unify_construction_with_proposition_coord(const ConstraintGraph *construction, const ConstraintGraph *proposition);
 ...
 * @return 合一状态
 */
LV00_PUBLIC_API UnifyStatus unify_construction_with_proposition_hash_filtered(const ConstraintGraph *construction,
 ...
 *
 *   获取命题图端口数的方法：
 LV00_PUBLIC_API *     int port_count = constraint_graph_port_count(proposition);
 LV00_PUBLIC_API *     int *buf = malloc(port_count * 2 * sizeof(int));
 *
 *   如果缓冲区过小，函数行为未定义（可能越界写入）。
 *   建议在调用前检查端口数并分配足够的缓冲区，或传入 NULL 以仅获取匹配计数。
 */
LV00_PUBLIC_API int unify_match_ports(const ConstraintGraph *construction, const ConstraintGraph *proposition,
 ...
 */
LV00_PUBLIC_API int unify_match_constraints(const ConstraintGraph *construction, const ConstraintGraph *proposition,
 ...
 */
LV00_PUBLIC_API int unify_coords_equal(const GeomNode *a, const GeomNode *b);
 ...
 */
LV00_PUBLIC_API int unify_match_coords(const SymbolicCoord *c1, const SymbolicCoord *c2);
 ...
 * @param info 失败信息指针
 */
LV00_PUBLIC_API void unify_failure_info_destroy(UnifyFailureInfo *info);
 ...
 * @return 是否成功声明
 */
LV00_PUBLIC_API bool unify_declare_proposition_equivalence(int prop_a_id, int prop_b_id, ConstraintGraph *transformation_rule);
 ...
 * @return 找到的等价命题数量
 */
LV00_PUBLIC_API int unify_find_equivalent_proposition(int prop_id, int *equivalent_ids, int max_count);

/**
 * @brief 清除所有等价声明
 */
LV00_PUBLIC_API void unify_clear_equivalences(void);
 ...
 */
LV00_PUBLIC_API int unify_equivalence_count(void);
 ...
 * @return 是否成功实例化
 */
LV00_PUBLIC_API bool unify_instantiate_proposition(ConstraintGraph *proposition, int type_var_node_id, const TypeRegion *concrete_type,
...
LV00_PUBLIC_API SimpleProposition *simple_proposition_create(const char *name, int *input_port_ids, int input_count,
                                             int *output_port_ids, int output_count);
LV00_PUBLIC_API void simple_proposition_destroy(SimpleProposition *prop);
LV00_PUBLIC_API SimpleProof *simple_proof_create(SimpleProposition *prop, ConstraintGraph *construction);
LV00_PUBLIC_API void simple_proof_destroy(SimpleProof *proof);

LV00_PUBLIC_API bool simple_proof_check(SimpleProof *proof);
LV00_PUBLIC_API void simple_proof_normalize(SimpleProof *proof);
