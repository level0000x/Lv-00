...
LV00_PUBLIC_API void recursion_set_stream_context(StreamContext *ctx);
 ...
 * 创建测度系统
 */
LV00_PUBLIC_API MeasureSystem *measure_system_create(void);

/**
 * 销毁测度系统
 */
LV00_PUBLIC_API void measure_system_destroy(MeasureSystem *ms);

/**
 * 创建符号测度
 */
LV00_PUBLIC_API Measure *measure_create_symbolic(const char *name, int kind, int ref_node_id);
 ...
 * 销毁测度
 */
LV00_PUBLIC_API void measure_destroy(Measure *m);

/**
 * 添加测度到系统
 */
LV00_PUBLIC_API bool measure_system_add(MeasureSystem *ms, Measure *m);

/**
 * 设置默认测度
 */
LV00_PUBLIC_API void measure_system_set_default(MeasureSystem *ms, Measure *m);

/**
 * 计算节点测度值
 */
LV00_PUBLIC_API SymbolicCoord *measure_compute_value(Measure *m, GeomNode *node, ConstraintGraph *graph);
 ...
 */
LV00_PUBLIC_API SymbolicCoord *measure_compute_value_symbolic(Measure *m, GeomNode *node, ConstraintGraph *graph);

/**
 * 比较两个测度值
 */
LV00_PUBLIC_API MeasureCompareResult measure_compare(Measure *m, SymbolicCoord *a, SymbolicCoord *b);

/**
 * 比较两个节点的测度
 */
LV00_PUBLIC_API MeasureCompareResult measure_compare_nodes(Measure *m, GeomNode *a, GeomNode *b, const ConstraintGraph *graph);
 ...
 * 创建递归上下文
 */
LV00_PUBLIC_API RecursionContext *recursion_context_create(int max_depth);

/**
 * 销毁递归上下文
 */
LV00_PUBLIC_API void recursion_context_destroy(RecursionContext *ctx);

/**
 * 设置活动测度
 */
LV00_PUBLIC_API void recursion_context_set_measure(RecursionContext *ctx, Measure *m);

/**
 * 进入递归调用
 */
LV00_PUBLIC_API RecursionCheckResult recursion_context_enter(RecursionContext *ctx, int func_block_id, const GeomNode *input,
                                             ConstraintGraph *graph);

/**
 * 退出递归调用
 */
LV00_PUBLIC_API void recursion_context_exit(RecursionContext *ctx);

/**
 * 检查测度递减性（修改1：验证整条调用链的单调递减）
 * 遍历整个 measure_values 数组，验证严格单调递减
 */
LV00_PUBLIC_API RecursionCheckResult recursion_context_check_decreasing(const RecursionContext *ctx, SymbolicCoord *new_value);

/**
 * 获取当前深度
 */
LV00_PUBLIC_API int recursion_context_get_depth(RecursionContext *ctx);

/**
 * 重置递归上下文
 */
LV00_PUBLIC_API void recursion_context_reset(RecursionContext *ctx);
 ...
 * @param user_data 用户数据
 */
LV00_PUBLIC_API void recursion_context_set_depth_callback(RecursionContext *ctx, RecursionDepthCallback callback, void *user_data);
 ...
 */
LV00_PUBLIC_API bool lv00_recursion_enter(void);
 ...
 */
LV00_PUBLIC_API void lv00_recursion_leave(void);
 ...
 */
LV00_PUBLIC_API bool lv00_recursion_circuit_breaker_triggered(void);
 ...
 * 应在开始新的递归链之前调用。
 */
LV00_PUBLIC_API void lv00_recursion_reset(void);
 ...
 */
LV00_PUBLIC_API int lv00_recursion_get_depth(void);
 ...
 * 创建选择器块
 */
LV00_PUBLIC_API SelectorBlock *selector_block_create(int id, ConstraintGraph *graph);

/**
 * 销毁选择器块
 */
LV00_PUBLIC_API void selector_block_destroy(SelectorBlock *sb);

/**
 * 设置测试条件
 */
LV00_PUBLIC_API bool selector_block_set_condition(SelectorBlock *sb, int point_id, int region_id);

/**
 * 设置分支
 */
LV00_PUBLIC_API bool selector_block_set_branches(SelectorBlock *sb, int true_root, int false_root);
 ...
 */
LV00_PUBLIC_API void selector_block_set_branch_nodes(SelectorBlock *sb, int *true_ids, int true_count, int *false_ids, int false_count);
 ...
 */
LV00_PUBLIC_API const int *selector_block_get_branch_nodes(SelectorBlock *sb, bool is_true_branch, int *out_count);

/**
 * 评估选择器块
 */
LV00_PUBLIC_API bool selector_block_evaluate(SelectorBlock *sb, ConstraintGraph *graph);

/**
 * 获取活跃分支
 */
LV00_PUBLIC_API int selector_block_get_active_branch(SelectorBlock *sb);

/**
 * 更新分支状态
 */
LV00_PUBLIC_API void selector_block_update_states(SelectorBlock *sb, BranchState true_state, BranchState false_state);
 ...
 */
LV00_PUBLIC_API RecursionCheckResult recursion_validate_measure(const RecursionContext *ctx, const Measure *measure,
 ...
 * @return 0 成功，-1 失败
 */
LV00_PUBLIC_API int selector_block_count_branch_nodes(const SelectorBlock *sb, int *out_true_count, int *out_false_count);
 ...
 */
LV00_PUBLIC_API bool selector_block_validate_branches(const SelectorBlock *sb);
 ...
 * @return 是否一致
 */
LV00_PUBLIC_API bool recursion_check_mutual(int *func_ids, int count, MeasureSystem *ms);
 ...
 * @return 是否通过验证
 */
LV00_PUBLIC_API bool recursion_check_mutual_with_contexts(RecursionContext *ctx_a, RecursionContext *ctx_b);
 ...
 * @return 注册是否成功
 */
LV00_PUBLIC_API bool measure_system_register_non_symbolic(MeasureSystem *ms, int measure_type_id, NonSymbolicComparator comparator,
 ...
 * @return 是否全部验证通过
 */
LV00_PUBLIC_API bool measure_system_validate_non_symbolic(MeasureSystem *ms);
 ...
 */
LV00_PUBLIC_API RecursionCheckResult recursion_validate_non_symbolic_measure(const Measure *measure, SymbolicCoord *before_value,
 ...
 * @return 是否全部通过
 */
LV00_PUBLIC_API bool recursion_run_measure_tests(const Measure *measure, int test_count, SymbolicCoord ***test_before_values,
 ...
 * 测度类型转字符串
 */
LV00_PUBLIC_API const char *measure_type_to_string(MeasureType type);

/**
 * 测度比较结果转字符串
 */
LV00_PUBLIC_API const char *measure_compare_result_to_string(MeasureCompareResult result);

/**
 * 递归检查结果转字符串
 */
LV00_PUBLIC_API const char *recursion_check_result_to_string(RecursionCheckResult result);

/**
 * 分支状态转字符串
 */
LV00_PUBLIC_API const char *branch_state_to_string(BranchState state);
 ...
 */
LV00_PUBLIC_API int recursion_run_builtin_tests(MeasureSystem *sys, RecursionTestResult **results, int *result_count);
 ...
 */
LV00_PUBLIC_API int recursion_validate_non_symbolic_with_axiom(MeasureSystem *sys, int measure_id, const char *axiom_template_name,
                                               void *axiom_pkg);
 ...
 */
LV00_PUBLIC_API const char *recursion_get_measure_validation_template(MeasureSystem *sys, int measure_id);
