...
struct RelExpr {
    RelExprType type;
    union {
        struct { struct Relation *rel; } atomic;
        struct { struct RelExpr *child; } unary;
        struct { struct RelExpr *left; struct RelExpr *right; } binary;
        struct { int var_count; struct RelExpr *body; } quant;
    } data;
};
