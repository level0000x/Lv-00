...
LV00_PUBLIC_API Relation *rel_union(const Relation *a, const Relation *b);
LV00_PUBLIC_API Relation *rel_intersection(const Relation *a, const Relation *b);
LV00_PUBLIC_API Relation *rel_difference(const Relation *a, const Relation *b);
LV00_PUBLIC_API Relation *rel_join(const Relation *a, const Relation *b);
LV00_PUBLIC_API Relation *rel_product(const Relation *a, const Relation *b);
LV00_PUBLIC_API Relation *rel_transpose(const Relation *r);
LV00_PUBLIC_API Relation *rel_transitive_closure(const Relation *r);
LV00_PUBLIC_API Relation *rel_reflexive_transitive_closure(const Relation *r);
 ...
 */
LV00_PUBLIC_API RelModel *relation_model_from_graph(const ConstraintGraph *graph);

/**
 * @brief 销毁关系模型，释放所有关联内存
 */
LV00_PUBLIC_API void relation_model_destroy(RelModel *model);

/**
 * @brief 向模型添加事实公式
 */
LV00_PUBLIC_API bool relation_model_add_fact(RelModel *model, RelFormula *formula);

/**
 * @brief 向模型添加断言公式
 */
LV00_PUBLIC_API bool relation_model_add_assertion(RelModel *model, RelFormula *formula);
 ...
 */
LV00_PUBLIC_API bool relation_check_satisfiability(RelModel *model, const SmallScopeConfig *scope);
 ...
 */
LV00_PUBLIC_API RelInstance *relation_find_instance(RelModel *model, const SmallScopeConfig *scope,
                                                   bool assertions);

/**
 * @brief 销毁关系实例
 */
LV00_PUBLIC_API void relation_instance_destroy(RelInstance *inst);
 ...
 */
LV00_PUBLIC_API Relation *relation_evaluate_expr(const RelModel *model, const RelInstance *inst,
                                                  const RelExpr *expr);
 ...
 */
LV00_PUBLIC_API bool relation_evaluate_formula(const RelModel *model, const RelInstance *inst,
 ...
 *
 * @param model  关系模型
 * @return Alloy 源码字符串（调用方需用 lv00_free 释放），失败返回 NULL
 */
LV00_PUBLIC_API char *relation_model_export_alloy(const RelModel *model);

/**
 * @brief 将关系实例导出为 XML 格式的字符串
 *
 * @param inst  关系实例
 * @return XML 字符串（调用方需用 lv00_free 释放），失败返回 NULL
 */
LV00_PUBLIC_API char *relation_instance_export_xml(const RelInstance *inst);
 ...
 * @brief 创建 SAT 编码上下文
 */
LV00_PUBLIC_API SatEncoding *sat_encoding_create(int initial_var_capacity, int initial_clause_capacity);

/**
 * @brief 销毁 SAT 编码上下文
 */
LV00_PUBLIC_API void sat_encoding_destroy(SatEncoding *enc);
 ...
 */
LV00_PUBLIC_API int sat_encoding_register_var(SatEncoding *enc, int arity, const int *atom_ids);
 ...
 */
LV00_PUBLIC_API int sat_encoding_lookup_var(const SatEncoding *enc, int arity, const int *atom_ids);
 ...
 */
LV00_PUBLIC_API int sat_encoding_add_clause(SatEncoding *enc, const SatLiteral *literals, int count);

/**
 * @brief 添加假设（单元子句）
 */
LV00_PUBLIC_API int sat_encoding_add_assumption(SatEncoding *enc, SatLiteral literal);
 ...
 * @brief 将约束图编码为 SAT
 */
LV00_PUBLIC_API SatResult constraint_graph_to_sat(const ConstraintGraph *graph, SatEncoding *enc);

/**
 * @brief 将关系模型编码为 SAT
 */
LV00_PUBLIC_API SatResult relation_model_to_sat(const RelModel *model, const SmallScopeConfig *scope,
                                                 SatEncoding *enc);
 ...
 */
LV00_PUBLIC_API SatResult sat_solve_and_decode(SatEncoding *enc, SatModel **out_model);

/**
 * @brief 增量求解：追加假设后求解
 */
LV00_PUBLIC_API SatResult sat_solve_incremental(SatEncoding *enc, const SatLiteral *literals,
                                                int count, SatModel **out_model);

/**
 * @brief 将 SAT 模型解码为约束图
 */
LV00_PUBLIC_API ConstraintGraph *sat_model_to_graph(const SatModel *model);

/**
 * @brief 将 SAT 模型解码为关系实例
 */
LV00_PUBLIC_API RelInstance *sat_model_to_instance(const SatEncoding *enc, const SatModel *model);
