 ...
 */
typedef enum {
    lv_EVOL_EULER = 0, /**< 显式 Euler 法（一阶，用于调试/快速测试） */
    lv_EVOL_RK4 = 1,   /**< 经典四阶 Runge-Kutta 法（非刚性） */
    lv_EVOL_ADAMS = 2, /**< Adams-Bashforth-Moulton 预测-校正法（非刚性，
 ...
 */
typedef enum {
    lv_EVOL_STATUS_IDLE = 0,      /**< 空闲（未开始） */
    lv_EVOL_STATUS_RUNNING = 1,   /**< 演化进行中 */
    lv_EVOL_STATUS_CONVERGED = 2, /**< 已收敛 */
    lv_EVOL_STATUS_STOPPED = 3,   /**< 已停止（达到终止时间） */
    lv_EVOL_STATUS_ERROR = 4      /**< 发生错误 */
} lvEvolStatus;

/* ==================== 回调类型 ==================== */

/** @cond 前向声明 */
typedef struct lvGeomEvol lvGeomEvol;
/** @endcond */
 ...
 */
typedef int (*lvGeomEvolRHSFunc)(double t, const double *param, double *dparam, lvGeomEvol *evol);
 ...
 * @param[in] param 当前参数
 */
typedef void (*lvGeomEvolPostStepFunc)(const lvGeomEvol *evol, double t, const double *param);
 ...
 * @return 成功返回 0
 */
typedef int (*lvGeomEvolRootFunc)(double t, const double *param, double *gout, lvGeomEvol *evol);
...
struct lvGeomEvol {
    /* ── 配置 ── */
    int dim;             /**< 参数向量维度 */
    lvEvolMethod method; /**< 演化方法 */
    lvEvolStatus status; /**< 当前状态 */
    ...
    /* ── PI 控制器 ── */
    lvGeomEvolPI pi; /**< PI 步长控制器 */

    /* ── 回调 ── */
    lvGeomEvolRHSFunc rhs_func;       /**< 右端函数（必须设置） */
    lvGeomEvolPostStepFunc post_step; /**< 步后处理回调（可选） */
    lvGeomEvolRootFunc root_func;     /**< 根检测函数（可选） */
    int root_ng;                      /**< 根函数数量 */
 ...
 */
static inline const lvGeomEvolStats *geoevol_get_step_stats(const lvGeomEvol *evol) {
 ...
 * @param[out] pi  PI 控制器
 */
static inline void geoevol_pi_init(lvGeomEvolPI *pi) {
 ...
 */
static inline double geoevol_pi_predict(lvGeomEvolPI *pi, double error, int order, double h_curr) {
    double exponent = pi->bias_factor / (double) (order + 1);
    double factor;
    if (pi->initialized && error > 0.0) {
        factor = pi->safety_factor * pow(1.0 / error, exponent) * pow(pi->error_prev / error, 1.0 - pi->bias_factor);
    } else {
        factor = pi->safety_factor * pow(1.0 / error, exponent);
    }
    /* 限制缩放范围 */
    if (factor > pi->max_scale)
        factor = pi->max_scale;
    if (factor < pi->min_scale)
        factor = pi->min_scale;
 ...
 *
 * // 创建并运行演化
 * lvGeomEvol *evol = geoevol_create(2, lv_EVOL_RK4, circle_rhs);
 * evol->param[0] = 1.0; evol->param[1] = 0.0;
 * evol->rel_tol  = 1e-6;
 * evol->abs_tol  = 1e-12;
 * geoevol_set_step(evol, 0.01, 1e-8, 0.1);
 * geoevol_evolve(evol, 2.0 * lv_PI);  // 演化一圈
 *
 * const lvGeomEvolStats *st = geoevol_get_step_stats(evol);
