 ...
 *
 * 使用示例：
 lv_PUBLIC_API *   RewriteRule *rule = rewrite_rule_create("simplify", pattern, replacement, 1);
 lv_PUBLIC_API *   RewriteMatch *match = find_rewrite_match(graph, rule, false);
...
#endif
#ifndef lv_REWRITE_WL_HASH_MULTIPLIER
#define lv_REWRITE_WL_HASH_MULTIPLIER 0x01000193U
#endif
 ...
 */
lv_PUBLIC_API GraphSnapshot *graph_snapshot_create(const ConstraintGraph *graph);
 ...
 */
lv_PUBLIC_API bool graph_snapshot_restore(GraphSnapshot *snapshot, ConstraintGraph *graph);

/**
 * @brief 销毁快照，释放所有深拷贝的数据
 */
lv_PUBLIC_API void graph_snapshot_destroy(GraphSnapshot *snapshot);
 ...
 */
lv_PUBLIC_API int rewrite_rules_load_from_file(const char *filepath, RewriteRule ***out_rules, int *out_count);
 ...
 */
lv_PUBLIC_API bool rewrite_rule_unload(RewriteRule ***rules, int *count, const char *rule_name);
 ...
 * @param rule 要销毁的重写规则
 */
lv_PUBLIC_API void rewrite_rule_destroy(RewriteRule *rule);
 ...
 * @return 重写状态
 */
lv_PUBLIC_API RewriteStatus apply_rewrite(ConstraintGraph *graph, RewriteRule *rule, RewriteMatch *match);
 ...
 * @return 重写状态
 */
lv_PUBLIC_API RewriteStatus detect_rewrite_loop_wl(ConstraintGraph *graph, WLHashHistory *hist);
 ...
 */
lv_PUBLIC_API void wl_history_init(WLHashHistory *hist);

/**
 * @brief 销毁 WL 哈希历史
 * @param[in,out] hist WL 哈希历史记录
 */
lv_PUBLIC_API void wl_history_destroy(WLHashHistory *hist);
 ...
 */
lv_PUBLIC_API RewriteMatch *find_best_match(ConstraintGraph *graph, RewriteRule *rule, bool local_equivalence_tolerant);
 ...
 */
lv_PUBLIC_API uint64_t rewrite_compute_wl_hash(const ConstraintGraph *graph);
...
/* ---- 策略树构造与销毁 ---- */
lv_PUBLIC_API RewriteStrategy *rewrite_strategy_create_idle(void);
lv_PUBLIC_API RewriteStrategy *rewrite_strategy_create_fail(void);
lv_PUBLIC_API RewriteStrategy *rewrite_strategy_create_apply_rule(int rule_id);
lv_PUBLIC_API RewriteStrategy *rewrite_strategy_create_match(const char *pattern);
RewriteStrategy *rewrite_strategy_create_test(int (*test)(void *), void *ctx);
lv_PUBLIC_API RewriteStrategy *rewrite_strategy_sequence(RewriteStrategy *left, RewriteStrategy *right);
lv_PUBLIC_API RewriteStrategy *rewrite_strategy_orelse(RewriteStrategy *left, RewriteStrategy *right);
...
/** 获取已注册的数值规则数量 */
lv_PUBLIC_API int rewrite_num_rule_count(void);
