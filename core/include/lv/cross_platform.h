 ...
 *   lv_i32 value = 42;           // 固定宽度有符号 32 位整数
 *   #if lv_PLATFORM_WINDOWS      // 编译期平台分支
 *       // Windows 特定代码
 *   #endif
 *
 * @version 1.1.0
...
#ifndef lv_CROSS_PLATFORM_H
#define lv_CROSS_PLATFORM_H
 ...
 *
 * 约定:
 *   - lv_i32: 恰好 32 位有符号整数（等于 int32_t）
 *   - lv_u32: 恰好 32 位无符号整数（等于 uint32_t）
 *   - lv_i64: 恰好 64 位有符号整数（等于 int64_t）
 *   - lv_u64: 恰好 64 位无符号整数（等于 uint64_t）
 *   - lv_iptr: 指针宽度有符号整数（用于指针运算）
 *   - lv_uptr: 指针宽度无符号整数（用于指针运算/哈希）
 *   - lv_byte: 恰好 8 位无符号整数（字节操作）
 *   - lv_bool: 布尔类型（依据 C 标准版本自动选择）
 * ==================================================================== */
 ...
 *
 * 命名:
 *   - lv_PLATFORM_WINDOWS: 目标为 Windows（含 MinGW/Cygwin）
 *   - lv_PLATFORM_LINUX:   目标为 Linux 内核
 *   - lv_PLATFORM_MACOS:   目标为 macOS / Darwin
 *   - lv_PLATFORM_UNIX:    目标为任意 Unix-like 系统（POSIX）
 *   - lv_PLATFORM_NAME:    目标平台的人类可读名称字符串
 * ==================================================================== */
 ...
 *
 * 命名:
 *   - lv_CC_GCC:   GNU Compiler Collection
 *   - lv_CC_CLANG: LLVM Clang / Apple Clang
 *   - lv_CC_MSVC:  Microsoft Visual C++
 *   - lv_CC_NAME:  编译器的人类可读名称字符串
 * ==================================================================== */
 ...
 *
 * 命名:
 *   - lv_ARCH_32BIT: 32 位架构（ILP32）
 *   - lv_ARCH_64BIT: 64 位架构（LP64 / LLP64）
 *   - lv_ARCH_NAME:  架构的人类可读名称字符串
 * ==================================================================== */
 ...
 *   #if lv_STACK_SIZE_KB < 2048
 *       char *buf = lv_malloc(lv_CONFIG_STREAM_JSON_BUFFER_SIZE);
 *   #else
 *       char buf[lv_CONFIG_STREAM_JSON_BUFFER_SIZE];
 *   #endif
 * ==================================================================== */
 ...
 *
 * 命名:
 *   - lv_ENDIAN_LITTLE: 小端序（LSB 在前）
 *   - lv_ENDIAN_BIG:    大端序（MSB 在前）
 * ==================================================================== */
 ...
 * 9. 导出/导入宏（用于共享库/DLL）
 *
 * 这些宏在 lv.h 的 lv_PUBLIC_API 中组合使用，
 * 此处提供底层构建块。
 * ==================================================================== */
...
/* ── 编译期验证固定宽度类型 ── */
lv_STATIC_ASSERT(sizeof(lv_i32) == 4, "lv_i32 must be exactly 4 bytes");
lv_STATIC_ASSERT(sizeof(lv_u32) == 4, "lv_u32 must be exactly 4 bytes");
lv_STATIC_ASSERT(sizeof(lv_i64) == 8, "lv_i64 must be exactly 8 bytes");
lv_STATIC_ASSERT(sizeof(lv_u64) == 8, "lv_u64 must be exactly 8 bytes");
lv_STATIC_ASSERT(sizeof(lv_byte) == 1, "lv_byte must be exactly 1 byte");
 ...
 */
static inline const char *lv_platform_name(void) {
    return lv_PLATFORM_NAME;
}
 ...
 */
static inline const char *lv_compiler_name(void) {
    return lv_CC_NAME;
}
 ...
 */
static inline const char *lv_arch_name(void) {
    return lv_ARCH_NAME;
}
 ...
 */
static inline const char *lv_endian_name(void) {
#if lv_ENDIAN_LITTLE
...
#else
    /* 运行时检测 */
    return lv_is_little_endian() ? "little-endian" : "big-endian";
#endif
}
 ...
 * @return 栈大小（字节）
 */
static inline size_t lv_stack_size(void) {
    return lv_STACK_SIZE_BYTES;
}
 ...
 * @return 缓存行大小（字节）
 */
static inline int lv_cache_line_size(void) {
    return lv_CACHE_LINE_SIZE;
}
