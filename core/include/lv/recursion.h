 ...
 *   - lv_recursion_enter / leave / reset   — 全局深度保护（熔断器）
 *   - selector_block_create / evaluate       — 选择器块
 *   - recursion_check_mutual                 — 互递归验证
 *
 * 使用示例：
 lv_PUBLIC_API *   RecursionContext *ctx = recursion_context_create(10000);
 lv_PUBLIC_API *   Measure *m = measure_create_symbolic("depth", MEASURE_KIND_DEPTH, -1);
 lv_PUBLIC_API *   recursion_context_set_measure(ctx, m);
 lv_PUBLIC_API *   RecursionCheckResult r = recursion_context_enter(ctx, fb_id, input, graph);
 *
 * ======================================================================== */
...
#ifndef lv_RECURSION_H
#define lv_RECURSION_H
 ...
 * 创建测度系统
 */
lv_PUBLIC_API MeasureSystem *measure_system_create(void);

/**
 * 销毁测度系统
 */
lv_PUBLIC_API void measure_system_destroy(MeasureSystem *ms);

/**
 * 创建符号测度
 */
lv_PUBLIC_API Measure *measure_create_symbolic(const char *name, int kind, int ref_node_id);
 ...
 * 销毁测度
 */
lv_PUBLIC_API void measure_destroy(Measure *m);

/**
 * 添加测度到系统
 */
lv_PUBLIC_API bool measure_system_add(MeasureSystem *ms, Measure *m);

/**
 * 设置默认测度
 */
lv_PUBLIC_API void measure_system_set_default(MeasureSystem *ms, Measure *m);

/**
 * 计算节点测度值
 */
lv_PUBLIC_API SymbolicCoord *measure_compute_value(Measure *m, GeomNode *node, ConstraintGraph *graph);
 ...
 */
lv_PUBLIC_API SymbolicCoord *measure_compute_value_symbolic(Measure *m, GeomNode *node, ConstraintGraph *graph);

/**
 * 比较两个测度值
 */
lv_PUBLIC_API MeasureCompareResult measure_compare(Measure *m, SymbolicCoord *a, SymbolicCoord *b);
 ...
 * 创建递归上下文
 */
lv_PUBLIC_API RecursionContext *recursion_context_create(int max_depth);

/**
 * 销毁递归上下文
 */
lv_PUBLIC_API void recursion_context_destroy(RecursionContext *ctx);

/**
 * 设置活动测度
 */
lv_PUBLIC_API void recursion_context_set_measure(RecursionContext *ctx, Measure *m);
 ...
 * 退出递归调用
 */
lv_PUBLIC_API void recursion_context_exit(RecursionContext *ctx);
 ...
 * 获取当前深度
 */
lv_PUBLIC_API int recursion_context_get_depth(RecursionContext *ctx);

/**
 * 重置递归上下文
 */
lv_PUBLIC_API void recursion_context_reset(RecursionContext *ctx);
 ...
 * 每次递归调用前调用此函数：
 * - 深度 +1
 * - 如果 depth > lv_MAX_RECURSION_DEPTH，触发熔断器（circuit breaker）
 *
 * 熔断器机制：
 * - 自动将全局 circuit_breaker_triggered 标志设为 true
 * - 后续所有 lv_recursion_enter() 调用都会返回 false
 * - 必须调用 lv_recursion_reset() 才能恢复
 *
 * 与 recursion_context_enter() 的区别：
 * - lv_recursion_enter/leave 是轻量级全局保护，无上下文依赖
 * - recursion_context_enter/exit 是上下文相关的完整测度验证
 *
 * @return true  进入成功（深度在安全范围内）
 * @return false 熔断器已触发或深度超限
 */
lv_PUBLIC_API bool lv_recursion_enter(void);
 ...
 */
lv_PUBLIC_API void lv_recursion_leave(void);
 ...
 */
lv_PUBLIC_API bool lv_recursion_circuit_breaker_triggered(void);
 ...
 * 应在开始新的递归链之前调用。
 */
lv_PUBLIC_API void lv_recursion_reset(void);
 ...
 */
lv_PUBLIC_API int lv_recursion_get_depth(void);
 ...
 * 创建选择器块
 */
lv_PUBLIC_API SelectorBlock *selector_block_create(int id, ConstraintGraph *graph);

/**
 * 销毁选择器块
 */
lv_PUBLIC_API void selector_block_destroy(SelectorBlock *sb);

/**
 * 设置测试条件
 */
lv_PUBLIC_API bool selector_block_set_condition(SelectorBlock *sb, int point_id, int region_id);

/**
 * 设置分支
 */
lv_PUBLIC_API bool selector_block_set_branches(SelectorBlock *sb, int true_root, int false_root);
 ...
 */
lv_PUBLIC_API const int *selector_block_get_branch_nodes(SelectorBlock *sb, bool is_true_branch, int *out_count);

/**
 * 评估选择器块
 */
lv_PUBLIC_API bool selector_block_evaluate(SelectorBlock *sb, ConstraintGraph *graph);

/**
 * 获取活跃分支
 */
lv_PUBLIC_API int selector_block_get_active_branch(SelectorBlock *sb);

/**
 * 更新分支状态
 */
lv_PUBLIC_API void selector_block_update_states(SelectorBlock *sb, BranchState true_state, BranchState false_state);
 ...
 * @return 0 成功，-1 失败
 */
lv_PUBLIC_API int selector_block_count_branch_nodes(const SelectorBlock *sb, int *out_true_count, int *out_false_count);
 ...
 */
lv_PUBLIC_API bool selector_block_validate_branches(const SelectorBlock *sb);
 ...
 * @return 是否一致
 */
lv_PUBLIC_API bool recursion_check_mutual(int *func_ids, int count, MeasureSystem *ms);
 ...
 * @return 是否通过验证
 */
lv_PUBLIC_API bool recursion_check_mutual_with_contexts(RecursionContext *ctx_a, RecursionContext *ctx_b);
 ...
 * @return 是否全部验证通过
 */
lv_PUBLIC_API bool measure_system_validate_non_symbolic(MeasureSystem *ms);
 ...
 * 测度类型转字符串
 */
lv_PUBLIC_API const char *measure_type_to_string(MeasureType type);

/**
 * 测度比较结果转字符串
 */
lv_PUBLIC_API const char *measure_compare_result_to_string(MeasureCompareResult result);

/**
 * 递归检查结果转字符串
 */
lv_PUBLIC_API const char *recursion_check_result_to_string(RecursionCheckResult result);

/**
 * 分支状态转字符串
 */
lv_PUBLIC_API const char *branch_state_to_string(BranchState state);
 ...
 */
lv_PUBLIC_API int recursion_run_builtin_tests(MeasureSystem *sys, RecursionTestResult **results, int *result_count);
 ...
 */
lv_PUBLIC_API const char *recursion_get_measure_validation_template(MeasureSystem *sys, int measure_id);

#ifdef __cplusplus
}
#endif

#endif /* lv_RECURSION_H */
