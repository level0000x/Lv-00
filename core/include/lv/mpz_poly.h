...
static inline bool mpz_poly_set(mpz_poly_t *dst, const mpz_poly_t *src) {
    mpz_poly_clear(dst);
    if (src->degree >= 0) {
        dst->coeffs = lv_malloc((src->degree + 1) * sizeof(mpz_t));
        if (!dst->coeffs) {
            dst->degree = -1;
            return false;
        }
...
static inline bool mpz_poly_alloc_result(mpz_poly_t *result, int max_deg) {
    ...
    result->coeffs = lv_malloc((size_t) (max_deg + 1) * sizeof(mpz_t));
    if (!result->coeffs) {
        result->degree = -1;
        return false;
    }
...
static inline void mpz_poly_mul(mpz_poly_t *result, const mpz_poly_t *a, const mpz_poly_t *b) {
    ...
    result->coeffs = lv_malloc(coeff_count * sizeof(mpz_t));
    if (!result->coeffs) {
        result->degree = -1;
        return;
    }
...
static inline void mpz_poly_div(mpz_poly_t *quotient, mpz_poly_t *dividend, const mpz_poly_t *divisor) {
    ...
    quotient->coeffs = lv_malloc((quotient->degree + 1) * sizeof(mpz_t));
    if (!quotient->coeffs) {
        quotient->degree = -1;
        return;
    }
