 ...
 * @date 2026-05-24
 */
#ifndef lv_INTERACTIVE_GEO_H
#define lv_INTERACTIVE_GEO_H
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
#define lv_GEO_SCRIPT_BUFFER_SIZE 65536
#endif
/** 状态导出 JSON 缓冲区大小 */
#ifndef lv_GEO_STATE_BUFFER_SIZE
#define lv_GEO_STATE_BUFFER_SIZE 131072
#endif
/** 随机化验证默认采样次数 */
 ...
 */
typedef struct lvInteractiveGeo {
    lvGeoCanvasState canvas_state;           /**< 画布状态 */
    lvRandomizedCheck rand_check;            /**< 随机化验证 */
    lvGeoScriptBinding script_binding;       /**< 脚本绑定 */
    lvContinuityTracker continuity;          /**< 连续性跟踪 */
 ...
 * @brief 初始化交互几何系统
 *
 * 分配并初始化 lvInteractiveGeo 上下文，设置默认模式为 GEO_MODE_SELECT，
 * 缩放级别 1.0，视口为单位矩阵。
 *
 * @param[in] engine_handle 关联的 lvEngine 句柄（可为 NULL 延迟绑定）
 * @return 新分配的交互几何上下文，失败返回 NULL
 */
lvInteractiveGeo *interactive_geo_init(lvEngine *engine_handle);
/**
