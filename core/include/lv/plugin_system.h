...
struct lvPluginContext {
    ...
    /* 日志回调 */
    void (*log)(lvPluginContext* ctx, int level, const char* message);

    /* 事件回调 */
    void (*on_event)(lvPluginContext* ctx, const lvPluginEvent* event);

    /* 接口查找 */
    lvPluginInterface* (*query_interface)(
        lvPluginContext* ctx,
        const char* name,
        uint32_t version
    );
...
struct lvPlugin {
    ...
    /* 生命周期回调 */
    int (*on_load)(lvPluginContext* ctx);
    int (*on_unload)(lvPluginContext* ctx);
    int (*on_activate)(lvPluginContext* ctx);
    int (*on_deactivate)(lvPluginContext* ctx);
    int (*on_configure)(lvPluginContext* ctx, const lvPluginConfig* config);
    int (*on_event)(lvPluginContext* ctx, const lvPluginEvent* event);
...
struct lvPluginSystem {
    ...
    /* 事件处理器 */
    void (*event_handler)(lvPluginSystem* system, const lvPluginEvent* event);
 ...
 */
lvPlugin** lv_plugin_get_by_type(
    lvPluginSystem* system,
    lvPluginType type,
    size_t* count
);
 ...
 */
lvPlugin** lv_plugin_get_by_state(
    lvPluginSystem* system,
    lvPluginState state,
    size_t* count
);
 ...
 * @return 成功返回 0，失败返回非零错误码
 */
int lv_plugin_register_interface(
    lvPlugin* plugin,
    lvPluginInterface* interface
);
 ...
 * @return 成功返回 0，失败返回非零错误码
 */
int lv_plugin_unregister_interface(
    lvPlugin* plugin,
    const char* name
);
 ...
 */
lvPluginInterface* lv_plugin_query_interface(
    lvPluginSystem* system,
    const char* name,
    uint32_t version
);
 ...
 */
lvPluginInterface** lv_plugin_query_interfaces(
    lvPluginSystem* system,
    const char* pattern,
    size_t* count
);
 ...
 * @return 成功返回 0，失败返回非零错误码
 */
int lv_plugin_config_save(
    const lvPluginConfig* config,
    const char* filepath
);
 ...
 */
const char* lv_plugin_config_get(
    const lvPluginConfig* config,
    const char* key,
 ...
 */
void lv_plugin_set_event_handler(
    lvPluginSystem* system,
    void (*handler)(lvPluginSystem*, const lvPluginEvent*)
);
/* ============ 依赖管理 ============ */
 ...
 * @return 成功返回 0，失败返回非零错误码
 */
int lv_plugin_resolve_dependencies(
    lvPluginSystem* system,
    lvPlugin* plugin
);
 ...
 */
int lv_plugin_check_dependencies(
    const lvPlugin* plugin
);
 ...
 */
lvPlugin** lv_plugin_get_dependents(
    lvPluginSystem* system,
    const lvPlugin* plugin,
    size_t* count
);
 ...
 * @return 成功返回 0，失败返回非零错误码
 */
int lv_plugin_system_add_search_path(
    lvPluginSystem* system,
    const char* path
);
 ...
 * @return 成功返回 0，失败返回非零错误码
 */
int lv_plugin_system_remove_search_path(
    lvPluginSystem* system,
    const char* path
);
 ...
 */
char** lv_plugin_system_get_search_paths(
    lvPluginSystem* system,
    size_t* count
);
/* ============ 自动加载 ============ */
 ...
 */
int lv_plugin_check_api_compatibility(
    uint32_t required,
    uint32_t provided
);
/* ============ 插件信息 ============ */
