 ...
 * @brief 创建缓存管理器
 */
lv_PUBLIC_API lvCacheManager *lv_cache_manager_create(const lvCacheConfig *config);
/**
 * @brief 销毁缓存管理器
 */
 ...
 * @brief 存储数据到缓存
 */
lv_PUBLIC_API bool lv_cache_put(lvCacheManager *manager, const char *key, const void *data, size_t size);
/**
 * @brief 从缓存获取数据
 */
lv_PUBLIC_API bool lv_cache_mgr_get(lvCacheManager *manager, const char *key, void **out_data, size_t *out_size);
/**
 * @brief 从缓存移除数据
 */
lv_PUBLIC_API bool lv_cache_mgr_remove(lvCacheManager *manager, const char *key);
/**
 * @brief 检查键是否存在
 */
lv_PUBLIC_API bool lv_cache_contains(lvCacheManager *manager, const char *key);
/**
 * @brief 创建缓存上下文
 */
lv_PUBLIC_API uint32_t lv_cache_context_create(lvCacheManager *manager, const char *name, uint32_t parent_id);
/**
 * @brief 切换当前活跃上下文
 */
lv_PUBLIC_API bool lv_cache_context_switch(lvCacheManager *manager, uint32_t context_id);
/**
 * @brief 获取当前活跃上下文ID
 */
 ...
 * @brief 获取全局统计信息
 */
lv_PUBLIC_API void lv_cache_mgr_get_stats(const lvCacheManager *manager, uint64_t *out_hits, uint64_t *out_misses,
                                          size_t *out_size);
/**
 * @brief 获取上下文统计信息
 */
lv_PUBLIC_API void lv_cache_get_context_stats(const lvCacheManager *manager, uint32_t context_id, uint64_t *out_hits,
 ...
 * @brief 获取配置
 */
lv_PUBLIC_API const lvCacheConfig *lv_cache_get_config(const lvCacheManager *manager);
/**
 * @brief 设置自定义析构函数
 */
lv_PUBLIC_API void lv_cache_set_destructor(lvCacheManager *manager, void (*destructor)(void *, size_t));
/**
 * @brief 清空缓存
 */
