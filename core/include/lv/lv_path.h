 ...
 *       缓冲区不足时返回 false。
 */
lv_PUBLIC_API bool lv_path_join(const char *dir, const char *file, char *out, size_t out_size);
 ...
 */
lv_PUBLIC_API const char *lv_path_basename(const char *path);
 ...
 */
lv_PUBLIC_API const char *lv_path_dirname(const char *path, size_t *out_len);
 ...
 * @note 无 '.' 时不变
 */
lv_PUBLIC_API char *lv_path_strip_ext(char *path);
 ...
 */
lv_PUBLIC_API int lv_path_mkdirs(const char *path);
 ...
 */
lv_PUBLIC_API int lv_path_remove(const char *path);
 ...
 */
lv_PUBLIC_API int lv_dir_foreach(const char *dir, lv_dir_entry_fn fn, void *ctx);
 ...
 */
lv_PUBLIC_API bool lv_temp_path(char *out, size_t out_size);
 ...
 */
lv_PUBLIC_API const char *lv_path_home_dir(void);
