...
#ifndef lv_CONFLICT_DETECTOR_H
#define lv_CONFLICT_DETECTOR_H
 ...
 */
lv_PUBLIC_API const ConflictDetectorConfig *lv_conflict_detector_default_config(void);
 ...
 */
lv_PUBLIC_API ConflictReport *lv_conflict_report_create(void);

/**
 * @brief 销毁冲突报告及所有关联的动态内存
 * @param report 要销毁的报告
 */
lv_PUBLIC_API void lv_conflict_report_destroy(ConflictReport *report);

/**
 * @brief 清空报告内容（保留数组容量）
 * @param report 要清空的报告
 */
lv_PUBLIC_API void lv_conflict_report_clear(ConflictReport *report);
 ...
 * @return true 表示存在冲突
 */
lv_PUBLIC_API bool lv_conflict_detect_quick(const ConstraintGraph *graph);
 ...
 * @return 类型名称字符串
 */
lv_PUBLIC_API const char *lv_conflict_type_name(ConflictType type);
 ...
 * @return 严重程度名称字符串
 */
lv_PUBLIC_API const char *lv_conflict_severity_name(ConflictSeverity severity);
 ...
 * @return true 表示存在冲突
 */
lv_PUBLIC_API bool lv_conflict_graph_has_conflicts(const ConstraintGraph *graph);
 ...
 */
lv_PUBLIC_API ConflictType lv_conflict_get_worst_type(const ConflictReport *report);

#ifdef __cplusplus
}
#endif

#endif /* lv_CONFLICT_DETECTOR_H */
