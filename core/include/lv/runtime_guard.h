 ...
 *
 *          设计原则：
 *          - 通过 lv_ENABLE_RUNTIME_GUARDS 编译开关控制启用/禁用
...
#ifndef lv_RUNTIME_GUARD_H
#define lv_RUNTIME_GUARD_H
...
/* ============================================================
 * 第一部分：编译期开关 —— lv_ENABLE_RUNTIME_GUARDS
 *
...
#define lv_RUNTIME_GUARD_READ_WARN_US 5000

/** @brief 写锁持有最大时间警告阈值（微秒） */
#define lv_RUNTIME_GUARD_WRITE_WARN_US 10000
 ...
 */
lv_PUBLIC_API bool lv_guard_init(lvGuardContext *guard);

/**
 * @brief 销毁运行时守卫上下文
 * @param guard 守卫上下文指针（非 NULL）
 */
lv_PUBLIC_API void lv_guard_destroy(lvGuardContext *guard);
 ...
 */
lv_PUBLIC_API void lv_guard_get_stats(const lvGuardContext *guard, lvGuardStats *stats);

/**
 * @brief 重置运行时保护统计信息
 * @param guard 守卫上下文指针（非 NULL）
 */
lv_PUBLIC_API void lv_guard_reset_stats(lvGuardContext *guard);

/* ============================================================
 * 核心宏：lv_RUNTIME_LOCK / lv_RUNTIME_UNLOCK
 *
 ...
 *   自旋次数由 lv_RUNTIME_GUARD_SPIN_ATTEMPTS 控制。
 * ============================================================ */
 ...
 * 数据完整性校验
 *
 * lv_verify_data_integrity(ctx) 执行一系列低成本检查，
 * 确保上下文中关键数据结构的一致性。
 *
 * 检查项目：
...
lv_PUBLIC_API bool lv_verify_data_integrity(struct lvContext *ctx);

#else /* !lv_ENABLE_RUNTIME_GUARDS */
 ...
 * 第四部分：安全断言宏（始终可用的辅助宏）
 *
 * 这些宏无论 lv_ENABLE_RUNTIME_GUARDS 是否启用都有效。
 * ============================================================ */

/**
 * @brief 运行时条件下断言：如果条件为假，记录错误并返回指定值
 *
 * 在 lv_ENABLE_RUNTIME_GUARDS 启用时执行额外的完整性检查，
 * 在禁用时退化为普通条件判断。
 *
 * @param ctx     lvContext 指针
 * @param cond    断言条件
 ...
 *
 * 在 lv_ENABLE_RUNTIME_GUARDS 启用时获取锁，
 * 退出作用域时自动释放（需与 lv_RUNTIME_UNLOCK 配对）。
 *
 * 使用示例：
 * @code
 *   lv_GUARDED_SECTION(ctx) {
 *       // 在此区域内 ctx 被保护，可安全读写
 *       ctx->main_graph = ...;
 *   }
 *   // 自动释放锁
