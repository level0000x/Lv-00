...
#ifndef lv_NORMALIZATION_H
#define lv_NORMALIZATION_H
 ...
 */
lv_PUBLIC_API void normalization_set_stream_context(StreamContext *ctx);
 ...
 */
lv_PUBLIC_API void normalization_set_merge_callback(MergeConfirmCallback cb, void *user_data);

/**
 * @brief 获取当前的合并确认回调
 * @return 回调函数指针，未设置返回 NULL
 */
lv_PUBLIC_API MergeConfirmCallback normalization_get_merge_callback(void);
 ...
 */
lv_PUBLIC_API NormalizationLog *normalization_log_create(int initial_capacity);

/**
 * @brief 销毁归一化日志
 * @param[in,out] log 要销毁的日志
 */
lv_PUBLIC_API void normalization_log_destroy(NormalizationLog *log);
 ...
 */
lv_PUBLIC_API void normalization_log_record(NormalizationLog *log, int old_id, int new_id, bool auto_merged);
 ...
 */
lv_PUBLIC_API NormalizationResult *graph_normalize(ConstraintGraph *graph, bool scope_aware);

/**
 * @brief 销毁归一化结果
 * @param[in,out] result 要销毁的结果
 */
lv_PUBLIC_API void normalization_result_destroy(NormalizationResult *result);
 ...
 */
lv_PUBLIC_API bool normalization_verify_idempotency(ConstraintGraph *graph);
 ...
 */
lv_PUBLIC_API int merge_line_segments(ConstraintGraph *graph, NormalizationLog *log);
 ...
 * @return 执行的合并数量，出错返回 -1
 */
lv_PUBLIC_API int merge_regions(ConstraintGraph *graph, NormalizationLog *log);
...
lv_PUBLIC_API NodeMergeCandidate *find_merge_candidates(const ConstraintGraph *graph, int *out_count);
 ...
 * @param[in] count 数组长度
 */
lv_PUBLIC_API void merge_candidates_destroy(NodeMergeCandidate *candidates, int count);
 ...
 * @return 执行的合并数量，出错返回 -1
 */
lv_PUBLIC_API int apply_merges(ConstraintGraph *graph, NodeMergeCandidate *candidates, int count, bool *user_confirmed);

/**
 * @brief 对图进行稳定的拓扑排序
 * @param[in,out] graph 约束图
 */
lv_PUBLIC_API void graph_topological_sort_stable(ConstraintGraph *graph);
 ...
 */
lv_PUBLIC_API RewriteHistory *rewrite_history_create(int capacity);

/**
 * @brief 销毁重写历史记录
 * @param[in,out] history 要销毁的历史记录
 */
lv_PUBLIC_API void rewrite_history_destroy(RewriteHistory *history);
 ...
 */
lv_PUBLIC_API bool rewrite_history_check_cycle(const RewriteHistory *history, const ConstraintGraph *graph);
 ...
 * @param[in] graph   约束图
 */
lv_PUBLIC_API void rewrite_history_add(RewriteHistory *history, ConstraintGraph *graph);

#ifdef __cplusplus
}
#endif

#endif /* lv_NORMALIZATION_H */
