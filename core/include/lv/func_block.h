 ...
 *
 * 使用示例：
 lv_PUBLIC_API *   FuncBlock *fb = func_block_create(1);
 lv_PUBLIC_API *   func_block_set_internal_nodes(fb, node_ids, count);
 lv_PUBLIC_API *   func_block_set_input_ports(fb, in_ports, in_count);
 lv_PUBLIC_API *   func_block_set_output_ports(fb, out_ports, out_count);
 *   PackConfig config = { ... };
 *   FuncBlock *result;
 lv_PUBLIC_API *   PackResult pr = func_block_pack_ex(graph, &config, &result);
 *
 * 根据 Lv-00 设计文档第8节实现：
 * - 打包操作（PackFunction）
 * - 确定性检查（静态层 + 动态层）
...
/* 距离默认值 */
#ifndef lv_DEFAULT_DISTANCE_SQUARED
#define lv_DEFAULT_DISTANCE_SQUARED 1e-6
#endif
...
/* 函数块ID偏移量 */
#ifndef lv_FUNC_BLOCK_ID_OFFSET
#define lv_FUNC_BLOCK_ID_OFFSET 0
#endif
...
/* ============== 例化结果 ============== */
typedef enum {
    lv_INSTANTIATE_OK,                  /* 例化成功 */
    lv_INSTANTIATE_NO_SOLUTION,         /* 无解 */
    lv_INSTANTIATE_MULTIPLE_SOLUTIONS,  /* 多解 */
    lv_INSTANTIATE_SELECTOR_NEEDED,     /* 需要选择器 */
    lv_INSTANTIATE_PRECONDITION_FAILED, /* 前置条件不满足 */
    lv_INSTANTIATE_OUT_OF_MEMORY        /* 内存不足 */
} InstantiateResult;
 ...
 *       统一释放，调用者不应单独释放这些成员。
 */
lv_PUBLIC_API FuncBlock *func_block_create(int id);
 ...
 *       调用后 fb 指针不可再使用。
 */
lv_PUBLIC_API void func_block_destroy(FuncBlock *fb);
 ...
 */
lv_PUBLIC_API bool func_block_set_internal_nodes(FuncBlock *fb, const int *node_ids, int count);
 ...
 * @return true 成功，false 失败
 */
lv_PUBLIC_API bool func_block_set_input_ports(FuncBlock *fb, const int *port_ids, int count);
 ...
 * @return true 成功，false 失败
 */
lv_PUBLIC_API bool func_block_set_output_ports(FuncBlock *fb, const int *port_ids, int count);
 ...
 *       函数块销毁时会自动释放关联的选择器。
 */
lv_PUBLIC_API bool func_block_set_selector(FuncBlock *fb, SolutionSelector *selector);
 ...
 * @return true 成功，false 失败
 */
lv_PUBLIC_API bool func_block_add_port_dependency(FuncBlock *fb, PortDependency *dep);
 ...
 * @return true 成功，false 失败
 */
lv_PUBLIC_API bool func_block_set_preconditions(FuncBlock *fb, const int *region_ids, int count);
 ...
 * @return true 成功，false 失败
 */
lv_PUBLIC_API bool func_block_set_name(FuncBlock *fb, const char *name);
 ...
 * @return true 成功，false 失败
 */
lv_PUBLIC_API bool func_block_set_description(FuncBlock *fb, const char *description);
 ...
 */
lv_PUBLIC_API int func_block_get_input_count(const FuncBlock *fb);
 ...
 */
lv_PUBLIC_API int func_block_get_output_count(const FuncBlock *fb);
 ...
 */
lv_PUBLIC_API int func_block_get_internal_count(const FuncBlock *fb);
 ...
 */
lv_PUBLIC_API int func_block_get_id(const FuncBlock *fb);
 ...
 */
lv_PUBLIC_API DeterminismState func_block_get_determinism(const FuncBlock *fb);
 ...
 */
lv_PUBLIC_API const char *func_block_get_name(const FuncBlock *fb);
 ...
 */
lv_PUBLIC_API FuncBlock *func_block_copy(const FuncBlock *src);
 ...
 * @return 确定性状态
 */
lv_PUBLIC_API DeterminismStatus func_block_determinism_check_static(FuncBlock *fb, const ConstraintGraph *graph);
 ...
 * @return 最终的确定性状态
 */
lv_PUBLIC_API DeterminismState func_block_verify_determinism(FuncBlock *fb, ConstraintGraph *graph, int step_limit);
 ...
 *       将所有权转移给函数块。
 */
lv_PUBLIC_API SolutionSelector *selector_create(SelectorType type);
 ...
 */
lv_PUBLIC_API SolutionSelector *selector_create_with_reference(SelectorType type, int reference_node_id);
 ...
 */
lv_PUBLIC_API SolutionSelector *selector_create_custom(SelectorFunction func, void *user_data);
 ...
 */
lv_PUBLIC_API void selector_destroy(SolutionSelector *selector);
 ...
 * @param graph 约束图指针
 */
lv_PUBLIC_API void selector_set_graph(SolutionSelector *selector, ConstraintGraph *graph);
 ...
 *       原函数块 f 和 g 的所有权不受影响。
 */
lv_PUBLIC_API bool func_block_compose(FuncBlock *f, FuncBlock *g, ConstraintGraph *graph, FuncBlock **out_composed);
 ...
 * @return 状态字符串
 */
lv_PUBLIC_API const char *determinism_state_to_string(DeterminismState state);
 ...
 * @return 结果字符串
 */
lv_PUBLIC_API const char *pack_result_to_string(PackResult result);
 ...
 * @return 结果字符串
 */
lv_PUBLIC_API const char *instantiate_result_to_string(InstantiateResult result);
 ...
 */
lv_PUBLIC_API char *func_block_serialize_state(const FuncBlock *fb);
 ...
 * @return 是否成功
 */
lv_PUBLIC_API bool func_block_deserialize_state(FuncBlock *fb, const char *data);
 ...
 * @param state 视图状态
 */
lv_PUBLIC_API void func_block_set_view_state(FuncBlock *fb, FuncBlockViewState state);
 ...
 * @return 视图状态
 */
lv_PUBLIC_API FuncBlockViewState func_block_get_view_state(const FuncBlock *fb);
 ...
 */
lv_PUBLIC_API void func_block_set_stream_context(StreamContext *ctx);

#ifdef __cplusplus
}
#endif

#endif /* lv_FUNC_BLOCK_H */
