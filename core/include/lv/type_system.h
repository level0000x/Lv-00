...
#ifndef lv_TYPE_SYSTEM_H
#define lv_TYPE_SYSTEM_H
 ...
 */
lv_PUBLIC_API void type_system_set_stream_context(StreamContext *ctx);
 ...
 *
 * 良基宇宙层级：
 * - Level 0: 基本几何体（点、线段、端口）
 * - Level 1: 类型区域（如"所有线段的集合"）
 * - Level 2+: 更高阶类型
 *
 * UNIVERSE_LEVEL_ANY: 关闭层级检查（非良基模式）
 */
typedef enum {
    UNIVERSE_LEVEL_0 = 0,   /* 基本几何体 */
    UNIVERSE_LEVEL_1 = 1,   /* 类型区域 */
    UNIVERSE_LEVEL_2 = 2,   /* 高阶类型 */
    UNIVERSE_LEVEL_3 = 3,   /* 更高阶 */
    UNIVERSE_LEVEL_ANY = -1 /* 关闭层级检查（非良基模式） */
} UniverseLevel;
 ...
 * @brief 创建重写路径
 */
lv_PUBLIC_API TypeRewritePath *type_rewrite_path_create(void);

/**
 * @brief 销毁重写路径
 */
lv_PUBLIC_API void type_rewrite_path_destroy(TypeRewritePath *path);
 ...
 */
lv_PUBLIC_API bool type_rewrite_path_replay(TypeRewritePath *path, int target_step);
 ...
 */
lv_PUBLIC_API const TypeRewritePath *type_system_get_rewrite_path(const TypeSystem *ts);
 ...
 * 创建类型系统
 */
lv_PUBLIC_API TypeSystem *type_system_create(void);

/**
 * 销毁类型系统
 */
lv_PUBLIC_API void type_system_destroy(TypeSystem *ts);

/**
 * 设置良基模式
 */
lv_PUBLIC_API void type_system_set_well_founded(TypeSystem *ts, bool well_founded);

/**
 * 设置累积性
 */
lv_PUBLIC_API void type_system_set_cumulative(TypeSystem *ts, bool cumulative);
 ...
 * 创建点类型
 */
lv_PUBLIC_API TypeRegion *type_create_point(TypeSystem *ts);

/**
 * 创建线段类型
 */
lv_PUBLIC_API TypeRegion *type_create_line_segment(TypeSystem *ts);

/**
 * 创建区域类型
 */
lv_PUBLIC_API TypeRegion *type_create_region(TypeSystem *ts, const int *contained_ids, int count);

/**
 * 创建函数类型
 */
lv_PUBLIC_API TypeRegion *type_create_function(TypeSystem *ts, TypeRegion *input, TypeRegion *output);

/**
 * 创建乘积类型
 */
lv_PUBLIC_API TypeRegion *type_create_product(TypeSystem *ts, TypeRegion *left, TypeRegion *right);

/**
 * 创建和类型
 */
lv_PUBLIC_API TypeRegion *type_create_sum(TypeSystem *ts, TypeRegion *first, TypeRegion *second);

/**
 * 创建类型变量
 */
lv_PUBLIC_API TypeRegion *type_create_variable(TypeSystem *ts, const char *name);

/**
 * 创建依赖类型
 */
lv_PUBLIC_API TypeRegion *type_create_dependent(TypeSystem *ts, int param_id, TypeRegion *body);

/**
 * 创建底部类型
 */
lv_PUBLIC_API TypeRegion *type_create_bottom(TypeSystem *ts);
 ...
 */
lv_PUBLIC_API bool type_check_predicate_subtype_value(TypeSystem *ts, TypeRegion *subtype, int node_id);
 ...
 */
lv_PUBLIC_API TypeRegion *type_predicate_subtype_get_base(TypeRegion *subtype);
 ...
 * 添加类型别名
 */
lv_PUBLIC_API bool type_add_alias(TypeRegion *tr, const char *alias);
 ...
 * 获取类型的宇宙层级
 */
lv_PUBLIC_API UniverseLevel type_get_level(TypeRegion *tr);
 ...
 * @return 是否有效
 */
lv_PUBLIC_API bool type_check_level_validity(TypeSystem *ts, TypeRegion *container, TypeRegion *contained);
 ...
 * @return 是否成功推断
 */
lv_PUBLIC_API bool type_infer_node(TypeSystem *ts, ConstraintGraph *graph, int node_id, TypeRegion **out_type);
 ...
 * @return 是否成功推断
 */
lv_PUBLIC_API bool type_infer_port(TypeSystem *ts, ConstraintGraph *graph, int port_id, TypeRegion **out_type);
 ...
 * @return 是否成功
 */
lv_PUBLIC_API bool type_instantiate_variable(TypeSystem *ts, int var_id, TypeRegion *concrete_type);
 ...
 * @return 是否存在循环
 */
lv_PUBLIC_API bool type_detect_cycle(TypeSystem *ts, TypeRegion *type);
 ...
 * @return 是否相容
 */
lv_PUBLIC_API bool type_check_non_well_founded_compatibility(TypeSystem *ts, TypeRegion *type);
 ...
 * @return 是否成功
 */
lv_PUBLIC_API bool type_normalize(TypeSystem *ts, TypeRegion *type, TypeRegion **out_normalized);
 ...
 */
lv_PUBLIC_API bool type_attach_to_node(TypeSystem *ts, int node_id, TypeRegion *type);
 ...
 */
lv_PUBLIC_API TypeRegion *type_get_node_type(const TypeSystem *ts, int node_id);
 ...
 */
lv_PUBLIC_API bool type_detach_node_type(TypeSystem *ts, int node_id);
 ...
 * 类型种类转字符串
 */
lv_PUBLIC_API const char *type_kind_to_string(TypeKind kind);

/**
 * 宇宙层级转字符串
 */
lv_PUBLIC_API const char *universe_level_to_string(UniverseLevel level);

/**
 * 类型等价结果转字符串
 */
lv_PUBLIC_API const char *type_equiv_result_to_string(TypeEquivResult result);

/**
 * 类型检查结果转字符串
 */
lv_PUBLIC_API const char *type_check_result_to_string(TypeCheckResult result);

/**
 * 打印类型
 */
lv_PUBLIC_API void type_print(const TypeRegion *tr, int indent);
 ...
 */
lv_PUBLIC_API const TypeInferenceRule *type_system_get_inference_rules(TypeSystem *ts, int *rule_count);

/**
 * 清除所有自定义推断规则（恢复为默认规则集）
 * @param ts 类型系统
 */
lv_PUBLIC_API void type_system_clear_inference_rules(TypeSystem *ts);
 ...
 */
lv_PUBLIC_API TypeEquivResult type_infer_by_rules(TypeSystem *ts, ConstraintGraph *graph, int node_id);
 ...
 */
lv_PUBLIC_API PathExplorer *path_explorer_create(TypeSystem *ts, TypeRegion *current, TypeRegion *target);

/**
 * @brief 销毁路径探索器
 * @param explorer 探索器指针
 */
lv_PUBLIC_API void path_explorer_destroy(PathExplorer *explorer);
 ...
 */
lv_PUBLIC_API ExplorerResult path_explorer_apply_rule(PathExplorer *explorer, int rule_index);
 ...
 */
lv_PUBLIC_API ExplorerResult path_explorer_undo(PathExplorer *explorer);
 ...
 */
lv_PUBLIC_API ExplorerResult path_explorer_check_goal(const PathExplorer *explorer, bool *reached);
 ...
 */
lv_PUBLIC_API ExplorerResult path_explorer_save_path(const PathExplorer *explorer, TypeRewritePath **out_path);
 ...
 * @return 步骤数量
 */
lv_PUBLIC_API int path_explorer_get_step_count(const PathExplorer *explorer);
 ...
 */
lv_PUBLIC_API const ExplorerStep *path_explorer_get_steps(const PathExplorer *explorer);
 ...
 */
lv_PUBLIC_API const TypeRegion *path_explorer_get_current(const PathExplorer *explorer);

#ifdef __cplusplus
}
#endif

#endif /* lv_TYPE_SYSTEM_H */
