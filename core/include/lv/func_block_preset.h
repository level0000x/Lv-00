 ...
 *
 * 使用示例：
 lv_PUBLIC_API *   func_block_preset_library_init();
 *   FuncBlock *fb;
...
#ifndef lv_FUNC_BLOCK_PRESET_H
#define lv_FUNC_BLOCK_PRESET_H
...
#endif /* lv_PRESET_PARAM_TYPE_ENUM_DEFINED */
 ...
 */
lv_PUBLIC_API bool func_block_preset_library_init(void);
 ...
 * 释放所有预设函数块资源，清理全局注册表。
 */
lv_PUBLIC_API void func_block_preset_library_cleanup(void);
 ...
 */
lv_PUBLIC_API const PresetMetadata *func_block_preset_get_metadata(const char *preset_name);
 ...
 * @param details 实例化详情
 */
lv_PUBLIC_API void func_block_preset_free_details(InstantiateDetails *details);
 ...
 */
lv_PUBLIC_API int func_block_preset_get_input_count(const char *preset_name);
 ...
 */
lv_PUBLIC_API int func_block_preset_get_output_count(const char *preset_name);
 ...
 */
lv_PUBLIC_API int func_block_preset_list(const char **out_names, int max_count, PresetCategory category);
 ...
 */
lv_PUBLIC_API bool func_block_preset_exists(const char *preset_name);

/**
 * @brief 获取当前已注册的预设函数块总数
 * @return 已注册的预设数量
 */
lv_PUBLIC_API int func_block_preset_count(void);
 ...
 */
lv_PUBLIC_API int func_block_preset_unregister(const char *name);
 ...
 * @return 类别名称字符串（静态存储）
 */
lv_PUBLIC_API const char *func_block_preset_category_string(PresetCategory category);
 ...
 * @return 类型名称字符串（静态存储）
 */
lv_PUBLIC_API const char *func_block_preset_param_type_string(PresetParamType type);
 ...
 */
lv_PUBLIC_API const char *func_block_preset_complexity_string(PresetComplexity complexity);
 ...
 * @return 实际写入的字符数
 */
lv_PUBLIC_API int func_block_preset_properties_string(PresetProperty properties, char *out_buffer, size_t buffer_size);
 ...
 */
lv_PUBLIC_API bool func_block_preset_compose(const char *f_name, const char *g_name, const char *new_preset_name);
 ...
 */
lv_PUBLIC_API const char *func_block_preset_get_inverse(const char *preset_name);
 ...
 */
lv_PUBLIC_API bool func_block_preset_register_custom(const PresetMetadata *metadata, const FuncBlock *template_fb);
 ...
 */
lv_PUBLIC_API size_t func_block_preset_generate_doc(const char *preset_name, char *out_buffer, size_t buffer_size);
 ...
 * @return 实际需要的缓冲区大小（包括\0），失败返回0
 */
lv_PUBLIC_API size_t func_block_preset_generate_index(char *out_buffer, size_t buffer_size);

#ifdef __cplusplus
}
#endif

#endif /* lv_FUNC_BLOCK_PRESET_H */
