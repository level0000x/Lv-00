 ...
 */
typedef struct {
    double lv_TOLERATED_FLOAT(center_val);    /**< 中心点处的函数值 f(center) */
    double lv_TOLERATED_FLOAT(*first_derivs); /**< 一阶偏导数数组 df/dxi|center */
    int *deriv_var_ids;                       /**< 导数对应的变量 ID 数组 */
    int deriv_count;                          /**< 偏导数数量（= 变量数） */
    double lv_TOLERATED_FLOAT(interval_lo);   /**< 区间下界（含泰勒余项） */
    double lv_TOLERATED_FLOAT(interval_hi);   /**< 区间上界（含泰勒余项） */
 ...
 */
typedef struct {
    double lv_TOLERATED_FLOAT(lo); /**< 区间下界 */
    double lv_TOLERATED_FLOAT(hi); /**< 区间上界 */
