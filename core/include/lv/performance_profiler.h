 ...
 *
 * @details 提供两大功能：
 *   1. 基准测试（lv_perf_benchmark_run）：对函数进行高精度计时，
 *      自动校准迭代次数，输出均值/最小值/最大值/标准差。
 *   2. 性能会话（lvPerfSession）：命名区域计时 + 内存分配追踪，
 *      支持文本报告和 JSON 导出。
 *
 * 使用示例：
 *   // 基准测试
 *   lvPerfBenchResult result;
 *   lv_perf_benchmark_run("my_func", my_func, NULL, &result);
 *   lv_perf_benchmark_print_result("my_func", &result, stdout);
 *
 *   // 性能会话
 *   lvPerfSession *s = lv_perf_session_create("my_session");
 *   lv_perf_begin(s, "region_a");
 *   // ... do work ...
 *   lv_perf_end(s, "region_a");
 *   lv_perf_session_record_alloc(s, "MyType", 1024);
 *   lv_perf_report_print(s, stdout);
 *   lv_perf_session_destroy(s);
 *
 * @version 1.0.0
 */

#ifndef lv_PERFORMANCE_PROFILER_H
#define lv_PERFORMANCE_PROFILER_H
 ...
 * - 报告输出（文本 / JSON）
 */
typedef struct lvPerfSession lvPerfSession;
...
#endif /* lv_PERFORMANCE_PROFILER_H */
