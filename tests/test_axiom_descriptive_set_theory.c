...
#define AXIOM_PKG_PATH "axiom_packages/descriptive_set_theory.lvz"
#define SAVE_TEST_PATH "axiom_packages/descriptive_set_theory_test_save.lvz"
...
static void test_save_load_roundtrip(void)
{
    ...
    TEST_ASSERT(strcmp(pkg2->name, "descriptive_set_theory") == 0,
...
static void test_dependency_validation(void)
{
    ...
    TEST_ASSERT(valid == false,
        "dependency validation should fail with no loaded packages (cross-package deps)");
...
static void test_external_references(void)
{
    ...
    for (int i = 0; i < pkg->unconstructible_count; i++) {
        ...
        TEST_ASSERT(strncmp(uc->external_ref, "https://", 8) == 0,
            "external reference should be a valid URL");
    }
...
static void test_key_templates_present(void)
{
    ...
    const char *key_templates[] = {
        "polish_space",
        "borel_sigma_algebra",
        "analytic_set_sigma_1_1",
        "projective_projection_sigma_1_n_plus_1",
        "borel_determinacy",
