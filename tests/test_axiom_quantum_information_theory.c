...
#define AXIOM_PKG_PATH "axiom_packages/quantum_information_theory.lvz"
#define SAVE_TEST_PATH "axiom_packages/quantum_information_theory_test_save.lvz"
...
static void test_templates(void)
{
    ...
    TEST_ASSERT(pkg->template_count == EXPECTED_TEMPLATE_COUNT,
        "should have 98 constraint templates");
...
static void test_unconstructibles(void)
{
    ...
    const char *expected_names[] = {
        "quantum_separability_problem",
        "quantum_channel_capacity_computation",
        "optimal_state_discrimination",
        "bqp_vs_np",
        "quantum_circuit_optimization",
        "general_entanglement_detection",
        "qec_code_optimization",
    ...
    for (int i = 0; i < EXPECTED_UNCONSTRUCTIBLE_COUNT; i++) {
        KnownUnconstructible *uc =
            axiom_package_lookup_unconstructible(pkg, expected_names[i]);
        if (uc) {
            found_count++;
            printf("  [%d] '%s' -> reduces_to: '%s', external_ref: '%s'\n",
                   i + 1, uc->name,
                   uc->reduces_to ? uc->reduces_to : "(null)",
    ...
    TEST_ASSERT(found_count == EXPECTED_UNCONSTRUCTIBLE_COUNT,
        "all expected unconstructible problems should be found");

    /* Verify specific external references */
    KnownUnconstructible *uc1 =
        axiom_package_lookup_unconstructible(pkg, "quantum_separability_problem");
    TEST_ASSERT(uc1 != NULL && uc1->external_ref != NULL,
        "quantum_separability_problem should have external_ref");
    TEST_ASSERT(uc1->green_verified == true,
        "quantum_separability_problem should be green_verified");

    /* BQP vs NP is an open problem */
    KnownUnconstructible *uc2 =
        axiom_package_lookup_unconstructible(pkg, "bqp_vs_np");
    TEST_ASSERT(uc2 != NULL,
        "bqp_vs_np should exist");
    TEST_ASSERT(uc2->green_verified == false,
        "bqp_vs_np should NOT be green_verified (open problem)");
...
static void test_round_trip_save_load(void)
{
    ...
    TEST_ASSERT(strcmp(pkg1->name, pkg2->name) == 0,
        "names should match after round-trip");
    TEST_ASSERT(strcmp(pkg1->version, pkg2->version) == 0,
        "versions should match after round-trip");
    TEST_ASSERT(pkg1->template_count == pkg2->template_count,
        "template counts should match after round-trip");
...
static void test_dependency_validation(void)
{
    ...
    TEST_ASSERT(uc->dependency_count >= 1,
        "quantum_separability_problem should have at least 1 dependency");

    uc = axiom_package_lookup_unconstructible(pkg, "bqp_vs_np");
    TEST_ASSERT(uc != NULL, "bqp_vs_np should exist");
    TEST_ASSERT(uc->dependency_count >= 1,
        "bqp_vs_np should have at least 1 dependency");
...
static void test_negative_lookups(void)
{
    ...
    KnownUnconstructible *uc =
        axiom_package_lookup_unconstructible(pkg, "non_existent_problem");
...
static void test_key_templates_present(void)
{
    ...
    TEST_ASSERT(axiom_package_get_template(pkg, "unitary_evolution") != NULL,
        "unitary_evolution should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "povm_measurement") != NULL,
        "povm_measurement should exist");

    /* Fundamental theorems */
    TEST_ASSERT(axiom_package_get_template(pkg, "no_cloning_theorem") != NULL,
        "no_cloning_theorem should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "quantum_teleportation") != NULL,
        "quantum_teleportation should exist");

    /* Entanglement */
    TEST_ASSERT(axiom_package_get_template(pkg, "bell_states") != NULL,
        "bell_states should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "entanglement_entropy") != NULL,
        "entanglement_entropy should exist");

    /* Quantum entropy */
    TEST_ASSERT(axiom_package_get_template(pkg, "von_neumann_entropy") != NULL,
        "von_neumann_entropy should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "holevo_bound") != NULL,
        "holevo_bound should exist");

    /* Quantum channels */
    TEST_ASSERT(axiom_package_get_template(pkg, "kraus_representation") != NULL,
        "kraus_representation should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "depolarizing_channel") != NULL,
        "depolarizing_channel should exist");

    /* Quantum gates */
    TEST_ASSERT(axiom_package_get_template(pkg, "cnot_gate") != NULL,
        "cnot_gate should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "universal_gate_set") != NULL,
        "universal_gate_set should exist");

    /* Quantum cryptography */
    TEST_ASSERT(axiom_package_get_template(pkg, "bb84_protocol") != NULL,
        "bb84_protocol should exist");

    /* Bell inequalities */
    TEST_ASSERT(axiom_package_get_template(pkg, "chsh_inequality") != NULL,
        "chsh_inequality should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "tsirelson_bound") != NULL,
        "tsirelson_bound should exist");
