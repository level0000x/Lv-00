...
#define AXIOM_PKG_PATH "axiom_packages/boolean_algebra.lvz"
#define SAVE_TEST_PATH "axiom_packages/boolean_algebra_test_save.lvz"
...
static void test_templates(void)
{
    ...
    TEST_ASSERT(pkg->template_count == EXPECTED_TEMPLATE_COUNT,
        "should have 30 constraint templates");
    ...
    const char *expected_templates[] = {
        ...
        "meet_identity",
        /* Distributive axioms */
        "meet_distributes_over_join",
        "join_distributes_over_meet",
        /* Complement axioms */
        "complement_join",
        "complement_meet",
...
static void test_logical_framework(void)
{
    ...
    TEST_ASSERT(pkg->bottom_geometry != NULL &&
                strcmp(pkg->bottom_geometry, "boolean_algebra_2element") == 0,
...
static void test_template_detailed_params(void)
{
    ...
    for (int i = 0; i < checks; i++) {
        ConstraintTemplate *t =
            axiom_package_get_template(pkg, param_checks[i].name);
        char msg[128];
        snprintf(msg, sizeof(msg), "%s should have %d params",
                 param_checks[i].name, param_checks[i].expected_params);
        TEST_ASSERT(t != NULL && t->param_count == param_checks[i].expected_params,
            msg);
    }
