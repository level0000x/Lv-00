...
static void test_templates(void)
{
    ...
    TEST_ASSERT(pkg->template_count == EXPECTED_TEMPLATE_COUNT,
        "should have 66 constraint templates");
    printf("  Template count: %d (expected %d)\n",
           pkg->template_count, EXPECTED_TEMPLATE_COUNT);

    const char *expected_templates[] = {
        /* Group I: σ-Algebra Axioms (7) */
        "sigma_algebra_contains_X",
        "sigma_algebra_complement",
        "sigma_algebra_countable_union",
        "sigma_algebra_countable_intersection",
        "sigma_algebra_contains_empty",
        "sigma_algebra_set_difference",
        "sigma_algebra_symmetric_difference",
        /* Group II: Core Measure Axioms (8) */
        "measure_null_empty_set",
        "measure_non_negativity",
        "measure_countable_additivity",
        "measure_finite_additivity",
        "measure_monotonicity",
        "measure_countable_subadditivity",
        "measure_continuity_from_below",
        "measure_continuity_from_above",
        /* Group III: Outer Measure Axioms (3) */
        "outer_measure_null_empty",
        "outer_measure_monotonicity",
        "outer_measure_countable_subadditivity",
        /* Group IV: Carathéodory Measurability (3) */
        "caratheodory_measurability",
        "measurable_sets_form_sigma_algebra",
        "outer_measure_restriction_complete",
        /* Group V: Null Sets and Completeness (4) */
        "null_set",
        "null_set_subset_measurable",
        "almost_everywhere",
        "measure_completion",
        ...
        "pre_measure",
        "caratheodory_outer_measure_construction",
...
static void test_unconstructible_problems(void)
{
    ...
    struct {
        ...
        { "vitali_set_non_measurable",
          "axiom_of_choice", 2, true },
        { "banach_tarski_paradox",
          "non_measurable_decomposition", 2, true },
        { "universal_measure_on_all_subsets",
          "non_measurable_set_existence", 2, true },
        { "all_sets_measurable_consistency",
          "independence_from_ZF_DC", 2, true },
        { "hausdorff_dimension_computation",
          "algorithmic_undecidability", 2, false },
        { "measure_space_isomorphism_classification",
          "isomorphism_problem", 2, false },
        { "measure_extension_uniqueness_without_sigma_finite",
          "non_uniqueness_of_extension", 2, true },
        { "riemann_integrability_decision",
          "lebesgue_measure_zero_set", 2, true },
    };
...
static void test_logical_framework(void)
{
    ...
    TEST_ASSERT(pkg->bottom_geometry != NULL &&
                strcmp(pkg->bottom_geometry, "measure_space_extended_reals") == 0,
        "bottom_geometry should be 'measure_space_extended_reals'");
    printf("  bottom_geometry: %s\n", pkg->bottom_geometry);

    TEST_ASSERT(pkg->negation_encoding != NULL &&
                strcmp(pkg->negation_encoding, "classical_equality") == 0,
...
static void test_external_refs(void)
{
    ...
    for (int i = 0; expected_urls[i] != NULL; i++) {
        ...
        for (int j = 0; j < pkg->unconstructible_count; j++) {
            ...
            if (uc->external_ref &&
                strcmp(uc->external_ref, expected_urls[i]) == 0) {
                found = true;
                break;
            }
        }
