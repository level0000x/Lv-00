...
static void test_templates(void)
{
    ...
    for (int i = 0; i < n_checks; i++) {
        ...
        snprintf(msg, sizeof(msg),
            "template '%s' should exist and have %d param(s)",
            checks[i].name, checks[i].expected_params);
...
static void test_unconstructible_problems(void)
{
    ...
    const char *expected_uc[] = {
        "mpt_isomorphism_undecidable",
        "kolmogorov_spectral_isomorphism_problem",
        "ks_entropy_computation_undecidable",
        "k_automorphism_classification",
        "sft_conjugacy_undecidable",
        "invariant_measure_existence_noncompact",
        "rokhlin_standardness_without_choice",
        "von_neumann_amenability_conjecture",
    };

    for (int i = 0; i < 8; i++) {
        KnownUnconstructible *uc =
            axiom_package_lookup_unconstructible(pkg, expected_uc[i]);
        char msg[256];
        snprintf(msg, sizeof(msg),
            "unconstructible '%s' should exist", expected_uc[i]);
        TEST_ASSERT(uc != NULL, msg);

        if (uc) {
            /* Verify external_ref is present */
            snprintf(msg, sizeof(msg),
                "unconstructible '%s' should have external_ref", expected_uc[i]);
            ...
            if (strcmp(expected_uc[i], "kolmogorov_spectral_isomorphism_problem") == 0 ||
                ...
                snprintf(msg, sizeof(msg),
                    "unconstructible '%s' should be green_verified=false (open)",
                    expected_uc[i]);
                TEST_ASSERT(uc->green_verified == false, msg);
            } else {
                snprintf(msg, sizeof(msg),
                    "unconstructible '%s' should be green_verified=true",
                    expected_uc[i]);
...
static void test_logical_framework(void)
{
    ...
    TEST_ASSERT(strcmp(pkg->bottom_geometry,
        "measure_preserving_dynamical_system") == 0,
...
static void test_content_hash(void)
{
    ...
    TEST_ASSERT(strcmp(hash, hash2) == 0,
...
static void test_round_trip(void)
{
    ...
    TEST_ASSERT(load2 == AXIOM_LOAD_OK,
        "reload of saved file should succeed");

    /* Verify name and version survived */
    TEST_ASSERT(pkg2->name != NULL &&
        strcmp(pkg2->name, "ergodic_theory") == 0,
        "reloaded package name should be 'ergodic_theory'");
    TEST_ASSERT(pkg2->version != NULL &&
        strcmp(pkg2->version, "1.0.0") == 0,
    ...
    TEST_ASSERT(pkg2->template_count == EXPECTED_TEMPLATE_COUNT,
        "reloaded template count should match");

    /* Compute reloaded hash */
    char *hash2 = axiom_package_compute_content_hash(pkg2);

    /* Hashes should match */
    TEST_ASSERT(strcmp(hash_orig, hash2) == 0,
        "round-trip content hash should match original");
...
static void test_negative_lookups(void)
{
    ...
    ConstraintTemplate *tmpl =
        axiom_package_get_template(pkg, "non_existent_template");
    ...
    TEST_ASSERT(uc == NULL,
        "non-existent unconstructible should return NULL");
...
static void test_cross_group_consistency(void)
{
    ...
    ConstraintTemplate *erg_def =
        axiom_package_get_template(pkg, "ergodicity_definition");
    ConstraintTemplate *birkhoff =
        axiom_package_get_template(pkg, "birkhoff_pointwise_ergodic_theorem");
    ConstraintTemplate *poincare =
        axiom_package_get_template(pkg, "poincare_recurrence");
    ConstraintTemplate *entropy =
        axiom_package_get_template(pkg, "kolmogorov_sinai_entropy");
    ConstraintTemplate *mixing =
        axiom_package_get_template(pkg, "strong_mixing");
    ConstraintTemplate *ornstein =
        axiom_package_get_template(pkg, "ornstein_isomorphism_theorem");

    TEST_ASSERT(erg_def != NULL && birkhoff != NULL && poincare != NULL,
        "core ergodic theory concepts should all exist");

    TEST_ASSERT(entropy != NULL,
        "Kolmogorov-Sinai entropy should exist");

    TEST_ASSERT(mixing != NULL,
        "strong mixing concept should exist");

    TEST_ASSERT(ornstein != NULL,
        "Ornstein isomorphism theorem should exist");

    /* Verify that mixing_implies_ergodic exists (hierarchy relationship) */
    ConstraintTemplate *mixing_implies =
        axiom_package_get_template(pkg, "mixing_implies_ergodic");
    TEST_ASSERT(mixing_implies != NULL,
        "mixing_implies_ergodic template should exist");

    /* Verify that core constructors all exist */
    const char *constructors[] = {
        "product_system", "induced_transformation", "natural_extension",
        "suspension_flow", "rokhlin_tower"
    };
    for (int i = 0; i < 5; i++) {
        ConstraintTemplate *c =
            axiom_package_get_template(pkg, constructors[i]);
        char msg[256];
        snprintf(msg, sizeof(msg),
            "constructor '%s' should exist", constructors[i]);
...
static void test_edge_cases(void)
{
    ...
    /* NULL package */
    TEST_ASSERT(axiom_package_load(NULL, AXIOM_PKG_PATH) != AXIOM_LOAD_OK,
        "load with NULL package should fail");
    ...
    TEST_ASSERT(axiom_package_load(pkg, NULL) != AXIOM_LOAD_OK,
        "load with NULL filepath should fail");
