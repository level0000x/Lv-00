...
static void test_templates(void)
{
    ...
    TEST_ASSERT(pkg->template_count == EXPECTED_TEMPLATE_COUNT,
        "should have 106 constraint templates");
...
static void test_unconstructibles(void)
{
    ...
    struct {
        ...
        { "kolmogorov_complexity_computation", "halting_problem", true },
...
static void test_logical_framework(void)
{
    ...
    printf("  Contradiction behavior: explosion_principle\n");
...
static void test_round_trip_save_load(void)
{
    ...
    TEST_ASSERT(load_status == AXIOM_LOAD_OK,
        "round-trip load should return AXIOM_LOAD_OK");
    ...
    if (hash1) lv00_free((void**)&hash1);
    if (hash2) lv00_free((void**)&hash2);
...
static void test_template_coverage(void)
{
    ...
    struct {
        ...
        { "Khinchin Axioms",        "entropy_non_negativity" },
    ...
    for (int i = 0; i < num_domains; i++) {
        ConstraintTemplate *tmpl = axiom_package_get_template(
            pkg, domains[i].representative_template);
        if (tmpl) {
            covered++;
            printf("  OK: %s (%s)\n", domains[i].domain,
                   domains[i].representative_template);
        } else {
            printf("  MISSING: %s\n", domains[i].domain);
            g_fail_count++;
        }
    ...
    TEST_ASSERT(covered == num_domains,
        "all information theory domains should be covered");
