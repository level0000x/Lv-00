...
#define AXIOM_PKG_PATH "axiom_packages/lattice_theory.lvz"
#define SAVE_TEST_PATH "axiom_packages/lattice_theory_test_save.lvz"
...
static void test_logical_framework(void)
{
    ...
    TEST_ASSERT(pkg->bottom_geometry != NULL &&
                strcmp(pkg->bottom_geometry, "lattice_partial_order") == 0,
        "bottom_geometry should be 'lattice_partial_order'");
    printf("  bottom_geometry: %s\n", pkg->bottom_geometry);

    TEST_ASSERT(pkg->negation_encoding != NULL &&
                strcmp(pkg->negation_encoding, "complement_in_complemented_lattice") == 0,
...
static void test_lattice_axiom_coherence(void)
{
    ...
    const char *core_constructors[] = {
        "meet", "join", "leq_from_meet", "leq_from_join",
        "top_element", "bottom_element", "complement",
        NULL
    };
    for (int i = 0; core_constructors[i] != NULL; i++) {
        ConstraintTemplate *tmpl =
            axiom_package_get_template(pkg, core_constructors[i]);
        TEST_ASSERT(tmpl != NULL, core_constructors[i]);
    }

    /* Verify special lattice type verifiers are present */
    const char *type_verifiers[] = {
        "is_distributive", "is_modular", "is_complemented",
        "is_boolean_algebra", "is_heyting_algebra", "is_complete", "is_chain",
        NULL
    };
    for (int i = 0; type_verifiers[i] != NULL; i++) {
        ConstraintTemplate *tmpl =
            axiom_package_get_template(pkg, type_verifiers[i]);
        TEST_ASSERT(tmpl != NULL, type_verifiers[i]);
    }

    /* Verify fundamental theorems are present */
    const char *theorems[] = {
        "whitman_condition", "dedekind_macneille_completion",
        "birkhoff_representation", "stone_representation",
