...
#define AXIOM_PKG_PATH "axiom_packages/zfc_set_theory.lvz"
#define SAVE_TEST_PATH "axiom_packages/zfc_test_save.lvz"
    ...
    for (int i = 0; i < expected_count; i++) {
        ...
        if (uc) {
            ...
            TEST_ASSERT(uc->dependency_count == expected[i].dep_count,
                "dependency_count should match");
            TEST_ASSERT(uc->green_verified == expected[i].green_verified,
                "green_verified should be true");
            ...
            TEST_ASSERT(strncmp(uc->external_ref, "https://", 8) == 0,
                "external_ref should be an HTTPS URL");
...
static void test_logical_framework(void)
{
    ...
    TEST_ASSERT(pkg->bottom_geometry != NULL &&
                strcmp(pkg->bottom_geometry, "zfc_cumulative_hierarchy") == 0,
        "bottom_geometry should be 'zfc_cumulative_hierarchy'");
    printf("  bottom_geometry: %s\n", pkg->bottom_geometry);

    TEST_ASSERT(pkg->negation_encoding != NULL &&
                strcmp(pkg->negation_encoding, "classical_complement_in_set_universe") == 0,
