...
static int test_symbolic_coord_basic(void) {
    ...
    SymbolicCoord *amber = symbolic_coord_downgrade_to_amber(r2, 1e-15, 
        "Test downgrade for large number handling");
    assert(amber != NULL);
...
static int test_unification(void) {
    ...
    SimpleProposition *prop = simple_proposition_create(
        "test_prop",
        NULL, 0,  /* No input ports for this test */
