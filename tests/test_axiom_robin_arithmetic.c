...
#define AXIOM_PKG_PATH "axiom_packages/robin_arithmetic.lvz"
#define SAVE_TEST_PATH "axiom_packages/robin_arithmetic_test_save.lvz"
...
static void test_templates(void)
{
    ...
    TEST_ASSERT(pkg->template_count == EXPECTED_TEMPLATE_COUNT,
        "should have 39 constraint templates");
...
static void test_unconstructible_problems(void)
{
    ...
    struct {
        ...
        { "consistency_of_Q",
          "godel_second_incompleteness", 7, true },
        { "godel_sentence_for_Q",
          "godel_first_incompleteness", 3, true },
        { "add_commutativity_general",
          "induction_requirement", 3, true },
        { "mul_commutativity_general",
          "induction_requirement", 4, true },
        { "add_associativity_general",
          "induction_requirement", 3, true },
        { "successor_not_self_general",
          "induction_requirement", 3, true },
        { "decidability_of_Q",
          "essential_undecidability", 7, true },
        { "decidability_of_any_extension_of_Q",
          "tarski_mostowski_robinson_theorem", 7, true },
        { "hilbert_tenth_problem_for_Q",
          "matiyasevich_davis_putnam_robinson_theorem", 3, true },
        { "nonstandard_model_characterization_of_Q",
          "tennenbaum_independence", 3, true },
        { "uniform_solvability_equations_in_Q",
          "unsolvability_of_diophantine_equations", 2, true },
        { "recursive_separability_of_Q_theorems_from_refutations",
          "essential_inseparability_of_Q", 7, true },
    };
...
static void test_logical_framework(void)
{
    ...
    TEST_ASSERT(pkg->bottom_geometry != NULL &&
                strcmp(pkg->bottom_geometry, "natural_number_zero_without_predecessor") == 0,
        "bottom_geometry should be 'natural_number_zero_without_predecessor'");
    printf("  bottom_geometry: %s\n", pkg->bottom_geometry);

    TEST_ASSERT(pkg->negation_encoding != NULL &&
                strcmp(pkg->negation_encoding, "classical_falsum_in_first_order_logic") == 0,
    ...
    for (int i = 0; i < (int)(sizeof(ref_checks)/sizeof(ref_checks[0])); i++) {
        ...
        if (uc) {
            TEST_ASSERT(uc->external_ref != NULL &&
                        strstr(uc->external_ref, ref_checks[i].expected_url_prefix) != NULL,
...
static void test_key_axioms_present(void)
{
    ...
    const char *core_axioms[] = {
        "zero_not_successor",
        "successor_injective",
        "every_number_zero_or_successor",
        "add_zero_right",
        "add_successor_right",
        "mul_zero",
        "mul_successor_right",
    ...
    for (int i = 0; i < 7; i++) {
        ConstraintTemplate *tmpl =
            axiom_package_get_template(pkg, core_axioms[i]);
