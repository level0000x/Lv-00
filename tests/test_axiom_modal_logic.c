...
#define AXIOM_PKG_PATH "axiom_packages/modal_logic.lvz"
#define SAVE_TEST_PATH "axiom_packages/modal_logic_test_save.lvz"
...
static void test_load_from_file(void)
{
    ...
    TEST_ASSERT(pkg->name != NULL && strcmp(pkg->name, "modal_logic") == 0,
        "package name should be 'modal_logic'");
...
static void test_templates(void)
{
    ...
    TEST_ASSERT(pkg->template_count == EXPECTED_TEMPLATE_COUNT,
        "should have 32 constraint templates");
    ...
    const char *expected_templates[] = {
        ...
        "kripke_frame",
        "kripke_model",
        "satisfaction_at_world",
        "validity_in_frame",
        /* Group X: Derived Modal Principles */
        "modal_modus_tollens",
        "box_distributes_over_and",
        "diamond_monotonicity",
        "modal_negation",
...
static void test_unconstructible_problems(void)
{
    ...
    struct {
        ...
        { "modal_satisfiability_K", "PSPACE_complete_problem", false },
    ...
    for (int i = 0; expected[i].name != NULL; i++) {
        ...
        if (uc) {
            ...
            TEST_ASSERT(uc->green_verified == expected[i].green_verified,
                "green_verified should match expected value");
        } else {
    ...
    TEST_ASSERT(uc && uc->external_ref != NULL,
        "modal_satisfiability_K should have external_ref");
    if (uc && uc->external_ref) {
        TEST_ASSERT(strstr(uc->external_ref, "wikipedia.org") != NULL ||
                    strstr(uc->external_ref, "PSPACE") != NULL,
    ...
    TEST_ASSERT(uc && uc->external_ref != NULL,
        "modal_satisfiability_S5 should have external_ref");
...
static void test_roundtrip_save_load(void)
{
    ...
    TEST_ASSERT(load_status == AXIOM_LOAD_OK,
        "loading saved file should succeed");
    ...
    TEST_ASSERT(pkg1->template_count == pkg2->template_count,
        "template count should be preserved");
...
static void test_external_refs(void)
{
    ...
    for (int i = 0; i < pkg->unconstructible_count; i++) {
        ...
        if (uc->external_ref) {
            ...
            bool is_valid = false;
            if (strncmp(uc->external_ref, "http://", 7) == 0 ||
    ...
    TEST_ASSERT(valid_url_count > 0,
        "at least some unconstructible problems should have valid URLs");
...
static void test_key_axioms_present(void)
{
    ...
    TEST_ASSERT(axiom_package_get_template(pkg, "necessitation") != NULL,
        "Necessitation rule should be present");

    /* System T axiom */
    TEST_ASSERT(axiom_package_get_template(pkg, "reflexivity_T") != NULL,
        "T axiom (Reflexivity) should be present");

    /* System S4 axioms */
    TEST_ASSERT(axiom_package_get_template(pkg, "transitivity_4") != NULL,
        "4 axiom (Transitivity) should be present");

    /* System S5 axioms */
    TEST_ASSERT(axiom_package_get_template(pkg, "euclidean_5") != NULL,
        "5 axiom (Euclidean) should be present");

    /* Modal duality */
    TEST_ASSERT(axiom_package_get_template(pkg, "possibility_dual") != NULL,
        "Possibility duality should be present");
    TEST_ASSERT(axiom_package_get_template(pkg, "necessity_dual") != NULL,
        "Necessity duality should be present");

    /* Kripke semantics */
    TEST_ASSERT(axiom_package_get_template(pkg, "kripke_frame") != NULL,
        "Kripke frame constructor should be present");
    TEST_ASSERT(axiom_package_get_template(pkg, "kripke_model") != NULL,
        "Kripke model constructor should be present");
