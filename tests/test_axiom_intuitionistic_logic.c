...
#define AXIOM_PKG_PATH "axiom_packages/intuitionistic_logic.lvz"
#define SAVE_TEST_PATH "axiom_packages/intuitionistic_logic_test_save.lvz"
...
static void test_load_from_file(void)
{
    ...
    TEST_ASSERT(pkg->name != NULL &&
                strcmp(pkg->name, "intuitionistic_logic") == 0,
...
static void test_templates(void)
{
    ...
    const char *group_1[] = {
        "axiom_then_1_weakening",
        "axiom_then_2_distribution",
        "axiom_and_1_elim_left",
        "axiom_and_2_elim_right",
        "axiom_and_3_intro",
        "axiom_or_1_intro_left",
        "axiom_or_2_intro_right",
        "axiom_or_3_elim",
        "axiom_false_efq",
        "modus_ponens",
    };
...
static void test_unconstructibles(void)
{
    ...
    TEST_ASSERT(pkg->unconstructible_count == EXPECTED_UNCONSTRUCTIBLE_COUNT,
        "should have 7 unconstructible entries");
...
static void test_logical_framework(void)
{
    ...
    TEST_ASSERT(strstr(pkg->bottom_geometry, "heyting") != NULL ||
                strstr(pkg->bottom_geometry, "intuitionistic") != NULL,
    ...
    TEST_ASSERT(strstr(pkg->negation_encoding, "brouwer") != NULL ||
                strstr(pkg->negation_encoding, "bhk") != NULL ||
...
static void test_round_trip(void)
{
    ...
    TEST_ASSERT(strcmp(pkg2->name, "intuitionistic_logic") == 0,
        "saved/loaded package name should be preserved");
    TEST_ASSERT(strcmp(pkg2->version, "1.0.0") == 0,
        "saved/loaded package version should be preserved");

    /* Verify template count preserved */
    TEST_ASSERT(pkg2->template_count == EXPECTED_TEMPLATE_COUNT,
        "saved/loaded template count should match");
...
static void test_dependency_validation(void)
{
    ...
    TEST_ASSERT(valid == true,
        "self-dependency validation should succeed (all deps are in-package)");
    ...
    TEST_ASSERT(valid_empty == true,
        "dependency validation with empty external list should succeed");
...
static void test_external_references(void)
{
    ...
    for (int i = 0; i < pkg->unconstructible_count; i++) {
        ...
        TEST_ASSERT(uc->external_ref[0] != '\0',
            "external_ref should be non-empty");
...
static void test_classical_boundary(void)
{
     ...
     * as unconstructible */
    ConstraintTemplate *lem_tmpl = axiom_package_get_template(pkg,
        "lem_boundary_marker");
    TEST_ASSERT(lem_tmpl != NULL,
        "LEM boundary marker should exist as template");

    ConstraintTemplate *dne_tmpl = axiom_package_get_template(pkg,
        "dne_boundary_marker");
    TEST_ASSERT(dne_tmpl != NULL,
        "DNE boundary marker should exist as template");

    ConstraintTemplate *peirce_tmpl = axiom_package_get_template(pkg,
        "peirce_boundary_marker");
    TEST_ASSERT(peirce_tmpl != NULL,
        "Peirce boundary marker should exist as template");

    /* And they must be in the unconstructible list */
    KnownUnconstructible *lem_uc = axiom_package_lookup_unconstructible(pkg,
        "law_of_excluded_middle_unconstructible");
    TEST_ASSERT(lem_uc != NULL && lem_uc->green_verified,
        "LEM should be green_verified as unconstructible");

    KnownUnconstructible *dne_uc = axiom_package_lookup_unconstructible(pkg,
        "double_negation_elim_unconstructible");
    TEST_ASSERT(dne_uc != NULL && dne_uc->green_verified,
        "DNE should be green_verified as unconstructible");

    /* Also confirm that double negation INTRO is a valid template */
    ConstraintTemplate *dni_tmpl = axiom_package_get_template(pkg,
        "double_negation_intro");
    TEST_ASSERT(dni_tmpl != NULL,
        "double_negation_intro should be a valid template (not boundary marker)");
