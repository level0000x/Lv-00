...
#define AXIOM_PKG_PATH "axiom_packages/computational_complexity_theory.lvz"
#define SAVE_TEST_PATH "axiom_packages/computational_complexity_theory_test_save.lvz"
...
static void test_load_from_file(void)
{
    ...
    TEST_ASSERT(pkg->name != NULL &&
                strcmp(pkg->name, "computational_complexity_theory") == 0,
        "package name should be 'computational_complexity_theory'");
    TEST_ASSERT(pkg->version != NULL &&
                strcmp(pkg->version, "1.0.0") == 0,
...
static void test_templates(void)
{
    ...
    TEST_ASSERT(pkg->template_count == EXPECTED_TEMPLATE_COUNT,
        "should have 54 constraint templates");
...
static void test_negative_lookups(void)
{
    ...
    ConstraintTemplate *tmpl =
        axiom_package_get_template(pkg, "nonexistent_template");
...
static void test_external_refs(void)
{
    ...
    for (int i = 0; i < (int)(sizeof(ref_checks)/sizeof(ref_checks[0])); i++) {
        ...
        if (uc) {
            int url_ok = (uc->external_ref != NULL &&
                         strncmp(uc->external_ref, ref_checks[i].expected_prefix,
...
static void test_complexity_axiom_coherence(void)
{
    ...
    AxiomPackage *pkg = axiom_package_create(
        "computational_complexity_theory", "1.0.0");
    ...
    const char *machine_models[] = {
        "deterministic_turing_machine",
        "non_deterministic_turing_machine",
    ...
    for (int i = 0; machine_models[i] != NULL; i++) {
        ConstraintTemplate *tmpl =
            axiom_package_get_template(pkg, machine_models[i]);
        TEST_ASSERT(tmpl != NULL, machine_models[i]);
    }

    /* Verify fundamental complexity classes are present */
    const char *classes[] = {
        "class_P", "class_NP", "class_PSPACE", "class_EXPTIME",
        "class_L", "class_NL", "class_BPP", "class_BQP",
        NULL
    };
    for (int i = 0; classes[i] != NULL; i++) {
        ConstraintTemplate *tmpl =
            axiom_package_get_template(pkg, classes[i]);
        TEST_ASSERT(tmpl != NULL, classes[i]);
    }

    /* Verify reduction and completeness axioms are present */
    const char *reductions[] = {
        "polynomial_time_reduction", "NP_completeness", "NP_hardness",
        "Cook_Levin_theorem", "Karp_reduction", "many_one_reduction",
        "Turing_reduction",
        NULL
    };
    for (int i = 0; reductions[i] != NULL; i++) {
        ConstraintTemplate *tmpl =
            axiom_package_get_template(pkg, reductions[i]);
    ...
    const char *hierarchy[] = {
        ...
        "Ladner_theorem", "Savitch_theorem", "hierarchy_separation",
        NULL
    };
    for (int i = 0; hierarchy[i] != NULL; i++) {
        ConstraintTemplate *tmpl =
            axiom_package_get_template(pkg, hierarchy[i]);
        TEST_ASSERT(tmpl != NULL, hierarchy[i]);
    }

    /* Verify class relationships are present */
    const char *relationships[] = {
        "P_subseteq_NP", "NP_subseteq_PSPACE",
        "PSPACE_subseteq_EXPTIME", "L_subseteq_NL",
        "NL_subseteq_P", "P_vs_NP_open",
        NULL
    };
    for (int i = 0; relationships[i] != NULL; i++) {
        ConstraintTemplate *tmpl =
            axiom_package_get_template(pkg, relationships[i]);
        TEST_ASSERT(tmpl != NULL, relationships[i]);
    }

    /* Verify important problems are present */
    const char *problems[] = {
        "SAT_problem", "3SAT_problem", "Hamiltonian_path_problem",
        "vertex_cover_problem", "clique_problem",
        NULL
    };
    for (int i = 0; problems[i] != NULL; i++) {
        ConstraintTemplate *tmpl =
            axiom_package_get_template(pkg, problems[i]);
        TEST_ASSERT(tmpl != NULL, problems[i]);
    }

    /* Verify specialized theorems are present */
    const char *theorems[] = {
        "Immerman_Szelepcsényi_theorem", "Fagin_theorem",
        "PCP_theorem", "IP_equals_PSPACE",
        NULL
    };
    for (int i = 0; theorems[i] != NULL; i++) {
        ConstraintTemplate *tmpl =
            axiom_package_get_template(pkg, theorems[i]);
        TEST_ASSERT(tmpl != NULL, theorems[i]);
    }

    printf("Test 10 passed: all complexity theory axioms, classes, reductions, "
