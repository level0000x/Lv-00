...
static void test_templates(void)
{
    ...
    TEST_ASSERT(pkg->template_count == EXPECTED_TEMPLATE_COUNT,
        "should have 50 constraint templates");
    ...
    const char *expected_templates[] = {
        ...
        "existence_of_affine_frame_3d",
        /* Group II: Parallelism Axioms (4) */
        "parallel_through_point",
        "parallelism_reflexive",
        "parallelism_transitive",
        "parallelism_no_intersection",
        /* Group III: Vector Space of Displacements (11) */
        "point_subtraction",
        "point_translation",
        "vector_addition_associative",
        "vector_addition_commutative",
        "zero_vector_exists",
        "vector_negation",
        "scalar_multiplication",
        "scalar_distributivity_vectors",
        "scalar_distributivity_scalars",
        "scalar_multiplication_associative",
        "scalar_unit",
        /* Group IV: Affine Structure Axioms (4) */
        "subtraction_identity",
        "subtraction_chain",
        "translation_subtraction_compat",
        "subtraction_translation_compat",
        /* Group V: Affine Combinations (7) */
        "affine_combination_two",
        "midpoint",
        "centroid_three_points",
        "general_barycenter",
        "affine_combination_associative",
        "affine_combination_commutative",
        "affine_combination_idempotent",
        /* Group VI: Affine Transformations (8) */
        "affine_map_preserves_combination",
        "translation_map",
        "affine_map_decomposition",
        "affine_map_preserves_parallelism",
        "affine_map_preserves_ratio",
        "affine_map_preserves_midpoint",
        "affine_map_preserves_centroid",
        "affine_map_preserves_barycenter",
        ...
        "affine_subspace_check",
        "affine_span_two_points",
        "affine_span_three_points",
        "affine_subspace_dimension",
        "affine_subspaces_parallel",
        /* Group X: Ratio and Division (4) */
        "simple_ratio",
        "thales_intercept_theorem",
        "ceva_theorem",
        "menelaus_theorem",
    ...
    TEST_ASSERT(t && t->param_count == 2,
        "line_through_two_points should have 2 params (point A, point B)");
...
static void test_unconstructible_problems(void)
{
    ...
    struct {
        ...
        { "perpendicular_bisector",
          "orthogonality requires metric structure", 2, true },
        { "angle_trisection",
          "requires metric structure and solving cubic equations", 2, true },
        { "circle_construction",
          "circles require metric distance notion absent in affine geometry", 2, true },
        { "finite_affine_plane_non_prime_power",
          "existence of finite affine planes of non-prime-power order is an open problem in combinatorics; equivalent to existence of finite projective planes", 3, false },
        ...
        { "metric_recovery_from_affine",
          "an affine space admits infinitely many inequivalent metric structures; no canonical choice without additional data", 2, true },
        { "area_computation",
          "area requires a notion of determinant or metric; only ratios of areas on parallel lines are affine invariants", 2, true },
    };
...
static void test_logical_framework(void)
{
    ...
    TEST_ASSERT(pkg->bottom_geometry != NULL &&
                strcmp(pkg->bottom_geometry, "affine_space") == 0,
...
static void test_content_hash(void)
{
    ...
    TEST_ASSERT(strcmp(hash1, hash2) == 0,
        "identical packages should have identical hashes");
...
static void test_round_trip_save_load(void)
{
    ...
    TEST_ASSERT(load_status == AXIOM_LOAD_OK,
        "re-loading saved file should return AXIOM_LOAD_OK");
    ...
    TEST_ASSERT(pkg2->template_count == pkg->template_count,
    ...
    TEST_ASSERT(strcmp(pkg2->name, pkg->name) == 0,
        "package name should match after round-trip");
    TEST_ASSERT(strcmp(pkg2->version, pkg->version) == 0,
        "package version should match after round-trip");
    ...
    TEST_ASSERT(hash1 != NULL && hash2 != NULL,
        "both hashes should be valid after round-trip");
    ...
    for (int i = 0; i < pkg->unconstructible_count; i++) {
        ...
        if (uc->external_ref && strlen(uc->external_ref) > 0) {
            ...
            TEST_ASSERT(is_https,
                "external_ref should use https:// URL format");
...
static void test_template_groups(void)
{
    ...
    for (int i = 0; i < pkg->template_count; i++) {
        const char *name = pkg->templates[i].name;
        if (strcmp(name, "line_through_two_points") == 0 ||
            strcmp(name, "line_has_two_points") == 0 ||
            strcmp(name, "existence_of_triangle") == 0 ||
            strcmp(name, "existence_of_affine_frame_3d") == 0)
            incidence_count++;
        else if (strncmp(name, "parallel", 8) == 0)
            parallelism_count++;
        else if (strncmp(name, "vector", 6) == 0 ||
                 strncmp(name, "scalar", 6) == 0 ||
                 strcmp(name, "zero_vector_exists") == 0)
            vector_count++;
        else if (strncmp(name, "subtraction", 11) == 0 ||
                 strncmp(name, "translation", 11) == 0)
            affine_struct_count++;
        else if (strncmp(name, "affine_combination", 18) == 0 ||
                 strncmp(name, "midpoint", 8) == 0 ||
                 strncmp(name, "centroid", 8) == 0 ||
                 strncmp(name, "general_barycenter", 18) == 0)
            combination_count++;
        else if (strncmp(name, "affine_map", 10) == 0)
            transform_count++;
        else if (strcmp(name, "desargues_theorem_affine") == 0 ||
                 strcmp(name, "pappus_theorem_affine") == 0)
            theorem_count++;
        else if (strncmp(name, "affine_subspace", 15) == 0 ||
                 strncmp(name, "affine_span", 11) == 0)
            subspace_count++;
        else if (strncmp(name, "simple_ratio", 12) == 0 ||
                 strncmp(name, "thales", 6) == 0 ||
                 strncmp(name, "ceva", 4) == 0 ||
                 strncmp(name, "menelaus", 8) == 0)
            ratio_count++;
        else if (strncmp(name, "parallelogram", 13) == 0)
            parallelogram_count++;
        else if (strncmp(name, "projective", 9) == 0 ||
                 strncmp(name, "line_at_infinity", 16) == 0 ||
    ...
    int total = incidence_count + parallelism_count + vector_count +
                affine_struct_count + combination_count + transform_count +
                theorem_count + subspace_count + ratio_count +
                parallelogram_count + projective_count;
    TEST_ASSERT(total == EXPECTED_TEMPLATE_COUNT,
        "group totals should equal overall template count");
