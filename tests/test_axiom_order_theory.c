...
#define AXIOM_PKG_PATH "axiom_packages/order_theory.lvz"
#define SAVE_TEST_PATH "axiom_packages/order_theory_test_save.lvz"
...
static void test_load_from_file(void)
{
    ...
    TEST_ASSERT(pkg->name != NULL && strcmp(pkg->name, "order_theory") == 0,
        "package name should be 'order_theory'");
...
static void test_unconstructible_problems(void)
{
    ...
    struct {
        ...
        { "poset_convex_realizability",    "undecidable: determining if a poset is isomorphic to the inclusion order of convex sets in R^d", 2, true },
...
static void test_logical_framework(void)
{
    ...
    TEST_ASSERT(pkg->bottom_geometry != NULL &&
                strcmp(pkg->bottom_geometry, "hasse_diagram_poset") == 0,
        "bottom_geometry should be 'hasse_diagram_poset'");
    printf("  bottom_geometry: %s\n", pkg->bottom_geometry);

    TEST_ASSERT(pkg->negation_encoding != NULL &&
                strcmp(pkg->negation_encoding, "classical_order_negation") == 0,
...
static void test_template_group_coverage(void)
{
    ...
    for (int i = 0; i < num_groups; i++) {
        ConstraintTemplate *tmpl =
            axiom_package_get_template(pkg, group_representatives[i]);
        if (tmpl) {
            groups_found++;
    ...
    TEST_ASSERT(groups_found == num_groups,
        "all 8 template groups should be represented");
