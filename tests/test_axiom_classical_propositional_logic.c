...
#define AXIOM_PKG_PATH "axiom_packages/classical_propositional_logic.lvz"
#define SAVE_TEST_PATH "axiom_packages/classical_propositional_logic_test_save.lvz"
...
static void test_load_from_file(void)
{
    ...
    TEST_ASSERT(pkg->name != NULL &&
                strcmp(pkg->name, "classical_propositional_logic") == 0,
...
static void test_templates(void)
{
    ...
    TEST_ASSERT(pkg->template_count == EXPECTED_TEMPLATE_COUNT,
        "should have 59 constraint templates");
    ...
    const char *expected_templates[] = {
        ...
        "axiom_C_contrapositive",
        /* Group II: Frege's Original Axioms (1879) */
        "frege_proposition_8",
        "frege_proposition_28",
        "frege_proposition_31",
        "frege_proposition_41",
        ...
        "absorption_conjunction_disjunction",
        "absorption_disjunction_conjunction",
...
static void test_logical_framework(void)
{
    ...
    TEST_ASSERT(pkg->bottom_geometry != NULL &&
                strcmp(pkg->bottom_geometry, "classical_propositional_2valued") == 0,
        "bottom_geometry should be 'classical_propositional_2valued'");
    printf("  bottom_geometry: %s\n", pkg->bottom_geometry);

    TEST_ASSERT(pkg->negation_encoding != NULL &&
                strcmp(pkg->negation_encoding, "material_implication_to_falsum") == 0,
...
static void test_round_trip(void)
{
    ...
    TEST_ASSERT(load_status == AXIOM_LOAD_OK,
        "re-load from saved file should succeed");
...
static void test_axiom_systems_coverage(void)
{
    ...
    /* Łukasiewicz P2 */
    TEST_ASSERT(axiom_package_get_template(pkg, "axiom_K_weakening") != NULL,
        "P2 axiom K should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "axiom_S_distribution") != NULL,
        "P2 axiom S should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "axiom_C_contrapositive") != NULL,
        "P2 axiom C should exist");

    /* Frege's Begriffsschrift (1879) */
    TEST_ASSERT(axiom_package_get_template(pkg, "frege_proposition_8") != NULL,
        "Frege prop 8 should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "frege_proposition_28") != NULL,
        "Frege prop 28 should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "frege_proposition_31") != NULL,
        "Frege prop 31 should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "frege_proposition_41") != NULL,
        "Frege prop 41 should exist");

    /* Russell-Whitehead Principia Mathematica */
    TEST_ASSERT(axiom_package_get_template(pkg, "RW_tautology") != NULL,
        "RW tautology should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "RW_addition") != NULL,
        "RW addition should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "RW_commutation") != NULL,
        "RW commutation should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "RW_association") != NULL,
        "RW association should exist");
    TEST_ASSERT(axiom_package_get_template(pkg, "RW_distribution") != NULL,
        "RW distribution should exist");
...
static void test_functional_completeness(void)
{
    ...
    TEST_ASSERT(nand != NULL && nand->param_count == 2,
        "Sheffer stroke (NAND) should exist with 2 params");

    ConstraintTemplate *nor = axiom_package_get_template(pkg, "peirce_arrow");
    TEST_ASSERT(nor != NULL && nor->param_count == 2,
        "Peirce arrow (NOR) should exist with 2 params");

    /* XOR should also be present */
    ConstraintTemplate *xor = axiom_package_get_template(pkg, "exclusive_or");
    TEST_ASSERT(xor != NULL && xor->param_count == 2,
        "Exclusive or (XOR) should exist with 2 params");

    /* Peirce's law (characterizes classical logic) */
    ConstraintTemplate *peirce = axiom_package_get_template(pkg, "peirces_law");
    TEST_ASSERT(peirce != NULL && peirce->param_count == 2,
        "Peirce's law should exist with 2 params");
