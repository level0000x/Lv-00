...
{
    printf("Test 1: Load dependent_type_theory.lvz from file...\n");
    ...
    TEST_ASSERT(pkg->name != NULL && strcmp(pkg->name, "dependent_type_theory") == 0,
        "package name should be 'dependent_type_theory'");
...
static void test_save_load_roundtrip(void)
{
    ...
    TEST_ASSERT(strcmp(pkg2->name, "dependent_type_theory") == 0,
...
/* ──────────────────────────────────────────────
 * Test 10: Key dependent type theory template checks
 * ────────────────────────────────────────────── */
static void test_key_templates(void)
{
    ...
    /* Core type formers */
    const char *type_formers[] = {
        "pi_type", "sigma_type", "identity_type",
        "natural_number_type", "universe_type"
    };
    ...
    const char *intro_elim[] = {
        "lambda_abstraction_dependent", "pair_dependent",
        "refl", "zero", "successor",
        "application_dependent", "projection_first",
        "projection_second", "induction_natural", "path_induction"
    };
    ...
    const char *computation[] = {
        "beta_reduction_dependent", "eta_expansion_dependent",
        "computation_natural", "computation_identity"
    };
