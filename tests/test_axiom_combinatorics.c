...
{
    printf("Test 1: Load combinatorics.lvz from file...\n");
    ...
    TEST_ASSERT(pkg->name != NULL && strcmp(pkg->name, "combinatorics") == 0,
        "package name should be 'combinatorics'");
...
static void test_save_load_roundtrip(void)
{
    ...
    TEST_ASSERT(strcmp(pkg2->name, "combinatorics") == 0,
...
/* ──────────────────────────────────────────────
 * Test 10: Key combinatorics template checks
 * ────────────────────────────────────────────── */
static void test_key_templates(void)
{
    ...
    const char *counting_core[] = {
        "pigeonhole_principle", "inclusion_exclusion",
        "binomial_coefficient", "multinomial_coefficient",
        "stars_and_bars", "generating_function"
    };
    ...
    /* Graph theory basics */
    const char *graph_basics[] = {
        "graph_vertex", "graph_edge", "graph_path",
        "graph_cycle", "graph_tree", "graph_connected",
        "graph_bipartite", "graph_planar"
    };
    ...
    /* Ramsey theory */
    const char *ramsey_core[] = {
        "ramsey_number", "ramsey_theorem",
        "schur_theorem", "van_der_waerden_theorem"
    };
