...
#define AXIOM_PKG_PATH "axiom_packages/presburger_arithmetic.lvz"
#define SAVE_TEST_PATH "axiom_packages/presburger_arithmetic_test_save.lvz"
...
static void test_templates(void)
{
    ...
    TEST_ASSERT(pkg->template_count == EXPECTED_TEMPLATE_COUNT,
        "should have 74 constraint templates");
...
static void test_logical_framework(void)
{
    ...
    TEST_ASSERT(pkg->bottom_geometry != NULL &&
                strcmp(pkg->bottom_geometry, "presburger_natural_numbers") == 0,
        "bottom_geometry should be 'presburger_natural_numbers'");
    printf("  bottom_geometry: %s\n", pkg->bottom_geometry);

    TEST_ASSERT(pkg->negation_encoding != NULL &&
                strcmp(pkg->negation_encoding, "classical_first_order") == 0,
    ...
    for (int i = 0; i < (int)(sizeof(ref_checks)/sizeof(ref_checks[0])); i++) {
        ...
        if (uc) {
            TEST_ASSERT(uc->external_ref != NULL &&
                        strstr(uc->external_ref, ref_checks[i].expected_domain) != NULL,
                ref_checks[i].name);
...
static void test_no_multiplication(void)
{
    ...
    TEST_ASSERT(tmpl == NULL,
        "general 'multiplication' template should NOT exist in Presburger arithmetic");

    /* But multiplication by constants SHOULD exist */
    tmpl = axiom_package_get_template(pkg, "multiply_by_constant");
    TEST_ASSERT(tmpl != NULL,
        "'multiply_by_constant' template SHOULD exist");

    tmpl = axiom_package_get_template(pkg, "multiply_by_2");
    TEST_ASSERT(tmpl != NULL,
        "'multiply_by_2' template SHOULD exist");
