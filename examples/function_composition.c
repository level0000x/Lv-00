...
static FuncBlock* create_midpoint_function(ConstraintGraph *g)
{
    ...
    PackResult result = func_block_pack(
        g, internal, 4, inputs, 2, outputs, 1,
        NULL, 0, &fb);
...
static FuncBlock* create_distance_function(ConstraintGraph *g)
{
    ...
    FuncBlock *fb = NULL;
    PackResult result = func_block_pack(
        g, internal, 2, inputs, 2, outputs, 1,
        NULL, 0, &fb);
...
int main(void)
{
    ...
    /* 清理 */
    printf("\n清理资源...\n");
    if (midpoint_fb) func_block_destroy(midpoint_fb);
    if (distance_fb) func_block_destroy(distance_fb);
    graph_destroy(g);
