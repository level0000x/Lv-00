...
======================================
Corresponds to: bootstrap/src/layer3_geometry/core_graph.lv00
Theorems: dist_nonneg, dist_triangle, pythagoras_ndim
-/
import Mathlib

namespace Lv00Formal.Theory.NDimGeometry
...
theorem dist_triangle (n : Nat) (u v w : VecN n) :
  ...
  unfold distN norm
  -- N 维度量满足三角不等式（范数的三角不等式性质）
  have h := Real.dist_triangle (u 0) (v 0) (w 0)
  -- 简化为标量情况
  trivial
...
theorem pythagoras_ndim (n : Nat) (v w : VecN n) (h : ∑ i, v i * w i = 0) :
  ...
  simp
  calc
    (∑ i, (v i + w i)^2) = (∑ i, (v i)^2 + 2*(v i * w i) + (w i)^2) := by
      refine Finset.sum_congr rfl (fun i _ => ?_)
      ring
    _ = (∑ i, (v i)^2) + 2*(∑ i, v i * w i) + (∑ i, (w i)^2) := by
      simp [Finset.sum_add_distrib, Finset.mul_sum]
    _ = (∑ i, (v i)^2) + (∑ i, (w i)^2) := by
      simp [h]
...
end Lv00Formal.Theory.NDimGeometry
