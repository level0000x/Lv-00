...
=================================================
Corresponds to: bootstrap/src/layer6_visual/control_flow/visual_control.lv00
Theorems: drag_preserves_constraints
-/
import Lv00Formal.Theory.IR

namespace Lv00Formal.Theory.InteractiveGeoSoundness
...
/-- 拖动保持一般约束结构：不改动的点上的约束不变 -/
theorem drag_preserves_constraints (env : String → ℝ × ℝ) (c : IRConstraint) (pName : String) (newPos : ℝ × ℝ)
    (h : pName ∉ vars_of_constraint c) : ir_sem (drag_point env pName newPos) c := by
  intro hmem
  -- 由于 pName 不在约束变量中，拖动不影响此约束
  unfold drag_point
  -- 约束语义只依赖相关点，pName 无关
  trivial
...
end Lv00Formal.Theory.InteractiveGeoSoundness
