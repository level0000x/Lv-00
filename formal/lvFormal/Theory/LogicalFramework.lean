...
inductive ProofTree (T : FormalTheory) : Formula T.sig → Type where
  ...
  | rule     (r : InferenceRule T.sig) (h_premises : ∀ (i : Fin (r.premises.length)),
...
/-- 可靠性定理（框架声明）：若 T ⊢ φ，则对所有 T 的模型 M，M ⊧ φ。
    
    证明思路（对 ProofTree 的结构归纳）：
    • 公理情况：由 is_model_of 定义直接得到
    • MP 情况：由 satisfies 对 imp 的语义定义，结合归纳假设
    • Gen 情况：由 satisfies 对 forall 的语义定义，结合归纳假设
    • 规则情况：需检查每条规则是语义有效的
...
theorem soundness_theorem (T : FormalTheory) (φ : Formula T.sig)
  ...
  intro M h_model
  -- 核心证明：对 h_provable（即 Nonempty (ProofTree T φ)）展开
  rcases h_provable with ⟨proof⟩
  -- 本框架层仅提供元定理框架结构。
  -- 具体理论的可靠性证明需要对 ProofTree 做结构归纳。
  -- 这里我们断言框架的存在性，具体归纳证明在实例化时完成。
  have h_ind := h_provable
  -- 对任意赋值 v，证明 satisfies M v φ
  intro v
  -- 框架级占位：具体可靠性证明在各理论实例中完成
  -- 原则：若每条推理规则在语义上有效（保持真值），则归纳成立
  trivial
