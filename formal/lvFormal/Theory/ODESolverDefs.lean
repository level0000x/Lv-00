...
noncomputable def rk4_step (f : ℝ → State → State) (h : ℝ) (t_n : ℝ) (y_n : State) : State :=
  let k1 := f t_n y_n
...
/-- 谐波振荡器能量：E = x² + (v/omega)² -/
noncomputable def harmonic_energy (omega : ℝ) (s : State) : ℝ :=
...
/-- 谐波振荡器的精确流（解析解） -/
noncomputable def harmonic_exact_flow (omega : ℝ) (h : ℝ) (t_n : ℝ) (y_n : State) : State :=
  let x0 := y_n.2.1
  let v0 := y_n.2.2
  (t_n + h, 
...
noncomputable def area_swept (s1 s2 : State) : ℝ :=
