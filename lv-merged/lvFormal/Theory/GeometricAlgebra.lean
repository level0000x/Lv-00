...
@[ext] theorem Multivector.ext (a b : Multivector) (h1 : a.scalar = b.scalar)
...
/-- 几何积结合律：∀ a b c, (ab)c = a(bc)
    ...
    ⚠️ 已知问题：当前 `gp` 的向量部分缺少叉积项（只含标量缩放），
    简化版几何积不满足结合律（标量部分的点积项与向量部分的缺项导致交叉项不消）。
    此定理在数学上不成立，需先修正 `gp` 定义（补叉积与二重向量传播）后再证明。
    v4.33 下同样无法证明（该文件从未编译通过）。 -/
...
theorem vector_anticommute (u v : Multivector) (hu : u.scalar = 0 ∧ u.bivector = (0, 0, 0))
  ...
  constructor
  · unfold gp; simp [hus, hvs, hub, hvb]; ring
  · unfold gp; simp [hus, hvs, hub, hvb]

/-- 几何积单位元：scalar_mv 1 是 gp 的单位元
    ∀ a, gp a 1 = a 且 gp 1 a = a

    ⚠️ 已知问题：当前 `gp` 的二重向量部分恒为零（简化实现），
    因此 `gp (scalar_mv 1) a` 会清零 `a.bivector`，单位元性质对任意 a 不成立。
    需先修正 `gp` 定义（完整传播二重向量）后再证明。 -/
