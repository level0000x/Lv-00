...
structure Group where
  ...
  inv_left : ∀ a : carrier, op (inv a) a = id
  inv_right : ∀ a : carrier, op a (inv a) = id

instance : CoeSort Group (Type u) := ⟨Group.carrier⟩
...
/-- 逆元的逆元是自身。 -/
theorem inv_inv (G : Group) (a : G) : G.inv (G.inv a) = a :=
  (group_inverse_unique G (G.inv a) a (G.inv_left a)).symm

/-- 逆元与 op 的反序关系：(ab)⁻¹ = b⁻¹a⁻¹。 -/
theorem inv_mul_rev (G : Group) (a b : G) : G.inv (G.op a b) = G.op (G.inv b) (G.inv a) :=
  (group_inverse_unique G (G.op a b) (G.op (G.inv b) (G.inv a)) (by
    calc
      ...
      _ = G.id := G.inv_right a)).symm

/-- 单位元的逆元是自身。 -/
theorem group_inv_id (G : Group) : G.inv G.id = G.id := by
  apply (group_inverse_unique G G.id G.id ?_).symm
  simpa using G.id_left G.id
...
def subgroup_Inter (G : Group) {ι : Type*} (H : ι → Subgroup G) : Subgroup G where
  ...
  op_closed := by
    intro a b ha hb
    have ha' : ∀ i, a ∈ (H i).carrier :=
      Set.mem_iInter.mp ha
    have hb' : ∀ i, b ∈ (H i).carrier :=
      Set.mem_iInter.mp hb
    apply Set.mem_iInter.mpr
    intro i
    exact (H i).op_closed a b (ha' i) (hb' i)
  inv_closed := by
    intro a ha
    have ha' : ∀ i, a ∈ (H i).carrier :=
      Set.mem_iInter.mp ha
...
def trivial_subgroup (G : Group) : Subgroup G where
  ...
  inv_closed := by
    intro a ha
    have ha' : a = G.id := by simpa using ha
    subst ha'
    simp [group_inv_id G]
  id_mem := by simp
...
theorem homomorphism_id_map (G H : Group) (φ : GroupHomomorphism G H) :
  ...
  exact mul_right_cancel H (φ.map G.id) (φ.map G.id) H.id hcalc
...
/-- 同态的核是正规子群。 -/
def kernel_is_normal (G H : Group) (φ : GroupHomomorphism G H) :
    NormalSubgroup G := by
  let sg : Subgroup G := {
    ...
    op_closed := by
      ...
      have h : φ.map (G.op a b) = H.id := by
        calc
          φ.map (G.op a b) = H.op (φ.map a) (φ.map b) := φ.map_op a b
          _ = H.op H.id H.id := by rw [ha', hb']
          _ = H.id := H.id_left H.id
      exact h
    inv_closed := by
      intro a ha
      have ha' : φ.map a = H.id := ha
      have h : φ.map (G.inv a) = H.id := by
        calc
          φ.map (G.inv a) = H.inv (φ.map a) := homomorphism_inv_map G H φ a
          _ = H.inv H.id := by rw [ha']
          _ = H.id := group_inv_id H
      exact h
    id_mem := by
      have h : φ.map G.id = H.id := homomorphism_id_map G H φ
      exact h
  }
  let normal_cond : ∀ (a : G) (b : G), b ∈ sg.carrier → G.op (G.op a b) (G.inv a) ∈ sg.carrier := by
    intro a b hb
    have hb' : φ.map b = H.id := hb
    have h : φ.map (G.op (G.op a b) (G.inv a)) = H.id := by
      calc
        φ.map (G.op (G.op a b) (G.inv a)) = H.op (φ.map (G.op a b)) (φ.map (G.inv a)) := φ.map_op (G.op a b) (G.inv a)
        _ = H.op (H.op (φ.map a) (φ.map b)) (φ.map (G.inv a)) := by rw [φ.map_op a b]
        _ = H.op (H.op (φ.map a) H.id) (φ.map (G.inv a)) := by rw [hb']
        _ = H.op (φ.map a) (φ.map (G.inv a)) := by rw [H.id_right]
        _ = H.op (φ.map a) (H.inv (φ.map a)) := by rw [homomorphism_inv_map G H φ a]
        _ = H.id := H.inv_right (φ.map a)
    exact h
  exact {
    toSubgroup := sg
...
def QuotientGroup (G : Group) (N : NormalSubgroup G) : Type _ :=
  Quot (fun x y : G => G.op (G.inv x) y ∈ N.carrier)

namespace QuotientGroup

private def rel (G : Group) (N : NormalSubgroup G) (x y : G) : Prop :=
  G.op (G.inv x) y ∈ N.carrier
...
def op (G : Group) (N : NormalSubgroup G) (x y : QuotientGroup G N) : QuotientGroup G N :=
  Quot.liftOn₂ x y (fun a b => mk G N (G.op a b))
    (by
      intro a₁ a₂ ha b₁ b₂ hb
      apply Quot.sound
...
instance : CoeSort Ring (Type u) := ⟨Ring.carrier⟩

/-- 从环提取加法群。 -/
def Ring.additiveGroup (R : Ring) : Group where
  carrier := R.carrier
  op := R.add
  id := R.zero
  inv := R.neg
  ...
  id_left := fun a => by
    calc
      R.add R.zero a = R.add a R.zero := R.add_comm R.zero a
      _ = a := R.add_zero a
  id_right := R.add_zero
  inv_left := R.add_left_neg
  inv_right := fun a => by
    calc
      R.add a (R.neg a) = R.add (R.neg a) a := R.add_comm a (R.neg a)
      _ = R.zero := R.add_left_neg a
...
theorem mul_zero (R : Ring) (a : R) : R.mul a R.zero = R.zero := by
  have h_add_neg : R.add (R.mul a R.zero) (R.neg (R.mul a R.zero)) = R.zero := by
    calc
      R.add (R.mul a R.zero) (R.neg (R.mul a R.zero)) = R.add (R.neg (R.mul a R.zero)) (R.mul a R.zero) :=
        R.add_comm _ _
      _ = R.zero := R.add_left_neg _
  have h_double : R.mul a R.zero = R.add (R.mul a R.zero) (R.mul a R.zero) := by
    calc
  ...
  calc
    ...
    _ = R.add (R.mul a R.zero) (R.add (R.mul a R.zero) (R.neg (R.mul a R.zero))) := by rw [h_add_neg]
    _ = R.add (R.add (R.mul a R.zero) (R.mul a R.zero)) (R.neg (R.mul a R.zero)) := by rw [R.add_assoc]
    _ = R.add (R.mul a R.zero) (R.neg (R.mul a R.zero)) := by rw [h_double]
    _ = R.zero := h_add_neg

/-- 乘法零乘性质：0 * a = 0。 -/
theorem zero_mul (R : Ring) (a : R) : R.mul R.zero a = R.zero := by
  have h_add_neg : R.add (R.mul R.zero a) (R.neg (R.mul R.zero a)) = R.zero := by
    calc
      R.add (R.mul R.zero a) (R.neg (R.mul R.zero a)) = R.add (R.neg (R.mul R.zero a)) (R.mul R.zero a) :=
        R.add_comm _ _
      _ = R.zero := R.add_left_neg _
  have h_double : R.mul R.zero a = R.add (R.mul R.zero a) (R.mul R.zero a) := by
    calc
  ...
  calc
    ...
    _ = R.add (R.mul R.zero a) (R.add (R.mul R.zero a) (R.neg (R.mul R.zero a))) := by rw [h_add_neg]
    _ = R.add (R.add (R.mul R.zero a) (R.mul R.zero a)) (R.neg (R.mul R.zero a)) := by rw [R.add_assoc]
    _ = R.add (R.mul R.zero a) (R.neg (R.mul R.zero a)) := by rw [h_double]
    _ = R.zero := h_add_neg
...
structure Field extends Ring where
  ...
  inv : carrier → carrier
  inv_mul_cancel : ∀ a : carrier, a ≠ zero → mul (inv a) a = one
  mul_inv_cancel : ∀ a : carrier, a ≠ zero → mul a (inv a) = one
  one_ne_zero : one ≠ zero

instance : CoeSort Field (Type u) := ⟨fun f => f.carrier⟩
...
theorem field_mul_ne_zero (F : Field) (a b : F) (ha : a ≠ F.zero) (hb : b ≠ F.zero) :
  ...
  have hcalc : F.mul (F.inv a) (F.mul a b) = F.one := by
    calc
      ...
      _ = b := F.one_mul b
  have hzero : F.mul (F.inv a) F.zero = F.zero := mul_zero (F : Ring) (F.inv a)
...
/-- 稳定子是子群。 -/
def stabilizer_is_subgroup (G : Group) (X : Type u) (action : GroupAction G X) (x : X) :
    Subgroup G where
  carrier := stabilizer G X action x
  op_closed := by
    intro a b ha hb
    have ha' : action.act a x = x := ha
    have hb' : action.act b x = x := hb
    have h : action.act (G.op a b) x = x := by
      calc
  ...
  inv_closed := by
    intro a ha
    have ha' : action.act a x = x := ha
    have h : action.act (G.inv a) x = x := by
      calc
...
/-- 对称群：集合 X 上的所有双射构成的群。 -/
def SymmetricGroup (X : Type u) : Group where
  carrier := Equiv.Perm X
  op := λ f g => f.trans g
  id := Equiv.refl X
  inv := λ f => f.symm
...
def QuotientRing (R : Ring) (I : Ideal R) : Type _ :=
  Quot (fun x y : R => I.carrier (R.add x (R.neg y)))

/-- 自然同态 π: R → R/I。 -/
def quotient_map (R : Ring) (I : Ideal R) (a : R) : QuotientRing R I :=
  Quot.mk (fun x y : R => I.carrier (R.add x (R.neg y))) a
