...
/-- 模板参数合理性可判定（供 `decide` 使用）。 -/
instance (t : PackageTemplate) : Decidable (TemplateParamCountReasonable t) := by
  unfold TemplateParamCountReasonable
  infer_instance
...
/-- 外部引用可判定（供 `decide` 使用）。 -/
instance (u : UnconstructibleProblem) : Decidable (HasExternalReference u) := by
  unfold HasExternalReference
  infer_instance
