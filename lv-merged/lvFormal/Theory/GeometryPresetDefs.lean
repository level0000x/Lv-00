...
lemma shoelace_sum_cons_cons (a b : Pt) (tail : List Pt) :
    shoelace_sum (a :: b :: tail) = (a.x * b.y - a.y * b.x) + shoelace_sum (b :: tail) := by sorry
theorem shoelace_triangulate (P0 P1 P2 : Pt) (tail : List Pt) :
  shoelace_sum (P0 :: P1 :: P2 :: tail) =
    shoelace_sum [P0, P1, P2] + shoelace_sum (P0 :: P2 :: tail) := by sorry
theorem heron_formula_standard_valid (a b c : ℝ) (h : a + b > c ∧ b + c > a ∧ c + a > b) :
  heron_formula_standard a b c = triangle_area_sss a b c := by sorry
lemma dot_product_scales (A B C D E F : Pt) (s θ tx ty : ℝ) (hs_pos : s > 0)
    ...
    (D.x - E.x) * (F.x - E.x) + (D.y - E.y) * (F.y - E.y) = s^2 * ((A.x - B.x) * (C.x - B.x) + (A.y - B.y) * (C.y - B.y)) := by sorry
