...
static void bench_pool_alloc_free(void) {
    TestObj *obj = (TestObj *)lv00_pool_alloc(g_test_pool);
    if (obj) {
        obj->data[0] = 1;
...
static void test_pool_performance(void) {
    ...
    int err = lv00_perf_benchmark_run("pool_alloc_free", bench_pool_alloc_free, NULL, &result);
...
static void test_malloc_vs_pool(void) {
    ...
    lv00_perf_benchmark_run("malloc_free", bench_malloc_free, NULL, &malloc_result);
    ...
    if (g_test_pool) {
        ...
        lv00_perf_benchmark_run("pool_alloc_free", bench_pool_alloc_free, NULL, &pool_result);
