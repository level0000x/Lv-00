...
static ConstraintGraph *create_graph_with_constraint(int id1, int id2) {
    ...
    if (ac != ADD_CONSTRAINT_OK) { graph_destroy(g); return NULL; }
...
static void test_empty_graph_hash(void) {
    ConstraintGraph *g = graph_create();
    if (!g) { FAIL("create"); F++; return; }

    TEST("compute_complete_graph_hash 空图");
    GraphHash *h = compute_complete_graph_hash(g);
    if (!h) { FAIL("NULL"); graph_destroy(g); return; }
    /* 空图哈希应非零（FNV offset basis 作为初始值） */
    if (h->hash != 0) PASS();
    else FAIL("hash is 0");

    TEST("空图 node_hashes 为 NULL");
    if (h->node_hashes == NULL && h->node_count == 0) PASS();
    else FAIL("node_hashes not NULL or node_count not 0");
...
static void test_determinism(void) {
    ...
    if (p0 < 0 || p1 < 0) { FAIL("add_point"); graph_destroy(g); F++; return; }

    GraphHash *h1 = compute_complete_graph_hash(g);
    if (!h1) { FAIL("h1 NULL"); graph_destroy(g); F++; return; }

    GraphHash *h2 = compute_complete_graph_hash(g);
    if (!h2) { FAIL("h2 NULL"); graph_hash_destroy(h1); graph_destroy(g); F++; return; }

    TEST("相同图两次哈希值相同");
    if (h1->hash == h2->hash) PASS();
    else FAIL("different hashes");

    TEST("相同图 node_hashes 逐元素相同");
    bool equal = true;
    if (h1->node_count != h2->node_count) { equal = false; }
    else {
        for (int i = 0; i < h1->node_count; i++) {
            if (h1->node_hashes[i] != h2->node_hashes[i]) { equal = false; break; }
        }
    }
    if (equal) PASS();
    else FAIL("node hashes differ");
...
static void test_diff_ids_diff_hash(void) {
    ...
    if (!g1 || !g2) { FAIL("setup"); if(g1)graph_destroy(g1); if(g2)graph_destroy(g2); F++; return; }

    GraphHash *h1 = compute_complete_graph_hash(g1);
    GraphHash *h2 = compute_complete_graph_hash(g2);
    if (!h1 || !h2) { FAIL("hash NULL"); goto cleanup; }

    TEST("不同 ID 产生不同哈希");
    if (h1->hash != h2->hash) PASS();
    else FAIL("hashes are same (unexpected)");

cleanup:
    if (h1) graph_hash_destroy(h1);
    if (h2) graph_hash_destroy(h2);
    graph_destroy(g1);
    graph_destroy(g2);
}
...
static void test_hash_equal(void) {
    ...
    GraphHash *h1 = compute_complete_graph_hash(g);
    GraphHash *h2 = compute_complete_graph_hash(g);
    if (!h1 || !h2) { FAIL("hash NULL"); graph_destroy(g); F++; return; }

    TEST("graph_hash_equal NULL, NULL");
    if (!graph_hash_equal(NULL, NULL)) PASS();
    else FAIL("should be false");

    TEST("graph_hash_equal NULL, h");
    if (!graph_hash_equal(NULL, h1)) PASS();
    else FAIL("should be false");

    TEST("graph_hash_equal h, NULL");
    if (!graph_hash_equal(h1, NULL)) PASS();
    else FAIL("should be false");

    TEST("graph_hash_equal 相同 hash");
    if (graph_hash_equal(h1, h2)) PASS();
    else FAIL("should be equal");
    ...
    if (!h3) { FAIL("h3 NULL"); goto cleanup; }

    TEST("graph_hash_equal 不同 hash");
    if (!graph_hash_equal(h1, h3)) PASS();
    else FAIL("should be not equal");

    if (h3) graph_hash_destroy(h3);
cleanup:
    graph_hash_destroy(h1);
    graph_hash_destroy(h2);
    graph_destroy(g);
...
static void test_hash_destroy(void) {
    ...
    if (!g) { FAIL("create"); F++; return; }
    GraphHash *h = compute_complete_graph_hash(g);
    if (!h) { FAIL("hash NULL"); graph_destroy(g); F++; return; }
    graph_hash_destroy(h);
    PASS();
    graph_destroy(g);
}
...
static void test_quick_hash(void) {
    ...
    if (compute_quick_graph_hash(NULL) == 0) PASS();
