 ...
 *
 * 注意：由于 Lv00Context 结构复杂，本测试采用简化的独立测试方式：
 * 通过 lv00_context_create() 创建上下文，直接验证其内嵌熔断器的行为。
 *
 * @details 熔断器状态机：
 *
...
/* ============================================================
 * 辅助宏 —— 用于直接检查熔断器状态字段
 * （lv00_circuit_breaker_is_open/is_closed/is_half_open
 ...
 */
static bool cb_is_open(const struct Lv00Context *ctx) {
 ...
 */
static bool cb_is_closed(const struct Lv00Context *ctx) {
 ...
 */
static bool cb_is_half_open(const struct Lv00Context *ctx) {
 ...
 * @brief 获取连续错误计数
 */
static int cb_get_failure_count(const struct Lv00Context *ctx) {
 ...
 * @brief 测试熔断器创建时的初始状态
 *
 * 验证通过 lv00_context_create() 创建的上下文，其内嵌熔断器
 * 处于 CLOSED（正常）状态。
 */
 ...
 *
 * 在刚创建的上下文中：
 * - lv00_circuit_breaker_check 应返回 true（允许操作）
 * - 熔断器应处于 CLOSED 态
 * - 熔断器不应处于 OPEN 态
 * - 熔断器不应处于 HALF_OPEN 态
 */
static void test_circuit_breaker_initial_state(void) {
    ...
    TEST_ASSERT_MSG(lv00_circuit_breaker_check(ctx),
 ...
 * @brief 测试熔断器 trip 操作
 *
 * 调用 lv00_circuit_breaker_trip 后：
 * - 熔断器应进入 OPEN 态
 * - lv00_circuit_breaker_check 应返回 false（拒绝操作）
 * - 熔断器不应再处于 CLOSED 态
 */
static void test_circuit_breaker_trip(void) {
    ...
    /* 触发熔断 */
    lv00_circuit_breaker_trip(ctx, "test trip reason");
...
static void test_circuit_breaker_reset(void) {
    ...
    /* 触发熔断 */
    lv00_circuit_breaker_trip(ctx, "test trip before reset");
    TEST_ASSERT_MSG(cb_is_open(ctx),
                    "circuit breaker should be OPEN after trip (before reset)");

    /* 重置熔断器 */
    lv00_circuit_breaker_reset(ctx);
 ...
 * @brief 测试熔断器状态名称函数
 *
 * 验证 lv00_circuit_breaker_state_name 在不同状态下
...
static void test_circuit_breaker_state_name(void) {
    ...
    /* CLOSED 态的名称应非空 */
    const char *name_closed = lv00_circuit_breaker_state_name(ctx);
    ...
    lv00_circuit_breaker_trip(ctx, "test state name");
    const char *name_open = lv00_circuit_breaker_state_name(ctx);
    ...
    lv00_circuit_breaker_reset(ctx);
    const char *name_closed2 = lv00_circuit_breaker_state_name(ctx);
...
static void test_circuit_breaker_failure_count(void) {
    ...
    lv00_circuit_breaker_trip(ctx, "test failure count");
    TEST_ASSERT_MSG(cb_get_failure_count(ctx) == 0,
...
static void test_circuit_breaker_context_lifecycle(void) {
    ...
    /* 正常使用：trip */
    lv00_circuit_breaker_trip(ctx, "lifecycle test trip");
    TEST_ASSERT_MSG(cb_is_open(ctx),
                    "circuit breaker should be OPEN after trip in lifecycle test");

    /* 正常使用：state_name */
    const char *name = lv00_circuit_breaker_state_name(ctx);
    ...
    int summary_len = lv00_circuit_breaker_summary(ctx, summary_buf, sizeof(summary_buf));
    ...
    /* 验证 ctx 的 trip_reason 已被正确清理（通过 reset 中的 lv00_free）。
