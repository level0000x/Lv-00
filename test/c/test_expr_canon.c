 ...
 *
 * 测试 Lv00ExprCanonical 规范多项式模块的所有公共 API，涵盖：
...
static void test_expr_create_destroy(void) {
    ...
    Lv00ExprCanonical *expr = lv00_expr_canonical_create(2, var_names);
    ...
    lv00_expr_canonical_destroy(&expr);
...
static void test_expr_add_term(void) {
    ...
    /* 添加项 3*x^1*y^0，即 3x */
    Lv00Rational *c1 = lv00_rational_create_from_si(3, 1);
    TEST_ASSERT_NOT_NULL(c1);
    int exp1[] = {1, 0};
    bool ok1 = lv00_expr_canonical_add_term(expr, c1, exp1);
    TEST_ASSERT_MSG(ok1, "add_term for 3x should succeed");
    TEST_ASSERT_EQ(expr->term_count, 1);

    /* 添加项 2*x^0*y^1，即 2y */
    Lv00Rational *c2 = lv00_rational_create_from_si(2, 1);
    TEST_ASSERT_NOT_NULL(c2);
    int exp2[] = {0, 1};
    bool ok2 = lv00_expr_canonical_add_term(expr, c2, exp2);
    TEST_ASSERT_MSG(ok2, "add_term for 2y should succeed");
    TEST_ASSERT_EQ(expr->term_count, 2);

    lv00_rational_destroy(&c1);
    lv00_rational_destroy(&c2);
    lv00_expr_canonical_destroy(&expr);
}
...
static void test_expr_canonicalize_merge(void) {
    ...
    Lv00ExprCanonical *expr = lv00_expr_canonical_create(1, var_names);
    TEST_ASSERT_NOT_NULL(expr);
    ...
    /* 规范化：应合并为 5x */
    bool canon_ok = lv00_expr_canonicalize(expr);
    TEST_ASSERT_MSG(canon_ok, "canonicalize should succeed");
    TEST_ASSERT_EQ(expr->term_count, 1);

    /* 验证系数为 5/1 */
    Lv00Rational *expected = lv00_rational_create_from_si(5, 1);
    TEST_ASSERT_NOT_NULL(expected);
    int cmp = lv00_rational_cmp(expr->terms[0].coeff, expected);
    ...
    lv00_rational_destroy(&c1);
    lv00_rational_destroy(&c2);
    lv00_rational_destroy(&expected);
    lv00_expr_canonical_destroy(&expr);
}
...
static void test_expr_canonicalize_sort(void) {
    ...
    Lv00Rational *c1 = lv00_rational_create_from_si(1, 1);
    TEST_ASSERT_NOT_NULL(c1);
    int exp_z[] = {0, 1}; /* x^0 * z^1 */
    lv00_expr_canonical_add_term(expr, c1, exp_z);

    /* 添加 x^1*z^0 -> 即 x^1 (指数: x=1, z=0) */
    int exp_x[] = {1, 0}; /* x^1 * z^0 */
    lv00_expr_canonical_add_term(expr, c1, exp_x);
...
static void test_expr_canonicalize_remove_zero(void) {
    ...
    /* 添加 0*x */
    Lv00Rational *zero = lv00_rational_create_from_si(0, 1);
    TEST_ASSERT_NOT_NULL(zero);
    int exp[] = {1};
    lv00_expr_canonical_add_term(expr, zero, exp);

    /* 也应添加一个有效的项以确保 canonicalize 不会把整个多项式判空 */
    Lv00Rational *one = lv00_rational_create_from_si(1, 1);
    TEST_ASSERT_NOT_NULL(one);
    int exp2[] = {2};
    lv00_expr_canonical_add_term(expr, one, exp2);
 ...
 * 验证 lv00_expr_is_canonical 返回 true。
...
static void test_expr_is_canonical(void) {
    ...
    /* 添加 1*x^2*y^0 */
    Lv00Rational *c = lv00_rational_create_from_si(1, 1);
    TEST_ASSERT_NOT_NULL(c);
    int exp[] = {2, 0};
    lv00_expr_canonical_add_term(expr, c, exp);

    /* 规范化后应为规范形式 */
    lv00_expr_canonicalize(expr);
    TEST_ASSERT_MSG(lv00_expr_is_canonical(expr),
                    "freshly canonicalized expr should be canonical");

    /* 添加新项后应不再是规范形式 */
    int exp2[] = {0, 1};
    lv00_expr_canonical_add_term(expr, c, exp2);
    TEST_ASSERT_MSG(!lv00_expr_is_canonical(expr),
...
static void test_expr_clone(void) {
    const char *var_names[] = {"x", "y"};
    Lv00ExprCanonical *orig = lv00_expr_canonical_create(2, var_names);
    TEST_ASSERT_NOT_NULL(orig);

    /* 添加 3x + 2y */
    Lv00Rational *c3 = lv00_rational_create_from_si(3, 1);
    Lv00Rational *c2 = lv00_rational_create_from_si(2, 1);
    TEST_ASSERT_NOT_NULL(c3);
    TEST_ASSERT_NOT_NULL(c2);
    int exp_x[] = {1, 0};
    int exp_y[] = {0, 1};
    lv00_expr_canonical_add_term(orig, c3, exp_x);
    lv00_expr_canonical_add_term(orig, c2, exp_y);
    lv00_expr_canonicalize(orig);

    /* Clone */
    Lv00ExprCanonical *copy = lv00_expr_canonical_clone(orig);
    ...
    for (int i = 0; i < copy->term_count; i++) {
        int cmp = lv00_rational_cmp(copy->terms[i].coeff, orig->terms[i].coeff);
    ...
    int exp_z[] = {0, 2};
    lv00_expr_canonical_add_term(copy, c2, exp_z);
    lv00_expr_canonicalize(copy);
    TEST_ASSERT_EQ(orig->term_count, 2); /* 原多项式仍为 2 项 */

    lv00_rational_destroy(&c3);
    lv00_rational_destroy(&c2);
    lv00_expr_canonical_destroy(&orig);
    lv00_expr_canonical_destroy(&copy);
}
...
static void test_expr_add(void) {
    ...
    /* a = x + y */
    Lv00ExprCanonical *a = lv00_expr_canonical_create(2, var_names);
    TEST_ASSERT_NOT_NULL(a);
    Lv00Rational *c1 = lv00_rational_create_from_si(1, 1);
    TEST_ASSERT_NOT_NULL(c1);
    int exp_x[] = {1, 0};
    int exp_y[] = {0, 1};
    lv00_expr_canonical_add_term(a, c1, exp_x);
    lv00_expr_canonical_add_term(a, c1, exp_y);
    lv00_expr_canonicalize(a);

    /* b = 2x - y */
    Lv00ExprCanonical *b = lv00_expr_canonical_create(2, var_names);
    TEST_ASSERT_NOT_NULL(b);
    Lv00Rational *c2 = lv00_rational_create_from_si(2, 1);
    Lv00Rational *cn1 = lv00_rational_create_from_si(-1, 1);
    TEST_ASSERT_NOT_NULL(c2);
    TEST_ASSERT_NOT_NULL(cn1);
    lv00_expr_canonical_add_term(b, c2, exp_x);
    lv00_expr_canonical_add_term(b, cn1, exp_y);
    lv00_expr_canonicalize(b);

    /* a + b = 3x */
    Lv00ExprCanonical *result = lv00_expr_canonical_add(a, b);
    ...
    Lv00Rational *expected = lv00_rational_create_from_si(3, 1);
    TEST_ASSERT_NOT_NULL(expected);
    int cmp = lv00_rational_cmp(result->terms[0].coeff, expected);
    TEST_ASSERT_EQ(cmp, 0);

    lv00_rational_destroy(&c1);
    lv00_rational_destroy(&c2);
    lv00_rational_destroy(&cn1);
    lv00_rational_destroy(&expected);
    lv00_expr_canonical_destroy(&a);
    lv00_expr_canonical_destroy(&b);
...
static void test_expr_sub(void) {
    ...
    Lv00ExprCanonical *b = lv00_expr_canonical_create(2, var_names);
    TEST_ASSERT_NOT_NULL(b);
    lv00_expr_canonical_add_term(b, c1, exp_x);
    lv00_expr_canonicalize(b);

    /* a - b = y */
    Lv00ExprCanonical *result = lv00_expr_canonical_sub(a, b);
    TEST_ASSERT_NOT_NULL(result);
    ...
    Lv00Rational *expected = lv00_rational_create_from_si(1, 1);
    TEST_ASSERT_NOT_NULL(expected);
    int cmp = lv00_rational_cmp(result->terms[0].coeff, expected);
    TEST_ASSERT_EQ(cmp, 0);
 ...
 *
 * 验证 lv00_canonical_compare_terms 的行为：
 * - 总次数高的项应排在前面
 * - 同次数按字典序（最后变量优先比较）
 * - 完全相同指数应返回 0
 */
static void test_expr_compare_terms(void) {
    ...
    int exp_high[] = {2, 0};
    int exp_low[]  = {1, 0};
    int cmp_deg = lv00_canonical_compare_terms(exp_high, exp_low, var_count);
    ...
    int cmp_lex = lv00_canonical_compare_terms(exp_z, exp_x, var_count);
    ...
    int exp_same1[] = {1, 1};
    int exp_same2[] = {1, 1};
    int cmp_eq = lv00_canonical_compare_terms(exp_same1, exp_same2, var_count);
...
static void test_expr_to_string(void) {
    ...
    int exp_x[] = {1, 0};
    int exp_y[] = {0, 1};
    lv00_expr_canonical_add_term(expr, c2, exp_x);
    lv00_expr_canonical_add_term(expr, c1, exp_y);
    lv00_expr_canonicalize(expr);

    char *s = lv00_expr_canonical_to_string(expr);
 ...
 * ============================================================ */
int main(void) {
    TEST_SUITE_BEGIN("Lv00ExprCanonical");
