...
/* Test 3：不可构造项（文件特有：逐条断言 + strstr 检查，保留原体） */
...
/* Test 4：逻辑框架（文件特有：strstr 检查 + 引号输出，保留原体） */
...
/* Test 6：往返保存/加载（文件特有：计数打印 + 无哈希校验，保留原体） */
...
/* Test 7：依赖验证（文件特有：valid==true 强断言，保留原体） */
...
/* Test 9：外部引用（文件特有：strstr https 检查，保留原体） */
...
TEST_MAIN_BEGIN("Intuitionistic Logic")
    LV_REGISTER_AXIOM_CASES("IntuitionisticLogic", kCases, K_CASES_COUNT);
