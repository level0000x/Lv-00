...
static int test_case_lifecycle(void) {
    ...
    assert(tc->type == TEST_CASE_FACTORY);
    assert(tc->param_count == 2);
    ...
    assert(tc != NULL);
    assert(strcmp(tc->template_name, "UserCustomTest") == 0);
...
static int test_runner_basic(void) {
...
static int test_runner_high_level(void) {
...
static int test_normal_form_verification(void) {
...
static int test_edge_cases(void) {
    ...
    assert(null_pkg_result.total == 0);
    ...
    assert(null_name_result.total == 0);
