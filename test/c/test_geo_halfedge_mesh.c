...
int main(void) {
    ...
    printf("\n[组 5] 半边遍历\n");
    {
        ...
        if (out_he != lv_HE_INVALID) { PASS(); tests_passed++; }
        ...
        if (twin != lv_HE_INVALID) { PASS(); tests_passed++; }
    ...
    printf("\n[组 6] 边操作\n");
    {
        ...
        lvFace f = lv_he_mesh_add_face_triangle(mesh, v0, v1,
            lv_he_mesh_add_vertex(mesh, 0, 1, 0));
    ...
    printf("\n[组 10] 迭代器\n");
    {
        ...
        if (it.current != lv_HE_INVALID) { PASS(); tests_passed++; }
        else { FAIL("迭代器初始化失败"); tests_failed++; }

        TEST("face_iter: 面邻接半边迭代");
        /* 简化测试：直接检查迭代器初始化 */
        lvHeFaceIterator fit = lv_he_face_iter_begin(mesh, f);
        if (fit.current != lv_HE_INVALID && fit.count == 3) { PASS(); tests_passed++; }
        else { FAIL("迭代器初始化失败"); tests_failed++; }
