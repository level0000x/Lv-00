...
static void test_load_from_file(void) {
    ...
    TEST_ASSERT(pkg->name != NULL && strcmp(pkg->name, "cartesian_closed_category") == 0,
...
static void test_logical_framework(void) {
    ...
    TEST_ASSERT(pkg->bottom_geometry != NULL &&
                    strcmp(pkg->bottom_geometry, "directed_multigraph_with_products_and_exponentials") == 0,
                "bottom_geometry should be 'directed_multigraph_with_products_and_exponentials'");
    printf("  bottom_geometry: %s\n", pkg->bottom_geometry);

    TEST_ASSERT(pkg->negation_encoding != NULL &&
                    strcmp(pkg->negation_encoding, "exponential_to_terminal_A_implies_false") == 0,
