...
/* ── 测试 5d: S2 管道端到端（声明值管道 → loader 消费） ── */
static void test_pipe_pipeline(void) {
    printf("\n[S2 管道端到端]\n");

    /* B |> segment(A) == segment(B, A)：管道重写后 loader 第二遍建线段 */
    const char *src =
        "Point A = (0, 0);\n"
        "Point B = (1, 1);\n"
        "Segment S := B |> segment(A);\n";
...
TEST_MAIN_BEGIN("lv bootstrap test")
    ...
    TEST_MAIN_RUN(test_pipe_pipeline);
