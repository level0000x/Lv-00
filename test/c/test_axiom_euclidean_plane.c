...
static void test_templates(void) {
    ...
    TEST_ASSERT(axiom_package_get_template_count(pkg) == EXPECTED_TEMPLATE_COUNT, "should have 22 constraint templates");
    printf("  Template count: %d (expected %d)\n", axiom_package_get_template_count(pkg), EXPECTED_TEMPLATE_COUNT);

    const char *expected_templates[] = {"line_through_two_points",
                                        "line_has_two_points",
...
static void test_logical_framework(void) {
    ...
    TEST_ASSERT(pkg->bottom_geometry != NULL && strcmp(pkg->bottom_geometry, "euclidean_plane") == 0,
