...
int main(void) {
    ...
    printf("[组 1] TikZ 导出\n");
    {
        ...
        if (!e) { FAIL("create"); F = 99; goto g1_end; }
...
g1_end: lv_cleanup();
    ...
    printf("[组 2] 证明编译器\n");
    {
        ...
        if (po) { lv_proof_object_destroy(po); PASS(); }
        else FAIL("NULL");

        TEST("proof_object NULL安全");
        lv_proof_object_destroy(NULL); PASS();

        TEST("proof_object 初始状态");
        po = lv_proof_object_create();
        if (po && lv_proof_object_get_step_count(po) == 0 &&
            !lv_proof_object_is_valid(po)) {
            lv_proof_object_destroy(po); PASS();
        } else { lv_proof_object_destroy(po); FAIL("!初始"); }

        TEST("compiler 创建/销毁");
        lvCompilerConfig cfg = lv_compiler_config_default();
        lvProofCompiler *pc = lv_proof_compiler_create(&cfg);
        if (pc) { lv_proof_compiler_destroy(pc); PASS(); }
        else FAIL("NULL");

        TEST("compiler NULL安全");
        lv_proof_compiler_destroy(NULL); PASS();

        TEST("compiler_config_default");
        cfg = lv_compiler_config_default();
        if (cfg.format == OUTPUT_FORMAT_TEXT) PASS();
        else FAIL("!TEXT");

        TEST("step_record 创建/销毁");
        lvProofStepRecord *sr = lv_proof_step_record_create();
        if (sr) { lv_proof_step_record_destroy(sr); PASS(); }
        else FAIL("NULL");

        TEST("step_record NULL安全");
        lv_proof_step_record_destroy(NULL); PASS();
    ...
    printf("[组 3] 插件系统\n");
    {
        ...
        if (pc) { lv_plugin_config_destroy(pc); PASS(); }
        else FAIL("NULL");

        TEST("config NULL安全");
        lv_plugin_config_destroy(NULL); PASS();

        TEST("system 创建");
        lvPluginSystem *ps = lv_plugin_system_create(NULL);
        if (ps) { lv_plugin_system_destroy(ps); PASS(); }
        else FAIL("NULL");

        TEST("system NULL安全");
        lv_plugin_system_destroy(NULL); PASS();
        ...
        if (lv_plugin_broadcast_event(NULL, 0, NULL, (size_t)0) < 0) PASS();
        else FAIL("!负值");

        TEST("register_interface NULL=-1");
        if (lv_plugin_register_interface(NULL, NULL) < 0) PASS();
        else FAIL("!负值");

        TEST("unregister_interface NULL=-1");
        if (lv_plugin_unregister_interface(NULL, "test") < 0) PASS();
        else FAIL("!负值");

        TEST("query_interface NULL=NULL");
        if (lv_plugin_query_interface(NULL, "test", 0) == NULL) PASS();
        else FAIL("!NULL");

        TEST("query_interfaces NULL=NULL");
        lvPluginInterface **ifaces = lv_plugin_query_interfaces(NULL, NULL, &cnt);
        if (ifaces == NULL && cnt == 0) PASS();
        else FAIL("!expected");

        TEST("send_event NULL=-1");
        if (lv_plugin_send_event(NULL, 0, 0, (size_t)0) < 0) PASS();
        else FAIL("!负值");

        TEST("apply_config NULL=-1");
        if (lv_plugin_apply_config(NULL, NULL) < 0) PASS();
        else FAIL("!负值");
    }
