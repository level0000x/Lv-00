...
void test_ga_mv_zero(void) {
    ...
    for (int i = 0; i < GA_MV_DIM; i++) {
        TEST_ASSERT(approx_eq(ga_mv_get(mv, i), 0.0), "All components of zero multivector should be zero");
    }
...
void test_ga_mv_scalar(void) {
    ...
    TEST_ASSERT(approx_eq(ga_mv_get(mv, GA_BLADE_1), 42.0), "Scalar component should be 42.0");

    /* All other components should be zero */
    for (int i = 1; i < GA_MV_DIM; i++) {
        TEST_ASSERT(approx_eq(ga_mv_get(mv, i), 0.0), "Non-scalar components should be zero");
    }
...
void test_ga_geometric_product(void) {
    ...
    TEST_ASSERT(approx_eq(ga_mv_get(e1_sq, GA_BLADE_1), 1.0), "e1 * e1 should equal 1 (scalar)");

    /* Other components should be zero */
    for (int i = 1; i < GA_MV_DIM; i++) {
        TEST_ASSERT(approx_eq(ga_mv_get(e1_sq, i), 0.0), "e1*e1 should have no other components");
    }
    ...
    TEST_ASSERT(approx_eq(ga_mv_get(e1_e2, GA_BLADE_E12), 1.0), "e1 * e2 should have e12 component = 1");

    /* Test 3: e2 * e1 = -e12 (anti-commutativity) */
    lvMultiVector *e2_e1 = ga_geometric_product(e2, e1);
    TEST_ASSERT_NOT_NULL(e2_e1);
    TEST_ASSERT(approx_eq(ga_mv_get(e2_e1, GA_BLADE_E12), -1.0), "e2 * e1 should have e12 component = -1");
    ...
    for (int i = 0; i < GA_MV_DIM; i++) {
        TEST_ASSERT(approx_eq(ga_mv_get(e0_sq, i), 0.0), "e0 * e0 should be zero");
    }
...
void test_ga_outer_product(void) {
    ...
    TEST_ASSERT(approx_eq(ga_mv_get(wedge, GA_BLADE_E12), 1.0), "e1 ^ e2 should have e12 = 1");

    /* Test 2: e1 ^ e1 = 0 */
    lvMultiVector *self_wedge = ga_outer_product(e1, e1);
    TEST_ASSERT_NOT_NULL(self_wedge);
    for (int i = 0; i < GA_MV_DIM; i++) {
        TEST_ASSERT(approx_eq(ga_mv_get(self_wedge, i), 0.0), "e1 ^ e1 should be zero");
    }
...
void test_ga_reverse(void) {
    ...
    TEST_ASSERT(approx_eq(ga_mv_get(s_rev, GA_BLADE_1), 5.0), "Reverse of scalar should be itself");
    ...
    TEST_ASSERT(approx_eq(ga_mv_get(e1_rev, GA_BLADE_E1), 1.0), "Reverse of e1 should be e1");
    ...
    TEST_ASSERT(approx_eq(ga_mv_get(e12_rev, GA_BLADE_E12), -1.0), "Reverse of e12 should be -e12");
    ...
    TEST_ASSERT(approx_eq(ga_mv_get(e123_rev, GA_BLADE_E123), -1.0), "Reverse of e123 should be -e123");
...
void test_ga_outer_product_comprehensive(void) {
    ...
    /* ---- grade-1 ^ grade-1 -> grade-2: e1 ^ e2 = e12 ---- */
    {
        ...
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E12), 1.0), "e1 ^ e2: e12 component should be 1");

        /* Verify all other components are zero */
        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E12)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e1 ^ e2: non-e12 components should be zero");
        }
    ...
    /* ---- grade-1 ^ grade-2 -> grade-3: e0 ^ e12 = e012 ---- */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E012), 1.0), "e0 ^ e12: e012 component should be 1");

        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E012)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e0 ^ e12: non-e012 components should be zero");
        }
    ...
    /* ---- grade-2 ^ grade-1 -> grade-3: e12 ^ e3 = e123 ---- */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E123), 1.0), "e12 ^ e3: e123 component should be 1");

        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E123)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e12 ^ e3: non-e123 components should be zero");
        }
    ...
    /* ---- grade-2 ^ grade-2 -> grade-4: e01 ^ e23 = e0123 ---- */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E0123), 1.0), "e01 ^ e23: e0123 component should be 1");

        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E0123)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e01 ^ e23: non-e0123 components should be zero");
        }
    ...
    /* ---- scalar ^ anything = anything: 3.0 ^ e1 = 3*e1 ---- */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E1), 3.0), "3.0 ^ e1: e1 component should be 3.0");

        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E1)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "3.0 ^ e1: non-e1 components should be zero");
        }
    ...
    /* ---- anything ^ scalar = anything: e1 ^ 3.0 = 3*e1 ---- */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E1), 3.0), "e1 ^ 3.0: e1 component should be 3.0");

        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E1)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e1 ^ 3.0: non-e1 components should be zero");
        }
    ...
    /* ---- same basis element ^ itself = 0: e1 ^ e1 = 0 ---- */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        for (int i = 0; i < GA_MV_DIM; i++) {
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e1 ^ e1: all components should be zero");
        }
    ...
    /* ---- additional: e2 ^ e2 = 0 ---- */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        for (int i = 0; i < GA_MV_DIM; i++) {
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e2 ^ e2: all components should be zero");
        }
    ...
    /* ---- additional: e0 ^ e0 = 0 (null basis) ---- */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        for (int i = 0; i < GA_MV_DIM; i++) {
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e0 ^ e0: all components should be zero");
        }
...
void test_ga_outer_product_anticommutativity(void) {
    ...
    for (int i = 0; i < GA_MV_DIM; i++) {
        ...
        TEST_ASSERT(approx_eq(sum, 0.0), "e1^e2 + e2^e1: all components should sum to zero (anticommutativity)");
    }

    /* Specifically check e12: e1^e2 = +e12, e2^e1 = -e12 */
    TEST_ASSERT(approx_eq(ga_mv_get(r12, GA_BLADE_E12), 1.0), "e1 ^ e2: e12 should be +1");
    TEST_ASSERT(approx_eq(ga_mv_get(r21, GA_BLADE_E12), -1.0), "e2 ^ e1: e12 should be -1");
    ...
    TEST_ASSERT(approx_eq(ga_mv_get(r03, GA_BLADE_E03), 1.0), "e0 ^ e3: e03 should be +1");
    TEST_ASSERT(approx_eq(ga_mv_get(r30, GA_BLADE_E03), -1.0), "e3 ^ e0: e03 should be -1");

    for (int i = 0; i < GA_MV_DIM; i++) {
        double sum = ga_mv_get(r03, i) + ga_mv_get(r30, i);
        TEST_ASSERT(approx_eq(sum, 0.0), "e0^e3 + e3^e0: all components should sum to zero (anticommutativity)");
    }
...
void test_ga_outer_product_associativity(void) {
    ...
    TEST_ASSERT(approx_eq(ga_mv_get(left, GA_BLADE_E012), 1.0), "(e0^e1)^e2: e012 should be 1");
    TEST_ASSERT(approx_eq(ga_mv_get(right, GA_BLADE_E012), 1.0), "e0^(e1^e2): e012 should be 1");
    ...
    TEST_ASSERT(approx_eq(ga_mv_get(left2, GA_BLADE_E123), 1.0), "(e1^e2)^e3: e123 should be 1");
    TEST_ASSERT(approx_eq(ga_mv_get(right2, GA_BLADE_E123), 1.0), "e1^(e2^e3): e123 should be 1");
    ...
    ga_mv_destroy(left);
    ga_mv_destroy(right);
    ga_mv_destroy(e12_v2);
    ga_mv_destroy(e23);
    ga_mv_destroy(left2);
    ga_mv_destroy(right2);
    printf("  PASSED\n");
}
...
void test_ga_geometric_product_full(void) {
    ...
    /* e12 * e12 = -1 (bivector squared gives negative scalar) */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_1), -1.0), "e12 * e12 should equal -1");
        for (int i = 1; i < GA_MV_DIM; i++) {
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e12 * e12: non-scalar components should be zero");
        }
        ga_mv_destroy(e12);
        ga_mv_destroy(r);
    }
    ...
    /* e12 * e23 = e13 */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E13), 1.0), "e12 * e23 should have e13 component = 1");
        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E13)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e12 * e23: non-e13 components should be zero");
        }
        ga_mv_destroy(e12);
        ga_mv_destroy(e23);
        ga_mv_destroy(r);
    ...
    /* e12 * e13 = -e23 */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E23), -1.0), "e12 * e13 should have e23 component = -1");
        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E23)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e12 * e13: non-e23 components should be zero");
        }
        ga_mv_destroy(e12);
        ga_mv_destroy(e13);
        ga_mv_destroy(r);
    ...
    /* e12 * e3 = e123 */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E123), 1.0), "e12 * e3 should have e123 component = 1");
        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E123)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e12 * e3: non-e123 components should be zero");
    ...
    /* e12 * e1 = -e2 (bivector contracting with contained vector) */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E2), -1.0), "e12 * e1 should have e2 component = -1");
        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E2)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e12 * e1: non-e2 components should be zero");
        }
        ga_mv_destroy(e12);
        ga_mv_destroy(e1);
        ga_mv_destroy(r);
    ...
    /* e1 * e12 = e2 */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E2), 1.0), "e1 * e12 should have e2 component = 1");
        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E2)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e1 * e12: non-e2 components should be zero");
        }
        ga_mv_destroy(e1);
        ga_mv_destroy(e12);
        ga_mv_destroy(r);
    ...
    /* e123 * e1 = e23 */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E23), 1.0), "e123 * e1 should have e23 component = 1");
        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E23)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e123 * e1: non-e23 components should be zero");
        }
        ga_mv_destroy(e123);
        ga_mv_destroy(e1);
        ga_mv_destroy(r);
    ...
    /* e1 * e123 = e23 */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E23), 1.0), "e1 * e123 should have e23 component = 1");
        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E23)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e1 * e123: non-e23 components should be zero");
        }
        ga_mv_destroy(e1);
        ga_mv_destroy(e123);
        ga_mv_destroy(r);
    ...
    /* e0123 * 1 = e0123 */
    {
        ...
        TEST_ASSERT_NOT_NULL(r);
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E0123), 1.0), "e0123 * 1 should have e0123 component = 1");
        for (int i = 0; i < GA_MV_DIM; i++) {
            if (i == GA_BLADE_E0123)
                continue;
            TEST_ASSERT(approx_eq(ga_mv_get(r, i), 0.0), "e0123 * 1: non-e0123 components should be zero");
        }
        ga_mv_destroy(e0123);
        ga_mv_destroy(s);
        ga_mv_destroy(r);
    ...
    /* ---- 6. Mixed multivector: (1 + e1 + e12) * (e2 + e23) ---- */
    {
        ...
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E1), 1.0), "(1+e1+e12)*(e2+e23): e1 component should be 1");
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E2), 1.0), "(1+e1+e12)*(e2+e23): e2 component should be 1");
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E12), 1.0), "(1+e1+e12)*(e2+e23): e12 component should be 1");
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E13), 1.0), "(1+e1+e12)*(e2+e23): e13 component should be 1");
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E23), 1.0), "(1+e1+e12)*(e2+e23): e23 component should be 1");
        TEST_ASSERT(approx_eq(ga_mv_get(r, GA_BLADE_E123), 1.0), "(1+e1+e12)*(e2+e23): e123 component should be 1");
