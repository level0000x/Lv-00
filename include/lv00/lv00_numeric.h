 ...
 */
typedef struct { double x, y; } Lv00Vec2;
 ...
 */
typedef struct { double x, y, z; } Lv00Vec3;
 ...
 */
typedef struct { double x, y, z, w; } Lv00Vec4;
 ...
 */
typedef struct { double m[9]; } Lv00Mat3;
 ...
 */
typedef struct { double m[16]; } Lv00Mat4;
 ...
 */
typedef struct { double w, x, y, z; } Lv00Quat;
...
static inline Lv00Mat4 lv00_mat4_mul(Lv00Mat4 a, Lv00Mat4 b) {
    ...
    for (int col = 0; col < 4; ++col) {
        for (int row = 0; row < 4; ++row) {
            int idx = col * 4 + row;
            r.m[idx] = a.m[0*4+row] * b.m[col*4+0]
...
static inline Lv00Mat4 lv00_mat4_inverse(Lv00Mat4 m) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (fabs(pivot) < LV00_EPSILON) {
            int swap_row = -1;
            for (int k = i + 1; k < 4; ++k) {
                if (fabs(a[k][i]) > LV00_EPSILON) { swap_row = k; break; }
            }
            if (swap_row < 0) {
                Lv00Mat4 zero;
            ...
            for (int j = 0; j < 4; ++j) {
                double tmp = a[i][j]; a[i][j] = a[swap_row][j]; a[swap_row][j] = tmp;
            }
            for (int j = 0; j < 16; ++j) {
                /* inv 的索引为 j*4 + row，即列主序储存 */
                int ri = j*4 + i, rs = j*4 + swap_row;
                double tmp = inv.m[ri]; inv.m[ri] = inv.m[rs]; inv.m[rs] = tmp;
            }
            pivot = a[i][i];
        }
        double inv_p = 1.0 / pivot;
        for (int j = 0; j < 4; ++j) a[i][j] *= inv_p;
        for (int j = 0; j < 16; ++j) { int ri = j*4 + i; inv.m[ri] *= inv_p; }

        for (int k = 0; k < 4; ++k) {
            if (k == i) continue;
            double factor = a[k][i];
            for (int j = 0; j < 4; ++j) a[k][j] -= factor * a[i][j];
            for (int j = 0; j < 16; ++j) { int rk = j*4 + k, ri = j*4 + i; inv.m[rk] -= factor * inv.m[ri]; }
        }
    }
    return inv;
}
...
static inline Lv00Mat4 lv00_mat4_rotation_x(double angle) {
    double c = cos(angle), s = sin(angle);
    Lv00Mat4 r;
    r.m[0] = 1.0; r.m[1] = 0.0; r.m[ 2] = 0.0; r.m[ 3] = 0.0;
    r.m[4] = 0.0; r.m[5] =   c; r.m[ 6] =   s; r.m[ 7] = 0.0;
    r.m[8] = 0.0; r.m[9] =  -s; r.m[10] =   c; r.m[11] = 0.0;
...
static inline Lv00Mat4 lv00_mat4_rotation_y(double angle) {
    double c = cos(angle), s = sin(angle);
    Lv00Mat4 r;
    r.m[0] =   c; r.m[1] = 0.0; r.m[ 2] =  -s; r.m[ 3] = 0.0;
    r.m[4] = 0.0; r.m[5] = 1.0; r.m[ 6] = 0.0; r.m[ 7] = 0.0;
    r.m[8] =   s; r.m[9] = 0.0; r.m[10] =   c; r.m[11] = 0.0;
    r.m[12]= 0.0; r.m[13]=0.0; r.m[14] = 0.0; r.m[15] = 1.0;
    return r;
}
...
static inline Lv00Mat4 lv00_mat4_rotation_z(double angle) {
    double c = cos(angle), s = sin(angle);
    Lv00Mat4 r;
    r.m[0] =   c; r.m[1] =   s; r.m[ 2] = 0.0; r.m[ 3] = 0.0;
    r.m[4] =  -s; r.m[5] =   c; r.m[ 6] = 0.0; r.m[ 7] = 0.0;
    r.m[8] = 0.0; r.m[9] = 0.0; r.m[10] = 1.0; r.m[11] = 0.0;
    r.m[12]= 0.0; r.m[13]=0.0; r.m[14] = 0.0; r.m[15] = 1.0;
    return r;
}
 ...
 * @return 正交投影矩阵
 */
static inline Lv00Mat4 lv00_mat4_ortho(double left, double right,
 ...
 * @return 透视投影矩阵
 */
static inline Lv00Mat4 lv00_mat4_perspective(double fov_y, double aspect,
                                              double near, double far) {
    Lv00Mat4 r;
    memset(r.m, 0, sizeof(r.m));
...
static inline Lv00Mat4 lv00_mat4_look_at(Lv00Vec3 eye, Lv00Vec3 center, Lv00Vec3 up) {
    ...
    Lv00Mat4 r;
    r.m[0]  =   s.x; r.m[4]  =   s.y; r.m[ 8] =   s.z; r.m[12] = -lv00_vec3_dot(s, eye);
    r.m[1]  =   u.x; r.m[5]  =   u.y; r.m[ 9] =   u.z; r.m[13] = -lv00_vec3_dot(u, eye);
    r.m[2]  =  -f.x; r.m[6]  =  -f.y; r.m[10] =  -f.z; r.m[14] =  lv00_vec3_dot(f, eye);
...
static inline Lv00Mat4 lv00_quat_to_mat4(Lv00Quat q) {
    ...
    Lv00Mat4 r;
    r.m[0]  = 1.0 - 2.0*(yy + zz); r.m[4]  = 2.0*(xy - wz);       r.m[ 8] = 2.0*(xz + wy);       r.m[12] = 0.0;
    r.m[1]  = 2.0*(xy + wz);       r.m[5]  = 1.0 - 2.0*(xx + zz); r.m[ 9] = 2.0*(yz - wx);       r.m[13] = 0.0;
    r.m[2]  = 2.0*(xz - wy);       r.m[6]  = 2.0*(yz + wx);       r.m[10] = 1.0 - 2.0*(xx + yy); r.m[14] = 0.0;
    r.m[3]  = 0.0;                 r.m[7]  = 0.0;                 r.m[11] = 0.0;                  r.m[15] = 1.0;
    return r;
}
 ...
 * @return 组合变换
 */
static inline Lv00GeomTransform lv00_transform_compose(Lv00GeomTransform a,
                                                        Lv00GeomTransform b) {
    Lv00GeomTransform t;
...
static inline void lv00_mat4_mul_sse2(const double a[16], const double b[16],
                                       double out[16]) {
...
static inline void lv00_vec4_add_sse2(const double a[4], const double b[4],
                                       double out[4]) {
 ...
 * @return 点到线段的最短距离
 */
static inline double lv00_point_to_line_distance_2d(Lv00Vec2 pt,
                                                     Lv00Vec2 line_a,
                                                     Lv00Vec2 line_b) {
