...
INCLUDE = os.path.join(BASE, "core", "include", "lv00")

# ── 1) Scan CMakeLists.txt for all referenced files and create missing stubs ──
with open(CMAKE, "r", encoding="utf-8") as f:
    cmake_text = f.read()

# Find all core/include/lv00/*.h and core/src/**/*.c references
refs = set(re.findall(r'(core/(?:include/lv00|src/[\w_/]+)/[\w_]+\.(?:h|c))', cmake_text))
