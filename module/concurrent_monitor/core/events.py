...
class EventBus:
    ...
    线程安全保证：
      ...
      - 发布事件时先获取处理器快照，再逐个调用
      - RLock（可重入锁）允许同一线程嵌套获取锁，避免死锁

    线程安全注意事项：
      - subscribe/unsubscribe 操作是原子的，多线程可以安全地同时订阅/取消订阅
      - publish 操作获取处理器快照后释放锁，处理器回调在锁外执行，
        因此回调函数内部不应依赖 EventBus 内部状态的一致性
      - 异步处理器通过 run_coroutine_threadsafe 跨线程调度到事件循环线程，
        异步处理器内部的所有操作都在事件循环线程中串行执行
    ...
    def _get_or_create_loop(self) -> asyncio.AbstractEventLoop:
        ...
        线程生命周期说明：
          - 事件循环运行在独立的守护线程中（daemon=True），
            当主线程退出时该线程会自动终止，不会阻止程序退出。
          - 事件循环一旦创建将持续运行（run_forever），直到被显式关闭
            或主进程退出。由于 EventBus 实例通常是全局单例，
            该线程的生命周期与应用生命周期一致。
          - 如果事件循环意外关闭（例如被外部代码调用 loop.close()），
            下次调用时会自动创建新的循环和线程，实现自愈。
          - 注意：守护线程中运行的异步处理器不应执行不可中断的阻塞操作，
            否则可能导致处理器无法及时响应。
