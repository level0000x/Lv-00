...
核心类：
  - lvModule: Lv-00 UI 相关模块的枚举定义
  - lvKnowledgeBase: 领域知识库，包含 API、代码模式、UI 规范
  - lvPromptEngine: 提示词生成引擎，基于知识库生成 AI 提示词

便捷函数：
  - get_lv_helper(): 获取知识库和提示词引擎实例（带缓存）
        ...
        return [
            {
                ...
                "steps": [
                    "在lv_web_bindings.c中编写EMSCRIPTEN_KEEPALIVE函数",
                    "处理参数类型转换",
                    "处理返回值",
                    "如果是字符串，用strdup分配内存",
                    "如果是数组，定义结构体并填充",
...
class lvPromptEngine:
    """
    Lv-00专用提示词引擎
    生成针对项目上下文优化的AI提示词
    """
    
    def __init__(self, knowledge_base: lvKnowledgeBase):
...
def get_lv_helper() -> tuple:
    ...
    Returns:
        (lvKnowledgeBase, lvPromptEngine) 元组
    """
    global _cached_helper, _cache_created_at
