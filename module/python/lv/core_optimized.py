...
核心类列表：
    ...
    - lvBaseError: 统一异常基类，提供 message 和 error_code 属性
    - lvError: 核心模块异常基类，涵盖库错误、参数错误、约束冲突等
    - lvLibraryError: 库加载或初始化失败异常

使用示例：
    >>> from lv.core import Graph, Point, SymbolicCoord
    >>>
    >>> # 创建约束图并添加几何对象
    >>> g = Graph()
...
与 C 库的绑定关系：
    本模块通过 ctypes 与底层 C 共享库（lv_core）交互，绑定关系如下：
...
class SymbolicCoord:
    ...
    def __pow__(self, other: Union['SymbolicCoord', int, float, Fraction]) -> 'SymbolicCoord':
        ...
        if exp_frac.denominator == 1:
            ...
            if exp > 0:
                ...
                return result
            # 负整数幂：计算倒数
            positive_result = self.__pow__(-exp_frac.numerator)
    ...
    def _sqrt(self) -> 'SymbolicCoord':
        ...
        try:
            ...
            if s:
                ...
                _lib.lv_free_ptr(s)
                try:
                    ptr = self._parse_string(f"sqrt({raw})")
                    if ptr:
                        return self._new_from_ptr(ptr)
                except lvError:
                    pass
    ...
    def _nth_root(self, n: int) -> 'SymbolicCoord':
        """计算 n 次方根。对完全 n 次幂有理数返回精确值，否则返回符号表示。"""
        import math as _math
        ...
        try:
            ...
            if s:
                ...
                _lib.lv_free_ptr(s)
                try:
                    ptr = self._parse_string(f"root({raw},{n})")
                    if ptr:
                        return self._new_from_ptr(ptr)
                except lvError:
                    pass
