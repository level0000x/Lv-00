...
使用方式:
    ...
    python stream_bridge.py --port 3456 --engine ../build/liblv.so
        ...
        if self.lib and self.ctx:
            ...
            for handler in self._event_handlers:
                try:
                    ...
                    import logging
                    logging.getLogger('lv.stream_bridge').error(
                        f"模拟事件处理器错误: {e}"
                    )
...
class EventPersistence:
    ...
    def __init__(self, file_path: Optional[str] = None) -> None:
        ...
        参数：
            file_path: JSONL 文件路径（默认 ~/.lv/stream_events.jsonl）
        """
        if file_path:
            self.file_path = os.path.expanduser(file_path)
        else:
            self.file_path = os.path.expanduser('~/.lv/stream_events.jsonl')
...
class MultiEngineManager:
    ...
    示例：
        ...
        >>> engine_a = EngineBridge('./build/liblv.so')
...
class AIAssistantProxy:
    ...
    def __init__(self, endpoint: str, api_key: str, model: Optional[str] = None) -> None:
        ...
        self.model = model or os.environ.get('lv_AI_MODEL', 'gpt-4o')
...
def main() -> None:
    ...
    parser.add_argument('--persist-file', help='持久化文件路径 (默认: ~/.lv/stream_events.jsonl)')
