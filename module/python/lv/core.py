...
class lvError(lvBaseError):
    """
    Lv-00 异常基类，所有核心模块异常的通用父类。

    所有 Lv-00 相关异常的父类，涵盖库错误、参数错误、
    约束冲突、求解失败等场景。继承自 lvBaseError，复用统一的
    message、error_code 属性和人类可读的 __str__ 格式化逻辑。

    子类包括：
        - lvLibraryError：库加载或初始化失败
        - lvArgumentError：参数无效或类型不兼容
        - lvConstraintError：几何约束冲突
        - lvSolverError：方程求解失败
    ...
    使用示例：
        >>> raise lvError("创建符号坐标失败", error_code=1001)
        lvError(1001): 创建符号坐标失败
    """
...
class lvLibraryError(lvError):
    ...
    使用示例：
        >>> try:
        ...     g = Graph()
        ... except lvLibraryError as e:
        ...     print(f"库加载失败：{e}")
    """
    pass


class lvArgumentError(lvError):
    """参数无效异常。"""
    pass


class lvConstraintError(lvError):
    """几何约束冲突异常。"""
    pass


class lvSolverError(lvError):
    """方程求解失败异常。"""
    pass
...
class SymbolicCoord:
    ...
    使用示例：
        >>> from lv.core import SymbolicCoord
        >>> from fractions import Fraction
        >>>
            ...
            if not self._ptr:
                ...
                try:
                    ...
                    try:
                        ...
                        raise lvError(f"无法解析符号坐标表达式: {value}")
        else:
...
class Point:
    ...
    使用示例：
        >>> from lv.core import Point, SymbolicCoord
        >>>
        >>> p = Point(0, 0)                         # 原点
...
class LineSegment:
    ...
    使用示例：
        >>> from lv.core import Point, LineSegment
        >>>
...
class Graph:
    ...
    def __init__(self) -> None:
        ...
        异常：
            lvLibraryError: C 库创建失败时抛出。通常由 DLL 加载失败、
                               内存不足或平台不兼容引起。
        """
    ...
    def add_point(self, x: Union[SymbolicCoord, int, float], 
        ...
        异常：
            lvConstraintError: C 层 graph_add_point() 返回非 OK 错误码时抛出，
                                  包含具体的错误码信息。常见原因：
                                  - 坐标类型不被 C 库支持
                                  - 底层图结构损坏
                                  - 内存分配失败
    ...
    def add_line_segment(self, p1: Point, p2: Point) -> LineSegment:
        ...
        异常：
            lvError: p1 或 p2 尚未添加到图中时抛出，
                       消息为 "点必须先通过 add_point 添加到图中"
            lvConstraintError: C 层 graph_add_line_segment() 返回
                                  非 OK 错误码时抛出
        ...
        if not (p1_in and p2_in):
            raise lvError("点必须先通过 add_point 添加到图中")
    ...
    def remove_node(self, node_id: int) -> int:
        ...
        if result == REMOVE_NODE_NOT_FOUND:
            raise lvConstraintError(f"移除节点失败: 节点 {node_id} 不存在")
        if result != REMOVE_NODE_OK:
            raise lvConstraintError(f"移除节点失败: 错误码 {result}")
        # 从 Python 追踪列表中移除
    ...
    def remove_constraint(self, constraint_id: int) -> int:
        ...
        if result == REMOVE_CONSTRAINT_NOT_FOUND:
            raise lvConstraintError(f"移除约束失败: 约束 {constraint_id} 不存在")
        if result != REMOVE_CONSTRAINT_OK:
            raise lvConstraintError(f"移除约束失败: 错误码 {result}")
        return result
    ...
    def add_node_with_id(self, node_id: int, node_type: int,
        ...
        if not ptr:
            raise lvConstraintError(f"带 ID 添加节点失败: id={node_id}")
        # 同步 Python 侧 ID 计数器
    ...
    def add_constraint_with_id(self, constraint_id: int,
        ...
        if not ptr:
            raise lvConstraintError(f"带 ID 添加约束失败: id={constraint_id}")
        return ptr
    ...
    def add_incidence(self, point_id: int, line_or_region_id: int) -> None:
        ...
        异常：
            lvConstraintError: C 层 graph_add_incidence() 返回非 OK 错误码时抛出。
                                  常见原因：
                                  - 节点 ID 不存在
                                  - 约束与已有约束冲突
                                  - 目标类型不支持关联约束
    ...
    def add_betweenness(self, p1_id: int, p2_id: int, p3_id: int) -> None:
        ...
        异常：
            lvConstraintError: C 层 graph_add_betweenness() 返回非 OK 错误码时抛出。
                                  常见原因：
                                  - 节点 ID 不存在
                                  - 三点不可能共线（与已有约束冲突）
    ...
    def add_intersection(self, line1_id: int, line2_id: int, 
        ...
        异常：
            lvConstraintError: C 层 graph_add_intersection() 返回非 OK 错误码时抛出。
                                  常见原因：
                                  - 节点 ID 不存在
                                  - 两线段平行（无交点）或重合（无穷多交点）
                                  - 交点与已有约束冲突
    ...
    def add_containment(self, inner_id: int, outer_id: int) -> None:
        ...
        异常：
            lvConstraintError: C 层 graph_add_containment() 返回非 OK 错误码时抛出。
                                  常见原因：
                                  - 节点 ID 不存在或非区域类型
                                  - 内区域无法被外区域完全包含
                                  - 与已有约束冲突（如循环包含）
    ...
    def add_connection(self, src_port_id: int, dst_port_id: int) -> None:
        ...
        异常：
            lvConstraintError: C 层 graph_add_connection() 返回非 OK 错误码时抛出。
                                  常见原因：
                                  - 节点 ID 不存在或非端口类型
                                  - 端口类型不匹配（如输出→输出）
                                  - 连接与已有约束冲突
...
def get_version() -> str:
    ...
    注意：
        lv_get_version 在 C 库中为 static inline 函数，不在 DLL 导出中。
...
def get_last_error() -> str:
    ...
    返回：
        str: 错误消息
    """
    msg = _lib.lv_get_last_error_message()
    if msg:
