...
register_preset(FuncBlockSpec(
    ...
    >>> from lv00.preset_blocks import create_harmonic_conjugate
    >>> g = Graph()
    >>> a = g.add_point(0, 0)
    >>> b = g.add_point(2, 0)
...
def create_harmonic_conjugate(
    ...
    # 使用分式线性变换的方法
    # 若 C 分 AB 为 t:(1-t)，则 D 分 AB 为 t:(t-1)
    
    # 计算参数 t
    # C = (1-t)A + tB => t = |AC|/|AB|
    
    # 简化处理：使用坐标公式
    # D_x = (2*a.x*b.x - c.x*(a.x + b.x)) / (a.x + b.x - 2*c.x)
    # 注意：这个公式仅在特定坐标系下成立
    
    # 更通用的方法：使用交比
    # (A, B; C, D) = (AC/CB) / (AD/DB) = -1
...
register_preset(FuncBlockSpec(
    ...
    example_usage="""
    >>> from lv00 import Graph
    >>> from lv00.preset_blocks import create_polar_line
    >>> g = Graph()
    >>> o = g.add_point(0, 0)
    >>> p = g.add_point(2, 0)
    >>> r = 1
...
register_preset(FuncBlockSpec(
    ...
    example_usage="""
    >>> from lv00 import Graph
    >>> from lv00.preset_blocks import create_pole_point
    >>> g = Graph()
    >>> o = g.add_point(0, 0)
    >>> a = g.add_point(1, 0)
    >>> b = g.add_point(1, 1)
